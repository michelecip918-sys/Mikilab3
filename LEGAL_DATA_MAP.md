...
## 29. V100 — La mappa di MikiLab
- **La mappa** (`Mappa.jsx`, /mappa): pagina statica di orientamento (elenco degli attrezzi con una riga di spiegazione e collegamenti); nessun dato trattato. `MIKILAB_STORIA.md` nella radice del progetto: cronologia delle versioni. Rimosse le cartelle `patch99`, `v99` e, a fine applicazione, `patch100`: il repository non contiene più cartelle di patch.
## 30. V101 — L'almanacco del fornaio
- **Almanacco** (`Almanacco.jsx`, /oggi; card `OggiInBottega.jsx` in Home): contenuti fissi (feste, proverbi, gesti) scelti in base alla data nel browser; il "pane di oggi" è scelto tra le ricette per giorno della settimana. I timbri dei giorni di visita (`mikilab_bottega_giorni`, solo date) e il promemoria scelto (`mikilab_promemoria`) restano in localStorage; nessun invio, nessuna notifica push: il promemoria è un file .ics ricorrente che l'utente salva nel proprio calendario. Nuova medaglia "Sette giorni in bottega". Cartelle di patch rimosse a fine applicazione.
## 31. V102 — Acqua e farina: la pasta di casa
- **Sezione creata dall'IA** (`PastaDiCasa.jsx`, `lib/pastaDiCasa.js`, /pasta): dodici ricette di pasta fresca scritte da Sitor secondo la tradizione, dichiarate in pagina come create dall'IA e distinte dalle ricette di Michele. Contenuti statici nel frontend, nessun dato trattato; le dosi a persona sono calcolate nel browser; il numero di persone e gli appunti restano in localStorage. Nessun claim salutistico; gli allergeni (glutine, uova nelle tagliatelle) sono evidenti dagli ingredienti.
## 32. V103 — Il pane dei piccoli; "Le mani in pasta"
- **Il pane dei piccoli** (`PaneDeiPiccoli.jsx`, `lib/paneDeiPiccoli.js`, /piccoli): sezione scritta da Sitor (IA) e dichiarata come tale, distinta dalle ricette di Michele: impasto base, sei forme, cinque regole di sicurezza (forno solo all'adulto, niente impasto crudo, forbici a punta tonda, acqua tiepida), diploma disegnato sul dispositivo con nome o soprannome scelto dall'adulto. Nessun dato inviato; numero di bambini in localStorage. Nessuna raccolta di dati di minori: il sito non chiede né salva nulla sul server.
...
## 35. V106 — Dati strutturati Ricetta completi
- Il blocco JSON-LD schema.org/Recipe di ogni ricetta include ora anche le istruzioni passo per passo (dal procedimento già pubblico), gli ingredienti aggiuntivi in grammi, la resa e il tempo di preparazione. Non vengono aggiunti valori nutrizionali (nessun dato di laboratorio, nessun claim), voti aggregati (non esiste un sistema di voti: inventarli violerebbe le regole di Google) né video (non ce ne sono). Nessun dato personale.
## 36. V107 — Il benvenuto, Il pane che salva
- **Benvenuto** (`BenvenutoMikiLab.jsx`, in Home): testo fisso e collegamenti; salva solo la data in cui è stato chiuso (`mikilab_benvenuto_visto`) in localStorage.
- **Il pane che salva** (`PaneCheSalva.jsx`, `lib/paneCheSalva.js`, /salva): sezione scritta da Sitor (IA), dichiarata e distinta dalle ricette di Michele; contenuti statici; la dispensa di scorta calcola nel browser (persone e giorni in localStorage `mikilab_scorta`). Avvertenze di sicurezza per il fuoco e per il bicarbonato. Nessun dato inviato, nessun claim salutistico. Rimossa di nuovo la cartella `capacitor-plugins`.
## 37. V108 — Cerca, Le collezioni, Le domande del fornaio
- **Cerca in tutto MikiLab** (`Cerca.jsx`, /cerca): ricerca locale su ricette già caricate e contenuti statici; il testo cercato resta in sessionStorage per la durata della scheda. **Le collezioni** (`Collezioni.jsx`, /collezioni): raggruppamenti calcolati dai dati delle ricette. **Le domande del fornaio** (`Domande.jsx`, /domande): risposte già presenti nel sito (glossario e pronto soccorso) con dati strutturati FAQPage (schema.org) per i motori di ricerca. Nessun dato inviato, nessuna IA, nessun claim salutistico. **Forno in °F** nell'attrezzo tazze e once.
## 38. V111 — Il pane dei piccoli: schede
- **IT:** 38. Il pane dei piccoli (/piccoli). Le cinque schede (dodici forme, la storia del pane, la notte del fornaio, il gioco del lievito, un'ora in classe) sono contenuti statici scritti da Sitor. Nel browser restano solo il numero di bambini (mikilab_piccoli), il soprannome facoltativo per il diploma (mikilab_cartolina_nome) e la medaglia «Piccolo fornaio» (mikilab_medaglie). Nulla viene inviato.
- **DE:** 38. Das Brot der Kleinen (/piccoli). Die fünf Reiter (zwölf Formen, Brotgeschichte, Nacht des Bäckers, Hefespiel, eine Stunde in der Klasse) sind statische Inhalte von Sitor. Im Browser bleiben nur die Kinderzahl (mikilab_piccoli), der optionale Spitzname für die Urkunde (mikilab_cartolina_nome) und die Medaille „Kleiner Bäcker“ (mikilab_medaglie). Nichts wird gesendet.
- **EN:** 38. Bread for little ones (/piccoli). The five tabs (twelve shapes, the story of bread, the baker's night, the yeast game, one hour in class) are static content written by Sitor. Only the number of children (mikilab_piccoli), the optional nickname for the diploma (mikilab_cartolina_nome) and the “Little baker” medal (mikilab_medaglie) stay in the browser. Nothing is sent.

## 39. V111 — MikiLab a scuola
- **IT:** 39. MikiLab a scuola (/scuola). Il quaderno della classe salva nel browser di chi insegna (localStorage, chiave mikilab_scuola) soltanto: il soprannome della classe, il numero di bambini, l'anno scolastico, il nome e la data di nascita del lievito madre della classe, le lezioni segnate come fatte, le date scelte per l'agenda, appunti liberi e la lingua scelta. Non vanno inseriti nomi di bambini. Nulla viene inviato a MikiLab o a terzi; i dati si cancellano con il tasto «Cancella il quaderno» o cancellando i dati del sito.
- **DE:** 39. MikiLab in der Schule (/scuola). Das Klassenheft speichert im Browser der Lehrkraft (localStorage, Schlüssel mikilab_scuola) nur: den Spitznamen der Klasse, die Anzahl der Kinder, das Schuljahr, Name und Geburtsdatum des Klassen-Sauerteigs, die als erledigt markierten Stunden, die gewählten Kalenderdaten, freie Notizen und die gewählte Sprache. Kindernamen dürfen nicht eingetragen werden. Nichts wird an MikiLab oder Dritte gesendet; die Daten lassen sich mit „Heft löschen“ oder durch Löschen der Website-Daten entfernen.
- **EN:** 39. MikiLab at school (/scuola). The class notebook stores in the teacher's browser (localStorage, key mikilab_scuola) only: the class nickname, the number of children, the school year, the name and birth date of the class sourdough, the lessons marked as done, the chosen calendar dates, free notes and the chosen language. Children's names must not be entered. Nothing is sent to MikiLab or to third parties; the data can be removed with "Delete the notebook" or by clearing the site data.

## 40. V127 — La calcolatrice del fornaio

- **La calcolatrice del fornaio** (`components/Calcolatrice.jsx`, `components/calcolatrice/*`, `lib/fornaio.js`; indirizzi /calcolatrice, /pizza, /rinfresco, /formule): tutti i calcoli avvengono nel browser, senza chiamate di rete. In localStorage: la formula aperta (`mikilab_calc_impasto`, `mikilab_calc_pizza`), le impostazioni del «lievito pronto» (`mikilab_calc_lievito`) e le formule salvate (`mikilab_formule`: nome scelto dall'utente e numeri, al massimo 60). Entrano nella Valigia come le altre chiavi mikilab_*.
- **Condivisione**: la formula è codificata nel link dopo il segno # (frammento). Il browser non invia il frammento al server; nessun registro, nessun tracciamento. Il file .ics del piano è creato sul dispositivo e condiviso solo col menu del telefono.
- **«Segna nel mio forno»**: nome e riassunto passano a «Il mio forno» tramite localStorage (`mikilab_mioforno_prefill`), cancellata appena letta. **«Chiedi a Sitor»**: apre la chat con il testo già scritto; parte solo se l'utente lo invia (come le altre domande a Sitor, sez. 5).
- **Scheda ricetta**: «Pezzi e peso» (`PezziPeso.jsx`) calcola la farina nel browser e usa lo stesso comando dei bottoni 250/500/750/1000; non salva nulla. **Home**: riquadro `CalcolaInHome.jsx` con un esempio calcolato nel browser.
