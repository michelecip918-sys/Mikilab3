...
export default function PrimoPane({ onBack }) {
  ...
  return (
    <div data-testid="primopane-page" className="max-w-2xl mx-auto px-4 py-6 space-y-4">
          ...
          return (
            <div key={d.n} data-testid={`pp-day-${d.n}`} className={`rounded-2xl border ${done[d.n] ? "border-salvia/50 bg-salvia/8" : isOpen ? "border-primary/50 bg-card" : "border-border bg-card"}`}>
              <button onClick={() => setOpen(isOpen ? null : d.n)} className="w-full flex items-center gap-3 p-3 text-left">
                ...
                <span className="flex-1 min-w-0"><span className="block text-[14px] font-bold text-foreground">{L(d.t)}</span><span className="block text-[11.5px] text-muted-foreground truncate">{L(d.goal)}</span></span>
                <ChevronRight className={`w-4 h-4 text-muted-foreground transition-transform ${isOpen ? "rotate-90" : ""}`} />
              </button>
              {isOpen && (
                <div className="px-3 pb-3 space-y-2.5">
                  <ol className="space-y-1.5">{L(d.steps).map((s, i) => <li key={i} className="flex gap-2 text-[12.5px] text-foreground/90 leading-snug"><span className="font-mono-data text-primary shrink-0">{i + 1}.</span><span>{s}</span></li>)}</ol>
                  <p className="text-[12px] text-salvia leading-snug">Sitor: {L(d.sitor)}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {rec && <button data-testid={`pp-recipe-${d.n}`} onClick={() => openRecipe(rec.id)} className="text-[11.5px] font-bold px-2.5 py-1.5 rounded-full bg-primary text-white active:scale-95">{tri("Apri la ricetta", "Rezept öffnen", "Open the recipe")}: {rLoc(rec, "name", lang)}</button>}
                    {(d.links || []).map((l) => <button key={l.route} data-testid={`pp-link-${l.route}`} onClick={() => nav(l.route)} className="text-[11.5px] font-semibold px-2.5 py-1.5 rounded-full border border-border bg-background text-foreground active:scale-95">{L(l)}</button>)}
                    <button data-testid={`pp-done-${d.n}`} onClick={() => toggle(d.n)} className={`ml-auto text-[11.5px] font-bold px-2.5 py-1.5 rounded-full border active:scale-95 ${done[d.n] ? "bg-background text-muted-foreground border-border" : "bg-salvia text-white border-salvia"}`}>{done[d.n] ? tri("Segna come da rifare", "Als offen markieren", "Mark as not done") : tri("Fatto ✓", "Erledigt ✓", "Done ✓")}</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {count === 7 && <p data-testid="pp-finished" className="text-[13px] font-bold text-salvia text-center">{tri("Percorso finito. Da oggi sei uno che fa il pane in casa.", "Weg geschafft. Ab heute bist du jemand, der zu Hause Brot backt.", "Path complete. From today you're someone who bakes bread at home.")}</p>}
    </div>
  );
}
