  ...
  return (
    <div data-testid="foglio-produzione" className="space-y-3">
      ...
      <div className="no-print flex gap-1.5">
        ...
        <button data-testid="fp-add" onClick={add} disabled={!pick} className="inline-flex items-center gap-1 text-[12px] font-bold px-3 rounded-lg bg-primary text-white disabled:opacity-40 active:scale-95"><Plus className="w-4 h-4" />{tri("Metti", "Rein", "Add")}</button>
      </div>
      <div className="print-area space-y-2">
        {lines.length === 0 && <p className="text-[12px] text-muted-foreground rounded-xl border border-border bg-card p-3">{tri("Nessuna ricetta ancora. Aggiungine una qui sopra.", "Noch kein Rezept. Füge oben eines hinzu.", "No recipe yet. Add one above.")}</p>}
        {lines.map(({ row, r, d }, i) => (
          <div key={i} data-testid={`fp-row-${i}`} className="rounded-xl border border-border bg-card p-3">
            ...
            <div className="no-print mt-1.5 flex flex-wrap items-center gap-2 text-[12px]">
        ...
        {lines.length > 0 && (
          <div data-testid="fp-totals" className="rounded-xl border border-salvia/40 bg-salvia/8 p-3">
            ...
            <p className="text-[12px] text-foreground mt-2">{tri("Sacchi da 25 kg", "Säcke à 25 kg", "25 kg sacks")}: <b className="font-mono-data">{(Math.ceil((totFlour / 25000) * 10) / 10).toString().replace(".", ",")}</b> · {tri("impasto totale", "Teig gesamt", "total dough")}: <b className="font-mono-data">{kgOrG(totDough)}</b></p>
          </div>
        )}
      </div>
      {lines.length > 0 && (
        <div className="no-print flex gap-1.5">
          <button data-testid="fp-print" onClick={() => window.print()} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12px] font-bold px-3 py-2 rounded-xl border border-border bg-card text-foreground active:scale-95"><Printer className="w-4 h-4" />{tri("Stampa", "Drucken", "Print")}</button>
          <button data-testid="fp-copy" onClick={() => { navigator.clipboard.writeText(asText()).then(() => toast.success(tri("Copiato.", "Kopiert.", "Copied."))).catch(() => toast.error(tri("Non riesco a copiare.", "Kopieren nicht möglich.", "Can't copy."))); }} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12px] font-bold px-3 py-2 rounded-xl border border-border bg-card text-foreground active:scale-95"><Copy className="w-4 h-4" />{tri("Copia come testo", "Als Text kopieren", "Copy as text")}</button>
        </div>
      )}
      <p className="text-[12px] text-salvia leading-snug">{tri("Sitor: in laboratorio si pesa una volta sola e si scrive. Il foglio in tasca vale più della memoria alle quattro del mattino.", "Sitor: in der Backstube wiegt man einmal und schreibt es auf. Das Blatt in der Tasche ist um vier Uhr morgens mehr wert als das Gedächtnis.", "Sitor: in the bakery you weigh once and write it down. The sheet in your pocket is worth more than memory at four in the morning.")}</p>
    </div>
  );
}
