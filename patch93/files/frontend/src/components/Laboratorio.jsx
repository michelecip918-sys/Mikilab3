...
import { ChevronLeft, ClipboardList, Snowflake, ArrowLeftRight, Flame } from "lucide-react";
  ...
  return (
    <div data-testid="laboratorio-page" className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1">
        {tabs.map((x) => (
          <button key={x.k} data-testid={`lab-tab-${x.k}`} onClick={() => setTab(x.k)} className={`shrink-0 inline-flex items-center gap-1.5 text-[12px] font-bold px-3 py-2 rounded-xl border active:scale-95 ${tab === x.k ? "bg-primary text-white border-primary" : "bg-card text-foreground border-border"}`}><x.I className="w-4 h-4" />{x.l}</button>
        ))}
      </div>
      {recipes.length === 0 && tab !== "forno" ? (
        <p className="text-[12px] text-muted-foreground">{tri("Carico le ricette…", "Lade Rezepte…", "Loading recipes…")}</p>
      ) : (
        <>
