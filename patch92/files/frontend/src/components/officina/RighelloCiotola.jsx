  ...
  return (
    <div data-testid="righello-ciotola" className="space-y-3">
      ...
      <div className="flex gap-3 items-stretch">
        ...
        <div className="flex-1 space-y-2.5">
          ...
          <label className="block">
            <span className="flex items-center justify-between text-[12px] text-muted-foreground"><span>{tri("Altezza adesso", "Höhe jetzt", "Height now")}</span><span className="font-mono-data font-bold text-foreground">{(Math.round(cur * 10) / 10).toString().replace(".", ",")} cm</span></span>
          ...
          <div>
            <span className="block text-[12px] text-muted-foreground mb-1">{tri("Deve crescere di", "Soll wachsen um", "It should grow by")}</span>
            <div className="flex gap-1.5">
              {[50, 75, 100].map((tg) => (
                <button key={tg} data-testid={`rc-target-${tg}`} onClick={() => set({ target: tg })}
                  className={`text-[11.5px] font-bold px-2.5 py-1 rounded-full border active:scale-95 ${s.target === tg ? "bg-salvia text-white border-salvia" : "bg-card text-muted-foreground border-border"}`}>+{tg} %</button>
              ))}
            </div>
          ...
          <div className="flex gap-2">
            <button data-testid="rc-mark" onClick={() => set({ readings: [...readings, { t: Date.now(), h: cur }].slice(-12) })}
              className="inline-flex items-center gap-1.5 bg-primary text-white font-semibold text-[12px] px-3 py-1.5 rounded-xl active:scale-95"><Check className="w-3.5 h-3.5" /> {tri("Segna questa lettura", "Messung speichern", "Save this reading")}</button>
            <button data-testid="rc-reset" onClick={() => setS(fresh())} className="inline-flex items-center gap-1 text-[12px] font-semibold text-muted-foreground px-2 active:scale-95"><RotateCcw className="w-3.5 h-3.5" /> {tri("Da capo", "Neu", "Reset")}</button>
          </div>
        </div>
      </div>
      ...
      <div data-testid="rc-result" className="rounded-xl border border-salvia/50 bg-salvia/10 p-3.5">
        ...
        <p className="text-[12.5px] text-foreground mt-2">
        ...
        {eta && <p className="text-[12px] text-muted-foreground mt-1">{tri("A questo ritmo", "In diesem Tempo", "At this pace")} ({(Math.round(ratePerH * 10) / 10).toString().replace(".", ",")} cm/h) {tri("arriva a", "erreicht er", "it reaches")} +{s.target} % {tri("verso le", "gegen", "at around")} <b className="font-mono-data text-foreground">{fmtTime(eta, lang)}</b>.</p>}
        {readings.length === 1 && <p className="text-[12px] text-muted-foreground mt-1">{tri("Segna una seconda lettura tra 20-30 minuti e ti dico l'orario.", "Speichere in 20-30 Minuten eine zweite Messung, dann sage ich dir die Uhrzeit.", "Save a second reading in 20-30 minutes and I'll tell you the time.")}</p>}
        {readings.length >= 1 && (
