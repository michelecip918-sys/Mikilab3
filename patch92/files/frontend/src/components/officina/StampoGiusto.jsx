...
export default function StampoGiusto({ r, lang, onScaleChange, scaleVal }) {
  ...
  if (!(baseFlour > 0)) return <p className="text-[12.5px] text-muted-foreground">{tri("Questa ricetta non ha la farina in grammi: non posso scalarla.", "Dieses Rezept hat kein Mehl in Gramm: ich kann es nicht skalieren.", "This recipe has no flour in grams: I can't scale it.")}</p>;
    ...
    <div data-testid="stampo-giusto" className="space-y-3">
      ...
      <div className="flex flex-wrap gap-1.5">
        {TYPES.map((x) => (
          <button key={x.k} data-testid={`sg-type-${x.k}`} onClick={() => setType(x.k)}
            className={`text-[11.5px] font-semibold px-2.5 py-1.5 rounded-full border transition-all active:scale-95 ${type === x.k ? "bg-primary text-white border-primary" : "bg-background text-muted-foreground border-border"}`}>
      ...
      <div className="rounded-xl border border-border bg-card p-3 space-y-2">
        <div className="flex flex-wrap gap-3">
          {T.dims.map((k) => (
            <label key={k} className="flex items-center gap-1.5 text-[12px] text-muted-foreground">
        ...
        <label className="flex items-center justify-between gap-2 text-[12px] text-muted-foreground pt-1 border-t border-border/60">
      ...
      <div data-testid="sg-result" className="rounded-xl border border-salvia/50 bg-salvia/10 p-3.5">
        <div className="flex items-end justify-between gap-3">
          <div>
            <p className="text-[12px] text-muted-foreground">{tri("Impasto per stampo", "Teig pro Form", "Dough per tin")}</p>
            <p className="font-display text-3xl font-bold text-foreground leading-none">{Math.round(perPiece)} g</p>
          </div>
          <div className="text-right">
            <p className="text-[12px] text-muted-foreground">{tri("Impasto totale", "Teig gesamt", "Total dough")}</p>
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          <button data-testid="sg-apply" disabled={!(newFlour > 0)} onClick={() => { onScaleChange(String(newFlour)); toast.success(tri(`Ricetta scalata: ${newFlour} g di farina`, `Rezept skaliert: ${newFlour} g Mehl`, `Recipe scaled: ${newFlour} g flour`)); }}
            className="inline-flex items-center gap-1.5 bg-primary text-white font-semibold text-[12.5px] px-3.5 py-2 rounded-xl active:scale-95 transition-all disabled:opacity-50">
          ...
          {Math.round(num(scaleVal) || baseFlour) !== Math.round(baseFlour) && (
            <button data-testid="sg-reset" onClick={() => onScaleChange(String(baseFlour))} className="text-[12px] font-semibold text-muted-foreground underline decoration-dotted active:scale-95">
