  ...
  return (
    <div data-testid="libro-page" className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="no-print">
        ...
        {loaded && recipes.length === 0 && <p className="mt-2 text-[12.5px] text-muted-foreground rounded-xl border border-border bg-card p-3">{tri("Nessuna ricetta nel cuore ancora: nel Ricettario tocca il cuore sulle ricette che vuoi nel libro.", "Noch keine Lieblingsrezepte: tipp im Rezeptbuch auf das Herz bei den Rezepten, die ins Buch sollen.", "No favourites yet: in the recipe book tap the heart on the recipes you want in the book.")}</p>}
        {recipes.length > 0 && <button data-testid="libro-print" onClick={() => window.print()} className="mt-3 inline-flex items-center gap-1.5 text-[12px] font-bold px-3 py-2 rounded-xl bg-primary text-white active:scale-95"><Printer className="w-4 h-4" />{tri(`Stampa il libro (${recipes.length} ricette)`, `Buch drucken (${recipes.length} Rezepte)`, `Print the book (${recipes.length} recipes)`)}</button>}
      </div>
      {recipes.length > 0 && (
        <div data-testid="libro-print-area" className="print-area space-y-6">
          <div className="rounded-2xl border border-border bg-background px-6 py-12 text-center" style={{ pageBreakAfter: "always" }}>
            ...
            <p className="text-[12px] text-muted-foreground">{recipes.length} {tri("ricette", "Rezepte", "recipes")} · {fmtDateLong(new Date(), lang)}</p>
          ...
          {recipes.map((r, idx) => {
            ...
            return (
              <article key={r.id} data-testid={`libro-ricetta-${idx}`} className="rounded-2xl border border-border bg-background px-6 py-6" style={{ pageBreakAfter: "always", breakInside: "avoid" }}>
                ...
                {r.flour_type && <p className="text-[12px] text-muted-foreground">{rLoc(r, "flour_type", lang) || r.flour_type}</p>}
                <div className="w-12 h-px bg-primary my-3" />
                {d && (
                  <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-[12.5px]">
      ...
      <p className="no-print text-[10.5px] text-muted-foreground">{tri("Le dosi sono quelle originali della ricetta. Per un'altra scala, apri la ricetta e stampala da lì.", "Die Mengen sind die Originalmengen. Für einen anderen Maßstab öffne das Rezept und drucke es dort.", "Quantities are the recipe's originals. For another scale, open the recipe and print from there.")}</p>
    </div>
  );
}
