  ...
  return (
    <div data-testid="valigia-page" className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="rounded-2xl border border-border bg-card p-4 space-y-2">
        ...
        <button data-testid="valigia-pick" onClick={() => fileRef.current && fileRef.current.click()} className="inline-flex items-center gap-1.5 text-[12px] font-bold px-3 py-2 rounded-xl border border-border bg-background text-foreground active:scale-95"><Upload className="w-4 h-4" />{tri("Scegli il file", "Datei wählen", "Choose the file")}</button>
        {preview && (
          <div data-testid="valigia-preview" className="rounded-xl border border-salvia/40 bg-salvia/8 p-3 space-y-1.5">
            <p className="text-[12.5px] text-foreground">{tri(`Valigia del ${preview.exported ? fmtDateLong(new Date(preview.exported), lang) : "?"}: ${preview.keys.length} cose.`, `Koffer vom ${preview.exported ? fmtDateLong(new Date(preview.exported), lang) : "?"}: ${preview.keys.length} Dinge.`, `Suitcase from ${preview.exported ? fmtDateLong(new Date(preview.exported), lang) : "?"}: ${preview.keys.length} things.`)}</p>
            <div className="flex gap-1.5">
              <button data-testid="valigia-import" onClick={importAll} className="text-[12px] font-bold px-3 py-2 rounded-xl bg-salvia text-white active:scale-95">{tri("Riapri qui", "Hier auspacken", "Unpack here")}</button>
              <button data-testid="valigia-cancel" onClick={() => setPreview(null)} className="text-[12px] font-semibold px-3 py-2 rounded-xl text-muted-foreground">{tri("Annulla", "Abbrechen", "Cancel")}</button>
            </div>
          </div>
        )}
      </div>
      <p className="text-[10.5px] text-muted-foreground flex items-start gap-1.5"><ShieldCheck className="w-3.5 h-3.5 shrink-0 mt-0.5" />{tri("Il file contiene solo le tue impostazioni e i tuoi appunti, con il soprannome se l'hai messo. Niente password, niente dati del server. Tienilo dove tieni le cose tue.", "Die Datei enthält nur deine Einstellungen und Notizen, mit Spitzname, falls gesetzt. Keine Passwörter, keine Serverdaten. Bewahre sie dort auf, wo du deine Sachen aufbewahrst.", "The file holds only your settings and notes, with the nickname if you set one. No passwords, no server data. Keep it where you keep your things.")}</p>
    </div>
  );
}
