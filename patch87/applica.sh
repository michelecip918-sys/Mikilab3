#!/bin/sh
# MikiLab v87 (include v84, v85, v86). Solo frontend. Uso dalla radice del progetto:  sh patch87/applica.sh
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"
if [ ! -d backend ] || [ ! -d frontend ]; then echo "ERRORE: lancia lo script dalla radice del progetto."; exit 1; fi
cp -R "$HERE/files/." .
rm -rf capacitor-plugins
for sec in 13 14 15 16; do
...
for f in FestaLancio Volantino AscoltaCrosta SvegliaPanettiere PaneDelPaese SenzaBilancia CuraLievito MioLievito MappaForno PrimoGiro LancioSegreto BancoProve MioForno; do
  test -f "frontend/src/components/$f.jsx" || { echo "ATTENZIONE: manca $f.jsx"; exit 1; }
done
test -f frontend/src/lib/recipeSeo.js || { echo "ATTENZIONE: manca lib/recipeSeo.js"; exit 1; }
...
echo "OK: v87 applicata. Ora: cd frontend && yarn build (deve riuscire), poi un solo commit e un solo deploy. Poi reinvia la sitemap in Search Console."
