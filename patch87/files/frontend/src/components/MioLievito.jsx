      ...
      {cur && !creating && (
        <>
          ...
          <section data-testid="lievito-traguardi" className="rounded-2xl border border-border bg-background p-4">
            ...
            <div className="mt-3 flex gap-2 flex-wrap">
              {!cur.firstBread && <button onClick={() => update({ firstBread: new Date().toISOString().slice(0, 10) })} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-ambra/20 border border-ambra/50 text-xs font-bold text-foreground active:scale-95"><Cake className="w-3.5 h-3.5 text-ambra" />{tri("Oggi ha fatto il primo pane!", "Heute das erste Brot gebacken!", "Made its first bread today!")}</button>}
