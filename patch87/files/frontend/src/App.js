  ...
  return (
    <ProfileProvider><AmbientProvider><TimerProvider><SoundFXProvider>
      ...
      <div className="holo-root min-h-screen font-sans selection:bg-muted selection:text-foreground">
        ...
        {festaOpen && <FestaLancio onClose={() => { setFestaOpen(false); try { localStorage.setItem("mikilab_festa_vista", "1"); const u = new URL(window.location.href); if (u.searchParams.has("festa")) { u.searchParams.delete("festa"); window.history.replaceState(window.history.state, "", u.toString()); } } catch { /* */ } }} onNav={navFromHome} />}
