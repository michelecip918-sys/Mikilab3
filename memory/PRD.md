...
## v69 (2026-09) — REPORT FINE TURNO AUTOMATICO (Sitor compila, il Capo approva)
- **Auto-compilazione**: appena un operaio invia il suo rapporto (`POST /api/floor/shift-report`), Sitor raccoglie da solo i dati reali del giorno e rigenera la bozza in background (`asyncio.create_task(_auto_shift_draft)`), nella lingua dell'ultimo rapporto.
- **Dati reali aggregati** (`_shift_snapshot`): rapporti operai del giorno (pezzi, scarti, problemi, note, pulizia), timbrature tamper-proof `compliance_timelog` → ore effettive/pause per operaio (`_arbzg_summary`, con flag ArbZG), obiettivi reparto (fatto/target), proposte di modifica degli operai.
- **Bozza AI (Claude Opus 4.8)**: `_sitor_shift_draft` genera il report in markdown con struttura obbligatoria (Sintesi / Ore effettive / Produzione / Problemi / Pulizia / Consiglio di Sitor), senza inventare numeri; fallback deterministico `_draft_fallback_text` se l'LLM non risponde. Collezione `sitor_shift_drafts` (bozza per giorno, status draft→approved).
...
## v72 (2026-09) — Ricettario UNIFICATO + home Master semplificata + metodo esteso
- **Un solo ricettario**: il Capo (admin) vede SEMPRE insieme ricette MikiLab + proprie ricette personali nel Master Ricettario (rimosso il toggle "Mostra anche le Mie", include_mine sempre attivo per il Capo). Badge ambra **"Mia"** (`recipe-mine-badge-<id>`) sulle card delle ricette personali.
- **Un solo modo per inserire**: "Aggiungi ricetta" apre il form con scelta in cima (`recipe-collection-choice`): **Ricetta MikiLab (condivisa)** vs **Ricetta mia (privata)** → `recipe-coll-mikilab` / `recipe-coll-personal`. RecipeDialog prop `collectionChoice` (solo in creazione); handleSave rispetta `payload.collection_name`. Backend già pronto (owner_id su personal). Testato via curl: creata personale dal flusso unico → appare nella master list con coll preservata (poi eliminata).
- **Home Master semplificata** (`Ricette.jsx`): rimossi la barra dei 6 pulsanti in alto e il "Sapore del Giorno". Resta: titolo + lista + Aggiungi. Gli extra (Novità MikiLab, Sapori di Casa, Custodite, Focacce, Enciclopedia, Farine, Backup) spostati IN FONDO sotto "Strumenti del ricettario".
...
## v-fork2 (2026-06-11) — 4 funzioni in sequenza (a→d), testate E2E 100%
- **(a) Sezioni Più Chiare**: `CapoGroup` ha una nuova prop `hint` → riquadro "cosa fa / quando usarlo" (data-testid `capo-group-hint-<id>`) mostrato in cima al corpo di ogni gruppo. Testi chiari IT/DE/EN/ES/FR/FA per piano, ordini, squadra, strumenti, sitor.
- **(b) Freezer nel Piano AI**: `AutoPlanReq` ora accetta `machines` e `freezer_stock` (prima ignorati per campi extra). `AutoPlan.jsx` legge `freezerApi.get()` e passa le giacenze a `/api/mike/autoplan` e `/api/mike/autoplan/options`. Helper `_autoplan_freezer_ctx()` → Sitor usa PRIMA il congelato e produce solo la differenza (verificato: "200 baguette → 120 dal freezer + 80 fresche"). Nota UI `autoplan-freezer-note`. FIX bug latente: `body.machines` ora funziona davvero (prima `getattr` restituiva sempre None).
- **(c) Tour Guidato Sitor**: nuovo `SitorTour.jsx` — overlay vocale a 6 step (welcome + 5 gruppi) che apre ogni `CapoGroup` (evento `mikilab:open-group`), legge la spiegazione via TTS, con dot di avanzamento, mute, Salta/Indietro/Avanti/Fine. Auto-avvio 1 volta per Capo (localStorage `mikilab_sitor_tour_done`); pulsante header `sitor-tour-open` per rivederlo.
- **(d) Semaforo Celle Live**: in `ColdStorage.jsx` scheda Celle, ogni cella ha un pallino di stato (`cold-cell-status-<dept>-<i>`) calcolato ONESTAMENTE da letture reali `/lab/sensors` (verde in norma / giallo da controllare / rosso fuori norma / grigio nessun sensore) + legenda `cold-legend`. Aggiunta una cella di tipo "Freezer" a ogni reparto nel catalogo `DEPARTMENTS` così il semaforo ha un bersaglio reale.
- Testato iteration_230: backend 100%, frontend 100%, 0 bug. Test: /app/backend/tests/test_iter230_autoplan_freezer_sensors.py.

## v-fork3 (2026-06-11) — Tour per Reparto + Freezer Auto-Scala dal Piano
- **Tour per Reparto**: `SitorTour.jsx` ora accetta `variant` ("capo"|"floor"). In Produzione parte un tour a 5 tappe dedicato all'operaio (postazione, compito di oggi, lista lavori, foto & aiuto, fine turno) che scorre agli elementi reali (floor-my-assignment, floor-day-tasks, floor-analyzer, floor-endshift). Auto-avvio 1 volta (localStorage `mikilab_floor_tour_done`), riavviabile dal pulsante header `floor-tour-open` (evento `mikilab:start-floor-tour`). Chiavi/eventi separati dal tour Capo. Isolamento Floor confermato.
- **Freezer Auto-Scala dal Piano**: `/api/mike/autoplan/dispatch` ora scala automaticamente le giacenze freezer dei prodotti usati (match per token del nome, ignorando descrittori generici tipo "semilavorate"). Ritorna `freezer_scaled` [{name, used, left}]. `AutoPlan.jsx` mostra un toast "🧊 Freezer aggiornato: …" e ricarica le giacenze. Verificato: "Baguette Tradizione 50" → freezer 120→70; "Focaccia" non tocca il freezer.
- Testato iteration_231: backend 100%, frontend 100%, 0 bug. Test: /app/backend/tests/test_iter231_freezer_autoscale.py. Match token migliorato dopo il report (advisory risolto).

## v-fork4 (2026-06-13) — Ingresso Capo/Operaio + voce, Sitor fluttuante snellito, Operaio senza piano
- **Ingresso Capo/Operaio (PublicGate)**: due grandi pulsanti "Sono il Capo" / "Sono un Operaio" (public-enter-capo / public-enter-operaio) + hint. Voce di benvenuto una volta per sessione (sessionStorage mikilab_welcome_spoken, rispetta mute) che invita a scegliere e accompagna l'operaio in produzione. AdminGate riceve prop `role`: header "PRODUZIONE · PIN 4 cifre" per l'operaio, "Capo · PIN 6 cifre" per il capo. Keypad invariato. VERIFICATO (screenshot mobile+desktop + testing_agent iter232).
- **Sitor fluttuante snellito (MikeMixSense)**: tutti gli strumenti Capo legacy (Audit, Linea/Pipeline, Vision AR, Clima, Inventario foto, Delega, Handoff+storico, Cella/Freezer, Batch Phoenix, Inviti, slider vetro/soglia, ShiftPowerBoard, Organico, Riposo, Sveglia) ora collassati dietro il toggle `mikemix-tools-toggle` (default nascosto). Di default il pannello mostra solo chat + avvisi + sensori + briefing + check-in. Codice verificato da testing_agent (live bloccato da flakiness OAuth/gate reload in automazione).
- **Audit Ricetta incorporato nelle Ricette**: nuovo `RecipeAuditButton` (ricette-audit-btn) montato nel pannello panel-ricette sopra il ricettario — è il suo contesto naturale. Codice verificato.
- **Operaio senza piano (FloorOpeningChecklist)**: quando non c'è piano/assegnazione, la Produzione mostra una checklist di apertura standard per reparto (5 voci con 🔊 e spunta) + pulsante "Chiedi a Sitor cosa fare" (deusApi.ask). VERIFICATO live (screenshot: floor-no-task + floor-opening-checklist + 5 item + ask-btn).
- Testato iteration_232: flussi diretti VERIFICATI; lato Capo autenticato verificato per ispezione del codice (harness non completa OAuth/gate).
- BACKLOG richiesto dall'utente per prossimi step: (P1) consolidare la console del Capo in 6 gruppi chiari (Oggi/Home notizie · Produzione · Ricette & Magazzino · Celle & Forni · Squadra · Sicurezza & Report) con funzioni distinte e non sovrapposte; (P2) rientro operaio con nome/riconoscimento facciale (FaceCheckIn già presente) al posto del PIN dalla seconda volta; (P2) spostare fisicamente gli altri strumenti legacy del fluttuante nelle sezioni giuste (non solo collassarli).

## v-fork5 (2026-06-13) — Console del Capo consolidata in 6 gruppi
- Da 5 gruppi (piano/ordini/squadra/strumenti/sitor, ~20 pannelli) a **6 gruppi chiari**: **Oggi · Sala Sitor** (SalaSitor + NexusConsole + Sitor su misura = punto d'incontro e notizie), **Produzione · Piano & Ordini** (weekly, pianoai, ordini extra, pasticceria, B2B), **Squadra**, **Ricette & Magazzino** (ricette+audit, thermal flow, magazzino, food cost, living recipe), **Celle & Forni** (ovenQC, cold chain, hardware/PLC, nuovi macchinari), **Sicurezza & Report** (report/PDF/MikiScore, sicurezza/PIN, emergenze).
- Default di apertura ora "oggi". CONSOLE_SECMAP aggiornato ai nuovi id. SitorTour CAPO_STEPS rimappato ai nuovi gruppi (produzione/ricette/squadra/celle/sicurezza).
...
## v-fork22 (2026-06-14) — Piano UNICO + pannello finto rimosso (richiesta utente)
...
- **Backend** (`server.py`): `AutoPlanReq.activity` + `_activity_plan_profile()`. `/api/mike/autoplan/options` costruisce un system prompt specifico per attività → 3 strategie/linee diverse: panificio (a catena, celle grandi, forni; Velocità/Qualità/Risparmio), pizzeria (flusso continuo/su richiesta, più macchine, celle piccole; Servizio continuo/Alta qualità impasto/Turno compatto), pasticceria (su commessa/eventi, a ritroso dalla consegna, abbattitore; Per consegne ed eventi/Precisione e abbattimento/Ottimizza commesse). Verificato via curl: 3 attività = 3 set distinti di label/linee.
- **Frontend** (`lib/activityProfile.js` NUOVO): profilo condiviso (paradigm, planHint, orderPlaceholder, toolsFocus, publicBlurb, publicFeatures) i18n a 6 lingue.
  - `PianoUnico.jsx`: prop `activity` (reattiva al selettore), banner `piano-activity-banner`/`piano-activity-label`, placeholder ordini per attività, passa activity a autoplanOptions.
  - `App.js`: `<PianoUnico activity={activity}/>`; banner `strumenti-activity-banner` con 3 tool prioritari per attività nella sezione Strumenti.
  - `PublicGate.jsx`: sotto i tab mondo, blocco `public-world-content` (blurb + 3 feature) reale per panificio/pizzeria/pasticceria; nascosto per il mondo Magazzino/banco.
- Testato: iteration_237 (banner reattivi OK, label pizzeria OK) + screenshot PublicGate (3 mondi distinti). Nota: 2ª generazione LLM consecutiva può andare in timeout (~70s) per latenza — non è un bug di codice (panificio confermato via curl). Possibile overlay del tour onboarding di Sitor da tenere d'occhio (pre-esistente).

## v-fork24 (2026-06-14) — Cleanup preciso: file inutilizzati + rimozione HACCP + tono professionale
- **Punto 1**: eliminati 4 file non usati — `sections/WeeklyPlan.jsx`, `BackwardScheduler.jsx`, `SmartPlannerStressZero.jsx`, `PianoProduzioneAI.jsx` (+ rimossi i 4 import orfani in App.js). Nessun import residuo, webpack compila.
- **Punto 2 — HACCP rimosso**: `operations.py` → cancellate rotte `/haccp-logs` (GET/POST/DELETE) e classe `HaccpLogReq` (+helper `_haccp_public`). In `/day-close` tolta la creazione voci HACCP (temperature/sanificazione); MANTENUTI scarico magazzino e archiviazione chiusura. Risposta senza più `haccp_created`. `api.js` → tolto `haccpApi`. `toolGuide.js` → tolta voce `haccp`. Rotte totali 489→486. NOTA: restano due stringhe di testo "HACCP" nella feature day-close mantenuta (titolo PDF chiusura) — fuori scope richiesto.
- **Punto 3 — tono professionale PublicGate**: welcome TTS non chiede più "Capo o operaio?" ma propone Direzione (piano produzione) / Produzione (reparto). Bottoni ingresso rinominati "Direzione"/"Produzione" (i18n: Direction/Production…). Titolo "Capo Supremo" tolto da sotto l'avatar MikiLab (resta solo il nome); il sottotitolo ora è condizionale. Lasciato intatto il testo guest di linea 185 (non richiesto).
- Verificato: backend /day-close ok senza haccp_created, /haccp-logs → 404; frontend compila; screenshot PublicGate mostra Direction/Production e avatar MikiLab senza sottotitolo.

## v-fork25 (2026-06-14) — Collegamento macchine reparto + dashboard Capo + ispezione + voce
- **PDF chiusura**: rimosso "· HACCP" dal titolo del registro chiusura turno (operations.py L762).
- **Punto 1 (macchine operaio)**: backend `POST/GET /api/depts/{dept}/machines` (stato: attiva/in manutenzione/spenta + valore opz. es. temperatura), collezione `dept_machines`, id validati dal catalogo DEPARTMENTS. `SharedWidgets.jsx` ora scrivibile: sotto-componente `DeptMachines` (select stato + input valore + Salva) per l'operaio; read-only per il Capo. `FloorOperatorDay` passa deptKey/operator.
- **Punto 2 (dashboard Capo)**: `GET /api/depts/machines/overview` + nuovo `components/console/CapoMachinesOverview.jsx`; HoloPanel `panel-machines-overview` nella sezione Piano mostra card per reparto con chip macchine colorate + conteggio attive, refresh 20s.
- **Punto 3 (ispeziona reparto)**: dal pannello overview il Capo sceglie un reparto (select o bottone card) e apre la stessa schermata operaio in SOLA SUPERVISIONE (modale, `FloorOperatorDay superviseDept=...`, banner `supervise-banner`, DeptMachines read-only). Nessun PIN/nome operaio richiesto.
- **Punto 4 (voce)**: all'ingresso dell'operaio, oltre al compito, Sitor annuncia lo stato delle macchine collegate del reparto (es. "Forno a piani attiva, 220°C").
- api.js: `deptApi.machinesGet/machinesSet/machinesOverview`. Rotte 486→489.
...
- **3 · PIN operatore legato all'azienda**: il token del cancello ora porta `org` (`issue_gate_token(ttl, org)`); helper `gate_org(request)` + dependency `effective_org` (sessione se loggato, altrimenti org dal cookie gate). Endpoint `/depts/*` gate-only ora usano `effective_org`. `admin-gate/verify` rilascia il cookie con l'org del PIN operatore/produzione (default org_default per master/legacy). PIN operatore e production_pin stampano `organization_id` alla creazione. Verificato: nuova azienda isolata (0 reparti/0 ricette).
- **4 · Scorciatoia corso da ogni lotto del piano**: pulsante `piano-batch-course-<day>-<i>` in `PianoUnico`/DayPlan → risolve prodotto→ricetta e apre `piano-course-modal` con il corso passo-passo. PASS test.
- **FIX CRITICO multi-tenant**: `auth.py` register — il proprietario di una NUOVA azienda (ramo codice attivazione) ora è `role="admin"` della propria azienda (prima restava `user` e non poteva accedere alla console Capo, bloccando la feature 1). Verificato via /auth/me → role=admin.
- **Testato**: iteration_243.json → Feature 4 PASS, Feature 2 PASS, Feature 1 sbloccata dopo fix ruolo (backend curl-verificato), Feature 3 curl-verificata. Regressione OK. Puliti gli account/ricette di test.

---
## Changelog — 15 Set 2026 (2 punti finali di isolamento)
- **PIN operatore isolati per azienda**: `operator_pin_list/del/level/renew` ora filtrano per `organization_id=_org_id(admin)`; `operator_pin_set` include l'org anche nella chiave di upsert (due aziende possono avere operai omonimi). `operator_pins` aggiunto a `ORG_SCOPED_COLLECTIONS` → migrazione startup stampa `org_default` sui PIN esistenti (verificato: il Capo vede ancora i suoi 3 PIN: Marco, Sara, Youssef). `operator_pin_verify` resta volutamente globale: è pre-login, è il PIN stesso a determinare l'azienda (già portata nel cookie gate dalla sessione precedente).
- **Proprietà ricetta cross-azienda**: `PUT`/`DELETE /recipes/{id}` ora verificano che una ricetta mikilab appartenga all'azienda di chi chiede (`organization_id`, default org_default per legacy) → 404 se altra azienda; personal già protette da owner_id. Verificato via curl: cross-org DELETE → 404; own DELETE → 200; org_default admin PUT → 200.
- **Verifica**: solo curl (backend), frontend non toccato. Puliti gli account di test.

---
## Changelog — 14 Set 2026 (Sessione: isolamento ricette + reparti completo)
- **A · Ricette isolate per azienda**: `create_recipe` ora stampa `organization_id=_org_id(user)` su OGNI ricetta (anche mikilab); `get_recipes` (ramo mikilab) filtra per `organization_id=_org_id(user)`; `seed_mikilab_if_empty` stampa `organization_id=ORG_DEFAULT`. Migrazione startup già stampa org_default sulle ricette esistenti → org_default conserva tutto il catalogo (verificato: Capo vede 149/150, ospite/operaio 150), una NUOVA azienda parte con ricettario proprio (vuoto). Scelto il filtro `organization_id` invece di rinominare `collection_name` (evita di toccare 25 punti fragili e i falsi positivi come `method="mikilab"` della generazione ricette). Nota: il modello `Recipe` non espone `organization_id` in risposta (è solo filtro server).
- **B · Reparti isolamento completo**: aggiunto filtro `organization_id` alle query residue su `dept_assignments`/`dept_objectives` prima non isolate — `_bakery_snapshot(admin)` (contesto Sitor), `_shift_snapshot(today, org)` + `_sitor_shift_draft(..., org)` (report turno, org passato dal chiamante admin, default org_default per auto/scheduled), `depts_history(admin)`, `depts_shift_report(admin)`. Tutti gli endpoint `/depts/*` ora coerenti. Verificato 200 OK su history/shift-report/board/proactive.
- **C · Corso ricette in produzione**: GIÀ implementato (sessione precedente) — sezione `floor-recipes-toggle`/`floor-recipes-panel` in `FloorOperatorDay.jsx` con `<RecipeList readOnly/>` e pulsante Corso. Confermato presente, nessuna modifica necessaria.
- **Verifica**: solo curl (backend) per risparmiare crediti; frontend non modificato in questa sessione. Nessun impatto su piano 7 giorni, voce unica Sitor, colore Sitor, differenziazione attività, macchine reparto operaio.
...
- **Punto 1 — Isolamento dati multi-azienda (session-based)**: `lab_warehouse` aggiunto a `ORG_SCOPED_COLLECTIONS` (migrazione a `org_default` allo startup → nessuna perdita dati). `GET/POST/DELETE /api/lab/warehouse` ora filtrano/stampano `organization_id=_org_id(user)`. Fix leak in `get_recipes`: le ricette PERSONALI dell'owner/admin iniettate nel Master ora sono filtrate per `organization_id` dell'utente corrente → una nuova azienda vede il Master di Michele condiviso ma NON le ricette personali di org_default. Scelta utente: Master di Michele CONDIVISO, isolate solo le ricette create da ciascuna azienda. `favorites`/`day_closures` già isolate per user_id/owner_id.
- **Punto 2 — Autonomia reparti macchine**: già implementato in sessione precedente e verificato non rotto — `FloorOperatorDay.jsx`→`SharedWidgets.jsx` (operaio scrive stato macchine del proprio reparto via `deptApi.machinesGet/Set`), `CapoMachinesOverview.jsx` (overview Capo via `deptApi.machinesOverview`, montato in App.js).
- **Punto 3 — Corso ricette con Sitor**: nuovo endpoint `GET /api/recipes/{recipe_id}/course?lang=` — genera UNA volta una spiegazione didattica estesa passo-passo (title/intro/phases[]/tips[]) via Claude, la salva in modo permanente in `db.recipe_courses` (chiave recipe_id+lang) e nelle chiamate successive la restituisce dalla cache (~0.8s). NON tocca mai ingredienti/dosi. Frontend: pulsante `course-recipe-<id>` (GraduationCap) nella barra azioni di `RecipeDetail` (RecipeList.jsx) → pannello inline `recipe-course-panel-<id>` con fasi numerate + consigli. Visibile a Capo e operai (stesso componente read view). `recipesApi.course` in api.js. Testato: prima generazione ~32s, poi cache <1s.
- **Punto 4 — Responsive mobile**: header non spezza più il testo dei pulsanti a metà parola. `PublicGate.jsx` header ora `flex-wrap` con `whitespace-nowrap shrink-0` sui pulsanti e tracking brand ridotto su mobile; `index.css` regole `header[data-testid=app-header]` mobile (nowrap pulsanti, tracking brand, row-gap). Verificato a 390px: 0 overflow orizzontale, pulsanti interi a capo come blocchi.
...
---
## Changelog — 15 Set 2026 (Comando Capo a Voce · Badge Risparmio · Cache Estrazioni) — COMPLETATO
- **Comando Capo a Voce**: in Sala Sitor (Console → tab Sitor) nuovo pulsante teal `sitor-delegate-btn` apre la Delega Vocale. In `VoiceDelegation.jsx` un comando diretto tipo «Sposta Sara ai forni» (regex MOVE_RE) chiama subito `POST /api/worker/capo-move` con PRIORITÀ ASSOLUTA (bypassa l'analisi IA e mostra conferma `capo-move-done`); qualunque altra frase resta la delega normale (`/api/delegation/parse`). Verificato end-to-end (iteration_247).
- **Badge Risparmio**: nuovo `CreditSavingsBadge.jsx` sempre visibile in Sala Sitor; legge `GET /api/ai/savings` (contatori mensili `ai_usage`: fast_calls, brain_calls, cache_hits) e mostra `-NN%` risparmiato + chip modello economico/cache. Backend traccia in `_track_saving` (await, non fire-and-forget). Verificato: -64% con 4 fast + 2 cache.
- **Cache Estrazioni**: memoization estesa a `delegation_parse` (max_tokens alzato a 1200 per evitare JSON troncato su Haiku) e a `_extract_order` (recipes.py): input identici → risposta riusata senza chiamare l'IA. Verificato: 2ª parse identica 158ms vs 7183ms.
- NOTA tecnica: il pannello di `MikeMixSense.jsx` è dead-code dietro `{false && (` — per questo i due elementi sono stati collocati in `SalaSitor.jsx` (raggiungibile). MikeMixSense non è stato modificato.
- File: `backend/server.py` (_track_saving async, /ai/savings, delegation_parse memo+1200tok, capo-move già presente), `backend/recipes.py` (_extract_order memo), `frontend/src/components/console/SalaSitor.jsx` (delegate btn + badge + modale), `frontend/src/components/VoiceDelegation.jsx` (comando move), `frontend/src/components/CreditSavingsBadge.jsx` (nuovo), `frontend/src/lib/api.js` (aiApi.savings).

---
## Changelog — 15 Set 2026 (Plancia Operatori · Storico Risparmio · Comando Voce Reale) — COMPLETATO
- **Plancia Stato Operatori**: nuovo `GET /api/worker/board` (unisce pool turno + worker_states + titolo task/ETA) e componente `OperatorStatusBoard.jsx` in Sala Sitor: elenco live di tutti gli operatori con pallino verde=libero/oro=occupato, compito corrente, ETA e badge DIREZIONE per chi è bloccato dal Capo. Aggiorna ogni 15s (consultabile offline). Verificato: 5 operatori, 4 liberi/1 occupato.
- **Storico Risparmio**: nuovo `GET /api/ai/savings/history` (ultimi N mesi da ai_usage) + mini grafico a barre mensile nel `CreditSavingsBadge` (compare con ≥2 mesi di dati; con 1 mese resta nascosto — atteso).
- **Comando Voce Reale**: in `VoiceDelegation.jsx` il microfono ora, a fine dettatura (`rec.onend`), esegue subito il comando di spostamento se la frase combacia (moveHandlerRef → tryCapoMove) — mani libere; le altre frasi restano da confermare con Analizza. Istruzione mic aggiornata.
...
---
## Changelog — 16 Set 2026 (Chiusura progetto — Punti 1-5)
### P1 Pulizia: eliminati /lab/departments (GET/POST/DELETE + /feature) in operations.py + droppate collezioni lab_departments, lab_dept_features. TENUTO /depts/templates* e dept_shift_templates (VIVI: usati da ShiftTemplates.jsx montato in App.js) — deviazione segnalata.
### P2 Centro Coordinamento: pannello unico Capo TeamCoordination (abilitazioni multi-reparto, presenza manuale+auto, soglie, chiamate/proposte, registro). Aggiunto LIMITE giornaliero richieste vocali per operatore (coordination_settings.voice_daily_limit + /coordination/voice-quota + collezione voice_usage) con enforcement in HeadphonesMode (oltre soglia Sitor tace, eventi critici bypassano). Audio hands-free operatore (PIN+wake-word+chiamate si/no) gia collegato. NON fatto: foto on-demand Sitor guarda qui, bridge audio nativo Bluetooth completo.
### P3 Verifica 4 feature: coordinamento auto (testato iter255), autisti/vista giro (iter255), avatar MikiLab (fatto) — tutti presenti e funzionanti.
...
### P5 Isolamento: TUTTE le collezioni elencate aggiunte a ORG_SCOPED_COLLECTIONS -> migrazione automatica backfill organization_id=org_default (verificato 0 doc senza org). Isolamento a livello di QUERY completo per: recipes, weekly_plan, inventory_items, favorites, day_closures, lab_warehouse, operator_pins, worker_states, deliveries, coordination_*. Le altre ~30 collezioni sono migrate (backfill) ma le loro query read/write vanno ancora filtrate per org (PENDENTE, da fare incrementale).
### TEST iter256: backend 9/9 PASS, frontend 100
---
## Changelog — 16 Set 2026 (Chiusura progetto — Punti 1-5)
### P1 Pulizia: eliminati /lab/departments (GET/POST/DELETE + /feature) in operations.py + droppate collezioni lab_departments, lab_dept_features. TENUTO /depts/templates* e dept_shift_templates (VIVI: usati da ShiftTemplates.jsx montato in App.js) — deviazione segnalata.
### P2 Centro Coordinamento: pannello unico Capo TeamCoordination (abilitazioni multi-reparto, presenza manuale+auto, soglie, chiamate/proposte, registro). Aggiunto LIMITE giornaliero richieste vocali per operatore (voice_daily_limit + /coordination/voice-quota + collezione voice_usage) con enforcement in HeadphonesMode (oltre soglia Sitor tace, eventi critici bypassano). NON fatto: foto on-demand "Sitor guarda qui", bridge audio nativo Bluetooth completo.
### P3 Verifica 4 feature: coordinamento auto (iter255), autisti/vista giro (iter255), avatar MikiLab — presenti e funzionanti.
### P4 Sitor Apprendista: /apprentice/recipe/{id} GET/PUT (recipe_apprentice, org-scoped) — pezzi per teglia + come formare, scritti dal Capo, letti/parlati da Sitor. Card ApprenticeCard nel dettaglio ricetta. NON tocca ingredienti/dosi (verificato).
...
---
## Changelog — 16 Set 2026 (P0 Isolamento multi-tenant COMPLETATO)
- Completato l'isolamento a livello di QUERY per tutte le collezioni aziendali rimaste: 115 sostituzioni esatte (script con assert, poi rimosso) in server.py (91), deck.py (16), sitor_ai.py (8).
- Collezioni chiuse: mike_machines, floor_change_requests, dept_shift_templates, capo_queue, floor_shift_reports (_shift_snapshot), lab_pulse_history, lab_sites (seed per-org + 7 rotte enterprise), lab_sensors, batch_links, lab_shift_plan (letture deck + _worker_pool), team_tasks (worker/coordinamento/delegation/briefing/timeline/shift-report), dough_batches, lab_consumption_log, deck_alarm_history, compliance_timelog (incl. catena hash per-org), compliance_training_ack, sos_events (raise/list/ack/history/telemetry/challenge/report), mikiscore_history, b2b_orders, lab_warehouse (mike_proactive + briefing floor).
- Pattern: rotte admin -> `_org_id(admin)`; rotte floor/pubbliche -> `org: str = Depends(effective_org)` (sessione o cookie gate). Scritture, conteggi e prune filtrati come le letture (coerenza: nessun dato nuovo invisibile).
- Collezioni GLOBALI (condivise, non toccate): news_items, newsletter_*, push_subs, access_requests/invites, entitlements, silos, oven_profiles, team_faces, recipe_temps, social_clicks, uptime_checks, security_log, shift_handoffs.
- RESIDUO noto (bassa priorita'): documenti singleton "_key" di stato live (lab_shift_state, floor_plan, lab_checkin, lab_rest_mode, lab_seq_block, lab_sensors_live, lab_staffing/lab_absences) e stores (owner_id-keyed) non ancora org-scoped nelle query interne org-less (_compute_pulse, deck_status_compute, _staffing).
- TEST iter257: backend 30/30 PASS (suite riutilizzabile /app/backend/tests/test_iter257_multitenant_isolation.py). Isolamento cross-tenant verificato con 2 aziende reali (marker invisibile alla seconda org su 11 endpoint; conteggi macchine separati). Nessun 500. Ricette invariate.

---
## Changelog — 16 Set 2026 (Stato Live per Azienda + Multi-Azienda + Inviti Operaio via Link)
### Stato Live per Azienda (Part A)
- Isolati i documenti singleton di stato live con chiave composita {_key, organization_id}: lab_shift_state, floor_plan, lab_checkin, lab_rest_mode, lab_seq_block, lab_sensors_live, lab_staffing, lab_absences, lab_wake, + shifts nel deck_status_compute. 40 sostituzioni in server.py/deck.py/recipes.py/community.py/operations.py. Helper resi org-aware: _compute_pulse(org), deck_status_compute(org), _staffing(org), _load/_save_batches(org). lab_wake aggiunto a ORG_SCOPED_COLLECTIONS (backfill org_default). Rotte floor/pubbliche usano effective_org (sessione o cookie gate).
### Multi-Azienda (Part B) — nuovo modulo backend orgs.py
- Collezione `organizations` {org_id, name, created_by} + `memberships` (array) sull'utente. Endpoints: GET /api/orgs (Owner vede TUTTE, altri le proprie), POST /api/orgs (crea + membership), POST /api/orgs/switch (persiste users.organization_id = org attiva; owner ovunque, altri solo membership → 403), PATCH /api/orgs/{id} (rinomina). Frontend: OrgSwitcher.jsx nel menu account del Capo (crea/rinomina/switch, ricarica al cambio).
...
---
## Changelog — 22 Set 2026, Riordino silos trascinabile
...
---
## Changelog — 22 Set 2026, sistemazione finale (reparti + Sitor unico + pulizia)
Verificato: testing_agent iteration_263 (retest_needed:false, nessun issue) + curl + screenshot.
1. **Reparti gestibili dal Capo** (ibrido come macchine/silos): server.py — `dept_config` per-org con `overrides` (name/icon/accent/hidden) + `custom`. Helper `_depts_merged/_dept_cfg/_dept_exists/_dept_name/_base_machines/_dept_label`. `/depts` ora per-org. Nuovi: POST `/depts` (crea custom), PATCH `/depts/{key}` (rinomina/icona/colore base via override o custom), DELETE `/depts/{key}` (base→nascondi, custom→rimuovi), POST `/depts/{key}/restore`, GET `/depts/hidden`. Tutte le funzioni macchine/assegnazioni/coordinamento aggiornate ad accettare reparti custom (guardie `not in DEPARTMENTS and not startswith('dept-')`, `_base_machines`, `_dept_label`). Frontend: nuovo `DeptManager.jsx` (panel-departments) con rinomina inline, ciclo icona, palette colori, nascondi/ripristina, aggiungi/elimina; MyMachines ascolta `mikilab-depts-updated`. api.js: deptApi.deptCreate/deptEdit/deptDelete/deptRestore.
2. **Sitor unico ovunque**: nuovo componente `SitorAvatar.jsx` (immagine ufficiale unica `sitor_official.jpg`). I 6 componenti legacy (LivingAvatar3D, AvatarWorld3D, NexusAvatar, MikiAvatar/HeroAvatar) riscritti come wrapper sottili di SitorAvatar. Tutte le `<img>` Sitor (avatar_nexus/avatar_sitor/sitor-full/sitor-oven) sostituite con `sitor_official.jpg` in 11 file. NOTA: `avatar_miki.jpg` = Michele/Capo (identità distinta) lasciato invariato. Nessuna immagine rotta.
3. **Pulizia codice morto (prudente)**: rimossi 6 endpoint mai chiamati dal frontend — /academy/catalog, /academy/coach, /academy/my, /subscription/status, /trial/activate (server.py), /admin-gate/guest (auth.py). Tutti ora 404. MANTENUTO /mike/access-requests (PIN ospite). api.js lasciato intatto deliberatamente (funzioni dubbie → per istruzione utente "lasciale stare" e per non rompere flussi funzionanti; academyApi.quiz/badge/leaderboard/sos ancora attivi e usati).
- NB: in PREVIEW → deploy verso mikilab.de. Dati di test ripuliti.

---
## Changelog — 22 Set 2026, Sitor unico + pulizia deliveries/ceste
Verificato: curl (deliveries server.py attiva, PATCH→405, ceste dicitura rimossa) + screenshot (hub Sitor con 6 card + chat, nessun "· Sitor" residuo). Backend py_compile OK, frontend webpack OK.
1. **Sitor in un'unica sezione**: nella sezione "Sitor" (SecBlock id=sitor, ora titolo semplice "Sitor") aggiunto un `sitor-task-menu` con 6 card compito (Genera il piano, Chiudi la giornata, Organizza consegne, Coordina la squadra, Riconosci macchina, Strumenti su misura) che portano alle sezioni funzione esistenti (#capo-sec-piano/team/strumenti); chat SalaSitor sempre presente sotto. Rimossi i suffissi "· Sitor" dai 6 pannelli sparsi (Piano di Produzione, Chiusura Giornata, Consegne & Furgoni, Riepilogo Squadra, Strumenti su misura, Nuovi Macchinari) — le funzioni restano dove sono, solo l'interazione con Sitor è centralizzata. FAB "Chiedi a Sitor" invariato (link alla sezione).
...
---
## Changelog — Set 2026, Console a 6 macro-sezioni con tab + fix reparti live (P0 UI)
Verificato: testing_agent iteration_264 (47/50 verde, retest_needed:false) + self-test drawer (fix z-index) + deployment_agent PASS. Backend py_compile OK, webpack compila (solo warning eslint preesistenti).
1. **Console ridotta a 6 macro-sezioni con schede interne**: App.js — eliminati gli HoloPanel impilati nella console Capo; nuovi `SecTabs`/`TabPanel`/`PANEL_TAB` + stato `capoTab`. Sezioni: Ricettario (Ricette/Thermal/Magazzino/FoodCost), Piano (Piano/Chiusura/Consegne/Stato Macchine), Ordini (Extra/B2B + Pasticceria-Pizzeria condizionali), Team (Coordinamento/Assegnazione/Riepilogo/Turni-tipo/Volti), Strumenti (Macchine+Atelier+avanzati/Reparti/Silos/Nuovi Macchinari/Celle), Sicurezza (Report/Accessi/Emergenze). Solo la tab attiva è montata. I deep-link (mikilab:open-panel/open-group) attivano la scheda giusta via PANEL_TAB.
2. **Sezione Sitor eliminata**: spariti SecBlock sitor + sitor-task-menu + voce nav. Quick-chip nei reparti giusti: piano-action-generate/close/deliver (sezione Piano), team-action-coordinate (Team), strumenti-action-recognize (→tab Nuovi Macchinari) + strumenti-action-atelier (→embed SitorAtelier nella tab Macchine, id sitor-atelier-embed). TodayFeed spostato in cima alla console (oggi-feed). RadioFornaio dentro il drawer (oggi-radio).
...
---
## Changelog — Set 2026, Mono-azienda + Font self-hosted + Nota "non in vendita"
Verificato: curl (POST /orgs owner=200, admin non-owner=403 "Solo il proprietario..."; org di test pulite) + screenshot con ispezione rete (0 richieste a googleapis/gstatic, 7 file font locali /fonts/*.woff2 caricati, nota visibile nel footer pubblico). webpack compiled successfully.
1. **Mono-azienda** (backend/orgs.py `orgs_create`): POST /orgs ora risponde 403 "Solo il proprietario può creare nuove aziende" se `_is_owner(admin)` è falso (stesso controllo usato altrove in orgs.py). Registrazione con codice di attivazione non toccata. Verificato: owner crea (200), admin non-owner bloccato (403), nessuna org "Vietata" creata.
2. **Google Fonts self-hosted** (GDPR): scaricate 46 facce woff2 (Playfair Display, Manrope, JetBrains Mono, Vazirmatn incl. subset arabo, Orbitron, Rajdhani; subset latin+latin-ext) in `frontend/public/fonts/` con `fonts.css` generato da `/app/scripts/fetch_fonts.py`. `index.css`: rimossi i 3 @import Google (inlining @font-face non possibile: css-loader li risolve → fonts.css caricato via `<link rel="stylesheet" href="/fonts/fonts.css">` in `public/index.html`). Rimossi anche preconnect + link Inter da index.html. Self-hostata anche l'icona Google del login (`/brand/google.svg` in AuthScreen.jsx). Verificato: 0 richieste di rete verso Google, font renderizzati.
3. **Nota "non in vendita"**: `NotForSaleNote` in `PrivacyNotice.jsx` (testo IT/DE forniti + EN/ES/FR/FA), discreta (10px italic) sotto il link Privacy in `PublicGate.jsx` (footer) e `FloorOperatorDay.jsx` (fine pagina operatore). Verificato su schermata pubblica.
- NB: deploy su mikilab.de in coda.

---
## Changelog — Set 2026, Impressum completo + AR/TR 100% + rimozione "Condividi"
Verificato: JSON valido, webpack compiled successfully, screenshot (UI pubblica in arabo senza fallback EN, bottoni condividi assenti, Guida presente).
1. **Impressum** (frontend/src/sections/LegalPage.jsx): aggiunte costanti STREET="Stitzenburgstraße 15", POSTAL="70182 Stuttgart" (righe 12-13); nel blocco impressum aggiunte righe Attività ("Panettiere"/"Bäcker"…), via, CAP+Deutschland (righe ~94-96); RIMOSSO l'avviso data-testid="impressum-todo" "DA COMPLETARE".
2. **Traduzioni AR/TR 100%** (frontend/src/i18n/triTranslations.json): completate 2830 stringhe mancanti in arabo e turco via LLM (Claude haiku, script /app/scripts/translate_ar_tr.py, batch 30, salvataggio incrementale) + 1 stringa a mano ("\n\nGrazie!") + nota "non in vendita". Copertura finale ar/tr = 4202/4202 (100%), pari a fr/fa. Struttura invariata: triMaps.js legge DATA[k].ar/.tr con fallback a triExtraArTr.js.
3. **Rimozione "Condividi"** (frontend/src/components/PublicGate.jsx): rimossi i bottoni data-testid="public-share-btn" e "vetrina-share-btn" col testo "Fai conoscere il laboratorio", la funzione doShare/shareUrl e l'import inutilizzato Share2. Resta il bottone Guida. Nient'altro toccato.
- NB: deploy su mikilab.de da pubblicare dalla piattaforma.
