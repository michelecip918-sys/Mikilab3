...
## Patch v84→v100 — serie funzioni frontend (22 settembre 2026)
- v84: segreto 16 ottobre, BancoProve, MioForno, PolpoFooter v2, sw.js v64. v86: pane che parla (TTS gratis), sveglia, pane del paese, lievito che viaggia, senza bilancia, modo notte, cura del lievito. v87: recipeSeo + FestaLancio + Volantino + sitemap.xml (da reinviare a Search Console dopo il deploy). v88: la tua bottega (BancoMichele, LaTuaCucina). v89 (corretta): Dedica "Da Miglionico a Stoccarda", rimossa cartella frontend/src/src duplicata (la prima v89 aveva doppio import Dedica in App.js — risolto da Michele nella v89 corretta). v90: il gusto di giocare (festa, medaglie, sorprendimi, Sitor dice). v92: Officina di Sitor (solo 6 righe aggiunte a RecipeList.jsx). v93+v93b: laboratorio, primo pane, bottega che risponde, voce gratis — backend: FEATURE_VOICE_SERVER=false in /api/features (v93b correggeva setLastRecipe mancante in lib/bottega.js). v94: sommelier del pane. v95: ospiti, carta dei pani, cartellini, attestato. v96: anno da fornaio, soprannome, privacy aggiornata. v97: valigia della bottega, libro di pane, pulizia. v98: appunti, tazze e once, da evitare. v99: comodità d'uso. v100 (ULTIMA): la mappa di MikiLab; lo script si auto-cancella — progetto senza cartelle patch.
- Tutte le build yarn OK; file Google verifica sempre intatto; index.html senza tracciamento. Ogni patch cancella la precedente (voluto). Deploy: sempre a carico di Michele (Publish).
