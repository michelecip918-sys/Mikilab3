...
_PERCHE_AVATAR = {
    "it": "\n\nSitor è il mio avatar trasformato in intelligenza artificiale: il volto è il mio, ma le risposte le genera l'IA.",
    "de": "\n\nSitor ist mein in eine künstliche Intelligenz verwandelter Avatar: das Gesicht ist meins, aber die Antworten erzeugt die KI.",
    "en": "\n\nSitor is my avatar turned into an artificial intelligence: the face is mine, but the answers are generated by AI.",
}
...
async def _seed_site_pages():
    ...
    # S4: frase sull'avatar IA nella bozza "Perché ho creato MikiLab" (idempotente, NON pubblica).
    marker = "avatar"
    for lang2, extra in _PERCHE_AVATAR.items():
        doc = await db.site_pages.find_one({"slug": "perche", "lang": lang2}, {"_id": 0, "body": 1})
        if doc and marker not in (doc.get("body") or "").lower():
            await db.site_pages.update_one({"slug": "perche", "lang": lang2},
                                           {"$set": {"body": (doc.get("body") or "") + extra}})
