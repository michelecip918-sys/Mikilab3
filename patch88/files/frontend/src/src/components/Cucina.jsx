...
export default function Cucina({ onBack }) {
  ...
  return (
    <div data-testid="cucina-page" className="max-w-2xl mx-auto px-4 py-6 space-y-5">
      ...
      {/* V85 — La mappa del tuo forno */}
      <button data-testid="cucina-mappaforno" onClick={() => { try { window.dispatchEvent(new CustomEvent("mikilab-nav", { detail: { route: "mappaforno" } })); } catch { /* */ } }}
        className="w-full text-left rounded-2xl border border-primary/40 bg-primary/10 p-4 active:scale-[0.99] transition-all">
        <p className="font-bold text-foreground text-sm">{tri("La mappa del tuo forno", "Die Karte deines Ofens", "The map of your oven")}</p>
        <p className="text-muted-foreground text-xs mt-0.5">{tri("Scopri con una teglia di fette di pane dove il tuo forno scalda di più.", "Finde mit einem Blech Brotscheiben heraus, wo dein Ofen am stärksten heizt.", "Find out with a tray of bread slices where your oven runs hot.")}</p>
      </button>
