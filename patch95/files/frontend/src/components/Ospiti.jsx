  ...
  return (
    <div data-testid="ospiti-page" className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="rounded-2xl border border-border bg-card p-3 space-y-2.5">
        <div className="flex flex-wrap gap-1.5">{OCC.map((o) => <button key={o.k} data-testid={`osp-occ-${o.k}`} onClick={() => set({ occ: o.k, picks: [] })} className={`text-[11.5px] font-semibold px-2.5 py-1.5 rounded-full border active:scale-95 ${st.occ === o.k ? "bg-primary text-white border-primary" : "bg-background text-muted-foreground border-border"}`}>{L(o)}</button>)}</div>
        <div className="grid grid-cols-2 gap-2">
          <label className="block text-[12px] text-muted-foreground">{tri("Quanti siete", "Wie viele", "How many")}<input data-testid="osp-people" type="number" min="1" max="60" value={st.people} onChange={(e) => set({ people: e.target.value })} className="mt-0.5 w-full font-mono-data font-bold text-foreground bg-background border border-border rounded-md px-2 py-1.5 outline-none" /></label>
          <label className="block text-[12px] text-muted-foreground">{tri("In tavola alle", "Auf dem Tisch um", "On the table at")}<input data-testid="osp-when" type="datetime-local" value={st.when || localInput(when)} onChange={(e) => set({ when: e.target.value, picks: [] })} className="mt-0.5 w-full font-mono-data font-bold text-foreground bg-background border border-border rounded-md px-2 py-1.5 outline-none" /></label>
        </div>
        <p className="text-[11.5px] text-muted-foreground">{tri(`Circa ${occ.g} g di pane a persona per ${L(occ).toLowerCase()}: ${Math.round(totalG)} g in tutto. Hai ${fmt1(Math.max(0, hoursLeft))} ore.`, `Etwa ${occ.g} g Brot pro Person für ${L(occ)}: ${Math.round(totalG)} g insgesamt. Du hast ${fmt1(Math.max(0, hoursLeft))} Stunden.`, `About ${occ.g} g of bread per person for ${L(occ).toLowerCase()}: ${Math.round(totalG)} g in total. You have ${fmt1(Math.max(0, hoursLeft))} hours.`)}</p>
      </div>
      {recipes.length === 0 ? <p className="text-[12px] text-muted-foreground">{tri("Carico le ricette…", "Lade Rezepte…", "Loading recipes…")}</p> : plan.length === 0 ? (
        <p className="text-[12.5px] text-muted-foreground rounded-xl border border-border bg-card p-3">{tri("Con questo tempo non ci sta nessun pane del sito: sposta l'ora, o scegli tu una ricetta qui sotto.", "In dieser Zeit passt kein Brot der Seite: verschiebe die Uhrzeit oder wähle unten selbst ein Rezept.", "No bread on the site fits this time: move the hour, or pick a recipe yourself below.")}</p>
        ...
        <div key={r.id} data-testid={`osp-bread-${i}`} className="rounded-2xl border border-border bg-card p-3 space-y-1.5">
          ...
          <p className="text-[12px] text-foreground"><b className="font-mono-data">{Math.round(d.total)} g</b> {tri("di impasto", "Teig", "of dough")} · {Math.round(d.totalFlour)} g {t("ing_flour").toLowerCase()} · {lh.F.lm ? tri("lievito madre", "Sauerteig", "sourdough") : lh.F.hasPre ? tri("con prefermento", "mit Vorteig", "with preferment") : tri("diretto", "direkt", "direct")} · ≈ {fmt1(lh.total)} h {tri("in tutto", "insgesamt", "in total")}</p>
          <p className="text-[12px] text-foreground flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-primary" />{tri("Inizia", "Start", "Start")}: <b className="font-mono-data">{fmtDate(start, lang)} {fmtTime(start, lang)}</b> · {tri("forno acceso", "Ofen an", "oven on")} {fmtTime(ovenOn, lang)} · {tri("in tavola", "auf dem Tisch", "on the table")} {fmtTime(when, lang)}</p>
          {TABLE[kind] && <p className="text-[11.5px] text-salvia">{tri("A tavola", "Dazu", "With it")}: {L(TABLE[kind])}</p>}
        </div>
      ))}
      {recipes.length > 0 && picks.length < 4 && (
        <select data-testid="osp-add" value="" onChange={(e) => addOne(e.target.value)} className="w-full text-[12px] font-semibold bg-card text-foreground border border-border rounded-lg px-2 py-2 outline-none">
      ...
      {plan.length > 0 && (
        <>
          <div data-testid="osp-shop" className="rounded-2xl border border-salvia/40 bg-salvia/8 p-3">
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[12.5px]">{shop.map((v, i) => <div key={i} className="flex justify-between"><span className="text-muted-foreground truncate">{v.label}</span><span className="font-mono-data font-bold text-foreground">{Math.round(v.g)} g</span></div>)}</div>
          </div>
          <div className="flex gap-1.5">
            <button data-testid="osp-ics" onClick={ics} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12px] font-bold px-3 py-2 rounded-xl bg-salvia text-white active:scale-95"><Calendar className="w-4 h-4" />{tri("Gli orari nel calendario", "Zeiten in den Kalender", "Times to calendar")}</button>
            <button data-testid="osp-copy" onClick={() => navigator.clipboard.writeText(asText()).then(() => toast.success(tri("Copiato.", "Kopiert.", "Copied."))).catch(() => {})} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12px] font-bold px-3 py-2 rounded-xl border border-border bg-card text-foreground active:scale-95"><Copy className="w-4 h-4" />{tri("Copia tutto", "Alles kopieren", "Copy all")}</button>
          </div>
          <p className="text-[12px] text-salvia leading-snug">{tri("Sitor: il pane fatto in casa è il regalo che si vede dalla porta. Fallo raffreddare prima che arrivino: il profumo farà il resto.", "Sitor: selbstgebackenes Brot ist das Geschenk, das man schon an der Tür sieht. Lass es auskühlen, bevor sie kommen: der Duft macht den Rest.", "Sitor: homemade bread is the gift you can see from the door. Let it cool before they arrive: the smell will do the rest.")}</p>
        </>
      )}
    </div>
  );
