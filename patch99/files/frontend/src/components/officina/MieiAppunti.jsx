  ...
  return (
    <div data-testid="miei-appunti" className="space-y-2.5">
      ...
      <div className="flex items-center gap-2">
        ...
        <button data-testid="ap-made" onClick={() => { if (!doneToday) save({ ...a, made: [...a.made, Date.now()].slice(-50) }); }} disabled={doneToday} className={`ml-auto inline-flex items-center gap-1 text-[12px] font-bold px-2.5 py-1.5 rounded-full border active:scale-95 ${doneToday ? "bg-salvia/15 text-salvia border-salvia/40" : "bg-salvia text-white border-salvia"}`}><Check className="w-3.5 h-3.5" />{doneToday ? tri("Fatta oggi", "Heute gebacken", "Made today") : tri("L'ho fatta oggi", "Heute gebacken", "I made it today")}</button>
      </div>
      {a.made.length > 0 && <p className="text-[12px] text-muted-foreground">{tri(`Fatta ${a.made.length} ${a.made.length === 1 ? "volta" : "volte"}`, `${a.made.length} Mal gebacken`, `Made ${a.made.length} time${a.made.length === 1 ? "" : "s"}`)}: {a.made.slice(-5).map((t) => fmtDate(new Date(t), lang)).join(" · ")}{a.made.length > 5 ? " …" : ""}</p>}
      <textarea data-testid="ap-text" value={a.text} onChange={(e) => save({ ...a, text: e.target.value.slice(0, 2000) })} rows={4} placeholder={tri("Es. 20 g d'acqua in meno con la mia farina; 5 minuti in più di forno; piaciuta a nonna.", "Z. B. 20 g weniger Wasser mit meinem Mehl; 5 Minuten länger im Ofen; Oma hat's geschmeckt.", "E.g. 20 g less water with my flour; 5 more minutes in the oven; grandma loved it.")} className="w-full text-[13px] bg-background text-foreground border border-border rounded-xl px-3 py-2 outline-none focus:border-primary leading-snug" />
      <p className="text-[11px] text-muted-foreground">{tri("Si salva da solo mentre scrivi.", "Speichert sich beim Schreiben von selbst.", "Saves itself as you type.")}</p>
    </div>
  );
}
