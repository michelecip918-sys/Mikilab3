    ...
    <div data-testid="cartolina-pane" className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {[["cartolina", tri("Cartolina", "Postkarte", "Postcard"), Share2], ["bigliettino", tri("Bigliettino da regalo", "Geschenkkarte", "Gift tag"), Gift]].map(([k, l, Icon]) => (
          <button key={k} data-testid={`cp-mode-${k}`} onClick={() => setMode(k)} className={`inline-flex items-center gap-1.5 text-[12.5px] font-bold px-3 py-1.5 rounded-full border active:scale-95 ${mode === k ? "bg-primary text-white border-primary" : "bg-card text-muted-foreground border-border"}`}><Icon className="w-3.5 h-3.5" /> {l}</button>
        ))}
      ...
      {mode === "cartolina" && (
        <label className="inline-flex items-center gap-2 text-[13px] font-semibold text-foreground bg-card border border-border rounded-xl px-3 py-2 cursor-pointer active:scale-95">
      ...
      {url ? <img data-testid="cp-preview" src={url} alt="" className="w-full rounded-xl border border-border shadow-md" /> : <p className="text-[12.5px] text-muted-foreground">{tri("Sto disegnando…", "Ich zeichne…", "Drawing…")}</p>}
      <div className="flex flex-wrap gap-2">
        <button data-testid="cp-share" onClick={share} className="inline-flex items-center gap-1.5 bg-primary text-white font-semibold text-[13px] px-3.5 py-2 rounded-xl active:scale-95"><Share2 className="w-4 h-4" /> {tri("Condividi", "Teilen", "Share")}</button>
        <button data-testid="cp-download" onClick={download} className="inline-flex items-center gap-1.5 bg-card border border-border text-foreground font-semibold text-[13px] px-3.5 py-2 rounded-xl active:scale-95"><Download className="w-4 h-4" /> {tri("Salva immagine", "Bild speichern", "Save image")}</button>
        {mode === "bigliettino" && <button data-testid="cp-print" onClick={print} className="inline-flex items-center gap-1.5 bg-card border border-border text-foreground font-semibold text-[13px] px-3.5 py-2 rounded-xl active:scale-95"><Printer className="w-4 h-4" /> {tri("Stampa", "Drucken", "Print")}</button>}
      </div>
