...
export default function Attestato({ lang, title }) {
  ...
  return (
    <div data-testid="attestato" className="space-y-2.5">
      ...
      {url ? <img ref={ref} data-testid="att-img" src={url} alt="" className="w-full rounded-xl border border-border shadow-md" /> : <p className="text-[12.5px] text-muted-foreground">{tri("Sto scrivendo…", "Ich schreibe…", "Writing…")}</p>}
      <button data-testid="att-share" onClick={share} className="inline-flex items-center gap-1.5 text-[12.5px] font-bold px-3 py-2 rounded-xl bg-primary text-white active:scale-95"><Share2 className="w-4 h-4" />{tri("Condividi o salva", "Teilen oder speichern", "Share or save")}</button>
      <p className="text-[11px] text-muted-foreground flex items-center gap-1"><Award className="w-3 h-3" />{tri("Disegnato nel tuo telefono. Meglio un soprannome che il nome vero: resta solo qui, e nessuno deve sapere chi sei per dirti bravo.", "Auf deinem Handy gezeichnet. Lieber ein Spitzname als der echte Name: bleibt nur hier, und niemand muss wissen, wer du bist, um dich zu loben.", "Drawn on your phone. A nickname is better than your real name: it stays only here, and nobody needs to know who you are to say well done.")}</p>
    </div>
  );
}
