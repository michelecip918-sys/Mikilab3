...
export default function OcchioDiSitor({ lang }) {
  ...
  return (
    <div data-testid="occhio-di-sitor" className="space-y-3">
      <p className="text-[13px] text-foreground/85 leading-snug">{tri("Fai una foto alla crosta o a una fetta: la leggo qui, nel tuo telefono, e ti dico cosa vedo.", "Mach ein Foto der Kruste oder einer Scheibe: ich lese es hier, in deinem Handy, und sage dir, was ich sehe.", "Take a photo of the crust or a slice: I read it here, on your phone, and tell you what I see.")}</p>
      <div className="flex gap-1.5">
        {[{ k: "crosta", I: Flame, l: tri("Crosta", "Kruste", "Crust") }, { k: "mollica", I: Layers, l: tri("Mollica (fetta)", "Krume (Scheibe)", "Crumb (slice)") }].map((m) => (
          <button key={m.k} data-testid={`ods-mode-${m.k}`} onClick={() => setMode(m.k)} className={`flex-1 inline-flex items-center justify-center gap-1.5 text-[12.5px] font-bold px-3 py-2 rounded-xl border active:scale-95 ${mode === m.k ? "bg-primary text-white border-primary" : "bg-card text-muted-foreground border-border"}`}><m.I className="w-3.5 h-3.5" />{m.l}</button>
        ))}
      </div>
      ...
      <div className="flex gap-1.5">
        <button data-testid="ods-shoot" onClick={() => camRef.current && camRef.current.click()} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12.5px] font-bold px-3 py-2 rounded-xl bg-salvia text-white active:scale-95"><Camera className="w-4 h-4" />{tri("Scatta", "Aufnehmen", "Shoot")}</button>
        <button data-testid="ods-pick" onClick={() => galRef.current && galRef.current.click()} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12.5px] font-bold px-3 py-2 rounded-xl border border-border bg-card text-foreground active:scale-95"><Eye className="w-4 h-4" />{tri("Dalla galleria", "Aus der Galerie", "From gallery")}</button>
      </div>
