...
function Slider({ label, value, onChange, min, max, testid, hint }) {
  return (
    <label className="block">
      <span className="flex items-center justify-between text-[13px] text-muted-foreground"><span>{label}</span><span className="font-mono-data font-bold text-foreground">{fmt1(value)} °C</span></span>
  ...
  return (
    <div data-testid="acqua-giusta" className="space-y-3">
      ...
      <div className="rounded-xl border border-border bg-card p-3 space-y-2.5">
        ...
        <div>
          <span className="block text-[13px] text-muted-foreground mb-1">{tri("Come impasti", "Wie du knetest", "How you knead")}</span>
      ...
      <div data-testid="ag-result" className={`rounded-xl border p-3.5 ${tooHot ? "" : "border-salvia/50 bg-salvia/10"}`} style={tooHot ? { borderColor: "hsl(var(--mattone) / 0.5)", background: "hsl(var(--mattone) / 0.1)" } : undefined}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[12.5px] text-muted-foreground">{tri("Usa l'acqua a", "Nimm Wasser mit", "Use water at")}</p>
            <p className="font-display text-3xl font-bold text-foreground leading-none">{Math.round(clamp(waterT, 0, 60))} °C</p>
          </div>
          {waterG > 0 && <p className="ml-auto text-right text-[12.5px] text-muted-foreground">{tri("per i", "für die", "for the")} <span className="font-mono-data font-bold text-foreground">{waterG} g</span> {tri("d'acqua della ricetta", "Wasser des Rezepts", "of water in the recipe")}</p>}
        </div>
        {tooHot && <p className="text-[12.5px] text-foreground mt-2">{tri("Sopra i 45 °C l'acqua rovina il lievito. Scalda piuttosto la cucina o la farina (vicino al termosifone) e impasta un po' più a lungo.", "Über 45 °C schadet das Wasser der Hefe. Wärme lieber Küche oder Mehl (nahe der Heizung) und knete etwas länger.", "Above 45 °C water harms the yeast. Warm the kitchen or the flour (near a radiator) instead and knead a little longer.")}</p>}
        {needIce && (
          <div data-testid="ag-ice" className="mt-2.5 rounded-lg bg-background border border-border p-2.5 text-[13px] text-foreground/90">
