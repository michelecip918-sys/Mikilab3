...
export default function LievitazioneACasa({ r, lang }) {
  ...
  if (rows.length === 0) {
    return <p className="text-[13px] text-muted-foreground">{tri("Questa ricetta non indica ore di lievitazione: segui il procedimento.", "Dieses Rezept nennt keine Garzeiten: folge dem Ablauf.", "This recipe gives no proofing hours: follow the method.")}</p>;
  }
    ...
    <div data-testid="lievitazione-casa" className="space-y-3">
      ...
      <div className="rounded-xl border border-border bg-card p-3 space-y-2.5">
        <label className="block">
          <span className="flex items-center justify-between text-[13px] text-muted-foreground"><span>{tri("Temperatura dove lievita l'impasto", "Temperatur, wo der Teig geht", "Temperature where the dough rises")}</span><span className="font-mono-data font-bold text-foreground">{temp} °C</span></span>
      ...
      <div data-testid="lc-result" className="rounded-xl border border-salvia/50 bg-salvia/10 p-3.5 space-y-2">
        {(() => { let acc = 0; return rows.map((row, i) => { const from = acc; acc += row.now; return (
          <div key={i} className="flex items-start justify-between gap-3 text-[13px]">
            <div>
              ...
              <p className="text-[12px] text-muted-foreground">{tri("Nella ricetta", "Im Rezept", "In the recipe")}: {hrs(row.base, lang)} · {tri("da te", "bei dir", "at your place")}: <b className="text-foreground">{hrs(row.now * 0.85, lang)} – {hrs(row.now * 1.15, lang)}</b></p>
            </div>
        ...
        <div className="flex items-center gap-2 pt-1 border-t border-salvia/30 text-[12.5px] text-muted-foreground">
