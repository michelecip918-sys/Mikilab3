    ...
    <div data-testid="quanto-costa" className="space-y-3">
      ...
      <div data-testid="qc-table" className="rounded-xl border border-border bg-card p-3">
        <div className="space-y-1">
          {lines.map((l, i) => (
            <div key={i} className="flex items-center justify-between text-[13px]"><span className="text-muted-foreground truncate pr-2">{l.name} <span className="font-mono-data">{l.g} g</span></span><span className="font-mono-data font-semibold text-foreground shrink-0">{eur(cost(l))}</span></div>
          ))}
          <div className="flex items-center justify-between text-[13px] pt-1 border-t border-border/60"><span className="text-muted-foreground flex items-center gap-1"><Flame className="w-3.5 h-3.5" /> {tri("Forno", "Ofen", "Oven")} ≈ {(Math.round(energyKwh * 10) / 10).toString().replace(".", ",")} kWh ({bakeMin} min + {tri("preriscaldo", "Vorheizen", "preheat")})</span><span className="font-mono-data font-semibold text-foreground">{eur(energy)}</span></div>
        </div>
      </div>
      <div data-testid="qc-result" className="rounded-xl border border-salvia/50 bg-salvia/10 p-3.5">
        <div className="flex items-end justify-between gap-3">
          <div>
            <p className="text-[12.5px] text-muted-foreground flex items-center gap-1"><Euro className="w-3.5 h-3.5" /> {tri("Questa infornata ti costa", "Diese Backrunde kostet dich", "This bake costs you")}</p>
            <p className="font-display text-3xl font-bold text-foreground leading-none">{eur(total)}</p>
          </div>
          <label className="text-right text-[12.5px] text-muted-foreground">
        ...
        <label className="flex items-center justify-between gap-2 text-[12.5px] text-muted-foreground mt-3 pt-2 border-t border-salvia/30">
        ...
        {shopN > 0 && <p className="text-[13px] text-foreground mt-1.5">{shopN > total / n ? tri(`Risparmi ${eur(shopN - total / n)} a pezzo (${Math.round((1 - total / n / shopN) * 100)} %).`, `Du sparst ${eur(shopN - total / n)} je Stück (${Math.round((1 - total / n / shopN) * 100)} %).`, `You save ${eur(shopN - total / n)} per piece (${Math.round((1 - total / n / shopN) * 100)} %).`) : tri("Costa quanto in panetteria: ma è tuo.", "Kostet so viel wie beim Bäcker: aber es ist deins.", "It costs the same as the bakery: but it's yours.")}</p>}
      </div>
      <details className="rounded-xl border border-border bg-card p-3">
        <summary className="text-[13px] font-semibold text-foreground cursor-pointer">{tri("I miei prezzi (€ al kg)", "Meine Preise (€ je kg)", "My prices (€ per kg)")}</summary>
        <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 mt-2">
          {fams.map((f) => (
            <label key={f} className="flex items-center justify-between gap-1 text-[12px] text-muted-foreground"><span className="truncate">{L(f)}</span>
          ...
          <label className="flex items-center justify-between gap-1 text-[12px] text-muted-foreground"><span>{tri("Corrente € al kWh", "Strom € je kWh", "Electricity € per kWh")}</span>
