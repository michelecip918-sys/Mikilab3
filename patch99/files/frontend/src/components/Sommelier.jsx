    ...
    <div data-testid="sommelier-page" className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      {DIMS.map((d) => (
        <div key={d.k} data-testid={`som-dim-${d.k}`} className="rounded-2xl border border-border bg-card p-3">
          <p className="text-[13px] font-bold text-foreground flex items-center gap-1.5"><d.I className="w-4 h-4 text-salvia" />{L(d.t)}<span className="font-normal text-[12px] text-muted-foreground">· {L(d.q)}</span></p>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {d.chips.map((c) => <button key={c.k} data-testid={`som-chip-${c.k}`} onClick={() => toggle(c.k)} className={`text-[12px] font-semibold px-2.5 py-1.5 rounded-full border active:scale-95 ${sel.has(c.k) ? "bg-salvia text-white border-salvia" : "bg-background text-foreground border-border"}`}>{L(c)}</button>)}
          </div>
        </div>
      ))}
      {card && (
        <div data-testid="som-card" className="rounded-2xl border border-salvia/40 bg-salvia/8 p-4 space-y-2.5">
          ...
          <p className="font-display text-xl font-bold text-foreground">{name || tri("Il tuo pane", "Dein Brot", "Your bread")} <span className="text-[12.5px] font-mono-data font-bold text-primary align-middle ml-1">{card.char}</span></p>
          <div className="text-[13px] text-foreground/90 leading-snug space-y-0.5">{card.prose.map((p, i) => <p key={i}>{p}</p>)}</div>
          <div className="text-[12.5px] text-salvia leading-snug space-y-1">{card.v.map((p, i) => <p key={i}>{i === 0 ? "Sitor: " : ""}{p}</p>)}</div>
          <div><p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">{tri("A tavola", "Am Tisch", "At the table")}</p>{card.pair.map((p, i) => <p key={i} className="text-[12.5px] text-foreground/90 leading-snug">{p}</p>)}</div>
          <div className="flex gap-1.5 pt-1">
            <button data-testid="som-share" onClick={share} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12.5px] font-bold px-3 py-2 rounded-xl bg-salvia text-white active:scale-95"><Share2 className="w-4 h-4" />{tri("Condividi", "Teilen", "Share")}</button>
            <button data-testid="som-save" onClick={save} className="flex-1 text-[12.5px] font-bold px-3 py-2 rounded-xl border border-border bg-card text-foreground active:scale-95">{tri("Salva nel quaderno", "Ins Heft", "Save to notebook")}</button>
            <button data-testid="som-reset" onClick={() => setSel(new Set())} className="text-[12.5px] font-semibold px-3 py-2 rounded-xl text-muted-foreground active:scale-95">{tri("Da capo", "Neu", "Reset")}</button>
          </div>
        </div>
      )}
      {saved.length > 0 && (
        <div data-testid="som-saved" className="rounded-2xl border border-border bg-card p-3">
          <p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground mb-1.5">{tri("Il quaderno degli assaggi", "Das Verkostungsheft", "The tasting notebook")}</p>
          {saved.map((s, i) => (
            <div key={s.ts} className="flex items-center gap-2 py-1 border-t border-border first:border-0 text-[12.5px]">
      ...
      <p className="text-[11px] text-muted-foreground">{tri("Le schede restano nel tuo telefono. Il sommelier non giudica: traduce in parole quello che la bocca sa già.", "Die Karten bleiben auf deinem Handy. Der Sommelier urteilt nicht: er übersetzt in Worte, was der Mund schon weiß.", "The cards stay on your phone. The sommelier doesn't judge: it puts into words what the mouth already knows.")}</p>
    </div>
  );
}
