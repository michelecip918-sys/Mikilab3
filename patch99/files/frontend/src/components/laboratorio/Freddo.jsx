...
export default function Freddo({ recipes, lang }) {
  ...
  if (!r) return <p className="text-[12.5px] text-muted-foreground">{tri("Nessuna ricetta con le dosi.", "Kein Rezept mit Mengen.", "No recipe with quantities.")}</p>;
  return (
    <div data-testid="freddo" className="space-y-3">
      <p className="text-[13px] text-foreground/85 leading-snug">{tri("Dimmi a che ora inforni e quante ore vuoi in cella: conto all'indietro forma, impasto e prefermento. Le ore in cella valgono poco per la lievitazione: te le calcolo.", "Sag mir, wann du backst und wie viele Stunden Kühlzelle du willst: ich rechne Formen, Kneten und Vorteig zurück. Kühle Stunden zählen für die Gare wenig: ich rechne sie ein.", "Tell me when you bake and how many hours in the cold room: I count back shaping, mixing and preferment. Cold hours count little for fermentation: I factor that in.")}</p>
      <div className="rounded-xl border border-border bg-card p-3 space-y-2.5">
        <select data-testid="fr-recipe" value={r.id} onChange={(e) => set({ id: e.target.value })} className="w-full text-[12.5px] font-semibold bg-background text-foreground border border-border rounded-lg px-2 py-2 outline-none">
          {recipes.map((x) => <option key={x.id} value={x.id}>{rLoc(x, "name", lang)}</option>)}
        </select>
        <div className="inline-flex rounded-lg border border-border overflow-hidden text-[12.5px]">
      ...
      <div data-testid="fr-plan" className="rounded-xl border border-salvia/40 bg-salvia/8 p-3 space-y-1.5">
        ...
        {plan.note && <p className="text-[12.5px] text-salvia mt-1">Sitor: {plan.note}</p>}
        <p className="text-[11px] text-muted-foreground">{tri(`Ricetta a 25 °C: massa ${fmt1(plan.F.bulk)} h, forma ${fmt1(plan.F.proof)} h. Il freddo rallenta ma non ferma: guarda sempre l'impasto.`, `Rezept bei 25 °C: Stockgare ${fmt1(plan.F.bulk)} h, Stückgare ${fmt1(plan.F.proof)} h. Kälte bremst, stoppt aber nicht: schau immer auf den Teig.`, `Recipe at 25 °C: bulk ${fmt1(plan.F.bulk)} h, proof ${fmt1(plan.F.proof)} h. Cold slows but doesn't stop: always look at the dough.`)}</p>
      </div>
      <div className="flex gap-1.5">
        <button data-testid="fr-ics" onClick={ics} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12.5px] font-bold px-3 py-2 rounded-xl bg-salvia text-white active:scale-95"><Calendar className="w-4 h-4" />{tri("Nel calendario", "In den Kalender", "To calendar")}</button>
        <button data-testid="fr-copy" onClick={() => navigator.clipboard.writeText(asText()).then(() => toast.success(tri("Copiato.", "Kopiert.", "Copied."))).catch(() => {})} className="flex-1 inline-flex items-center justify-center gap-1.5 text-[12.5px] font-bold px-3 py-2 rounded-xl border border-border bg-card text-foreground active:scale-95"><Copy className="w-4 h-4" />{tri("Copia", "Kopieren", "Copy")}</button>
      </div>
    </div>
  );
}
