...
export default function CaricoForno({ lang }) {
  ...
  return (
    <div data-testid="carico-forno" className="space-y-3">
      ...
      {plan.waves > 1 && (
        <p className="text-[12.5px] text-salvia leading-snug">{tri(`Sitor: sfalsa la forma di ${plan.stagger} minuti tra un'infornata e l'altra (o tieni in cella i pezzi delle infornate dopo la seconda): così ogni pezzo entra al punto giusto di lievitazione, non quando tocca a lui.`, `Sitor: staffle das Formen um ${plan.stagger} Minuten zwischen den Backgängen (oder halte die Stücke ab dem dritten Backgang in der Kühlzelle): so kommt jedes Stück am richtigen Garpunkt in den Ofen, nicht wenn es an der Reihe ist.`, `Sitor: stagger the shaping by ${plan.stagger} minutes between bakes (or keep the pieces for the third bake onwards in the cold room): that way every piece goes in at the right proof, not when its turn comes.`)}</p>
      )}
      <button data-testid="cf-copy" onClick={() => navigator.clipboard.writeText(asText()).then(() => toast.success(tri("Copiato.", "Kopiert.", "Copied."))).catch(() => {})} className="inline-flex items-center gap-1.5 text-[12.5px] font-bold px-3 py-2 rounded-xl border border-border bg-card text-foreground active:scale-95"><Copy className="w-4 h-4" />{tri("Copia il piano", "Plan kopieren", "Copy the plan")}</button>
    </div>
  );
}
