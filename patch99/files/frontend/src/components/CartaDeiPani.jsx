    ...
    <div data-testid="carta-page" className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="no-print rounded-2xl border border-border bg-card p-3 space-y-2">
        ...
        <div className="flex gap-1.5">
          <select data-testid="carta-pick" value={pick} onChange={(e) => setPick(e.target.value)} className="flex-1 text-[12.5px] font-semibold bg-background text-foreground border border-border rounded-lg px-2 py-2 outline-none">
          ...
          <button data-testid="carta-add" onClick={() => { if (pick && (st.items || []).length < 10) { set({ items: [...(st.items || []), { id: pick, price: "", note: "" }] }); setPick(""); } }} disabled={!pick} className="inline-flex items-center gap-1 text-[12.5px] font-bold px-3 rounded-lg bg-primary text-white disabled:opacity-40 active:scale-95"><Plus className="w-4 h-4" /></button>
        </div>
        <label className="flex items-center gap-2 text-[12.5px] text-foreground"><input data-testid="carta-prices" type="checkbox" checked={!!st.prices} onChange={(e) => set({ prices: e.target.checked })} className="accent-[hsl(var(--primary))]" />{tri("Mostra i prezzi (per il banco)", "Preise zeigen (für die Theke)", "Show prices (for the counter)")}</label>
        {items.map((it, i) => (
        ...
        <div className="space-y-5 max-w-md mx-auto">
          {items.map((it, i) => (
            <div key={it.id} data-testid={`carta-item-${i}`}>
              ...
              <p className="text-[12.5px] text-muted-foreground text-left leading-snug">{describe(it.r, lang, tri)}</p>
              {it.note && <p className="text-[12.5px] italic text-foreground/80 text-left">{it.note}</p>}
            </div>
          ))}
        </div>
        <div className="w-16 h-px bg-primary mx-auto my-5" />
