...
export default function ProfileSelect() {
  ...
  const Card = ({ id, Icon, title, desc, points }) => (
    <button data-testid={`profile-select-${id}`} onClick={() => chooseProfile(id)}
      className="w-full text-left rounded-3xl p-5 bg-[#1B2A38] border-2 border-[#3E9C93]/50 hover:border-[#3E9C93] active:scale-98 transition-all">
      <div className="flex items-center gap-3 mb-2">
        <span className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] flex items-center justify-center shrink-0">
        ...
        <ChevronRight className="w-6 h-6 text-[#3E9C93] shrink-0" />
      </div>
      <ul className="mt-2 space-y-1">
        {points.map((p, i) => (
          <li key={i} className="flex items-start gap-2 text-[13px] text-[#e4eff8] leading-snug">
            <span className="w-1.5 h-1.5 rounded-full bg-[#3E9C93] mt-1.5 shrink-0" /> {p}
          </li>
        ))}
      </ul>
    </button>
      ...
      <div className="max-w-md mx-auto px-5 py-10">
        {profile && (
          <button data-testid="profile-select-close" onClick={() => chooseProfile(profile)}
            className="absolute top-4 right-4 w-10 h-10 rounded-full bg-[#1B2A38] border border-[#2A3B49] flex items-center justify-center text-white active:scale-95">
            <X className="w-5 h-5" />
          </button>
        )}
