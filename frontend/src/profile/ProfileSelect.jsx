...
export default function ProfileSelect() {
  ...
  const Card = ({ id, Icon, title, desc, points }) => (
    <button data-testid={`profile-select-${id}`} onClick={() => chooseProfile(id)}
      className="w-full text-left rounded-3xl p-5 bg-[#1e1e1e] border-2 border-[#ff6b00]/50 hover:border-[#ff6b00] active:scale-98 transition-all">
      <div className="flex items-center gap-3 mb-2">
        <span className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#ff6b00] to-[#c94f00] flex items-center justify-center shrink-0">
        ...
        <ChevronRight className="w-6 h-6 text-[#ff6b00] shrink-0" />
      </div>
      <ul className="mt-2 space-y-1">
        {points.map((p, i) => (
          <li key={i} className="flex items-start gap-2 text-[13px] text-[#e4eff8] leading-snug">
            <span className="w-1.5 h-1.5 rounded-full bg-[#ff6b00] mt-1.5 shrink-0" /> {p}
          </li>
        ))}
      </ul>
    </button>
