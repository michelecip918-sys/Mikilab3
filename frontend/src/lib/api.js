...
export const coordinationApi = {
  ...
  roster: () => api.get(`/coordination/roster`).then((r) => r.data).catch(() => ({ operators: [], free: 0, busy: 0, total: 0 })),
...
export const mikeApi = {
  ...
  autoplanWeekOptions: (payload) => api.post(`/mike/autoplan/week/options`, payload).then((r) => r.data),
  autoplanWeekDetail: (payload) => api.post(`/mike/autoplan/week/detail`, payload).then((r) => r.data),
  ...
  reorderProposals: () => api.get(`/mike/reorder/proposals`).then((r) => r.data),
  reorderConfigSet: (autoSend, autoThreshold) => api.put(`/mike/reorder/config`, { auto_send: autoSend, auto_threshold: autoThreshold }).then((r) => r.data),
  reorderSend: (pid) => api.post(`/mike/reorder/proposals/${pid}/send`).then((r) => r.data),
  reorderDismiss: (pid) => api.post(`/mike/reorder/proposals/${pid}/dismiss`).then((r) => r.data),
  reorderReceive: (pid) => api.post(`/mike/reorder/proposals/${pid}/receive`).then((r) => r.data),
  reorderHistory: () => api.get(`/mike/reorder/history`).then((r) => r.data),
  reorderSmartThresholds: () => api.get(`/mike/reorder/smart-thresholds`).then((r) => r.data),
  reorderApplyThreshold: (name, source) => api.post(`/mike/reorder/apply-threshold`, { name, source }).then((r) => r.data),
...
export const operatorPinsApi = {
  ...
  purgeByCode: (code) => api.post(`/operator-pins/purge-by-code`, { code }).then((r) => r.data),
