  ...
  return (
    <div className="pb-24">
      ...
      <div className="mb-4 rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-5">
        <div className="flex items-end gap-3">
          <div className="flex-1">
            ...
            <input data-testid="termo-lab-now" type="number" value={labNow} onChange={(e) => setLabNow(e.target.value)}
              className="mt-1 w-full bg-[#e4eff8] dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] rounded-2xl shadow-md border border-amber-900/40 p-3 text-base outline-none focus:border-[#ff6b00]" />
          </div>
        ...
        {climateMsg && (
          <div data-testid="termo-climate-msg" className={`mt-3 text-sm rounded-2xl shadow-md border border-amber-900/40 px-3 py-2 border ${delta && Math.abs(delta) >= 1 ? "bg-[#ff6b00]/15 border-[#ff6b00]/40 text-[#ff6b00] dark:text-[#8FB0C2]" : "bg-[#ff6b00]/12 border-[#ff6b00]/30 text-[#ff6b00] dark:text-[#a9d2ec]"}`}>
      ...
      <div className="rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-5">
        ...
        <select data-testid="termo-recipe-select" value={sel} onChange={(e) => pickRecipe(e.target.value)}
          className="w-full bg-[#e4eff8] dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] rounded-2xl shadow-md border border-amber-900/40 p-3 text-base outline-none focus:border-[#ff6b00]">
        ...
        {sel && (
          <div className="mt-3 space-y-3" data-testid="termo-recipe-detail">
            {advice && (
              <div data-testid="termo-advice" className={`text-sm rounded-2xl shadow-md border border-amber-900/40 px-3 py-2 border flex items-start gap-2 ${advice.type === "ok" ? "bg-[#ff6b00]/12 border-[#ff6b00]/30 text-[#ff6b00] dark:text-[#a9d2ec]" : "bg-[#ff6b00]/15 border-[#ff6b00]/40 text-[#ff6b00] dark:text-[#8FB0C2]"}`}>
            ...
            <button data-testid="termo-save" onClick={saveTemp} disabled={measured === ""}
              className="w-full bg-[#ff6b00] hover:bg-[#ff8a33] disabled:opacity-50 text-white font-semibold px-4 py-2.5 rounded-2xl shadow-md border border-amber-900/40 flex items-center justify-center gap-2">
