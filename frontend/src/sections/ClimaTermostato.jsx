...
export default function ClimaTermostato() {
  ...
  return (
    <div className="pb-24">
      ...
      {/* Orologio */}
      <div data-testid="termo-clock" className="mb-4 rounded-2xl bg-white dark:bg-[#2A211D] border border-[#E8DEC8] dark:border-[#3D302A] p-5 flex items-center gap-3">
        <Clock className="w-6 h-6 text-[#B34A26]" />
        <div>
          <p className="text-[10px] uppercase tracking-wide text-[#8C7567]">{t("termo_clock")}</p>
          <p className="font-mono-data text-3xl font-bold text-[#2C221E] dark:text-[#F5EFE6]">{hh}</p>
        </div>
      </div>

      {/* Clima */}
      <div className="mb-4 rounded-2xl bg-white dark:bg-[#2A211D] border border-[#E8DEC8] dark:border-[#3D302A] p-5">
        <div className="flex items-end gap-3">
          <div className="flex-1">
            <label className="text-xs font-semibold uppercase tracking-wide text-[#8C7567]">{t("termo_lab_now")}</label>
            <input data-testid="termo-lab-now" type="number" value={labNow} onChange={(e) => setLabNow(e.target.value)}
              className="mt-1 w-full bg-[#F5EFE6] dark:bg-[#332823] border border-[#E8DEC8] dark:border-[#3D302A] rounded-xl p-3 text-base outline-none focus:border-[#B34A26]" />
          </div>
          <div className="text-center shrink-0 pb-1">
            <p className="text-[10px] uppercase tracking-wide text-[#8C7567]">{t("termo_std")}</p>
            <p className="font-mono-data text-xl font-bold text-[#6B8E62]">{std}°C</p>
          </div>
        </div>
        <button data-testid="termo-bt-connect" onClick={connectBt} disabled={btState === "connecting" || btState === "reading"}
          className="mt-3 w-full bg-[#5b8fb0] hover:bg-[#4d7d9c] disabled:opacity-60 text-white font-semibold px-4 py-2.5 rounded-xl flex items-center justify-center gap-2">
        ...
        {climateMsg && (
          <div data-testid="termo-climate-msg" className={`mt-3 text-sm rounded-xl px-3 py-2 border ${delta && Math.abs(delta) >= 1 ? "bg-[#D99B26]/15 border-[#D99B26]/40 text-[#8C3A1D] dark:text-[#E5AC3A]" : "bg-[#6B8E62]/12 border-[#6B8E62]/30 text-[#4d6b45] dark:text-[#9ec48f]"}`}>
      ...
      <div className="rounded-2xl bg-white dark:bg-[#2A211D] border border-[#E8DEC8] dark:border-[#3D302A] p-5">
        <div className="flex items-center gap-2 mb-3 text-[#B34A26]">
          <History className="w-4 h-4" />
          <h2 className="font-display text-base font-semibold text-[#2C221E] dark:text-[#F5EFE6]">{t("termo_recipe_mem")}</h2>
        </div>
        <select data-testid="termo-recipe-select" value={sel} onChange={(e) => pickRecipe(e.target.value)}
          className="w-full bg-[#F5EFE6] dark:bg-[#332823] border border-[#E8DEC8] dark:border-[#3D302A] rounded-xl p-3 text-base outline-none focus:border-[#B34A26]">
        ...
        {sel && (
          <div className="mt-3 space-y-3" data-testid="termo-recipe-detail">
            {advice && (
              <div data-testid="termo-advice" className={`text-sm rounded-xl px-3 py-2 border flex items-start gap-2 ${advice.type === "ok" ? "bg-[#6B8E62]/12 border-[#6B8E62]/30 text-[#4d6b45] dark:text-[#9ec48f]" : "bg-[#D99B26]/15 border-[#D99B26]/40 text-[#8C3A1D] dark:text-[#E5AC3A]"}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] font-semibold uppercase tracking-wide text-[#8C7567]">{t("termo_target")}</label>
              ...
              <div>
                <label className="text-[10px] font-semibold uppercase tracking-wide text-[#8C7567]">{t("termo_measured")}</label>
            ...
            <button data-testid="termo-save" onClick={saveTemp} disabled={measured === ""}
              className="w-full bg-[#B34A26] hover:bg-[#963B1C] disabled:opacity-50 text-white font-semibold px-4 py-2.5 rounded-xl flex items-center justify-center gap-2">
