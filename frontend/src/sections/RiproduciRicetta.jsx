...
export default function RiproduciRicetta({ onBack, onNavigate }) {
  ...
  return (
    <div data-testid="riproduci-ricetta" className="pb-8">
      {onBack && (
        <button data-testid="riproduci-back" onClick={onBack} className="flex items-center gap-1 text-[#3E9C93] font-medium mb-3">
      ...
      <div className="relative mb-4">
        ...
        <input data-testid="riproduci-search" value={q} onChange={(e) => setQ(e.target.value)}
          ...
          className="w-full bg-[#1B2A38] border border-[#2A3B49] rounded-2xl pl-10 pr-3 py-3 text-sm text-white outline-none focus:border-[#3E9C93]" />
      </div>
          ...
          {filtered.map((r) => {
            ...
            return (
              <button key={r.id} data-testid={`riproduci-recipe-${r.id}`} onClick={() => setActive(r)}
                className="w-full text-left flex items-center gap-3 rounded-2xl bg-[#1B2A38] border border-[#2A3B49] p-3 active:scale-98 hover:border-[#3E9C93]/60 transition-all">
                {r.image_url ? (
                  ...
                  <div className="w-14 h-14 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/15 flex items-center justify-center shrink-0"><ChefHat className="w-6 h-6 text-[#3E9C93]" /></div>
                )}
                ...
                <ChevronRight className="w-5 h-5 text-[#3E9C93]/70 shrink-0" />
              </button>
            );
          })}
        </div>
