    ...
    <div className="pb-8" data-testid="sapori-casa">
      ...
      {tab === "forno" && (
        <div className="space-y-3" data-testid="sapori-forno">
          {BREADS.map((b, i) => (
            <div key={i} data-testid={`bread-${i}`} className="rounded-2xl bg-background dark:bg-card border border-border dark:border-border p-4 shadow-sm">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-2xl">{b.emoji}</span>
                <p className="font-display text-lg font-bold text-primary dark:text-foreground leading-tight flex-1">{b.name}</p>
              </div>
              <span className="inline-block text-[10px] font-bold uppercase tracking-wide bg-card text-primary px-2 py-0.5 rounded-full mb-2">{b.tag}</span>
              <p className="text-[13.5px] text-primary dark:text-muted-foreground leading-relaxed">{b.body}</p>
            </div>
          ))}
        </div>
      )}
