          ...
          {visible.map((p) => {
            ...
            return (
              <div key={p.id} data-testid={`community-post-${p.id}`} className="bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] rounded-2xl p-4 shadow-sm">
                ...
                {p.text && <p className="text-sm text-[#2B303B] dark:text-[#e4eff8] whitespace-pre-line leading-relaxed">{(lang === "de" && p.text_de) ? p.text_de : (lang === "en" && p.text_en) ? p.text_en : p.text}</p>}
