...
const CATS = [
  ...
  { id: "idea", Icon: Sparkles, color: "#3E9C93" },
  { id: "foto", Icon: Camera, color: "#3E9C93" },
  { id: "ricetta", Icon: BookOpen, color: "#3E9C93" },
  { id: "domanda", Icon: HelpCircle, color: "#3E9C93" },
  { id: "evento", Icon: CalendarDays, color: "#2e8b6f" },
  { id: "pane", Icon: Wheat, color: "#E0A458" },
  { id: "pizza", Icon: Pizza, color: "#3E9C93" },
  { id: "dolci", Icon: Cookie, color: "#3E9C93" },
  { id: "sos", Icon: LifeBuoy, color: "#ff3b5c" },
];
// Categorie mostrate nel feed ma non selezionabili dall'utente (es. traguardi automatici).
const FEED_CATS = [...CATS, { id: "traguardo", Icon: Trophy, color: "#3E9C93" }, { id: "auguri", Icon: Cake, color: "#3E9C93" }];
  ...
  if (!user) {
    return (
      <div className="pb-40" data-testid="community-auth-gate">
        <div className="relative overflow-hidden rounded-3xl p-7 text-white shadow-xl text-center border border-[#3E9C93]/40"
          style={{ background: "linear-gradient(135deg,#3E9C93 0%,#1B2A38 85%)" }}>
          <div className="w-16 h-16 rounded-2xl bg-[#3E9C93]/20 border border-[#3E9C93]/40 flex items-center justify-center mx-auto mb-4">
            <Users className="w-8 h-8 text-[#3E9C93]" />
          </div>
          ...
          <button data-testid="community-register-btn" onClick={() => setAuthOpen(true)}
            className="mt-5 w-full bg-[#3E9C93] text-white font-bold px-5 py-3.5 rounded-2xl shadow-[0_4px_14px_rgba(255,107,0,0.4)] active:scale-98 hover:bg-[#ff8226] transition-all">
    ...
    <div className="pb-40" data-testid="community">
      ...
      <div className="grid grid-cols-4 gap-2 mb-4" data-testid="community-quick-actions">
        <button data-testid="open-my-profile" onClick={() => setProfileUser(user.user_id)}
          className="flex flex-col items-center gap-1.5 py-3 rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] shadow-sm active:scale-95 hover:border-[#3E9C93]/60 transition-all">
          <div className="relative w-9 h-9 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/15 flex items-center justify-center">
            <UserPlus className="w-5 h-5 text-[#3E9C93]" />
          </div>
        ...
        <button data-testid="community-friends-btn" onClick={() => setFriendsOpen(true)}
          ...
          <div className="relative w-9 h-9 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/15 flex items-center justify-center">
            <UserPlus className="w-5 h-5 text-[#3E9C93]" />
            {friendReqCount > 0 && <span data-testid="friends-req-badge" className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] px-1 rounded-full bg-[#3E9C93] text-white text-[10px] font-extrabold flex items-center justify-center ring-2 ring-white">{friendReqCount}</span>}
          </div>
        ...
        <button data-testid="community-messages-btn" onClick={() => { if (needLogin()) return; setChatUser(null); setChatOpen(true); }}
          className="flex flex-col items-center gap-1.5 py-3 rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] shadow-sm active:scale-95 hover:border-[#3E9C93]/60 transition-all">
          <div className="relative w-9 h-9 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/15 flex items-center justify-center">
            <MessageCircle className="w-5 h-5 text-[#3E9C93]" />
            {msgUnread > 0 && <span data-testid="messages-unread-badge" className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] px-1 rounded-full bg-[#3E9C93] text-white text-[10px] font-extrabold flex items-center justify-center ring-2 ring-white">{msgUnread > 9 ? "9+" : msgUnread}</span>}
          </div>
      ...
      {(() => {
        ...
        if (!done) return null;
        return (
          <div data-testid="community-badge" className="flex items-center gap-2 mb-4 rounded-2xl bg-gradient-to-r from-[#3E9C93] to-[#3E9C93] text-white px-4 py-2.5 shadow">
      ...
      <div data-testid="community-composer" className="bg-[#3E9C93]/10 border border-[#3E9C93]/30 rounded-2xl p-4 mb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 mb-2">
          {CATS.map(({ id, Icon, color }) => (
            <button key={id} data-testid={`community-cat-${id}`} onClick={() => setCat(id)}
              className={`flex flex-col items-center gap-1 py-2 rounded-2xl shadow-md border border-amber-900/40 border text-[11px] font-semibold transition-all ${cat === id ? "text-white border-transparent" : "bg-white dark:bg-[#1B2A38] text-[#3F4A54] dark:text-[#AEB8BF] border-[#2A3B49] dark:border-[#2A3B49]"}`}
        ...
        <div className="flex items-center gap-2 mt-2">
          <label data-testid="community-photo-btn" className="flex items-center gap-1.5 px-3 py-2 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] text-sm font-semibold text-[#3F4A54] dark:text-[#AEB8BF] cursor-pointer">
            {uploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <ImagePlus className="w-4 h-4 text-[#3E9C93]" />} {tri("Foto", "Foto", "Photo", "Foto")}
      ...
      <div className="flex items-center justify-between mb-1.5 min-h-[20px]">
        <button data-testid="community-manage-follows" onClick={() => { if (needLogin()) return; setManageOpen(true); }} className="text-[12px] font-semibold text-[#3E9C93] active:scale-95 flex items-center gap-1">
        ...
        {CATS.some((c) => hasNew(c.id)) && (
          <button data-testid="community-mark-all-read" onClick={markAllSeen} className="text-[12px] font-semibold text-[#3E9C93] active:scale-95 flex items-center gap-1">
      ...
      {manageOpen && (
        <div data-testid="follows-manager" className="fixed inset-0 z-[300] flex items-end sm:items-center justify-center p-4" onClick={() => setManageOpen(false)}>
          ...
          <div className="relative w-full max-w-md bg-[#14212C] border border-[#2A3B49] rounded-3xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#2A3B49]">
              ...
              <button data-testid="follows-manager-close" onClick={() => setManageOpen(false)} className="w-8 h-8 rounded-full bg-[#1B2A38] flex items-center justify-center text-[#7E8A93] active:scale-95"><X className="w-4 h-4" /></button>
            </div>
            <div className="max-h-[60vh] overflow-y-auto p-2">
              <div data-testid="channel-email-pref" className="mb-2 p-3 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/8 border border-[#3E9C93]/25">
                <p className="text-[12px] font-bold text-[#e4eff8] mb-2 flex items-center gap-1.5"><Mail className="w-3.5 h-3.5 text-[#3E9C93]" />{tri("Email dei canali seguiti", "E-Mails der abonnierten Kanäle", "Followed-channel emails", "Emails de canales seguidos", "E-mails des canaux suivis")}</p>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-3 -mx-1 px-1" data-testid="community-filters">
        <button data-testid="community-filter-all" onClick={() => setFilter("all")} className={`px-3 py-1.5 rounded-full text-sm font-semibold whitespace-nowrap border ${filter === "all" ? "bg-[#3E9C93] text-white border-[#3E9C93]" : "bg-white dark:bg-[#1B2A38] text-[#3F4A54] dark:text-[#AEB8BF] border-[#2A3B49] dark:border-[#2A3B49]"}`}>{tri("Tutti", "Alle", "All", "Todos")}</button>
        {CATS.map((c) => (
      ...
      <div className="flex items-center gap-2 mb-2 mt-1" data-testid="community-board-heading">
        <span className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-[#3E9C93]/15" style={{ boxShadow: "inset 0 0 0 1px #3E9C93" }}><Users className="w-4 h-4 text-[#3E9C93]" /></span>
      ...
      <div data-testid="feed-toggle" className="flex gap-2 mb-3">
        {[["all", tri("Recenti", "Neueste", "Recent", "Recientes")], ["popular", tri("Popolari", "Beliebt", "Popular", "Populares")], ["friends", tri("Amici", "Freunde", "Friends", "Amigos")]].map(([id, lbl]) => (
          <button key={id} data-testid={`feed-tab-${id}`} onClick={() => { if (id === "friends" && needLogin()) return; setFeed(id); }}
            className={`flex-1 py-2 rounded-2xl shadow-md border border-amber-900/40 text-sm font-semibold transition-all ${feed === id ? "bg-[#1B2A38] text-white shadow-sm" : "bg-[#e4eff8] dark:bg-[#1B2A38] text-[#7E8A93]"}`}>{lbl}</button>
        ))}
      </div>
      {loading ? (
        ...
        <div className="space-y-3" data-testid="community-feed">
          {(() => {
            ...
            const WI = W.Icon;
            return (
              <div data-testid={`forum-welcome-${filter}`} className="rounded-2xl border border-[#3E9C93]/50 bg-gradient-to-br from-[#2a1a0d] to-[#1a1a1a] p-4 shadow-md">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-[#3E9C93]/20" style={{ boxShadow: "inset 0 0 0 1px #3E9C93" }}><WI className="w-4.5 h-4.5 text-[#3E9C93]" /></span>
                  <div className="min-w-0">
                    <p className="text-[10px] font-extrabold uppercase tracking-wider text-[#3E9C93]">📌 {tri("Fissato · MikiLab", "Angepinnt · MikiLab", "Pinned · MikiLab", "Fijado · MikiLab")}</p>
          ...
          {visible.map((p) => {
            ...
            return (
              <div key={p.id} data-testid={`community-post-${p.id}`} className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-4 shadow-md hover:border-[#3E9C93]/40 transition-colors" style={{ borderLeft: `3px solid ${C.color}` }}>
                ...
                <div className="flex items-center gap-4 mt-3 pt-2 border-t border-[#2A3B49] dark:border-[#2A3B49]">
                  ...
                  <div className="mt-3 space-y-2">
                    {(p.comments || []).map((c) => (
                      <div key={c.id} className="flex gap-2 text-sm items-start">
                        <div className="w-6 h-6 rounded-full overflow-hidden shrink-0 bg-[#3E9C93] flex items-center justify-center text-white text-[10px] font-bold">{c.author_avatar ? <img src={c.author_avatar} alt="" className="w-full h-full object-cover" /> : (c.author_name || "F")[0].toUpperCase()}</div>
                        <div className="min-w-0"><span className="font-semibold text-[#3E9C93]">{c.author_name}:</span> <span className="text-[#3F4A54] dark:text-[#AEB8BF]">{lang === "de" ? (c.text_de || c.text) : lang === "es" ? (c.text_es || c.text_en || c.text) : lang === "en" ? (c.text_en || c.text) : c.text}</span></div>
                      </div>
                    ))}
                    {commentFor === p.id && (
                      <div className="flex gap-2 mt-1">
                        ...
                        <button data-testid={`community-comment-send-${p.id}`} onClick={() => sendComment(p.id)} className="px-3 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93] text-white"><Send className="w-4 h-4" /></button>
                      </div>
                    )}
                  </div>
                )}
