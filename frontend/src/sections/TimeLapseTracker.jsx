  ...
  return (
    <div className="pb-8" data-testid="timelapse">
      ...
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#0D1520] shadow-xl mb-5" style={{ background: "linear-gradient(135deg,#3E9C93,#3E9C93 60%,#3E9C93)" }}>
        ...
        <p className="text-[#0D1520]/85 text-sm mt-2 leading-snug">{L("Fotografa l'impasto all'inizio e adesso, segna quanto è cresciuto: ti dico se ha raddoppiato ed è pronto.", "Fotografiere Start & Jetzt, markiere den Anstieg.", "Photograph start & now, mark the rise: I'll tell you when it's doubled.", "Fotografía inicio y ahora, marca el crecimiento.")}</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {[["startPhoto", L("Inizio", "Start", "Start", "Inicio")], ["nowPhoto", L("Adesso", "Jetzt", "Now", "Ahora")]].map(([k, label]) => (
          <label key={k} data-testid={`timelapse-photo-${k}`} className="rounded-2xl bg-[#0D1520] dark:bg-[#1B2A38] border border-dashed border-[#2A3B49] dark:border-[#2A3B49] p-3 flex flex-col items-center gap-2 cursor-pointer min-h-[120px] justify-center">
      ...
      <div className="flex gap-2">
        <button data-testid="timelapse-start" onClick={start} className="flex-1 flex items-center justify-center gap-2 bg-[#3E9C93] hover:bg-[#3E9C93] text-[#0D1520] font-semibold py-3 rounded-2xl active:scale-98 transition-all"><Play className="w-4 h-4" /> {L("Avvia timer", "Timer starten", "Start timer", "Iniciar")}</button>
