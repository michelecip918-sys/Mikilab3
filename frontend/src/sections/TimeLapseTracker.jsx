...
export default function TimeLapseTracker({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="timelapse">
      {onBack && <button data-testid="timelapse-back" onClick={onBack} className="flex items-center gap-1 text-[#c94f00] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back")}</button>}
      ...
      <div className="flex flex-col items-center mb-5">
        <svg width="140" height="140" viewBox="0 0 140 140" data-testid="timelapse-ring">
          ...
          <circle cx="70" cy="70" r={R} fill="none" stroke={ready ? "#2e8b6f" : "#c94f00"} strokeWidth="12" strokeLinecap="round"
            strokeDasharray={C} strokeDashoffset={C - (pct / 100) * C} transform="rotate(-90 70 70)" style={{ transition: "stroke-dashoffset .5s" }} />
          <text x="70" y="66" textAnchor="middle" className="font-display" fontSize="26" fontWeight="bold" fill="#c94f00">+{rise}%</text>
          <text x="70" y="88" textAnchor="middle" fontSize="11" fill="#c94f00">{ready ? L("PRONTO", "FERTIG", "READY", "LISTO") : L("in crescita", "wächst", "rising", "creciendo")}</text>
        </svg>
        <p className="text-[13px] text-[#c94f00] mt-2">⏱️ {eh ? `${eh}h ` : ""}{em}m {L("dall'inizio", "seit Start", "since start", "desde inicio")}</p>
      </div>

      {/* Slider rise */}
      <div className="rounded-2xl bg-[#121212] dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-4 shadow-sm mb-4">
        <p className="text-[12px] font-semibold text-[#c94f00] dark:text-[#AEB8BF] mb-2">{L("Quanto è cresciuto? (0 = uguale, 100 = raddoppiato)", "Wie stark gewachsen? (100 = verdoppelt)", "How much did it grow? (100 = doubled)", "¿Cuánto creció? (100 = duplicado)")}</p>
        <input data-testid="timelapse-rise" type="range" min="0" max="150" value={rise} onChange={(e) => setRiseVal(Number(e.target.value))} className="w-full accent-[#c94f00]" />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {[["startPhoto", L("Inizio", "Start", "Start", "Inicio")], ["nowPhoto", L("Adesso", "Jetzt", "Now", "Ahora")]].map(([k, label]) => (
          <label key={k} data-testid={`timelapse-photo-${k}`} className="rounded-2xl bg-[#121212] dark:bg-[#1e1e1e] border border-dashed border-[#2e2e2e] dark:border-[#2e2e2e] p-3 flex flex-col items-center gap-2 cursor-pointer min-h-[120px] justify-center">
            {data?.[k] ? <img src={data[k]} alt={label} className="w-full h-24 object-cover rounded-2xl shadow-md border border-amber-900/40" /> : (uploading === k ? <Loader2 className="w-6 h-6 animate-spin text-[#c94f00]" /> : <Camera className="w-7 h-7 text-[#c94f00]" />)}
            <span className="text-[12px] font-bold text-[#c94f00]">{label}</span>
      ...
      <div className="flex gap-2">
        <button data-testid="timelapse-start" onClick={start} className="flex-1 flex items-center justify-center gap-2 bg-[#c94f00] hover:bg-[#c94f00] text-[#121212] font-semibold py-3 rounded-2xl active:scale-98 transition-all"><Play className="w-4 h-4" /> {L("Avvia timer", "Timer starten", "Start timer", "Iniciar")}</button>
        <button data-testid="timelapse-reset" onClick={reset} className="px-4 flex items-center justify-center bg-[#1e1e1e] text-[#c94f00] font-semibold py-3 rounded-2xl active:scale-98 transition-all"><RotateCcw className="w-4 h-4" /></button>
      </div>
    </div>
  );
}
