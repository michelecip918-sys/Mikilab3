...
export default function CalcolatoreMetodo({ onBack }) {
  ...
  const lbl = "text-[12px] font-semibold text-[#6B5546] dark:text-[#AEB8BF] mb-1 flex items-center gap-1.5";
  ...
  return (
    <div className="pb-8" data-testid="calc-metodo">
      ...
      {/* Risultati */}
      <div data-testid="calc-results" className="rounded-3xl bg-[#FAF5EC] dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] overflow-hidden shadow-md">
        <div className="bg-[#8C4A27] text-[#FFFDF9] px-5 py-3"><p className="font-display text-lg font-bold">{L("Ricetta calcolata", "Berechnetes Rezept", "Calculated recipe", "Receta calculada")}</p></div>
        <div className="divide-y divide-[#E6D8C3] dark:divide-[#38424B]">
          {[
            ...
            <div key={i} data-testid={`calc-row-${i}`} className="flex items-center justify-between px-5 py-2.5">
              <span className="text-[13px] text-[#6B5546] dark:text-[#AEB8BF]">{k}</span>
              <span className="font-mono-data font-bold text-[#2C1E16] dark:text-[#e4eff8]">{v}</span>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-px bg-[#E6D8C3] dark:bg-[#38424B]">
          <div className="bg-[#FEF3C7] dark:bg-[#2A2118] p-4 text-center">
            <p className="text-[11px] font-semibold text-[#92400E] flex items-center justify-center gap-1"><Thermometer className="w-3.5 h-3.5" /> {L("Temperatura acqua", "Wassertemperatur", "Water temperature", "Temp. agua")}</p>
            <p data-testid="calc-water-temp" className="font-display text-2xl font-bold text-[#8C4A27] mt-1">{r.waterTemp}°C</p>
          </div>
          <div className="bg-[#FEF3C7] dark:bg-[#2A2118] p-4 text-center">
            <p className="text-[11px] font-semibold text-[#92400E] flex items-center justify-center gap-1"><Clock className="w-3.5 h-3.5" /> {L("Impasto (spirale)", "Kneten (Spirale)", "Mixing (spiral)", "Amasado (espiral)")}</p>
            <p data-testid="calc-mix-min" className="font-display text-2xl font-bold text-[#8C4A27] mt-1">{r.mixMin} min</p>
          </div>
        </div>
      </div>

      <div className="mt-5 rounded-2xl bg-[#F2E8D5] border border-[#E6D8C3] p-4">
        <p className="font-display text-base font-bold text-[#8C4A27] flex items-center gap-2 mb-2"><Lightbulb className="w-4.5 h-4.5" /> {L("Alta alveolatura — consigli", "Offene Porung — Tipps", "Open crumb — tips", "Alveolado abierto — consejos")}</p>
        <ul className="text-[13px] text-[#6B5546] leading-relaxed space-y-1.5 list-disc pl-4">
