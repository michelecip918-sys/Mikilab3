...
export default function MikiLab() {
    ...
    return (
      <div className="pb-4">
        ...
        <div data-testid="lievito-page" className="space-y-4">
          <div className="rounded-2xl p-5 bg-[#3E9C93]/12 border border-[#3E9C93]/40">
            <div className="flex items-center gap-2 mb-2">
              <Wheat className="w-5 h-5 text-[#3E9C93]" />
          ...
          {lm.sections.map((s, i) => (
            <div key={i} data-testid={`lm-section-${i}`} className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-5">
  ...
  return (
    <div>
      ...
      <div data-testid="bio-card" className="mb-5 rounded-3xl overflow-hidden bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] text-white">
        <div className="p-5">
          <div className="flex justify-center mb-3">
            <img
              ...
              className="w-24 h-24 rounded-2xl object-cover ring-2 ring-[#3E9C93]/70 shadow-lg"
            />
          </div>
        ...
        <div className="px-5 pb-5 pt-4 border-t border-white/15">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex items-center gap-2">
              <Heart className="w-5 h-5 text-[#3E9C93]" />
          ...
          <figure className="mb-4">
            <img data-testid="bio-cartoon" src={`${process.env.PUBLIC_URL}/michele-cartoon.jpg`} alt="Michele — MikiLab"
              className="w-40 h-40 mx-auto rounded-3xl object-cover border-2 border-[#3E9C93]/50 shadow-lg" />
            <figcaption className="text-center text-[11px] text-white/70 mt-2 italic">{t("bio_dough_caption")}</figcaption>
          </figure>
      ...
      <div data-testid="method-section" className="mt-2 mb-5 rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-5">
        <div className="flex items-center gap-2 mb-2">
        ...
        <div className="grid gap-3 mt-4 sm:grid-cols-2">
          {[["_606t-4KXT4", "method_video1_title"], ["HpOycYo1Cvc", "method_video2_title"]].map(([vid, tk]) => (
            <div key={vid} data-testid={`method-video-${vid}`} className="rounded-2xl shadow-md border border-amber-900/40 overflow-hidden border border-[#2A3B49] dark:border-[#2A3B49]">
      ...
      <div data-testid="thanks-card" className="mt-5 rounded-3xl p-5 bg-[#3E9C93]/12 border border-[#3E9C93]/30">
        <div className="flex items-center gap-2 mb-2">
          <BookHeart className="w-5 h-5 text-[#3E9C93] dark:text-[#a9d2ec]" />
...
function BackBtn({ onClick }) {
  return (
    <button data-testid="mikilab-back-btn" onClick={onClick} className="flex items-center gap-1 text-[#3E9C93] font-medium mb-4">
...
function AccessBtn({ testid, Icon, title, sub, onClick }) {
  return (
    <button data-testid={testid} onClick={onClick} className="w-full mb-3 flex items-center gap-4 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-4 shadow-sm active:scale-98 transition-all text-left">
      <div className="w-12 h-12 rounded-2xl bg-[#3E9C93]/15 border border-[#3E9C93]/30 flex items-center justify-center shrink-0">
        <Icon className="w-6 h-6 text-[#3E9C93]" />
      </div>
