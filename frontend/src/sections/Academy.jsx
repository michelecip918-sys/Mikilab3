...
export default function Academy() {
  ...
  return (
    <div data-testid="academy-page" className="pb-4 space-y-6">
      ...
      {/* Corsi */}
      <div>
        <h2 className="font-display text-sm font-bold uppercase tracking-wide text-[#ff6b00] mb-3 flex items-center gap-2">
              ...
              <div key={c.id} data-testid={`academy-course-${c.id}`} className="rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] shadow-sm overflow-hidden">
                <div className="p-4">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="shrink-0 font-mono-data font-bold text-[#ff6b00]">{euro(c.price_cents)}</span>
                  </div>
          ...
          <div className="rounded-2xl bg-[#ff6b00]/10 border border-[#ff6b00]/30 p-4">
            <div className="flex items-start justify-between gap-2">
              <p className="text-sm text-[#3F4A54] dark:text-[#AEB8BF] leading-snug flex items-center gap-1.5">
                <Sparkles className="w-4 h-4 text-[#ff6b00]" /> {tri("Videochiamata su misura col Maestro per ricette, forni e produzione.", "Maßgeschneiderter Videocall mit dem Meister zu Rezepten, Öfen und Produktion.", "Tailored video call with the Master on recipes, ovens and production.")}
              </p>
              <span className="shrink-0 font-mono-data font-bold text-[#ff6b00]">{euro(cat.consult.price_cents)}</span>
            </div>
