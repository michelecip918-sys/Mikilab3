...
export default function OrdersManager({ store, stores }) {
  ...
  const STATUS = {
    ...
    ricevuto: { label: tri("Ricevuto", "Erhalten", "Received"), color: "#ff6b00" },
  };
  ...
  return (
    <div className="pb-40" data-testid="orders-manager">
      <p className="text-xs text-[#7E8A93] mb-3">{tri("Ordini del negozio", "Bestellungen der Filiale", "Orders for store")}: <b className="text-[#ff6b00]">{store ? storeName(store) : tri("tutti", "alle", "all")}</b></p>
      ...
      {showForm && (
        <div data-testid="order-form" className="bg-[#ff6b00]/10 border border-[#ff6b00]/30 rounded-2xl p-4 mb-5 space-y-3">
          <div>
          ...
          <div className="space-y-2">
            <p className="text-xs font-bold uppercase text-[#ff6b00]">{tri("Articoli", "Artikel", "Items")}</p>
            {form.items.map((it) => (
            ...
            <button data-testid="order-item-add" onClick={addItem} className="text-sm font-semibold text-[#ff6b00] flex items-center gap-1"><Plus className="w-4 h-4" /> {tri("Aggiungi riga", "Zeile hinzufügen", "Add row")}</button>
          </div>
          ...
          <button data-testid="order-create" onClick={create} disabled={busy} className="w-full flex items-center justify-center gap-2 bg-[#ff6b00] hover:bg-[#336a94] text-white font-semibold py-3 rounded-xl active:scale-98 disabled:opacity-50">
          ...
          {orders.map((o) => {
            ...
            return (
              <div key={o.id} data-testid={`order-card-${o.id}`} className="bg-white dark:bg-[#1e1e1e] border border-[#2b2b2b] dark:border-[#2e2e2e] rounded-2xl p-4 shadow-sm">
                ...
                {o.total > 0 && <p className="text-sm font-bold text-[#ff6b00] mt-1">{tri("Totale", "Summe", "Total")}: € {o.total}</p>}
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {o.status !== "ricevuto" && <button data-testid={`order-mark-received-${o.id}`} onClick={() => setStatus(o, "ricevuto")} className="flex items-center gap-1 text-xs font-semibold text-[#ff6b00]"><PackageCheck className="w-3.5 h-3.5" /> {tri("Segna ricevuto", "Als erhalten", "Mark received")}</button>}
