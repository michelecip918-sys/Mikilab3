...
export default function OrdersManager({ store, stores }) {
  ...
  const STATUS = {
    ...
    ricevuto: { label: tri("Ricevuto", "Erhalten", "Received"), color: "#6B8E62" },
  };
  ...
  return (
    <div className="pb-40" data-testid="orders-manager">
      <p className="text-xs text-[#7E8A93] mb-3">{tri("Ordini del negozio", "Bestellungen der Filiale", "Orders for store")}: <b className="text-[#5E8B7E]">{store ? storeName(store) : tri("tutti", "alle", "all")}</b></p>
      ...
      {showForm && (
        <div data-testid="order-form" className="bg-[#6E8CA0]/10 border border-[#6E8CA0]/30 rounded-2xl p-4 mb-5 space-y-3">
          ...
          <div className="space-y-2">
            <p className="text-xs font-bold uppercase text-[#5E8B7E]">{tri("Articoli", "Artikel", "Items")}</p>
            {form.items.map((it) => (
            ...
            <button data-testid="order-item-add" onClick={addItem} className="text-sm font-semibold text-[#5E8B7E] flex items-center gap-1"><Plus className="w-4 h-4" /> {tri("Aggiungi riga", "Zeile hinzufügen", "Add row")}</button>
          </div>
          ...
          <button data-testid="order-create" onClick={create} disabled={busy} className="w-full flex items-center justify-center gap-2 bg-[#6B8E62] hover:bg-[#5a7a53] text-white font-semibold py-3 rounded-xl active:scale-98 disabled:opacity-50">
      ...
      {loading ? (
        ...
        <div className="space-y-3" data-testid="orders-list">
          ...
          {orders.map((o) => {
            ...
            return (
              <div key={o.id} data-testid={`order-card-${o.id}`} className="bg-white dark:bg-[#232A31] border border-[#D7E1DB] dark:border-[#38424B] rounded-2xl p-4 shadow-sm">
                <div className="flex items-start gap-2">
                  <div className="w-10 h-10 rounded-xl bg-[#6E8CA0]/15 flex items-center justify-center shrink-0"><Truck className="w-5 h-5 text-[#6E8CA0]" /></div>
                  <div className="min-w-0 flex-1">
                    <p className="font-display text-lg font-bold text-[#2B303B] dark:text-[#EAF0EC] leading-tight">{o.supplier}</p>
                ...
                {o.total > 0 && <p className="text-sm font-bold text-[#6B8E62] mt-1">{tri("Totale", "Summe", "Total")}: € {o.total}</p>}
                ...
                <div className="grid grid-cols-3 gap-2 mt-3">
                  ...
                  <button data-testid={`order-print-${o.id}`} onClick={() => printOrder(o)} className="flex items-center justify-center gap-1 bg-white dark:bg-[#1F252B] border border-[#D7E1DB] dark:border-[#38424B] text-[#2B303B] dark:text-[#EAF0EC] text-xs font-semibold py-2.5 rounded-xl active:scale-95"><Printer className="w-4 h-4 text-[#6B8E62]" /> {tri("Stampa", "Druck", "Print")}</button>
                </div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {o.status !== "ricevuto" && <button data-testid={`order-mark-received-${o.id}`} onClick={() => setStatus(o, "ricevuto")} className="flex items-center gap-1 text-xs font-semibold text-[#6B8E62]"><PackageCheck className="w-3.5 h-3.5" /> {tri("Segna ricevuto", "Als erhalten", "Mark received")}</button>}
