...
export default function OrdersManager({ store, stores }) {
  ...
  const STATUS = {
    ...
    inviato: { label: tri("Inviato", "Gesendet", "Sent"), color: "#3E9C93" },
    ricevuto: { label: tri("Ricevuto", "Erhalten", "Received"), color: "#3E9C93" },
  };
  ...
  return (
    <div className="pb-40" data-testid="orders-manager">
      ...
      {showForm && (
        <div data-testid="order-form" className="bg-[#3E9C93]/10 border border-[#3E9C93]/30 rounded-2xl p-4 mb-5 space-y-3">
          <div>
          ...
          <div className="space-y-2">
            <p className="text-xs font-bold uppercase text-[#3E9C93]">{tri("Articoli", "Artikel", "Items")}</p>
            {form.items.map((it) => (
            ...
            <button data-testid="order-item-add" onClick={addItem} className="text-sm font-semibold text-[#3E9C93] flex items-center gap-1"><Plus className="w-4 h-4" /> {tri("Aggiungi riga", "Zeile hinzufügen", "Add row")}</button>
          </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3" data-testid="orders-list">
          ...
          {orders.map((o) => {
            ...
            return (
              <div key={o.id} data-testid={`order-card-${o.id}`} className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-4 shadow-sm">
                <div className="flex items-start gap-2">
                  <div className="w-10 h-10 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/15 flex items-center justify-center shrink-0"><Truck className="w-5 h-5 text-[#3E9C93]" /></div>
                ...
                {o.total > 0 && <p className="text-sm font-bold text-[#3E9C93] mt-1">{tri("Totale", "Summe", "Total")}: € {o.total}</p>}
                {o.note && <p className="text-xs text-[#7E8A93] mt-1 italic">{o.note}</p>}

                {/* Invio */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mt-3">
                  <button data-testid={`order-send-email-${o.id}`} onClick={() => sendEmail(o)} className="flex items-center justify-center gap-1 bg-[#3E9C93] text-white text-xs font-semibold py-2.5 rounded-2xl shadow-md border border-amber-900/40 active:scale-95"><Mail className="w-4 h-4" /> Email</button>
                  ...
                  <button data-testid={`order-print-${o.id}`} onClick={() => printOrder(o)} className="flex items-center justify-center gap-1 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] text-[#2B303B] dark:text-[#e4eff8] text-xs font-semibold py-2.5 rounded-2xl shadow-md border border-amber-900/40 active:scale-95"><Printer className="w-4 h-4 text-[#3E9C93]" /> {tri("Stampa", "Druck", "Print")}</button>
                </div>

                {/* Stato + elimina */}
                <div className="flex items-center gap-2 mt-2">
                  {o.status !== "inviato" && <button data-testid={`order-mark-sent-${o.id}`} onClick={() => setStatus(o, "inviato")} className="flex items-center gap-1 text-xs font-semibold text-[#3E9C93]"><Send className="w-3.5 h-3.5" /> {tri("Segna inviato", "Als gesendet", "Mark sent")}</button>}
                  {o.status !== "ricevuto" && <button data-testid={`order-mark-received-${o.id}`} onClick={() => setStatus(o, "ricevuto")} className="flex items-center gap-1 text-xs font-semibold text-[#3E9C93]"><PackageCheck className="w-3.5 h-3.5" /> {tri("Segna ricevuto", "Als erhalten", "Mark received")}</button>}
