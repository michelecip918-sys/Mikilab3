  ...
  return (
    <div className={embedded ? "" : "pb-24"}>
      ...
      <div className="rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-6 text-center">
        ...
        {loading ? (
          <div data-testid="scan-loading" className="inline-flex items-center gap-2 bg-[#c94f00] text-white font-semibold px-5 py-3.5 rounded-2xl opacity-70">
        ...
        {!loading && (
          <div className="mt-4 pt-4 border-t border-[#2e2e2e] dark:border-[#2e2e2e]">
            ...
            <button data-testid="scan-manual-btn" onClick={() => { setScanned(null); setDialogOpen(true); }}
              className="inline-flex items-center gap-2 bg-white dark:bg-[#1e1e1e] text-[#c94f00] dark:text-[#e4eff8] font-semibold px-5 py-3 rounded-2xl border-2 border-[#c94f00]/40 active:scale-97 transition-all">
              <PenLine className="w-5 h-5 text-[#c94f00]" /> {tri("Scrivi a mano", "Von Hand schreiben", "Write by hand")}
            </button>
          </div>
        )}
      </div>
      ...
      {pdfRecipes.length > 1 && (
        <div data-testid="pdf-recipes-list" className="mt-4 rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-4">
              ...
              return (
                <li key={i}>
                  <button data-testid={`pdf-recipe-${i}`} onClick={() => openPdfRecipe(i)}
                    className={`w-full flex items-center gap-3 text-left px-3.5 py-3 rounded-2xl shadow-md border border-amber-900/40 border transition-all active:scale-98 ${done ? "bg-[#2e8b6f]/10 border-[#2e8b6f]/40" : "bg-[#121212] dark:bg-[#181818] border-[#2e2e2e] dark:border-[#2e2e2e] hover:border-[#c94f00]"}`}>
                    {pageThumbs[String(r.page)] ? (
                      ...
                      done ? <CheckCircle2 className="w-5 h-5 text-[#2e8b6f] shrink-0" /> : <ScanLine className="w-5 h-5 text-[#c94f00] shrink-0" />
                    )}
                    <span className="flex-1 min-w-0">
                      ...
                      {r.page && <span className="block text-[10px] text-[#c94f00] font-semibold">{tri(`Pag. ${r.page}`, `S. ${r.page}`, `Page ${r.page}`, `Pág. ${r.page}`)}</span>}
                    </span>
          ...
          <button data-testid="pdf-recipes-done" onClick={() => { setPdfRecipes([]); setSavedIdx([]); }}
            className="mt-3 text-[12px] font-bold text-[#c94f00] underline">
        ...
        <div data-testid="inbound-email-panel" className="mt-4 rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-5">
          <div className="flex items-center gap-2 mb-2 text-[#c94f00]">
            ...
            {inbound.enabled
              ...
              : <span className="ml-auto text-[10px] font-bold text-[#c94f00] bg-[#c94f00]/10 px-2 py-0.5 rounded-full">{tri("In arrivo", "Bald", "Coming soon", "Próximamente")}</span>}
          </div>
          ...
          {inbound.enabled && inbound.inbound_address && (
            <button data-testid="inbound-copy-address" onClick={() => { navigator.clipboard?.writeText(inbound.inbound_address); toast.success(tri("Indirizzo copiato", "Adresse kopiert", "Address copied", "Dirección copiada")); }}
              className="w-full inline-flex items-center gap-2 bg-[#121212] dark:bg-[#181818] border border-[#c94f00]/40 text-[#c94f00] dark:text-[#a9d2ec] font-semibold px-4 py-2.5 rounded-2xl shadow-md border border-amber-900/40 active:scale-98">
              <span className="flex-1 text-left truncate text-sm">{inbound.inbound_address}</span>
              <Copy className="w-4 h-4 shrink-0" />
            </button>
          )}
          {inbound.webhook_url && (
            <div data-testid="inbound-webhook-admin" className="mt-3 rounded-2xl shadow-md border border-amber-900/40 bg-[#c94f00]/10 border border-[#c94f00]/30 p-3">
              <p className="text-[11px] font-bold text-[#c94f00] mb-1">{tri("Admin · URL webhook per la Route Mailgun", "Admin · Webhook-URL für Mailgun-Route", "Admin · Webhook URL for Mailgun Route", "Admin · URL webhook Mailgun")}</p>
              <button data-testid="inbound-copy-webhook" onClick={() => { navigator.clipboard?.writeText(inbound.webhook_url); toast.success(tri("URL webhook copiato", "Webhook-URL kopiert", "Webhook URL copied", "URL copiada")); }}
                className="w-full inline-flex items-center gap-2 bg-white dark:bg-[#181818] border border-[#c94f00]/40 text-[#c94f00] dark:text-[#a9d2ec] font-mono-data text-[11px] px-3 py-2 rounded-lg active:scale-98">
