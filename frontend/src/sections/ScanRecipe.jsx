  ...
  return (
    <div className={embedded ? "" : "pb-24"}>
      ...
      <div className="rounded-2xl bg-white dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] p-6 text-center">
        ...
        {loading ? (
          <div data-testid="scan-loading" className="inline-flex items-center gap-2 bg-[#8C4A27] text-white font-semibold px-5 py-3.5 rounded-2xl opacity-70">
        ...
        {!loading && (
          <div className="mt-4 pt-4 border-t border-[#E6D8C3] dark:border-[#38424B]">
            ...
            <button data-testid="scan-manual-btn" onClick={() => { setScanned(null); setDialogOpen(true); }}
              className="inline-flex items-center gap-2 bg-white dark:bg-[#232A31] text-[#6E371C] dark:text-[#e4eff8] font-semibold px-5 py-3 rounded-2xl border-2 border-[#8C4A27]/40 active:scale-97 transition-all">
              <PenLine className="w-5 h-5 text-[#8C4A27]" /> {tri("Scrivi a mano", "Von Hand schreiben", "Write by hand")}
            </button>
          </div>
        )}
      </div>
      ...
      {pdfRecipes.length > 1 && (
        <div data-testid="pdf-recipes-list" className="mt-4 rounded-2xl bg-white dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] p-4">
              ...
              return (
                <li key={i}>
                  <button data-testid={`pdf-recipe-${i}`} onClick={() => openPdfRecipe(i)}
                    className={`w-full flex items-center gap-3 text-left px-3.5 py-3 rounded-xl border transition-all active:scale-98 ${done ? "bg-[#2e8b6f]/10 border-[#2e8b6f]/40" : "bg-[#FAF5EC] dark:bg-[#1F252B] border-[#E6D8C3] dark:border-[#38424B] hover:border-[#8C4A27]"}`}>
                    {pageThumbs[String(r.page)] ? (
                      <img data-testid={`pdf-recipe-thumb-${i}`} src={pageThumbs[String(r.page)]} alt=""
                        className="w-12 h-16 object-cover rounded-md border border-[#E6D8C3] dark:border-[#38424B] shrink-0 bg-white" />
                    ) : (
                      done ? <CheckCircle2 className="w-5 h-5 text-[#2e8b6f] shrink-0" /> : <ScanLine className="w-5 h-5 text-[#8C4A27] shrink-0" />
                    )}
          ...
          <button data-testid="pdf-recipes-done" onClick={() => { setPdfRecipes([]); setSavedIdx([]); }}
            className="mt-3 text-[12px] font-bold text-[#8C4A27] underline">
      ...
      {inbound && inbound.your_email && (
        <div data-testid="inbound-email-panel" className="mt-4 rounded-2xl bg-white dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] p-5">
          <div className="flex items-center gap-2 mb-2 text-[#8C4A27]">
          ...
          {inbound.enabled && inbound.inbound_address && (
            <button data-testid="inbound-copy-address" onClick={() => { navigator.clipboard?.writeText(inbound.inbound_address); toast.success(tri("Indirizzo copiato", "Adresse kopiert", "Address copied", "Dirección copiada")); }}
              className="w-full inline-flex items-center gap-2 bg-[#FAF5EC] dark:bg-[#1F252B] border border-[#8C4A27]/40 text-[#6E371C] dark:text-[#a9d2ec] font-semibold px-4 py-2.5 rounded-xl active:scale-98">
