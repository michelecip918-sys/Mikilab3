...
export default function AngoloRecupero({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="angolo-recupero">
      {onBack && <button data-testid="recupero-back" onClick={onBack} className="flex items-center gap-1 text-[#c94f00] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back", "Atrás")}</button>}
      ...
      <div className="flex gap-1.5 bg-[#1e1e1e] p-1.5 rounded-2xl mb-5 border border-[#2e2e2e]">
        {[["raffermo", L("Pane raffermo", "Altes Brot", "Stale bread", "Pan duro"), Croissant], ["esubero", L("Esubero LM", "Sauerteig-Rest", "Sourdough discard", "Descarte MM"), Sparkles]].map(([id, label, Icon]) => (
          <button key={id} data-testid={`recupero-tab-${id}`} onClick={() => setTab(id)} className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-2xl shadow-md border border-amber-900/40 text-[13px] font-bold transition-all ${tab === id ? "bg-[#2e8b6f] text-[#121212] shadow" : "text-[#c94f00]"}`}><Icon className="w-4 h-4" /> {label}</button>
        ))}
      </div>
        ...
        {list.map((r, i) => (
          <div key={i} data-testid={`recupero-item-${i}`} className="rounded-2xl bg-[#121212] dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="font-display text-lg font-bold text-[#c94f00] dark:text-[#e4eff8] leading-tight">{r.name}</p>
            </div>
            <p className="text-[13.5px] text-[#c94f00] dark:text-[#AEB8BF] leading-relaxed">{r.body}</p>
          </div>
        ))}
      </div>
    </div>
