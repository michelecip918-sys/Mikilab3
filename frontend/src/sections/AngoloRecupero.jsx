...
export default function AngoloRecupero({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="angolo-recupero">
      {onBack && <button data-testid="recupero-back" onClick={onBack} className="flex items-center gap-1 text-[#8C4A27] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back", "Atrás")}</button>}
      ...
      <div className="flex gap-1.5 bg-[#F2E8D5] p-1.5 rounded-2xl mb-5 border border-[#E6D8C3]">
        {[["raffermo", L("Pane raffermo", "Altes Brot", "Stale bread", "Pan duro"), Croissant], ["esubero", L("Esubero LM", "Sauerteig-Rest", "Sourdough discard", "Descarte MM"), Sparkles]].map(([id, label, Icon]) => (
          <button key={id} data-testid={`recupero-tab-${id}`} onClick={() => setTab(id)} className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-[13px] font-bold transition-all ${tab === id ? "bg-[#2e8b6f] text-[#FFFDF9] shadow" : "text-[#6B5546]"}`}><Icon className="w-4 h-4" /> {label}</button>
        ))}
      </div>

      <div className="space-y-3" data-testid="recupero-list">
        {list.map((r, i) => (
          <div key={i} data-testid={`recupero-item-${i}`} className="rounded-2xl bg-[#FAF5EC] dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-2xl">{r.emoji}</span>
              <p className="font-display text-lg font-bold text-[#2C1E16] dark:text-[#e4eff8] leading-tight">{r.name}</p>
            </div>
            <p className="text-[13.5px] text-[#6B5546] dark:text-[#AEB8BF] leading-relaxed">{r.body}</p>
          </div>
        ))}
      </div>
    </div>
