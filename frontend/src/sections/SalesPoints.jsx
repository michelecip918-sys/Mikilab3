  ...
  return (
    <div className="pb-40" data-testid="salespoints">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold text-[#2B303B] dark:text-[#EAF0EC]">{tri("Punti Vendita", "Verkaufspunkte", "Sales Points")}</h1>
      ...
      <div className="bg-white dark:bg-[#232A31] border border-[#D7E1DB] dark:border-[#38424B] rounded-2xl p-4 mb-4 space-y-3">
        <p className="text-xs font-bold uppercase text-[#5E8B7E]">{tri("Nuovo punto vendita", "Neuer Verkaufspunkt", "New sales point")}</p>
        ...
        <button data-testid="salespoint-add" onClick={add} className="w-full flex items-center justify-center gap-2 bg-[#5E8B7E] hover:bg-[#4C7368] text-white font-bold py-3 rounded-2xl active:scale-98"><Plus className="w-5 h-5" /> {tri("Aggiungi punto vendita", "Verkaufspunkt hinzufügen", "Add sales point")}</button>
      </div>
      ...
      {points.length === 0 ? (
        ...
        <div className="space-y-2" data-testid="salespoint-list">
          {points.map((p) => (
            <div key={p.id} data-testid={`salespoint-item-${p.id}`} className="flex items-start justify-between bg-white dark:bg-[#232A31] border border-[#D7E1DB] dark:border-[#38424B] rounded-xl px-3 py-2.5">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-[#2B303B] dark:text-[#EAF0EC] flex items-center gap-1.5"><Store className="w-3.5 h-3.5 text-[#5E8B7E]" /> {p.name}</p>
              ...
              <button data-testid={`salespoint-remove-${p.id}`} onClick={() => remove(p.id)} className="text-[#7E8A93] hover:text-[#C0574D] shrink-0 ml-2"><Trash2 className="w-4 h-4" /></button>
            </div>
          ))}
        </div>
      )}
