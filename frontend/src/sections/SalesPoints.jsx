  ...
  return (
    <div className="pb-40" data-testid="salespoints">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-[#3E9C93] flex items-center justify-center"><Store className="w-6 h-6 text-white" /></div>
        <div>
      ...
      <div className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-4 mb-4 space-y-3">
        <p className="text-xs font-bold uppercase text-[#3E9C93]">{tri("Nuovo punto vendita", "Neuer Verkaufspunkt", "New sales point")}</p>
      ...
      {points.length === 0 ? (
        ...
        <div className="space-y-2" data-testid="salespoint-list">
          {points.map((p) => (
            <div key={p.id} data-testid={`salespoint-item-${p.id}`} className="flex items-start justify-between bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 px-3 py-2.5">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-[#2B303B] dark:text-[#e4eff8] flex items-center gap-1.5"><Store className="w-3.5 h-3.5 text-[#3E9C93]" /> {p.name}</p>
              ...
              <button data-testid={`salespoint-remove-${p.id}`} onClick={() => remove(p.id)} className="text-[#7E8A93] hover:text-[#3E9C93] shrink-0 ml-2"><Trash2 className="w-4 h-4" /></button>
            </div>
          ))}
        </div>
      )}
