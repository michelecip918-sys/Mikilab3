...
export default function Marketplace() {
  ...
  const [items, setItems] = useState(() => {
    try { return JSON.parse(localStorage.getItem("mikilab_market") || "[]"); } catch { return []; }
  });
  ...
  useEffect(() => { try { localStorage.setItem("mikilab_market", JSON.stringify(items)); } catch { /* quota */ } }, [items]);
  ...
  return (
    <div className="pb-40">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-[#5E8B7E] flex items-center justify-center"><Store className="w-6 h-6 text-white" /></div>
        <div>
          <h1 className="font-display text-2xl font-bold text-[#2B303B] dark:text-[#EAF0EC]">{tri("Marketplace Usato", "Gebraucht-Markt", "Used Marketplace")}</h1>
      ...
      {showForm && (
        <div data-testid="market-form" className="bg-[#6E8CA0]/10 border border-[#6E8CA0]/30 rounded-2xl p-4 mb-5 space-y-3">
          ...
          <button data-testid="market-publish" onClick={publish} className="w-full bg-[#6B8E62] hover:bg-[#5a7a53] text-white font-semibold py-3 rounded-xl active:scale-98 transition-all">{tri("Pubblica", "Veröffentlichen", "Publish")}</button>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-3 -mx-1 px-1" data-testid="market-filters">
        <button data-testid="market-filter-all" onClick={() => setFilter("all")} className={`px-3 py-1.5 rounded-full text-sm font-semibold whitespace-nowrap border ${filter === "all" ? "bg-[#5E8B7E] text-white border-[#5E8B7E]" : "bg-white dark:bg-[#232A31] text-[#3F4A54] dark:text-[#AEB8BF] border-[#D7E1DB] dark:border-[#38424B]"}`}>{tri("Tutti", "Alle", "All")}</button>
        {CATS.map((c) => (
          <button key={c.id} data-testid={`market-filter-${c.id}`} onClick={() => setFilter(c.id)} className={`px-3 py-1.5 rounded-full text-sm font-semibold whitespace-nowrap border ${filter === c.id ? "bg-[#5E8B7E] text-white border-[#5E8B7E]" : "bg-white dark:bg-[#232A31] text-[#3F4A54] dark:text-[#AEB8BF] border-[#D7E1DB] dark:border-[#38424B]"}`}>{c.label}</button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3" data-testid="market-list">
        ...
        {visible.map((it) => (
          <div key={it.id} data-testid={`market-card-${it.id}`} className="bg-white dark:bg-[#232A31] border border-[#D7E1DB] dark:border-[#38424B] rounded-2xl overflow-hidden shadow-sm flex flex-col">
            {it.photo
            ...
            <div className="p-3 flex flex-col flex-1">
              <p className="font-display text-sm font-semibold text-[#2B303B] dark:text-[#EAF0EC] leading-tight line-clamp-2">{it.title}</p>
              <div className="flex items-center gap-1 mt-1 flex-wrap">
                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[#6E8CA0]/15 text-[#5E8B7E] font-semibold flex items-center gap-0.5"><Tag className="w-2.5 h-2.5" />{catLabel(it.cat)}</span>
              ...
              <div className="mt-auto pt-2">
                {it.price && <p className="font-mono-data text-lg font-bold text-[#6B8E62]">€ {priceFmt(it.price)}</p>}
                <div className="flex items-center gap-1.5 mt-1.5">
                  ...
                  <button data-testid={`market-remove-${it.id}`} onClick={() => remove(it.id)} className="p-2 rounded-lg border border-[#D7E1DB] dark:border-[#38424B] text-[#7E8A93] hover:text-[#E4572E]"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          </div>
