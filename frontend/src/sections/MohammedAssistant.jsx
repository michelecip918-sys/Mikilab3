          ...
          <div ref={listRef} className="max-h-72 overflow-y-auto space-y-2 pr-1">
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`markdown-body max-w-[85%] rounded-2xl px-3 py-2 text-sm ${m.role === "user" ? "bg-[#3E9C93] text-white" : "bg-[#e4eff8] dark:bg-[#1B2A38] text-[#2B303B] dark:text-[#e4eff8]"}`}>
                  {m.role === "assistant" && !m.content ? <Loader2 className="w-4 h-4 animate-spin text-[#3E9C93]" /> : <ReactMarkdown>{m.content}</ReactMarkdown>}
                </div>
              </div>
            ))}
          </div>
