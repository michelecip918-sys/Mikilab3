...
export default function ConvertitoreLieviti({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="conv-lieviti">
      {onBack && <button data-testid="conv-back" onClick={onBack} className="flex items-center gap-1 text-[#c94f00] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back", "Atrás")}</button>}
      ...
      <div className={card + " mb-3"}>
        <p className="font-display text-base font-bold text-[#c94f00] mb-2 flex items-center gap-1.5"><Beaker className="w-4.5 h-4.5" /> {L("Lievito di birra: fresco ⇄ secco", "Hefe: frisch ⇄ trocken", "Yeast: fresh ⇄ dry", "Levadura: fresca ⇄ seca")}</p>
        <div className="grid grid-cols-2 gap-3 items-end">
          ...
          <div><p className={lbl}>{L("Secco / istantaneo (g)", "Trocken (g)", "Dry (g)", "Seca (g)")}</p><div data-testid="conv-secco" className="rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] text-[#c94f00] font-mono-data font-bold px-3 py-2.5 text-center">{secco.toLocaleString(lang === "en" ? "en" : "it", { maximumFractionDigits: 1 })} g</div></div>
        </div>
        <p className="text-[11px] text-[#d4a373] mt-1.5">{L("Regola: 3 g di fresco = 1 g di secco.", "Regel: 3 g frisch = 1 g trocken.", "Rule: 3 g fresh = 1 g dry.", "Regla: 3 g fresca = 1 g seca.")}</p>
      </div>

      {/* 2 */}
      <div className={card + " mb-3"}>
        <p className="font-display text-base font-bold text-[#c94f00] mb-2">{L("Da lievito di birra a Lievito Madre", "Von Hefe zu Sauerteig", "From yeast to sourdough", "De levadura a masa madre")}</p>
        <div className="grid grid-cols-2 gap-3 items-end">
          ...
          <div><p className={lbl}>{L("Lievito Madre (g)", "Sauerteig (g)", "Sourdough (g)", "Masa madre (g)")}</p><div data-testid="conv-lm-out" className="rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] text-[#c94f00] font-mono-data font-bold px-3 py-2.5 text-center">{num(lmFromBirra)} g</div></div>
        </div>
        <p className="text-[11px] text-[#d4a373] mt-1.5">{L("Stima: ~20 g di LM maturo per 1 g di birra. Allunga la lievitazione di 2-4 ore.", "~20 g reifer Sauerteig pro 1 g Hefe. Gärzeit +2-4 h.", "~20 g mature sourdough per 1 g yeast. Add 2-4 h proofing.", "~20 g de masa madre por 1 g de levadura. +2-4 h de fermentación.")}</p>
      </div>

      {/* 3 */}
      <div className={card}>
        <p className="font-display text-base font-bold text-[#c94f00] mb-2 flex items-center gap-1.5"><Droplets className="w-4.5 h-4.5" /> {L("Converti il prefermento (idratazione)", "Vorteig umrechnen (Hydratation)", "Convert preferment (hydration)", "Convertir prefermento (hidratación)")}</p>
        ...
        <div data-testid="conv-pref-out" className="rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] p-3 text-[13px] text-[#c94f00] font-semibold space-y-1">
