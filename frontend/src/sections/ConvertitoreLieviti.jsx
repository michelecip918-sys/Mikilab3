...
export default function ConvertitoreLieviti({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="conv-lieviti">
      {onBack && <button data-testid="conv-back" onClick={onBack} className="flex items-center gap-1 text-[#8C4A27] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back", "Atrás")}</button>}
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#FFFDF9] shadow-xl mb-5" style={{ background: "linear-gradient(135deg,#8C4A27,#6E371C 60%,#4A3222)" }}>
        ...
        <p className="text-[#FFFDF9]/85 text-sm mt-2 leading-snug">{L("Passa da un tipo di lievito all'altro senza sbagliare le dosi: birra, Lievito Madre, Li.Co.Li., biga e poolish.", "Wechsle zwischen Hefearten ohne Dosierfehler.", "Switch between leavening types without dose errors.", "Cambia entre tipos de levadura sin errores.")}</p>
      </div>

      {/* 1 */}
      <div className={card + " mb-3"}>
        <p className="font-display text-base font-bold text-[#8C4A27] mb-2 flex items-center gap-1.5"><Beaker className="w-4.5 h-4.5" /> {L("Lievito di birra: fresco ⇄ secco", "Hefe: frisch ⇄ trocken", "Yeast: fresh ⇄ dry", "Levadura: fresca ⇄ seca")}</p>
        <div className="grid grid-cols-2 gap-3 items-end">
          ...
          <div><p className={lbl}>{L("Secco / istantaneo (g)", "Trocken (g)", "Dry (g)", "Seca (g)")}</p><div data-testid="conv-secco" className="rounded-xl bg-[#FEF3C7] text-[#8C4A27] font-mono-data font-bold px-3 py-2.5 text-center">{secco.toLocaleString(lang === "en" ? "en" : "it", { maximumFractionDigits: 1 })} g</div></div>
        </div>
        <p className="text-[11px] text-[#8C7362] mt-1.5">{L("Regola: 3 g di fresco = 1 g di secco.", "Regel: 3 g frisch = 1 g trocken.", "Rule: 3 g fresh = 1 g dry.", "Regla: 3 g fresca = 1 g seca.")}</p>
      </div>

      {/* 2 */}
      <div className={card + " mb-3"}>
        <p className="font-display text-base font-bold text-[#8C4A27] mb-2">{L("Da lievito di birra a Lievito Madre", "Von Hefe zu Sauerteig", "From yeast to sourdough", "De levadura a masa madre")}</p>
        <div className="grid grid-cols-2 gap-3 items-end">
          ...
          <div><p className={lbl}>{L("Lievito Madre (g)", "Sauerteig (g)", "Sourdough (g)", "Masa madre (g)")}</p><div data-testid="conv-lm-out" className="rounded-xl bg-[#FEF3C7] text-[#8C4A27] font-mono-data font-bold px-3 py-2.5 text-center">{num(lmFromBirra)} g</div></div>
        </div>
        <p className="text-[11px] text-[#8C7362] mt-1.5">{L("Stima: ~20 g di LM maturo per 1 g di birra. Allunga la lievitazione di 2-4 ore.", "~20 g reifer Sauerteig pro 1 g Hefe. Gärzeit +2-4 h.", "~20 g mature sourdough per 1 g yeast. Add 2-4 h proofing.", "~20 g de masa madre por 1 g de levadura. +2-4 h de fermentación.")}</p>
      </div>

      {/* 3 */}
      <div className={card}>
        <p className="font-display text-base font-bold text-[#8C4A27] mb-2 flex items-center gap-1.5"><Droplets className="w-4.5 h-4.5" /> {L("Converti il prefermento (idratazione)", "Vorteig umrechnen (Hydratation)", "Convert preferment (hydration)", "Convertir prefermento (hidratación)")}</p>
        ...
        <div data-testid="conv-pref-out" className="rounded-xl bg-[#FEF3C7] p-3 text-[13px] text-[#8C4A27] font-semibold space-y-1">
