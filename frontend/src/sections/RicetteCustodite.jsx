...
import { Landmark, Wheat, Share2, Printer, ChevronLeft, Scale, MapPin, Clock, Sparkles, ShieldCheck, Heart, BookOpen } from "lucide-react";
...
const CATS = [
  { id: "sud", label: { it: "Sud Italia", de: "Süditalien", en: "Southern Italy", es: "Sur de Italia", fr: "Sud de l'Italie" } },
...
// V78 — dati condivisi con "Sapori di Casa" e collegamento alle schede del ricettario.
export const CUSTODITE_RECIPES = RECIPES;
export const CUSTODITE_PLACE = PLACE;
// Scheda equivalente nel ricettario (stesso nome): da lì prendono foto vere e apertura diretta.
export const CUSTODITE_SEED_NAME = {
  matera: "Pane di Matera IGP", pane_basilicata: "Pane Lucano di Grano Duro", focaccia_lucana: "Focaccia Lucana ai Peperoni Cruschi",
  pane_patate_lucano: "Pane di Patate Lucano", pane_cafone_lucano: "Pane Cafone Lucano", altamura: "Pane di Altamura DOP",
  focaccia_barese: "Focaccia Barese", taralli: "Taralli Pugliesi", friselle: "Friselle Pugliesi", puccia: "Puccia Salentina",
  panettone_classico: "Panettone Artigianale MikiLab — Uvetta e Canditi (Classico)", pandoro: "Pandoro Artigianale MikiLab — Classico Vanigliato",
  colomba: "Colomba Artigianale MikiLab — Classica alle Mandorle", panettone_cioccolato: "Panettone al Cioccolato",
  veneziana_salata: "Panettone Salato Artigianale MikiLab — Speck e Formaggio", pane_colorato: "Pane Arcobaleno Naturale",
  baguette_colorata: "Baguette Colorata (Innovazione)", panettone_colorato: "Panettone Colorato (Innovazione)",
  pane_carbone: "Pane Nero al Carbone Vegetale", pane_zucca: "Pane alla Zucca", brezel: "Brezel",
  laugenbroetchen: "Laugenbrötchen (Panini di Laugen)", roggenbrot: "Roggenbrot (Pane di Segale)",
  vollkornbrot: "Vollkornbrot (Pane Integrale ai Semi)", kaisersemmel: "Kaisersemmel (Panino Kaiser)",
};
...
export default function RicetteCustodite({ initialId = null, onOpenRecipe = null }) {
  ...
  const [cat, setCat] = useState(initialId ? "all" : "sud"); // V76: si apre sul Sud Italia (Basilicata + Puglia)
  const [flour, setFlour] = useState(1000);
  const [qr, setQr] = useState("");
  const [miglIdx, setMiglIdx] = useState(0);
  // V78 — foto vere e collegamento alla scheda del ricettario (se esiste la ricetta con lo stesso nome).
  const [seedBy, setSeedBy] = useState({});
  useEffect(() => {
    let alive = true;
    recipesApi.list("mikilab").then((rows) => {
      if (!alive) return;
      const byName = {};
      (rows || []).forEach((x) => { byName[x.name] = x; });
      const m = {};
      Object.entries(CUSTODITE_SEED_NAME).forEach(([cid, nm]) => { if (byName[nm]) m[cid] = { id: byName[nm].id, image_url: byName[nm].image_url }; });
      Object.entries(CUSTODITE_IMG_ONLY).forEach(([cid, nm]) => { if (byName[nm] && !m[cid]) m[cid] = { image_url: byName[nm].image_url }; });
      setSeedBy(m);
    }).catch(() => {});
    return () => { alive = false; };
  }, []);
  ...
  const list = useMemo(() => {
    if (cat === "sud") return RECIPES.filter((r) => r.region === "basilicata" || r.region === "puglia").sort((a, b) => CAT_ORDER.indexOf(a.region) - CAT_ORDER.indexOf(b.region));
  ...
  if (recipe) {
    return (
      <div className="pb-40" data-testid="custodite-detail">
        ...
        <div className="print-area">
          <div className="rounded-3xl overflow-hidden border border-border dark:border-border bg-card dark:bg-card shadow-sm">
            <div className="relative h-40">
              <img src={imgOf(recipe)} onError={onImgErr} alt="" className="w-full h-full object-cover" />
        ...
        {seedBy[recipe.id] && seedBy[recipe.id].id && onOpenRecipe && (
          <button data-testid="custodite-open-ricettario" onClick={() => onOpenRecipe(seedBy[recipe.id].id)}
            className="no-print mt-4 w-full flex items-center justify-center gap-2 bg-card dark:bg-card border border-primary/40 text-foreground font-semibold py-3 rounded-2xl shadow-md active:scale-95 transition-all">
            <BookOpen className="w-4 h-4 text-primary" /> {L({ it: "Vedi la scheda nel ricettario", de: "Rezept im Rezeptbuch ansehen", en: "See it in the recipe book", es: "Ver la ficha en el recetario", fr: "Voir la fiche dans le livre de recettes" })}
          </button>
        )}
        ...
        {list.map((r) => (
          <div key={r.id} className="relative">
            <button data-testid={`custodite-open-${r.id}`} onClick={() => setOpenId(r.id)}
              ...
              <img src={imgOf(r)} onError={onImgErr} alt="" className="w-24 h-24 object-cover shrink-0" />
