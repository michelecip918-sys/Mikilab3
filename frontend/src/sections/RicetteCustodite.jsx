...
export default function RicetteCustodite({ initialId = null }) {
  ...
  const inp = "bg-[#0B0E14] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-2xl shadow-md border border-amber-900/40 px-3 py-2.5 outline-none text-[#2B303B] dark:text-[#e4eff8] focus:border-[#F26419] font-mono-data text-center w-28";

  if (recipe) {
    return (
      <div className="pb-40" data-testid="custodite-detail">
        <button data-testid="custodite-back" onClick={() => setOpenId(null)}
          className="inline-flex items-center gap-1.5 mb-4 px-4 py-2 rounded-full bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] text-[#F26419] dark:text-[#a9d2ec] font-semibold text-sm shadow-sm active:scale-95 transition-all">
          <ChevronLeft className="w-4 h-4" /> {L({ it: "Tutte le ricette", de: "Alle Rezepte", en: "All recipes", es: "Todas", fr: "Toutes les recettes" })}
        </button>

        <div className="print-area">
          <div className="rounded-3xl overflow-hidden border border-[#26324A] dark:border-[#26324A] bg-white dark:bg-[#18202E] shadow-sm">
            <div className="relative h-40">
              ...
              <button type="button" data-testid={`custodite-fav-detail-${recipe.id}`} aria-pressed={isFav(`custodite:${recipe.id}`)}
                ...
                className="no-print absolute top-3 right-3 z-10 bg-white/90 dark:bg-[#0B0E14]/80 rounded-full p-2 shadow active:scale-90 transition-transform">
            ...
            <div className="p-4">
              ...
              {(() => {
                ...
                return (
                  <div data-testid="badge-idratazione" className="mt-3 inline-flex items-center gap-2 rounded-full px-3.5 py-1.5" style={{ background: "linear-gradient(90deg,#F6D27A,#E8A838)", color: "#3A2408", boxShadow: "0 0 16px rgba(231,178,60,.55)", border: "1px solid #F6D27A" }}>
          ...
          <div data-testid="custodite-lab-photo" className="mt-4 rounded-2xl overflow-hidden border border-[#26324A] dark:border-[#26324A] relative">
          ...
          <div className="mt-4 bg-[#F26419] rounded-2xl p-4 text-white" data-testid="custodite-improver-note">
          ...
          <div className="mt-4 bg-[#F26419]/10 border border-[#F26419]/30 rounded-2xl p-4">
            <p className="text-xs font-bold uppercase tracking-wide text-[#F26419] mb-2 flex items-center gap-1.5"><Scale className="w-4 h-4" /> {L({ it: "Adatta alle mie dosi", de: "An meine Mengen anpassen", en: "Adapt to my amounts", es: "Adapta a mis dosis", fr: "Adapte à mes quantités" })}</p>
              ...
              <div className="flex gap-1.5 ms-auto">
                {[500, 1000, 2000].map((v) => (
                  <button key={v} data-testid={`custodite-quick-${v}`} onClick={() => setFlour(v)}
                    className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] active:scale-95">{v >= 1000 ? `${v / 1000}kg` : `${v}g`}</button>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 rounded-2xl border border-[#26324A] dark:border-[#26324A] bg-white dark:bg-[#18202E] overflow-hidden">
            <div className="px-4 py-2.5 bg-[#0B0E14] dark:bg-[#18202E] flex items-center gap-2 border-b border-[#26324A] dark:border-[#26324A]">
              <Wheat className="w-4 h-4 text-[#F26419]" />
              ...
              {rows.map((r, i) => (
                <div key={i} className="flex items-center justify-between px-4 py-2.5 border-b last:border-0 border-[#F0E7D6] dark:border-[#2C343C]">
                  ...
                  <span className="flex items-baseline gap-2">
                    <span className="font-mono-data font-bold text-[#F26419] dark:text-[#d8a679]">{r.grams} g</span>
          ...
          {/* Procedimento */}
          <div className="mt-4 rounded-2xl border border-[#26324A] dark:border-[#26324A] bg-white dark:bg-[#18202E] p-4">
            <p className="font-display font-semibold text-[#2B303B] dark:text-[#e4eff8] mb-2 flex items-center gap-1.5"><Clock className="w-4 h-4 text-[#F26419]" /> {L({ it: "Procedimento", de: "Zubereitung", en: "Method", es: "Procedimiento", fr: "Préparation" })}</p>
          ...
          <div className="mt-4 rounded-2xl border-2 border-dashed border-[#F26419]/40 bg-[#0B0E14] dark:bg-[#18202E] p-4 flex items-center gap-4">
            {qr && <img data-testid="custodite-qr" src={qr} alt="QR" className="w-24 h-24 rounded-lg bg-white p-1 shrink-0" />}
            <div className="min-w-0">
              <p className="font-display font-semibold text-[#2B303B] dark:text-[#e4eff8] flex items-center gap-1.5"><Sparkles className="w-4 h-4 text-[#F26419]" /> {L({ it: "Scheda da condividere", de: "Karte zum Teilen", en: "Shareable card", es: "Ficha para compartir", fr: "Fiche à partager" })}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mt-4 no-print">
          <button data-testid="custodite-share" onClick={doShare} className="flex items-center justify-center gap-2 bg-[#F26419] text-white font-semibold py-3 rounded-2xl shadow-md border border-amber-900/40 active:scale-97"><Share2 className="w-5 h-5" /> {L({ it: "Condividi", de: "Teilen", en: "Share", es: "Compartir", fr: "Partager" })}</button>
          <button data-testid="custodite-copy" onClick={copyText} className="flex items-center justify-center gap-2 bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] text-[#2B303B] dark:text-[#e4eff8] font-semibold py-3 rounded-2xl shadow-md border border-amber-900/40 active:scale-97">{L({ it: "Copia", de: "Kopieren", en: "Copy", es: "Copiar", fr: "Copier" })}</button>
          <button data-testid="custodite-print" onClick={() => window.print()} className="flex items-center justify-center gap-2 bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] text-[#2B303B] dark:text-[#e4eff8] font-semibold py-3 rounded-2xl shadow-md border border-amber-900/40 active:scale-97"><Printer className="w-5 h-5" /> {L({ it: "Stampa", de: "Druck", en: "Print", es: "Imprimir", fr: "Imprimer" })}</button>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-wrap gap-2 mt-4 pb-1" data-testid="custodite-filters">
        {CATS.map((c) => (
          <button key={c.id} data-testid={`custodite-cat-${c.id}`} onClick={() => setCat(c.id)}
            className={`shrink-0 px-3.5 py-1.5 rounded-full text-sm font-semibold border transition-all active:scale-95 ${cat === c.id ? "bg-[#F26419] text-white border-[#F26419]" : "bg-white dark:bg-[#18202E] text-[#F26419] dark:text-[#a9d2ec] border-[#26324A] dark:border-[#26324A]"}`}>
            {L(c.label)}
          </button>
        ))}
      </div>
        ...
        {list.map((r) => (
          <div key={r.id} className="relative">
            <button data-testid={`custodite-open-${r.id}`} onClick={() => setOpenId(r.id)}
              className="w-full flex items-center gap-3 text-start rounded-2xl overflow-hidden border border-[#26324A] dark:border-[#26324A] bg-white dark:bg-[#18202E] shadow-sm active:scale-98 transition-all hover:border-[#F26419]/50">
            ...
            <button type="button" data-testid={`custodite-fav-${r.id}`} aria-pressed={isFav(`custodite:${r.id}`)}
              ...
              className="absolute top-2 right-2 z-10 inline-flex items-center gap-1 bg-white/90 dark:bg-[#0B0E14]/80 rounded-full pl-1.5 pr-2 py-1.5 shadow active:scale-90 transition-transform">
