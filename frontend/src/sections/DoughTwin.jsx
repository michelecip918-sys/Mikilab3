  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <span className="text-[11px] font-semibold uppercase text-[#7E8A93] flex items-center gap-1"><Icon className="w-3.5 h-3.5 text-[#3f7cac]" />{label}</span>
        <span className="font-mono-data text-sm font-bold text-[#2B303B] dark:text-[#e4eff8]">{value}{unit}</span>
      </div>
      <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => set(Number(e.target.value))}
        className="w-full accent-[#3f7cac]" data-testid={testid} />
    </div>
  );
}
  ...
  return (
    <div className="pb-40">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#5aa0cf] to-[#234b6e] flex items-center justify-center"><FlaskConical className="w-6 h-6 text-white" /></div>
        <div>
      ...
      <div className="bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] rounded-2xl p-4 mb-4 space-y-3">
        <div>
          <span className="text-[11px] font-semibold uppercase text-[#7E8A93] flex items-center gap-1 mb-1"><ChefHat className="w-3.5 h-3.5 text-[#3f7cac]" />{tri("Parti da una ricetta", "Von einem Rezept starten", "Start from a recipe")}</span>
        ...
        <div>
          <span className="text-[11px] font-semibold uppercase text-[#7E8A93] flex items-center gap-1 mb-1"><Clock className="w-3.5 h-3.5 text-[#3f7cac]" />{tri("Ora d'inizio impasto", "Startzeit Teig", "Dough start time")}</span>
          <input data-testid="twin-start-time" type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)}
            className="w-full bg-[#e4eff8] dark:bg-[#2A323A] border border-[#d5e4f0] dark:border-[#38424B] rounded-xl p-2.5 text-sm outline-none focus:border-[#3f7cac] font-mono-data" />
        </div>
      </div>
            ...
            <div className="flex gap-1.5">
              {[["ldb", tri("Birra", "Hefe", "Yeast")], ["madre", tri("Madre", "Sauer", "Sourdough")]].map(([id, lb]) => (
                <button key={id} data-testid={`twin-type-${id}`} onClick={() => { setYeastType(id); setYeast(id === "ldb" ? 1 : 20); }}
                  className={`flex-1 py-2 rounded-xl text-sm font-semibold border ${yeastType === id ? "bg-[#3f7cac] text-white border-[#3f7cac]" : "bg-white dark:bg-[#232A31] text-[#3F4A54] dark:text-[#AEB8BF] border-[#d5e4f0] dark:border-[#38424B]"}`}>{lb}</button>
              ))}
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="rounded-2xl bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] p-4">
          ...
          <p data-testid="twin-volume" className="font-mono-data text-2xl font-bold text-[#5aa0cf] mt-1">{sim.vMax.toFixed(1)}×</p>
      ...
      <div className="rounded-2xl bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] p-3 mb-4" data-testid="twin-chart">
        ...
        <ResponsiveContainer width="100%" height={180}>
          <LineChart data={sim.pts} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="v" stroke="#3f7cac" strokeWidth={2.5} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>

      {/* Anteprima alveolatura */}
      <div className="rounded-2xl overflow-hidden border border-[#d5e4f0] dark:border-[#38424B] mb-4" data-testid="twin-crumb">
      ...
      <div className="space-y-2" data-testid="twin-advice">
        {advice.map((a, i) => (
          <div key={i} className="flex items-start gap-2 bg-[#5aa0cf]/10 border border-[#5aa0cf]/25 rounded-xl p-3">
            <Sparkles className="w-4 h-4 text-[#5aa0cf] shrink-0 mt-0.5" />
