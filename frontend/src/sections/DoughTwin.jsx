  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <span className="text-[11px] font-semibold uppercase text-[#7E8A93] flex items-center gap-1"><Icon className="w-3.5 h-3.5 text-[#3E9C93]" />{label}</span>
        <span className="font-mono-data text-sm font-bold text-[#2B303B] dark:text-[#e4eff8]">{value}{unit}</span>
      </div>
      <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => set(Number(e.target.value))}
        className="w-full accent-[#3E9C93]" data-testid={testid} />
    </div>
  );
}
  ...
  return (
    <div className="pb-40">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] flex items-center justify-center"><FlaskConical className="w-6 h-6 text-white" /></div>
        <div>
      ...
      <div className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-4 mb-4 space-y-3">
        <div>
          <span className="text-[11px] font-semibold uppercase text-[#7E8A93] flex items-center gap-1 mb-1"><ChefHat className="w-3.5 h-3.5 text-[#3E9C93]" />{tri("Parti da una ricetta", "Von einem Rezept starten", "Start from a recipe")}</span>
          <select data-testid="twin-recipe" value={recipeId} onChange={(e) => applyRecipe(e.target.value)}
            className="w-full bg-[#e4eff8] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 p-2.5 text-sm outline-none focus:border-[#3E9C93]">
        ...
        <div>
          <span className="text-[11px] font-semibold uppercase text-[#7E8A93] flex items-center gap-1 mb-1"><Clock className="w-3.5 h-3.5 text-[#3E9C93]" />{tri("Ora d'inizio impasto", "Startzeit Teig", "Dough start time")}</span>
          <input data-testid="twin-start-time" type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)}
            className="w-full bg-[#e4eff8] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 p-2.5 text-sm outline-none focus:border-[#3E9C93] font-mono-data" />
        </div>
      </div>

      {/* Avviso PICCO del volume */}
      <div data-testid="twin-peak-alert" className="rounded-2xl bg-gradient-to-br from-[#E4572E] to-[#1B2A38] text-white p-4 mb-4 shadow-md">
        ...
        {alarmAt ? (
          ...
          <button data-testid="twin-alarm-set" onClick={schedulePeakAlarm}
            className="mt-3 w-full flex items-center justify-center gap-2 bg-white text-[#1B2A38] font-bold py-2.5 rounded-2xl shadow-md border border-amber-900/40 active:scale-97 transition-all">
      ...
      {/* Parametri */}
      <div className="bg-[#3E9C93]/10 border border-[#3E9C93]/30 rounded-2xl p-4 mb-4 space-y-3">
            ...
            <div className="flex gap-1.5">
              {[["ldb", tri("Birra", "Hefe", "Yeast")], ["madre", tri("Madre", "Sauer", "Sourdough")]].map(([id, lb]) => (
                <button key={id} data-testid={`twin-type-${id}`} onClick={() => { setYeastType(id); setYeast(id === "ldb" ? 1 : 20); }}
                  className={`flex-1 py-2 rounded-2xl shadow-md border border-amber-900/40 text-sm font-semibold border ${yeastType === id ? "bg-[#3E9C93] text-white border-[#3E9C93]" : "bg-white dark:bg-[#1B2A38] text-[#3F4A54] dark:text-[#AEB8BF] border-[#2A3B49] dark:border-[#2A3B49]"}`}>{lb}</button>
              ))}
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-4">
        ...
        <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-4">
          ...
          <p data-testid="twin-volume" className="font-mono-data text-2xl font-bold text-[#3E9C93] mt-1">{sim.vMax.toFixed(1)}×</p>
        ...
        <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-4">
          ...
          <p data-testid="twin-rechyd" className="font-mono-data text-2xl font-bold text-[#3E9C93] mt-1">~{sim.recHyd}%</p>
      ...
      <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-3 mb-4" data-testid="twin-chart">
        ...
        <ResponsiveContainer width="100%" height={180}>
          <LineChart data={sim.pts} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="v" stroke="#3E9C93" strokeWidth={2.5} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>

      {/* Anteprima alveolatura */}
      <div className="rounded-2xl overflow-hidden border border-[#2A3B49] dark:border-[#2A3B49] mb-4" data-testid="twin-crumb">
      ...
      <div className="space-y-2" data-testid="twin-advice">
        {advice.map((a, i) => (
          <div key={i} className="flex items-start gap-2 bg-[#3E9C93]/10 border border-[#3E9C93]/25 rounded-2xl shadow-md border border-amber-900/40 p-3">
