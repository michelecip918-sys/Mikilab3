...
export default function LabPizzeria({ onBack, onOpenTool }) {
  ...
  return (
    <div className="pb-8" data-testid="lab-pizzeria">
      {onBack && <button data-testid="pizzeria-back" onClick={onBack} className="flex items-center gap-1 text-[#c94f00] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Back")}</button>}
      ...
      <div className="flex flex-wrap gap-1.5 bg-[#1e1e1e] p-1.5 rounded-2xl mb-5 border border-[#2e2e2e]">
        {TABS.map(({ id, Icon, label }) => (
          <button key={id} data-testid={`pizzeria-tab-${id}`} onClick={() => setTab(id)} className={`flex-1 min-w-[30%] flex flex-col items-center gap-1 py-2 rounded-2xl shadow-md border border-amber-900/40 text-[11px] font-bold transition-all ${tab === id ? "bg-[#c94f00] text-[#121212] shadow" : "text-[#c94f00]"}`}><Icon className="w-4 h-4" /> {label}</button>
        ))}
      </div>
      ...
      {tab === "prefermenti" && (
        <div className={card} data-testid="pizzeria-prefermenti">
          ...
          <div data-testid="pz-pref-out" className="rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] p-3 text-[13px] text-[#c94f00] font-semibold space-y-1">
      ...
      {tab === "matrix" && (
        <div className={card} data-testid="pizzeria-matrix">
          ...
          <div className="grid grid-cols-2 gap-2 mb-2">
            <div className="rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] p-3 text-center"><p className="text-[11px] font-semibold text-[#c94f00]">{L("Maturazione in frigo", "Cold maturation")}</p><p data-testid="pz-fridge" className="font-display text-xl font-bold text-[#c94f00]">{matrix.fridge}</p></div>
            <div className="rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] p-3 text-center"><p className="text-[11px] font-semibold text-[#c94f00]">{L("Appretto (fuori frigo)", "Appretto (room)")}</p><p data-testid="pz-appretto" className="font-display text-xl font-bold text-[#c94f00]">{matrix.appretto}</p></div>
          </div>
          <p className="text-[12.5px] text-[#c94f00] dark:text-[#AEB8BF] leading-snug">{matrix.note}</p>
        </div>
      )}
      ...
      {tab === "acqua" && (
        <div className={card} data-testid="pizzeria-acqua">
          <p className="text-[12.5px] text-[#c94f00] mb-3">{L("Teorema della temperatura acqua: (T° impasto × fattore macchina) − (T° ambiente + T° farina + T° attrito).", "Water temperature theorem: (dough T° × machine factor) − (room T° + flour T° + friction T°).")}</p>
          <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <div data-testid="pz-water-out" className="rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] p-4 text-center">
            <p className="text-[12px] font-semibold text-[#c94f00]">{L("Temperatura acqua consigliata", "Recommended water temperature")}</p>
            <p className="font-display text-4xl font-bold text-[#c94f00]">{waterT}°C</p>
          </div>
        </div>
      )}
      ...
      {tab === "settimana" && (
        <div className={card} data-testid="pizzeria-settimana">
          <p className="text-[12.5px] text-[#c94f00] mb-3">{L("Pianifica impasti e palline da Lunedì a Domenica.", "Plan doughs and dough balls from Monday to Sunday.")}</p>
          <div className="space-y-2 mb-3">
            {DAYS.map((d, i) => (
              <div key={i} className="flex items-center gap-2" data-testid={`pz-week-${i}`}>
                <span className="w-24 shrink-0 text-[13px] font-semibold text-[#c94f00]">{d}</span>
