...
export default function SimulatoreForno({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="sim-forno">
      {onBack && <button data-testid="simforno-back" onClick={onBack} className="flex items-center gap-1 text-[#F26419] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back")}</button>}
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#0B0E14] shadow-xl mb-5" style={{ background: "linear-gradient(135deg,#F26419,#F26419 60%,#F26419)" }}>
        ...
        <p className="text-[#0B0E14]/85 text-sm mt-2 leading-snug">{L("Tempistiche esatte per vapore, pietra refrattaria e spiffero: cuoci come un pro anche col forno di casa.", "Genaue Zeiten für Dampf, Stein und Ofen.", "Exact timing for steam, stone and oven quirks.", "Tiempos exactos para vapor, piedra y horno.")}</p>
      </div>
      ...
      <div className={card + " mb-4 space-y-3"}>
        ...
        <label className="flex items-center gap-2 text-[13px] font-semibold text-[#F26419] dark:text-[#AEB8BF]">
          <input data-testid="simforno-stone" type="checkbox" checked={stone} onChange={(e) => setStone(e.target.checked)} className="w-4 h-4 accent-[#F26419]" />
          {L("Uso la pietra refrattaria", "Ich nutze den Backstein", "I use a baking stone", "Uso piedra refractaria")}
        </label>
      </div>

      <div data-testid="simforno-result" className="rounded-2xl bg-[#ffffff] border border-[#26324A] p-4 space-y-2.5 text-[13.5px] text-[#F26419]">
        <p className="flex items-center gap-2"><Flame className="w-4.5 h-4.5 text-[#F26419]" /><span className="font-bold text-[#F26419]">{L("Imposta il forno a:", "Ofen einstellen auf:", "Set the oven to:", "Ajusta el horno a:")} </span><span className="font-mono-data font-bold">{r.setTempC}°C</span>{r.dryHint && ` (${L("ventilato: -10°C perché asciuga di più", "Umluft: -10°C", "fan: -10°C, it dries more", "ventilado: -10°C")})`}</p>
        <p className="flex items-center gap-2"><Clock className="w-4.5 h-4.5 text-[#F26419]" /><span className="font-bold text-[#F26419]">{L("Preriscalda:", "Vorheizen:", "Preheat:", "Precalienta:")} </span><span className="font-mono-data font-bold">{r.preheat} min</span> {stone ? L("(pietra ben calda)", "(Stein heiß)", "(stone hot)", "(piedra caliente)") : ""}</p>
        <p className="flex items-center gap-2"><Droplets className="w-4.5 h-4.5 text-[#F26419]" /><span className="font-bold text-[#F26419]">{L("Vapore:", "Dampf:", "Steam:", "Vapor:")} </span>{r.steamMin > 0 ? <><span className="font-mono-data font-bold">{r.steamMin} min</span> {L("all'inizio (teglia con acqua bollente), poi apri e fai uscire il vapore.", "am Anfang, dann Dampf ablassen.", "at the start, then open to release steam.", "al inicio, luego libera el vapor.")}</> : L("niente vapore (crosta sottile).", "kein Dampf.", "no steam.", "sin vapor.")}</p>
        <p className="flex items-start gap-2"><Info className="w-4.5 h-4.5 text-[#F26419] shrink-0 mt-0.5" />{L("Spiffero: negli ultimi 5 minuti tieni lo sportello leggermente aperto (cucchiaio di legno) per crosta croccante.", "Spalt: letzte 5 Min Tür leicht offen für knusprige Kruste.", "Vent: last 5 min keep the door slightly open (wooden spoon) for a crisp crust.", "Rendija: últimos 5 min puerta entreabierta para corteza crujiente.")}</p>
      </div>
    </div>
  );
}
