  ...
  return (
    <div className="pb-8" data-testid="sim-forno">
      ...
      <div className={card + " mb-4 space-y-3"}>
        ...
        <label className="flex items-center gap-2 text-[13px] font-semibold text-[#3E9C93] dark:text-[#AEB8BF]">
          <input data-testid="simforno-stone" type="checkbox" checked={stone} onChange={(e) => setStone(e.target.checked)} className="w-4 h-4 accent-[#3E9C93]" />
          {L("Uso la pietra refrattaria", "Ich nutze den Backstein", "I use a baking stone", "Uso piedra refractaria")}
        </label>
      </div>

      <div data-testid="simforno-result" className="rounded-2xl bg-[#ffffff] border border-[#2A3B49] p-4 space-y-2.5 text-[13.5px] text-[#3E9C93]">
        <p className="flex items-center gap-2"><Flame className="w-4.5 h-4.5 text-[#3E9C93]" /><span className="font-bold text-[#3E9C93]">{L("Imposta il forno a:", "Ofen einstellen auf:", "Set the oven to:", "Ajusta el horno a:")} </span><span className="font-mono-data font-bold">{r.setTempC}°C</span>{r.dryHint && ` (${L("ventilato: -10°C perché asciuga di più", "Umluft: -10°C", "fan: -10°C, it dries more", "ventilado: -10°C")})`}</p>
        <p className="flex items-center gap-2"><Clock className="w-4.5 h-4.5 text-[#3E9C93]" /><span className="font-bold text-[#3E9C93]">{L("Preriscalda:", "Vorheizen:", "Preheat:", "Precalienta:")} </span><span className="font-mono-data font-bold">{r.preheat} min</span> {stone ? L("(pietra ben calda)", "(Stein heiß)", "(stone hot)", "(piedra caliente)") : ""}</p>
        <p className="flex items-center gap-2"><Droplets className="w-4.5 h-4.5 text-[#3E9C93]" /><span className="font-bold text-[#3E9C93]">{L("Vapore:", "Dampf:", "Steam:", "Vapor:")} </span>{r.steamMin > 0 ? <><span className="font-mono-data font-bold">{r.steamMin} min</span> {L("all'inizio (teglia con acqua bollente), poi apri e fai uscire il vapore.", "am Anfang, dann Dampf ablassen.", "at the start, then open to release steam.", "al inicio, luego libera el vapor.")}</> : L("niente vapore (crosta sottile).", "kein Dampf.", "no steam.", "sin vapor.")}</p>
        <p className="flex items-start gap-2"><Info className="w-4.5 h-4.5 text-[#3E9C93] shrink-0 mt-0.5" />{L("Spiffero: negli ultimi 5 minuti tieni lo sportello leggermente aperto (cucchiaio di legno) per crosta croccante.", "Spalt: letzte 5 Min Tür leicht offen für knusprige Kruste.", "Vent: last 5 min keep the door slightly open (wooden spoon) for a crisp crust.", "Rendija: últimos 5 min puerta entreabierta para corteza crujiente.")}</p>
      </div>
    </div>
  );
}
