  ...
  return (
    <div className="pb-8" data-testid="sim-forno">
      ...
      <div className={card + " mb-4 space-y-3"}>
        ...
        <label className="flex items-center gap-2 text-[13px] font-semibold text-[#ff6b00] dark:text-[#AEB8BF]">
          <input data-testid="simforno-stone" type="checkbox" checked={stone} onChange={(e) => setStone(e.target.checked)} className="w-4 h-4 accent-[#ff6b00]" />
      ...
      <div data-testid="simforno-result" className="rounded-2xl bg-[#ffffff] border border-[#2e2e2e] p-4 space-y-2.5 text-[13.5px] text-[#ff6b00]">
        ...
        <p className="flex items-start gap-2"><Info className="w-4.5 h-4.5 text-[#ff6b00] shrink-0 mt-0.5" />{L("Spiffero: negli ultimi 5 minuti tieni lo sportello leggermente aperto (cucchiaio di legno) per crosta croccante.", "Spalt: letzte 5 Min Tür leicht offen für knusprige Kruste.", "Vent: last 5 min keep the door slightly open (wooden spoon) for a crisp crust.", "Rendija: últimos 5 min puerta entreabierta para corteza crujiente.")}</p>
      </div>
    </div>
  );
}
