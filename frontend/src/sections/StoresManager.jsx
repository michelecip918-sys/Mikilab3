  ...
  return (
    <div className="pb-40" data-testid="stores-manager">
      ...
      <div className="space-y-3" data-testid="stores-list">
        ...
        {stores.map((s) => (
          <div key={s.id} data-testid={`store-card-${s.id}`} className={`rounded-2xl p-4 border shadow-sm ${current === s.id ? "bg-[#8C4A27]/10 border-[#8C4A27]" : "bg-white dark:bg-[#232A31] border-[#E6D8C3] dark:border-[#38424B]"}`}>
            {editing === s.id ? (
              ...
              <>
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#8C4A27] flex items-center justify-center shrink-0"><Store className="w-5 h-5 text-white" /></div>
                ...
                <div className="flex items-center gap-2 mt-3">
                  {current === s.id ? (
                    <span data-testid={`store-active-${s.id}`} className="flex-1 flex items-center justify-center gap-1 text-sm font-semibold text-[#8C4A27] bg-[#8C4A27]/10 rounded-xl py-2"><Check className="w-4 h-4" /> {tri("Attivo", "Aktiv", "Active")}</span>
                  ) : (
                    <button data-testid={`store-select-${s.id}`} onClick={() => setCurrent(s.id)} className="flex-1 text-sm font-semibold text-[#2B303B] dark:text-[#e4eff8] bg-[#FAF5EC] dark:bg-[#1F252B] border border-[#E6D8C3] dark:border-[#38424B] rounded-xl py-2 active:scale-98">{tri("Rendi attivo", "Aktivieren", "Set active")}</button>
                  )}
                  <button data-testid={`store-edit-${s.id}`} onClick={() => setEditing(s.id)} className="p-2 rounded-xl border border-[#E6D8C3] dark:border-[#38424B] text-[#B45309]"><Pencil className="w-4 h-4" /></button>
                  <button data-testid={`store-remove-${s.id}`} onClick={() => remove(s.id)} className="p-2 rounded-xl border border-[#E6D8C3] dark:border-[#38424B] text-[#7E8A93] hover:text-[#E4572E]"><Trash2 className="w-4 h-4" /></button>
                </div>
              </>
            )}
          </div>
...
function StoreForm({ form, setForm, inp, onSave, onCancel, busy, tri, testidPrefix }) {
  return (
    <div data-testid={`${testidPrefix}-form`} className="bg-[#B45309]/10 border border-[#B45309]/30 rounded-2xl p-4 mb-4 space-y-2">
      ...
      <div className="flex gap-2">
        <button data-testid={`${testidPrefix}-save`} onClick={onSave} disabled={busy} className="flex-1 flex items-center justify-center gap-1 bg-[#8C4A27] text-white font-semibold py-2.5 rounded-xl active:scale-98 disabled:opacity-50">
          {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />} {tri("Salva", "Speichern", "Save")}
        </button>
        <button data-testid={`${testidPrefix}-cancel`} onClick={onCancel} className="px-4 rounded-xl border border-[#E6D8C3] dark:border-[#38424B] text-[#7E8A93]"><X className="w-4 h-4" /></button>
      </div>
    </div>
  );
}
