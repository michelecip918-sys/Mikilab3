  ...
  return (
    <div className="pb-40" data-testid="stores-manager">
      ...
      <div className="space-y-3" data-testid="stores-list">
        ...
        {stores.map((s) => (
          <div key={s.id} data-testid={`store-card-${s.id}`} className={`rounded-2xl p-4 border shadow-sm ${current === s.id ? "bg-[#c94f00]/10 border-[#c94f00]" : "bg-white dark:bg-[#1e1e1e] border-[#2e2e2e] dark:border-[#2e2e2e]"}`}>
            {editing === s.id ? (
              ...
              <>
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-2xl shadow-md border border-amber-900/40 bg-[#c94f00] flex items-center justify-center shrink-0"><Store className="w-5 h-5 text-white" /></div>
                ...
                <div className="flex items-center gap-2 mt-3">
                  {current === s.id ? (
                    <span data-testid={`store-active-${s.id}`} className="flex-1 flex items-center justify-center gap-1 text-sm font-semibold text-[#c94f00] bg-[#c94f00]/10 rounded-2xl shadow-md border border-amber-900/40 py-2"><Check className="w-4 h-4" /> {tri("Attivo", "Aktiv", "Active")}</span>
                  ) : (
                  ...
                  <button data-testid={`store-edit-${s.id}`} onClick={() => setEditing(s.id)} className="p-2 rounded-2xl shadow-md border border-amber-900/40 border border-[#2e2e2e] dark:border-[#2e2e2e] text-[#c94f00]"><Pencil className="w-4 h-4" /></button>
...
function StoreForm({ form, setForm, inp, onSave, onCancel, busy, tri, testidPrefix }) {
  return (
    <div data-testid={`${testidPrefix}-form`} className="bg-[#c94f00]/10 border border-[#c94f00]/30 rounded-2xl p-4 mb-4 space-y-2">
      ...
      <div className="flex gap-2">
        <button data-testid={`${testidPrefix}-save`} onClick={onSave} disabled={busy} className="flex-1 flex items-center justify-center gap-1 bg-[#c94f00] text-white font-semibold py-2.5 rounded-2xl shadow-md border border-amber-900/40 active:scale-98 disabled:opacity-50">
