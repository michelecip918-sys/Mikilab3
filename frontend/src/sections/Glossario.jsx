...
export default function Glossario({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="glossario">
      {onBack && <button data-testid="glossario-back" onClick={onBack} className="flex items-center gap-1 text-primary font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back")}</button>}
      <div className="relative overflow-hidden rounded-3xl p-6 text-foreground shadow-xl mb-4" style={{ background: "linear-gradient(135deg,hsl(var(--primary)),hsl(var(--primary)) 60%,hsl(var(--primary)))" }}>
        <div className="w-14 h-14 rounded-2xl bg-foreground/15 border border-foreground/30 flex items-center justify-center mb-3"><BookOpen className="w-7 h-7" /></div>
        ...
        <p className="text-foreground/85 text-sm mt-2 leading-snug">{L("I termini tecnici spiegati in parole semplici. Cerca quello che ti serve.", "Fachbegriffe einfach erklärt.", "Technical terms in plain words. Search what you need.", "Términos técnicos en palabras sencillas.")}</p>
      </div>
      ...
      <div className="relative mb-4">
        ...
        <input data-testid="glossario-search" value={q} onChange={(e) => setQ(e.target.value)} placeholder={L("Cerca un termine…", "Begriff suchen…", "Search a term…", "Busca un término…")}
          className="w-full bg-background dark:bg-card border border-border dark:border-border rounded-2xl pl-10 pr-3 py-3 outline-none text-primary dark:text-foreground focus:border-primary" />
      </div>

      <div className="space-y-2.5" data-testid="glossario-list">
        {list.map((x, i) => (
          <div key={x.t} data-testid={`glossario-term-${i}`} className="rounded-2xl bg-background dark:bg-card border border-border dark:border-border p-4 shadow-sm">
            <p className="font-display text-[16px] font-bold text-primary leading-tight">{gt(x)}</p>
            <p className="text-[13.5px] text-primary dark:text-muted-foreground leading-relaxed mt-1">{gd(x)}</p>
          </div>
        ))}
        {list.length === 0 && <p className="text-center text-sm text-muted-foreground py-8">{L("Nessun termine trovato.", "Kein Begriff gefunden.", "No term found.", "Ningún término.")}</p>}
      </div>
    </div>
  );
}
