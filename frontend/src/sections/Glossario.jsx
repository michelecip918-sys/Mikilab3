  ...
  return (
    <div className="pb-8" data-testid="glossario">
      ...
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#0D1520] shadow-xl mb-4" style={{ background: "linear-gradient(135deg,#3E9C93,#3E9C93 60%,#3E9C93)" }}>
        ...
        <p className="text-[#0D1520]/85 text-sm mt-2 leading-snug">{L("I termini tecnici spiegati in parole semplici. Cerca quello che ti serve.", "Fachbegriffe einfach erklärt.", "Technical terms in plain words. Search what you need.", "Términos técnicos en palabras sencillas.")}</p>
      </div>

      <div className="relative mb-4">
        <Search className="w-4.5 h-4.5 text-[#64748B] absolute left-3 top-1/2 -translate-y-1/2" />
        <input data-testid="glossario-search" value={q} onChange={(e) => setQ(e.target.value)} placeholder={L("Cerca un termine…", "Begriff suchen…", "Search a term…", "Busca un término…")}
          className="w-full bg-[#0D1520] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl pl-10 pr-3 py-3 outline-none text-[#3E9C93] dark:text-[#e4eff8] focus:border-[#3E9C93]" />
      </div>

      <div className="space-y-2.5" data-testid="glossario-list">
        {list.map((x, i) => (
          <div key={x.t} data-testid={`glossario-term-${i}`} className="rounded-2xl bg-[#0D1520] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-4 shadow-sm">
        ...
        {list.length === 0 && <p className="text-center text-sm text-[#64748B] py-8">{L("Nessun termine trovato.", "Kein Begriff gefunden.", "No term found.", "Ningún término.")}</p>}
      </div>
    </div>
  );
}
