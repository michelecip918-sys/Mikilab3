...
export default function SoundDiagnosi() {
  ...
  return (
    <div className="pb-24">
      ...
      <div className="rounded-2xl bg-[#3E9C93]/12 border border-[#3E9C93]/30 p-4 mb-4 text-sm text-[#3E9C93] dark:text-[#8FB0C2] leading-relaxed">
      ...
      {!recording ? (
        ...
        <button data-testid="sound-stop-btn" onClick={stop}
          className="w-full bg-[#3E9C93] hover:bg-[#a8483f] text-white font-semibold px-5 py-3.5 rounded-2xl shadow-md active:scale-98 transition-all flex items-center justify-center gap-2 animate-pulse">
          <Square className="w-5 h-5" /> {tri("Sto ascoltando… tocca per fermare", "Ich höre zu… tippen zum Stoppen", "Listening… tap to stop")}
        </button>
      )}

      {recording && (
        <div data-testid="sound-wave" className="mt-4 rounded-2xl bg-[#3E9C93] p-4">
      ...
      {result && (
        <div className="mt-5">
          <div data-testid="sound-result" className="markdown-body bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-5 text-sm leading-relaxed text-[#2B303B] dark:text-[#e4eff8]">
