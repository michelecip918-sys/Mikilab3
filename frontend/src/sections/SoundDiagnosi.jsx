...
export default function SoundDiagnosi() {
  ...
  return (
    <div className="pb-24">
      ...
      <div className="rounded-2xl bg-[#F26419]/12 border border-[#F26419]/30 p-4 mb-4 text-sm text-[#F26419] dark:text-[#8FB0C2] leading-relaxed">
      ...
      {!recording ? (
        ...
        <button data-testid="sound-stop-btn" onClick={stop}
          className="w-full bg-[#F26419] hover:bg-[#a8483f] text-white font-semibold px-5 py-3.5 rounded-2xl shadow-md active:scale-98 transition-all flex items-center justify-center gap-2 animate-pulse">
          <Square className="w-5 h-5" /> {tri("Sto ascoltando… tocca per fermare", "Ich höre zu… tippen zum Stoppen", "Listening… tap to stop")}
        </button>
      )}

      {recording && (
        <div data-testid="sound-wave" className="mt-4 rounded-2xl bg-[#F26419] p-4">
      ...
      {result && (
        <div className="mt-5">
          <div data-testid="sound-result" className="markdown-body bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-2xl p-5 text-sm leading-relaxed text-[#2B303B] dark:text-[#e4eff8]">
          ...
          <ListenButton text={result} who="momy" testid="sound-listen-btn"
            className="mt-2 w-full bg-[#F26419] hover:bg-[#E8A838] text-white font-medium px-4 py-3 rounded-2xl flex items-center justify-center gap-2 active:scale-98 transition-all" />
        </div>
      )}
    </div>
  );
