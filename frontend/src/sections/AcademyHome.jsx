...
export default function AcademyHome({ onNavigate }) {
  ...
  return (
    <div className="pb-4" data-testid="academy-home">
      {/* Hero */}
      <div className="relative rounded-3xl overflow-hidden mb-5 bg-gradient-to-br from-[#3E9C93] to-[#3a2415] p-6 text-white">
        ...
        <div className="h-1 w-12 rounded-full bg-[#3E9C93] mt-2" />
      ...
      <div data-testid="academy-path" className="mb-5">
        <div className="flex items-center justify-between mb-2 px-1">
          ...
          <span data-testid="academy-path-progress" className="text-xs font-bold text-[#3E9C93]">{doneCount}/3 {tri("completati", "erledigt", "done", "completados")}</span>
        </div>
          ...
          {STEPS.map(({ id, n, Icon, label, desc }, i) => {
            ...
            return (
              <motion.button key={id} data-testid={`academy-path-${id}`} onClick={() => setSub(id)}
                ...
                className={`group w-full flex items-center gap-3.5 min-h-[68px] rounded-2xl px-4 text-start active:scale-98 transition-all border ${on ? "bg-[#3E9C93]/12 border-[#3E9C93]" : done ? "bg-[#3E9C93]/8 border-[#3E9C93]/45" : "bg-[#1B2A38] border-[#2A3B49] hover:border-[#3E9C93]/60"}`}>
                <span className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 font-bold ${done ? "bg-[#3E9C93] text-white" : "bg-[#3E9C93]/15 border border-[#3E9C93]/40 text-[#3E9C93]"}`}>
                ...
                <ChevronRight className="w-5 h-5 text-[#3E9C93] shrink-0 rtl:rotate-180" />
              </motion.button>
            );
          })}
        </div>
        {doneCount === 3 && (
          <div data-testid="academy-path-complete" className="mt-3 rounded-2xl bg-[#3E9C93] text-white p-4 text-center shadow-lg">
          ...
          {EXTRA.map(({ id, Icon, label }) => {
            ...
            return (
              <button key={id} data-testid={`academy-tab-${id}`} onClick={() => setSub(id)}
                className={`flex items-center gap-2.5 min-h-[54px] rounded-2xl px-4 border active:scale-97 transition-all text-start ${on ? "bg-[#3E9C93]/12 border-[#3E9C93]" : "bg-[#1B2A38] border-[#2A3B49] hover:border-[#3E9C93]/60"}`}>
                <span className="w-9 h-9 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/15 border border-[#3E9C93]/30 flex items-center justify-center shrink-0"><Icon className="w-5 h-5 text-[#3E9C93]" /></span>
      ...
      {sub === "diagnosi" && (
        <div className="space-y-4" data-testid="academy-diagnosi">
          <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-5 text-center">
            <div className="w-14 h-14 rounded-full bg-[#3E9C93]/15 flex items-center justify-center mx-auto mb-3">
              <Camera className="w-7 h-7 text-[#3E9C93]" />
            </div>
...
function DynamicRecipes() {
  ...
  const downloadPdf = () => {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>MikiLab · ${nm.it}</title>
      <style>
        ...
        .head{text-align:center;border-bottom:3px solid #3E9C93;padding-bottom:10px;margin-bottom:18px}
        .head h1{margin:0;color:#3E9C93}
        .head p{margin:2px 0 0;color:#7E8A93;font-size:13px}
        .card{border:1px solid #2A3B49;border-radius:12px;padding:14px 18px;margin-bottom:14px;page-break-inside:avoid}
        ...
        .hint{margin:10px 0 0;color:#3E9C93;font-size:13px}
        .flag{font-size:12px;font-weight:bold;color:#3E9C93;text-transform:uppercase;letter-spacing:1px;margin:0 0 4px}
  ...
  return (
    <div className="space-y-4" data-testid="dynamic-recipes">
      <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-4 space-y-3">
      ...
      {/* Risultato */}
      <div className="print-area rounded-2xl bg-gradient-to-br from-[#3E9C93]/10 to-[#3E9C93]/5 border border-[#3E9C93]/30 p-5" data-testid="calc-result">
        ...
        <p className="text-sm text-[#3E9C93] dark:text-[#a9d2ec] mt-3">💡 {L(r.hint)}</p>
      </div>
...
function Row({ label, val, bold }) {
  return (
    <div className={`flex items-center justify-between ${bold ? "font-bold" : ""}`}>
      ...
      <span className="text-[#3E9C93] dark:text-[#8FB0C2]">{val}</span>
    </div>
  );
}
  ...
  return (
    <div className="space-y-3" data-testid="flour-db">
      ...
      <div className="rounded-2xl overflow-hidden border border-[#2A3B49] dark:border-[#2A3B49]">
        ...
        {rows.map((f, i) => (
          <div key={f.name} data-testid={`flour-row-${i}`} className="grid grid-cols-[1fr_1fr_auto] gap-2 px-3 py-2.5 text-sm border-t border-[#2A3B49] dark:border-[#2A3B49] bg-white dark:bg-[#1B2A38]">
            <div>
            ...
            <span className="font-mono-data text-[#3E9C93] dark:text-[#8FB0C2] self-center">{f.w}</span>
          </div>
        ))}
      </div>
    </div>
