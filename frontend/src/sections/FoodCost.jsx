  ...
  return (
    <div className="pb-40">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-[#ff6b00] flex items-center justify-center"><Euro className="w-6 h-6 text-white" /></div>
        <div>
      ...
      <div className="rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-2 mb-4">
        ...
        {s.ingr.map((r) => (
          <div key={r.id} className="grid grid-cols-[1fr_64px_72px_28px] gap-1 items-center mb-1">
            ...
            <button onClick={() => set({ ingr: s.ingr.filter((x) => x.id !== r.id) })} className="text-[#ff6b00] flex justify-center"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        ))}
        <button data-testid="fc-add-ingr" onClick={() => set({ ingr: [...s.ingr, { id: Date.now(), name: "", kg: 0, price: 0 }] })} className="text-xs font-semibold text-[#ff6b00] flex items-center gap-1 mt-1"><Plus className="w-4 h-4" /> {tri("Aggiungi", "Hinzufügen", "Add")}</button>
      </div>
      ...
      <div className="rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-2 mb-2">
        ...
        {s.energy.map((r) => (
          <div key={r.id} className="grid grid-cols-[1fr_64px_64px_28px] gap-1 items-center mb-1">
            ...
            <button onClick={() => set({ energy: s.energy.filter((x) => x.id !== r.id) })} className="text-[#ff6b00] flex justify-center"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        ))}
        <button data-testid="fc-add-energy" onClick={() => set({ energy: [...s.energy, { id: Date.now(), name: "", kw: 0, hours: 0 }] })} className="text-xs font-semibold text-[#ff6b00] flex items-center gap-1 mt-1"><Plus className="w-4 h-4" /> {tri("Aggiungi", "Hinzufügen", "Add")}</button>
      </div>
      ...
      {/* Risultato */}
      <div data-testid="fc-result" className="rounded-3xl bg-gradient-to-br from-[#ff6b00] to-[#ff6b00] text-white p-5 shadow-lg">
