  ...
  return (
    <div className="pb-40">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-[#3E9C93] flex items-center justify-center"><Euro className="w-6 h-6 text-white" /></div>
        <div>
      ...
      {/* Materie prime */}
      <h2 className="text-xs font-bold uppercase tracking-wide text-[#3E9C93] mb-2">{tri("Materie prime", "Rohstoffe", "Ingredients")}</h2>
      <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-2 mb-4">
        ...
        {s.ingr.map((r) => (
          <div key={r.id} className="grid grid-cols-[1fr_64px_72px_28px] gap-1 items-center mb-1">
            ...
            <button onClick={() => set({ ingr: s.ingr.filter((x) => x.id !== r.id) })} className="text-[#3E9C93] flex justify-center"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        ))}
        <button data-testid="fc-add-ingr" onClick={() => set({ ingr: [...s.ingr, { id: Date.now(), name: "", kg: 0, price: 0 }] })} className="text-xs font-semibold text-[#3E9C93] flex items-center gap-1 mt-1"><Plus className="w-4 h-4" /> {tri("Aggiungi", "Hinzufügen", "Add")}</button>
      </div>

      {/* Energia */}
      <h2 className="text-xs font-bold uppercase tracking-wide text-[#3E9C93] mb-2 flex items-center gap-1"><Zap className="w-3.5 h-3.5" /> {tri("Energia", "Energie", "Energy")}</h2>
      <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-2 mb-2">
        ...
        {s.energy.map((r) => (
          <div key={r.id} className="grid grid-cols-[1fr_64px_64px_28px] gap-1 items-center mb-1">
            ...
            <button onClick={() => set({ energy: s.energy.filter((x) => x.id !== r.id) })} className="text-[#3E9C93] flex justify-center"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        ))}
        <button data-testid="fc-add-energy" onClick={() => set({ energy: [...s.energy, { id: Date.now(), name: "", kw: 0, hours: 0 }] })} className="text-xs font-semibold text-[#3E9C93] flex items-center gap-1 mt-1"><Plus className="w-4 h-4" /> {tri("Aggiungi", "Hinzufügen", "Add")}</button>
      </div>
      ...
      {/* Resa / calo peso */}
      <h2 className="text-xs font-bold uppercase tracking-wide text-[#3E9C93] mb-2 flex items-center gap-1"><Flame className="w-3.5 h-3.5" /> {tri("Resa & calo cottura", "Ausbeute & Backverlust", "Yield & baking loss")}</h2>
      ...
      {/* Risultato */}
      <div data-testid="fc-result" className="rounded-3xl bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] text-white p-5 shadow-lg">
