  ...
  return (
    <div className="pb-4">
      <div className="flex items-center gap-3 mb-1">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold text-[#2B303B] dark:text-[#e4eff8]">{t("sd_title")}</h1>
      ...
      <div className="mt-5 bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] rounded-2xl px-4 py-3">
        ...
        <input
          ...
          className="mt-1 w-full font-mono-data bg-[#e4eff8] dark:bg-[#2A323A] border border-[#d5e4f0] dark:border-[#38424B] rounded-lg px-3 py-2 outline-none focus:border-[#3f7cac]"
        />
      </div>

      {/* Persone al lavoro */}
      <div className="mt-3 bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] rounded-2xl px-4 py-3">
        <div className="flex items-center gap-2">
          <Users className="w-4 h-4 text-[#3f7cac]" />
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="w-8 h-8 rounded-lg bg-[#e4eff8] dark:bg-[#2A323A] border border-[#d5e4f0] dark:border-[#38424B] font-bold text-[#3f7cac]"
            >−</button>
            <input
              data-testid="lv-people" type="number" min="1" value={people}
              onChange={(e) => setPeople(e.target.value)}
              className="w-12 text-center font-mono-data font-bold text-[#234b6e] dark:text-[#8FB0C2] bg-[#e4eff8] dark:bg-[#2A323A] border border-[#d5e4f0] dark:border-[#38424B] rounded-lg px-1 py-1.5 outline-none"
            />
            <button
      ...
      <div className="mt-3">
        ...
        <select
          ...
          className="mt-1 w-full bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#3f7cac]"
        >
      ...
      <div className="space-y-2 mt-4">
        {rows.map((r) => (
          <div key={r.id} className="bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] rounded-2xl p-3">
            <div className="flex items-center gap-2">
              <select
                data-testid={`sd-recipe-${r.id}`} value={r.recipe_id}
                onChange={(e) => onRecipe(r.id, e.target.value)}
                className="flex-1 min-w-0 bg-[#e4eff8] dark:bg-[#2A323A] border border-[#d5e4f0] dark:border-[#38424B] rounded-lg px-2 py-2 text-sm outline-none focus:border-[#3f7cac]"
              >
      ...
      <button
        ...
        className="w-full mt-2 bg-[#e4eff8] dark:bg-[#2A323A] text-[#2B303B] dark:text-[#e4eff8] font-medium px-4 py-2.5 rounded-xl border border-[#d5e4f0] dark:border-[#38424B] flex items-center justify-center gap-2 disabled:opacity-40"
      >
      ...
      <div className="mt-3 flex items-center gap-2 bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] rounded-2xl px-4 py-2.5">
        ...
        <input
          ...
          className="w-16 text-right font-mono-data font-bold text-[#234b6e] dark:text-[#8FB0C2] bg-[#e4eff8] dark:bg-[#2A323A] border border-[#d5e4f0] dark:border-[#38424B] rounded-lg px-2 py-1.5 outline-none"
        />
      ...
      <button
        ...
        className="w-full mt-3 bg-[#3f7cac] hover:bg-[#336a94] text-white font-semibold px-5 py-3.5 rounded-2xl shadow-md active:scale-98 transition-all"
      >
        {t("lv_compute")}
      </button>
      ...
      {schedule && (
        <div data-testid="sd-result" className="mt-5 space-y-2">
          {schedule.first && (
            <div data-testid="lv-first-result" className="flex items-center gap-2 bg-[#5aa0cf]/12 border border-[#5aa0cf]/35 rounded-2xl px-4 py-3">
              <ChefHat className="w-5 h-5 text-[#2e6690] dark:text-[#a9d2ec] shrink-0" />
          ...
          {[...schedule.items].sort((a, b) => a.mixStart - b.mixStart).map((it, i) => (
            <div key={i} className="bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] rounded-2xl px-4 py-3">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-full bg-[#6E8CA0]/20 text-[#234b6e] dark:text-[#8FB0C2] font-mono-data font-bold text-sm flex items-center justify-center shrink-0">{i + 1}</div>
                <p className="text-sm font-medium text-[#2B303B] dark:text-[#e4eff8] flex-1 truncate">{it.name}</p>
              ...
              <div className="flex flex-wrap gap-2 mt-2 pl-9 font-mono-data text-xs">
                <span className="inline-flex items-center gap-1 text-[#3F4A54] dark:text-[#AEB8BF]">
                  <Cog className="w-3.5 h-3.5 text-[#3f7cac]" /> {t("sd_col_mix")} {fmt(it.mixStart, lang)}–{fmt(it.mixEnd, lang)}
                </span>
                <span className="inline-flex items-center gap-1 text-[#3F4A54] dark:text-[#AEB8BF]">
                  <Hand className="w-3.5 h-3.5 text-[#5aa0cf]" /> {t("sd_col_form")} {fmt(it.formStart, lang)}–{fmt(it.formEnd, lang)}
                </span>
                {it.bake && (it.bake.temp != null || it.bake.mins != null) && (
                  <span className="inline-flex items-center gap-1 text-[#234b6e] dark:text-[#8FB0C2]">
          ...
          <button
            ...
            className={`w-full mt-2 font-semibold px-5 py-3 rounded-2xl flex items-center justify-center gap-2 transition-all ${
              alarmsOn ? "bg-[#5aa0cf] text-white" : "bg-[#e4eff8] dark:bg-[#2A323A] text-[#2B303B] dark:text-[#e4eff8] border border-[#d5e4f0] dark:border-[#38424B]"
            }`}
          >
  ...
  return (
    <div className="flex flex-col">
      ...
      <input
        ...
        className="w-full text-right font-mono-data font-bold text-[#234b6e] dark:text-[#8FB0C2] bg-[#e4eff8] dark:bg-[#2A323A] border border-[#d5e4f0] dark:border-[#38424B] rounded-lg px-1.5 py-1.5 outline-none"
      />
    </div>
  );
}
