...
export default function EsuberoZero({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="esubero-zero">
      {onBack && <button data-testid="esubero-back" onClick={onBack} className="flex items-center gap-1 text-[#ff6b00] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back")}</button>}
      ...
      <div className="rounded-2xl bg-[#121212] dark:bg-[#1e1e1e] border border-[#2b2b2b] dark:border-[#2e2e2e] p-4 shadow-sm mb-4">
        ...
        <input data-testid="esubero-input" type="number" value={g} onChange={(e) => setG(e.target.value)}
          className="w-full bg-white dark:bg-[#181818] border border-[#2b2b2b] dark:border-[#2e2e2e] rounded-xl px-3 py-2.5 outline-none text-[#ff6b00] dark:text-[#e4eff8] focus:border-[#ff6b00] font-mono-data text-lg" />
      </div>

      <div className="space-y-3" data-testid="esubero-recipes">
        {RECIPES.map((r, i) => (
          <div key={r.key} data-testid={`esubero-recipe-${i}`} className="rounded-2xl bg-[#121212] dark:bg-[#1e1e1e] border border-[#2b2b2b] dark:border-[#2e2e2e] overflow-hidden shadow-sm">
            <div className="bg-[#2e8b6f] text-[#161616] px-4 py-2.5"><p className="font-display text-base font-bold">{rL(r)}</p></div>
            <div className="divide-y divide-[#2b2b2b] dark:divide-[#2e2e2e]">
              {r.ing(grams).map(([name, val], j) => (
                <div key={j} className="flex items-center justify-between px-4 py-2 text-[13px]">
                  <span className="text-[#ff6b00] dark:text-[#AEB8BF]">{name}</span>
                  <span className="font-mono-data font-bold text-[#ff6b00] dark:text-[#e4eff8]">{name === "Uovo" ? `${val} pz` : `${num(val)} g`}</span>
                </div>
              ))}
            </div>
          </div>
