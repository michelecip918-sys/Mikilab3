...
import { Trash2, Plus, Recycle, Droplets, Mic, Check } from "lucide-react";
...
export default function RegistroScarti() {
  ...
  const addScarto = (q, p, r = "") => {
    const n = parseFloat(String(q).replace(",", ".")) || 0;
    if (!n && !p) return;
    setScarti((s) => [{ id: `${Date.now()}`, ts: new Date().toISOString(), qty: n, product: p || tri("Prodotto", "Produkt", "Product", "Producto"), reason: r }, ...s]);
  };
  const submit = () => { addScarto(qty, product, reason); setQty(""); setProduct(""); setReason(""); };
    ...
    <div className="pb-24" data-testid="registro-scarti">
      ...
      <div className="rounded-2xl bg-[#1e1e1e] border border-[#2e2e2e] p-3 mb-4 space-y-2">
        ...
        <input data-testid="scarto-reason" value={reason} onChange={(e) => setReason(e.target.value)} placeholder={tri("Motivo (opzionale)", "Grund (optional)", "Reason (optional)", "Motivo (opcional)")}
          className="w-full bg-[#151515] border border-[#2e2e2e] rounded-lg p-2 text-sm text-[#e4eff8] outline-none focus:border-[#ff6b00]" />
