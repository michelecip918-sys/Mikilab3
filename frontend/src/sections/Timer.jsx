...
export default function Timer() {
  ...
  return (
    <div className="pb-40">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mb-5">
        {PRESETS.map((p) => (
          <button key={p.key} data-testid={`timer-preset-${p.key}`} onClick={() => addTimer(p.label, p.min, p.repeat)}
            className="relative flex flex-col items-center gap-0.5 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl py-3 active:scale-95 hover:border-[#E4572E]/50 transition-all">
            {p.repeat && <span className="absolute top-1.5 right-1.5"><RefreshCw className="w-3.5 h-3.5 text-[#3E9C93]" /></span>}
      ...
      <div className="bg-[#3E9C93]/10 border border-[#3E9C93]/30 rounded-2xl p-4 mb-5">
        <p className="text-xs font-bold uppercase tracking-wide text-[#3E9C93] mb-2">{tri("Timer personalizzato", "Eigener Timer", "Custom timer")}</p>
        ...
        <label data-testid="timer-repeat" className="flex items-center gap-2 mt-2 text-sm text-[#3F4A54] dark:text-[#AEB8BF] cursor-pointer select-none">
          <input type="checkbox" checked={repeat} onChange={(e) => setRepeat(e.target.checked)} className="w-4 h-4 accent-[#3E9C93]" />
          <RefreshCw className="w-4 h-4 text-[#3E9C93]" /> {tri("Ripeti automaticamente (pieghe/rinfreschi)", "Automatisch wiederholen (Falten/Auffrischen)", "Auto-repeat (folds/refreshes)")}
        </label>
        <button data-testid="timer-add" onClick={() => { addTimer(name.trim(), mins, repeat); setName(""); }}
          className="mt-2 w-full flex items-center justify-center gap-2 bg-[#3E9C93] hover:bg-[#3E9C93] text-white font-semibold py-3 rounded-2xl shadow-md border border-amber-900/40 active:scale-98 transition-all">
        ...
        {timers.map((t) => {
          ...
          return (
            <div key={t.id} data-testid={`timer-card-${t.id}`}
              className={`rounded-3xl p-5 shadow-sm border ${done ? "bg-[#E4572E]/10 border-[#E4572E]/40" : "bg-white dark:bg-[#1B2A38] border-[#2A3B49] dark:border-[#2A3B49]"}`}>
              <div className="flex items-center justify-between mb-2">
                <span className="font-display text-base font-semibold text-[#2B303B] dark:text-[#e4eff8] truncate flex items-center gap-1.5">{t.name || t.label}{t.repeat && <RefreshCw className="w-4 h-4 text-[#3E9C93] shrink-0" />}</span>
              ...
              <div className="grid grid-cols-2 gap-2 mt-4">
                ...
                <button data-testid={`timer-reset-${t.id}`} onClick={() => reset(t.id)}
                  className="flex items-center justify-center gap-2 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] text-[#2B303B] dark:text-[#e4eff8] font-bold text-lg py-4 rounded-2xl active:scale-97 transition-all">
