...
export default function CalcolatoreSequenze({ onBack }) {
  ...
  const card = "rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-4";
  const inp = "w-full bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 px-3 py-2.5 outline-none text-[#3E9C93] dark:text-[#e4eff8] focus:border-[#3E9C93] text-sm";
  const lbl = "text-[12px] font-semibold text-[#3E9C93] dark:text-[#AEB8BF] mb-1";

  return (
    <div className="pb-8" data-testid="calc-sequenze">
      {onBack && <button data-testid="seq-back" onClick={onBack} className="flex items-center gap-1 text-[#3E9C93] font-medium mb-4">
      ...
      {machines.length > 0 && (
        <div data-testid="seq-machines" className="mb-4 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/12 border border-[#3E9C93]/40 p-3 text-[12px] text-[#3E9C93] dark:text-[#3E9C93]">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className={`${card} text-center`}><p className="text-[11px] font-semibold text-[#3E9C93] flex items-center justify-center gap-1"><Zap className="w-3.5 h-3.5" />{L("1ª velocità", "1. Gang", "1st speed", "1ª vel.")}</p><p data-testid="seq-v1" className="font-display text-2xl font-bold text-[#3E9C93] mt-1">{plan.v1}′</p></div>
        <div className={`${card} text-center`}><p className="text-[11px] font-semibold text-[#3E9C93] flex items-center justify-center gap-1"><Zap className="w-3.5 h-3.5" />{L("2ª velocità", "2. Gang", "2nd speed", "2ª vel.")}</p><p data-testid="seq-v2" className="font-display text-2xl font-bold text-[#3E9C93] mt-1">{plan.v2}′</p></div>
        <div className={`${card} text-center`}><p className="text-[11px] font-semibold text-[#3E9C93] flex items-center justify-center gap-1"><Snail className="w-3.5 h-3.5" />{L("Puntata", "Stockgare", "Bulk", "Fermentación")}</p><p className="font-display text-lg font-bold text-[#3E9C93] mt-1">{fmtMin(plan.puntata)}</p></div>
        <div className={`${card} text-center`}><p className="text-[11px] font-semibold text-[#3E9C93] flex items-center justify-center gap-1"><Clock className="w-3.5 h-3.5" />{L("Appretto", "Stückgare", "Final proof", "Fermentación final")}</p><p className="font-display text-lg font-bold text-[#3E9C93] mt-1">{fmtMin(plan.appretto)}</p></div>
      </div>

      <div data-testid="seq-sequence" className={card}>
        <p className="font-display text-base font-bold text-[#3E9C93] flex items-center gap-2 mb-3"><ListOrdered className="w-4.5 h-4.5" />{L("Sequenza delle lavorazioni", "Arbeitsabfolge", "Work sequence", "Secuencia de trabajo")}</p>
        <ol className="space-y-2">
          {plan.seq.map((s, i) => (
            <li key={i} data-testid={`seq-step-${i}`} className="flex items-start gap-2.5 text-sm text-[#3F4A54] dark:text-[#AEB8BF]">
              <span className="shrink-0 w-6 h-6 rounded-full bg-[#3E9C93] text-white font-bold text-[12px] flex items-center justify-center">{i + 1}</span>
              <span className="leading-snug pt-0.5">{s}</span>
            </li>
          ))}
        </ol>
        <div className="mt-4 pt-3 border-t border-[#2A3B49] dark:border-[#2A3B49] space-y-1.5">
          {plan.pauses.map((p, i) => (
            <div key={i} className="flex items-center justify-between text-[13px]">
              <span className="text-[#3E9C93] dark:text-[#AEB8BF]">{p.label}</span>
              <span className="font-mono-data font-bold text-[#3E9C93]">{p.v}</span>
            </div>
          ))}
        </div>
      </div>
