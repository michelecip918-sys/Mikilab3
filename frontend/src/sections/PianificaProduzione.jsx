  ...
  return (
    <div className="pb-4">
      <div className="flex items-center gap-3 mb-1">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold text-[#2B303B] dark:text-[#EAF0EC]">{t("plan_title")}</h1>
      ...
      <div className="space-y-2 mt-4">
        {rows.map((r) => (
          <div key={r.id} className="flex items-center gap-2 bg-white dark:bg-[#232A31] border border-[#D7E1DB] dark:border-[#38424B] rounded-2xl px-3 py-2.5">
            <select
      ...
      {plan && (
        <div data-testid="plan-result" className="mt-5 space-y-2">
          ...
          {plan.items.map((it, i) => {
            ...
            return (
            <div key={i} className="flex items-center gap-3 bg-white dark:bg-[#232A31] border border-[#D7E1DB] dark:border-[#38424B] rounded-2xl px-4 py-3">
              <div className="w-8 h-8 rounded-full bg-[#6E8CA0]/20 text-[#33564E] dark:text-[#8FB0C2] font-mono-data font-bold text-sm flex items-center justify-center shrink-0">{i + 1}</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-[#2B303B] dark:text-[#EAF0EC] truncate">{it.name}</p>
                <p className="font-mono-data text-xs text-[#7E8A93]">{t("sd_start_mix")} {fmt(it.start, lang)} · {t("sd_rest_min")} {it.rest}</p>
                <p data-testid={`inf-bake-${i}`} className="font-mono-data text-xs text-[#33564E] dark:text-[#8FB0C2] mt-0.5">
          ...
          <div className="flex items-center gap-3 bg-[#5E8B7E] rounded-2xl px-4 py-3 text-white">
          ...
          {selectedOven && (
            <div data-testid="inf-oven-suggestion" className="bg-white dark:bg-[#232A31] border border-[#D7E1DB] dark:border-[#38424B] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-2">
                <Flame className="w-4 h-4 text-[#5E8B7E]" />
                <span className="text-xs font-bold uppercase tracking-wide text-[#33564E] dark:text-[#8FB0C2]">
              ...
              {[["phase1", t("tl_phase1")], ["phase2", t("tl_phase2")], ["phase3", t("tl_phase3")]].map(([k, label]) => {
                ...
                return (
                  <div key={k} className="flex items-center justify-between font-mono-data text-sm py-0.5">
                    ...
                    <span className="text-[#33564E] dark:text-[#8FB0C2] font-bold">
