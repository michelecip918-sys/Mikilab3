  ...
  return (
    <div className="pb-8" data-testid="banca-lievito">
      ...
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#0E1620] shadow-xl mb-5" style={{ background: "linear-gradient(135deg,#2e8b6f,#1c5c49 70%,#3E9C93)" }}>
        ...
        <p className="text-[#0E1620]/85 text-sm mt-2 leading-snug">{L("La mappa della community per scambiare starter (lievito madre / Sauerteig) e consigli con i fornai vicino a te.", "Community-Karte zum Tauschen von Sauerteig-Starter.", "Community map to exchange sourdough starter and tips nearby.", "Mapa de la comunidad para intercambiar masa madre.")}</p>
      </div>

      <button data-testid="banca-open-map" onClick={() => setMapOpen(true)}
        className="w-full flex items-center justify-center gap-2 rounded-2xl bg-[#2e8b6f] hover:bg-[#1c5c49] text-[#0E1620] font-semibold px-5 py-3.5 mb-4 active:scale-98 transition-all">
      ...
      {list === null ? (
        ...
        <p className="text-center text-sm text-[#5E8CA8] py-8">{L("Ancora nessuno sulla mappa. Sii il primo a offrire il tuo starter!", "Noch niemand. Sei der Erste!", "Nobody yet. Be the first to offer your starter!", "Nadie aún. ¡Sé el primero!")}</p>
      ) : (
        <div className="space-y-2.5" data-testid="banca-list">
          {list.slice(0, 30).map((b, i) => (
            <div key={i} data-testid={`banca-baker-${i}`} className="flex items-start gap-3 rounded-2xl bg-[#0E1620] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-3.5 shadow-sm">
