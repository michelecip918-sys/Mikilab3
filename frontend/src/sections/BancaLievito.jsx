...
export default function BancaLievito({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="banca-lievito">
      {onBack && <button data-testid="banca-back" onClick={onBack} className="flex items-center gap-1 text-[#c94f00] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back")}</button>}
      ...
      <p className="font-display text-lg font-bold text-[#c94f00] dark:text-[#e4eff8] mb-2">{L("Fornai nella community", "Bäcker in der Community", "Bakers in the community", "Panaderos en la comunidad")}</p>
      {list === null ? (
        <div className="flex justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-[#2e8b6f]" /></div>
      ) : list.length === 0 ? (
        <p className="text-center text-sm text-[#d4a373] py-8">{L("Ancora nessuno sulla mappa. Sii il primo a offrire il tuo starter!", "Noch niemand. Sei der Erste!", "Nobody yet. Be the first to offer your starter!", "Nadie aún. ¡Sé el primero!")}</p>
      ) : (
          ...
          {list.slice(0, 30).map((b, i) => (
            <div key={i} data-testid={`banca-baker-${i}`} className="flex items-start gap-3 rounded-2xl bg-[#121212] dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-3.5 shadow-sm">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-display text-[15px] font-bold text-[#c94f00] dark:text-[#e4eff8] leading-tight">{b.name}</p>
                {b.city && <p className="text-[12px] text-[#c94f00] flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> {b.city}</p>}
                {b.bio && <p className="text-[12.5px] text-[#c94f00] dark:text-[#AEB8BF] leading-snug mt-0.5">{b.bio}</p>}
