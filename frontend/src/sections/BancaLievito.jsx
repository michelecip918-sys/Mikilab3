...
export default function BancaLievito({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="banca-lievito">
      {onBack && <button data-testid="banca-back" onClick={onBack} className="flex items-center gap-1 text-[#F26419] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back")}</button>}
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#0B0E14] shadow-xl mb-5" style={{ background: "linear-gradient(135deg,#2e8b6f,#1c5c49 70%,#F26419)" }}>
        ...
        <p className="text-[#0B0E14]/85 text-sm mt-2 leading-snug">{L("La mappa della community per scambiare starter (lievito madre / Sauerteig) e consigli con i fornai vicino a te.", "Community-Karte zum Tauschen von Sauerteig-Starter.", "Community map to exchange sourdough starter and tips nearby.", "Mapa de la comunidad para intercambiar masa madre.")}</p>
      </div>

      <button data-testid="banca-open-map" onClick={() => setMapOpen(true)}
        className="w-full flex items-center justify-center gap-2 rounded-2xl bg-[#2e8b6f] hover:bg-[#1c5c49] text-[#0B0E14] font-semibold px-5 py-3.5 mb-4 active:scale-98 transition-all">
      ...
      <p className="font-display text-lg font-bold text-[#F26419] dark:text-[#e4eff8] mb-2">{L("Fornai nella community", "Bäcker in der Community", "Bakers in the community", "Panaderos en la comunidad")}</p>
      {list === null ? (
        <div className="flex justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-[#2e8b6f]" /></div>
      ) : list.length === 0 ? (
        <p className="text-center text-sm text-[#E8A838] py-8">{L("Ancora nessuno sulla mappa. Sii il primo a offrire il tuo starter!", "Noch niemand. Sei der Erste!", "Nobody yet. Be the first to offer your starter!", "Nadie aún. ¡Sé el primero!")}</p>
      ) : (
        <div className="space-y-2.5" data-testid="banca-list">
          {list.slice(0, 30).map((b, i) => (
            <div key={i} data-testid={`banca-baker-${i}`} className="flex items-start gap-3 rounded-2xl bg-[#0B0E14] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] p-3.5 shadow-sm">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-display text-[15px] font-bold text-[#F26419] dark:text-[#e4eff8] leading-tight">{b.name}</p>
                {b.city && <p className="text-[12px] text-[#F26419] flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> {b.city}</p>}
                {b.bio && <p className="text-[12.5px] text-[#F26419] dark:text-[#AEB8BF] leading-snug mt-0.5">{b.bio}</p>}
