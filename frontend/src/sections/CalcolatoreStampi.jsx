    ...
    <div className="pb-8" data-testid="calc-stampi">
      ...
      {mode === "pirottini" ? (
        <div className={card}>
          ...
          <div data-testid="stampi-pirottini-out" className="rounded-2xl shadow-md border border-amber-900/40 bg-[#e4eff8] dark:bg-[#1B2A38] border border-[#2A3B49] p-4 text-center">
            <p className="text-[11px] font-semibold text-[#3E9C93]">{L("Impasto totale necessario", "Benötigter Teig gesamt", "Total dough needed", "Masa total necesaria")}</p>
            <p className="font-display text-3xl font-bold text-[#3E9C93] mt-1">{num(totImpasto)} g</p>
            <p className="text-[12px] text-[#3E9C93] mt-1">{num(PIROTTINI[size])} g {L("per pirottino", "pro Form", "per mould", "por molde")} ({L("include la testa che lievita oltre il bordo", "inkl. Überstand", "includes rise over the rim", "incluye la cúpula")})</p>
          </div>
        </div>
      ) : (
        <div className={card}>
          ...
          <div data-testid="stampi-teglia-out" className="rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] p-4 text-center">
            <p className="text-[11px] font-semibold text-[#3E9C93]">{L("Impasto consigliato", "Empfohlener Teig", "Recommended dough", "Masa recomendada")}</p>
            <p className="font-display text-3xl font-bold text-[#3E9C93] mt-1">{num(teglia.dough)} g</p>
            <p className="text-[12px] text-[#3E9C93] mt-1">{L("Superficie", "Fläche", "Area", "Superficie")}: {num(teglia.area)} cm²</p>
          </div>
        </div>
      )}
    </div>
