...
const C = { cream: "#0D1520", surf: "#1B2A38", border: "#2A3B49", gold: "#64748B", title: "#64748B", dark: "#F7F9FC", muted: "#94A3B8" };
...
export default function RicettaDelGiorno() {
    ...
    return (
      <div data-testid="rdg-detail" className="min-h-[70vh] rounded-3xl p-5 pb-28" style={{ background: C.cream, color: C.dark }}>
        ...
        <div className="space-y-3" data-testid="rdg-doses">
          {rows.map((row, i) => (
            <div key={i} className="flex items-baseline justify-between gap-3 rounded-2xl px-4 py-3" style={{ background: C.surf, border: `2px solid ${C.border}` }}>
              ...
              <span className="font-mono-data font-extrabold whitespace-nowrap" style={{ fontSize: "clamp(24px,7vw,38px)", color: "#64748B" }}>
          ...
          {STEP_STATUSES.map((st) => {
            ...
            return (
              <button key={st} data-testid={`rdg-status-${st}`} onClick={() => setBatchStatus(batchFor(sel), st)}
                ...
                style={{ background: on ? (STATUS_COLOR[st] || C.gold) : C.surf, border: `2px solid ${on ? (STATUS_COLOR[st] || C.gold) : C.border}`, color: on ? "#1B2A38" : C.dark }}>
                {statusLabel(st, tri)}
              </button>
            );
          })}
        ...
        <button data-testid="rdg-done" onClick={async () => {
          ...
          <CheckCircle2 className="w-5 h-5" style={{ color: "#2A3B49" }} /> {tri("Lotto completato \u2192 prossimo", "Charge fertig \u2192 n\u00e4chste", "Batch done \u2192 next", "Lote hecho \u2192 siguiente", "Lot termin\u00e9 \u2192 suivant", "\u062f\u0633\u062a\u0647 \u062a\u0645\u0627\u0645 \u2192 \u0628\u0639\u062f\u06cc")}
        </button>
      </div>
    );
  }
  ...
  return (
    <div data-testid="ricetta-del-giorno" className="min-h-[70vh] rounded-3xl p-5 pb-28" style={{ background: C.cream, color: C.dark }}>
      ...
      {autonomia && (
        <div className="rounded-2xl shadow-md border border-amber-900/40 px-3 py-2 mb-3" style={{ background: "#1B2A38", border: `2px solid ${C.gold}` }}>
          ...
          {deadline && <p className="text-[12px] font-extrabold mt-1" style={{ color: "#64748B" }}>⏰ {tri("Puoi lavorare in autonomia fino alle", "Autonom bis", "Work autonomously until", "Autonomía hasta", "Autonomie jusqu'à", "خودگردان تا")} {fmtHM(deadline, lang)}.</p>}
        </div>
      )}
