...
export default function SmartWeatherBaker({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="weather-baker">
      {onBack && <button data-testid="weather-back" onClick={onBack} className="flex items-center gap-1 text-[#F26419] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back")}</button>}
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#0B0E14] shadow-xl mb-5" style={{ background: "linear-gradient(135deg,#F26419,#F26419 55%,#F26419)" }}>
        ...
        <p className="text-[#0B0E14]/85 text-sm mt-2 leading-snug">{L("Adatta acqua e lievitazione al clima di casa tua: una giornata fredda in Germania non è come il caldo del Sud Italia.", "Passe Wasser & Gare an dein Klima an.", "Adapt water & proofing to your local climate.", "Adapta agua y fermentación a tu clima.")}</p>
      </div>
      ...
      <div data-testid="weather-result" className="rounded-2xl bg-[#ffffff] border border-[#26324A] p-4 space-y-3">
        <div className="flex items-center gap-2">
          <Droplets className="w-5 h-5 text-[#F26419]" />
          <p className="text-[13.5px] text-[#F26419]"><span className="font-bold text-[#F26419]">{L("Acqua:", "Wasser:", "Water:", "Agua:")} </span>
        ...
        <div className="flex items-center gap-2">
          <Clock className="w-5 h-5 text-[#F26419]" />
          <p className="text-[13.5px] text-[#F26419]"><span className="font-bold text-[#F26419]">{L("Puntata stimata:", "Stockgare:", "Bulk proof:", "Fermentación:")} </span>
        ...
        <div className="flex items-center gap-2">
          <CloudSun className="w-5 h-5 text-[#F26419]" />
          <p className="text-[13.5px] text-[#F26419]"><span className="font-bold text-[#F26419]">{L("Temperatura acqua consigliata:", "Empf. Wassertemp.:", "Suggested water temp:", "Temp. agua sugerida:")} </span><span className="font-mono-data font-bold">{r.waterTemp}°C</span></p>
        </div>
        {city && <p className="text-[11px] text-[#E8A838] italic">📍 {city}</p>}
      </div>
    </div>
  );
}
