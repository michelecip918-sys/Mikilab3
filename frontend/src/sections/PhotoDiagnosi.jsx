...
export default function PhotoDiagnosi() {
  ...
  return (
    <div className="pb-24">
      ...
      {praised && (
        <div data-testid="photo-compliment" className="mt-4 flex items-center gap-3 bg-[#3E9C93]/15 border border-[#3E9C93]/40 rounded-2xl p-4">
          <PartyPopper className="w-6 h-6 text-[#3E9C93] dark:text-[#a9d2ec] shrink-0" />
          <p className="text-sm font-bold text-[#3E9C93] dark:text-[#a9d2ec]">{t("photo_compliment")}</p>
        </div>
      )}
      ...
      {result && (
        <div className="print-area">
          ...
          <div data-testid="photo-result" className="markdown-body mt-5 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-5 text-sm leading-relaxed text-[#2B303B] dark:text-[#e4eff8]">
      ...
      {result && (
        <button data-testid="photo-share-btn" onClick={() => shareContent(`${modeLabel(mode)} — MikiLab`, result, lang)}
          className="mt-2 w-full bg-[#e4eff8] dark:bg-[#1B2A38] text-[#2B303B] dark:text-[#e4eff8] font-medium px-4 py-3 rounded-2xl border border-[#2A3B49] dark:border-[#2A3B49] flex items-center justify-center gap-2 active:scale-98 transition-all">
        ...
        <div data-testid="diagnosi-recenti" className="mt-8">
          ...
          <div className="space-y-2.5">
            {recent.map((d) => {
              ...
              return (
                <div key={d.id} data-testid={`diagnosi-item-${d.id}`} className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] overflow-hidden">
                  <div className="flex items-center gap-3 p-3">
                    <div className="w-12 h-12 rounded-2xl shadow-md border border-amber-900/40 bg-[#e4eff8] dark:bg-[#1B2A38] flex items-center justify-center shrink-0 overflow-hidden relative">
                    ...
                    <button data-testid={`diagnosi-share-${d.id}`} onClick={() => shareContent(`${modeLabel(d.mode)} — MikiLab`, d.result, lang)} className="p-2 text-[#3E9C93] active:scale-90 shrink-0" aria-label="share"><Share2 className="w-4 h-4" /></button>
                    <button data-testid={`diagnosi-delete-${d.id}`} onClick={() => deleteRecent(d.id)} className="p-2 text-[#3E9C93] active:scale-90 shrink-0" aria-label="delete"><Trash2 className="w-4 h-4" /></button>
                  ...
                  {isOpen && (
                    <div className="markdown-body px-4 pb-4 text-sm leading-relaxed text-[#2B303B] dark:text-[#e4eff8] border-t border-[#e4eff8] dark:border-[#2A3B49] pt-3">
