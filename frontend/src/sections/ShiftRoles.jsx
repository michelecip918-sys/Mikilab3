  ...
  return (
    <div className="pb-4">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-[#F26419] flex items-center justify-center"><Users className="w-6 h-6 text-white" /></div>
        <div>
      ...
      <div className="space-y-3 pb-24" data-testid="shift-list">
        ...
        {people.map((p) => (
          <div key={p.id} data-testid={`shift-${p.id}`} className="rounded-2xl bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] p-3 space-y-2">
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => del(p.id)} className="text-[#F26419] p-1"><X className="w-4 h-4" /></button>
            </div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 min-w-0 flex items-center gap-2 bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] rounded-lg p-2">
                <Headphones className="w-4 h-4 text-[#F26419] shrink-0" />
            ...
            <div className="grid grid-cols-2 gap-2">
              <select data-testid={`shift-zone-${p.id}`} value={p.zone || "impasti"} onChange={(e) => upd(p.id, { zone: e.target.value })}
                className="bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] rounded-lg p-2 text-sm outline-none focus:border-[#F26419]">
              ...
              <input type="time" data-testid={`shift-start-${p.id}`} value={p.shiftStart || ""} onChange={(e) => upd(p.id, { shiftStart: e.target.value })}
                className="bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] rounded-lg p-2 text-sm outline-none focus:border-[#F26419]" title={mkTri(lang)("Inizio turno", "Schichtbeginn", "Shift start", "Inicio turno")} />
            </div>
            {/* Cambio mansione programmato (avviso vocale automatico) */}
            <div className="grid grid-cols-2 gap-2 items-center rounded-lg bg-[#F26419]/8 border border-[#F26419]/25 p-2">
              <div className="col-span-2 text-[11px] font-bold uppercase tracking-wide text-[#F26419]">{mkTri(lang)("Cambio mansione (avviso vocale)", "Aufgabenwechsel (Sprachhinweis)", "Role change (voice alert)", "Cambio de tarea (aviso de voz)")}</div>
              ...
              <input type="time" data-testid={`shift-changeat-${p.id}`} value={p.changeAt || ""} onChange={(e) => upd(p.id, { changeAt: e.target.value })}
                className="bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] rounded-lg p-2 text-sm outline-none focus:border-[#F26419]" title={mkTri(lang)("Orario cambio", "Wechselzeit", "Change time", "Hora del cambio")} />
            </div>
          </div>
        ))}
      </div>

      <button data-testid="shift-add" onClick={add} className="w-full mt-4 bg-[#F26419] hover:bg-[#E8A838] text-white font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all flex items-center justify-center gap-2">
