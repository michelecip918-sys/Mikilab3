...
export default function TrovaFarina({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="trova-farina">
      {onBack && <button data-testid="farina-back" onClick={onBack} className="flex items-center gap-1 text-[#F26419] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back")}</button>}
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#0B0E14] shadow-xl mb-5" style={{ background: "linear-gradient(135deg,#F26419,#F26419 60%,#F26419)" }}>
        ...
        <p className="text-[#0B0E14]/85 text-sm mt-2 leading-snug">{L("Quale farina comprare al supermercato (in Germania, Italia, Francia o Spagna) per replicare la ricetta giusta.", "Welches Supermarkt-Mehl du kaufen sollst, um das Rezept zu treffen.", "Which supermarket flour to buy to match the recipe.", "Qué harina comprar en el supermercado para replicar la receta.")}</p>
      </div>

      <div className="space-y-3">
        {ROWS.map((r, i) => (
          <div key={i} data-testid={`farina-row-${i}`} className="rounded-2xl bg-[#0B0E14] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              <Wheat className="w-4.5 h-4.5 text-[#F26419]" />
              <p className="font-display text-[15px] font-bold text-[#F26419] dark:text-[#e4eff8] flex-1 leading-tight">{uL(r.use)}</p>
              {r.w !== "—" && <span className="text-[10px] font-bold uppercase bg-[#ffffff] text-[#F26419] px-2 py-0.5 rounded-full">{r.w}</span>}
            </div>
            <div className="grid grid-cols-2 gap-1.5 text-[12.5px]">
              {flagRow.map(([k, flag]) => (
                <p key={k} className="text-[#F26419] dark:text-[#AEB8BF]"><span className="mr-1">{flag}</span>{r[k]}</p>
              ))}
            </div>
          </div>
        ))}
