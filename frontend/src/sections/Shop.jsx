...
function TestRegistry() {
  ...
  const field = (key, ph, type = "text") => (
    <input
      ...
      placeholder={ph}
      className="w-full rounded-xl bg-[#121722] border border-[#26324A]/40 px-3 py-2 text-sm text-[#F7F9FC] placeholder-[#64748B] focus:border-[#E8A838] focus:outline-none"
    />
  );

  return (
    <div data-testid="flour-test-registry" className="lab-3d-card rounded-3xl p-6 bg-[#121722]/85 border border-[#26324A]/60 backdrop-blur-md shadow-xl">
      <div className="flex items-center gap-3 mb-4">
        <span className="w-11 h-11 rounded-2xl bg-[#E8A838]/15 border border-[#E8A838]/40 flex items-center justify-center shrink-0">
      ...
      <div data-testid="flour-scan-block" className="mb-4 rounded-2xl bg-[#121722] border border-[#E8A838]/30 p-4">
        <div className="flex items-center gap-2 mb-2 text-[#E8A838]">
        ...
        {scanning ? (
          <div data-testid="flour-scan-loading" className="inline-flex items-center gap-2 rounded-2xl bg-[#E8A838] text-[#121722] font-semibold px-5 py-3 opacity-80">
      ...
      <button
        ...
        onClick={add}
        className="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-br from-[#E8A838] to-[#F26419] text-white font-semibold py-2.5 shadow-lg active:scale-98 transition-all"
      >
      ...
      <div className="mt-4 space-y-2.5" data-testid="flour-test-list">
        ...
        {tests.map((tst) => (
          <div key={tst.id} data-testid={`flour-test-row-${tst.id}`} className="flex items-start gap-3 rounded-2xl bg-[#121722] border border-[#26324A]/40 p-3.5">
            <div className="min-w-0 flex-1">
              <p className="font-semibold text-[#F7F9FC] text-sm truncate">{tst.name}</p>
              <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[12px] text-[#E8A838] font-medium mt-0.5">
            ...
            <button data-testid={`flour-test-del-${tst.id}`} onClick={() => remove(tst.id)} className="shrink-0 text-[#64748B] hover:text-[#F26419] transition-colors">
...
export default function Shop({ onNavigate }) {
  ...
  return (
    <div data-testid="shop-page" className="pb-4 space-y-6">
      {/* Hero */}
      <div className="rounded-3xl bg-gradient-to-br from-[#3a2410] to-[#121722] text-white p-7 text-center shadow-xl border border-[#26324A]/50 relative overflow-hidden">
        <div className="absolute inset-0 opacity-[0.06] pointer-events-none" style={{ backgroundImage: "radial-gradient(circle at 20% 20%, #E8A838 1px, transparent 1px)", backgroundSize: "22px 22px" }} />
        <Microscope className="w-12 h-12 mx-auto mb-2 text-[#E8A838]" />
      ...
      <div data-testid="lab-manifesto" className="p-6 rounded-3xl border border-[#26324A]/60 bg-[#121722]/85 backdrop-blur-md shadow-xl text-[#F7F9FC]">
        <div className="flex flex-col md:flex-row gap-6 items-center">
          <img
            src={LAB_PHOTO}
            onError={(e) => { e.currentTarget.src = `${PUB}/michele-real-lab.jpg`; }}
            className="w-32 h-32 md:w-44 md:h-44 object-cover rounded-2xl shadow-lg border border-[#E8A838]/40 flex-shrink-0"
            alt={tri("Reparto Analisi Impasti", "Teiganalyse-Abteilung", "Dough Analysis Department", "Departamento de Análisis de Masas")}
          />
          <div>
            <h2 className="text-2xl font-bold mb-3 text-[#E8A838] flex items-center gap-3">
            ...
            <div className="text-xs text-[#E8A838] font-semibold tracking-wide uppercase">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {params.map((p) => (
            <div key={p.id} data-testid={`flour-param-${p.id}`} className="lab-3d-card rounded-2xl p-5 bg-[#121722] border border-[#26324A]/50 shadow-sm flex gap-3">
              <span className="w-11 h-11 rounded-2xl bg-[#E8A838]/15 border border-[#E8A838]/40 flex items-center justify-center shrink-0">
                <p.Icon className="w-6 h-6 text-[#E8A838]" />
              </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button data-testid="shop-go-recipes" onClick={() => onNavigate && onNavigate("ricette")}
          className="lab-3d-card text-left rounded-2xl p-5 bg-[#121722] border border-[#26324A]/50 shadow-sm active:scale-98 transition-all flex items-center gap-3">
          <div className="w-11 h-11 rounded-2xl bg-[#E8A838]/15 border border-[#E8A838]/40 flex items-center justify-center shrink-0"><BookOpen className="w-6 h-6 text-[#E8A838]" /></div>
          <div className="min-w-0">
        ...
        <button data-testid="shop-go-academy" onClick={() => onNavigate && onNavigate("impara")}
          className="lab-3d-card text-left rounded-2xl p-5 bg-[#121722] border border-[#26324A]/50 shadow-sm active:scale-98 transition-all flex items-center gap-3">
          <div className="w-11 h-11 rounded-2xl bg-[#E8A838]/15 border border-[#E8A838]/40 flex items-center justify-center shrink-0"><GraduationCap className="w-6 h-6 text-[#E8A838]" /></div>
          <div className="min-w-0">
