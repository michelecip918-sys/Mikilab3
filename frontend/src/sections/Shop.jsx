  ...
  const Card = ({ p }) => (
    <div data-testid={`shop-product-${p.id}`} className="rounded-2xl overflow-hidden bg-white dark:bg-[#2A211D] border border-[#E8DEC8] dark:border-[#3D302A] shadow-sm">
      ...
      <div className="p-4">
        ...
        {p.allergens ? <p className="text-[11px] text-[#8C7567] mt-2"><b>{tri("Allergeni","Allergene","Allergens")}:</b> {p.allergens}</p> : null}
        {data.enabled ? (
    ...
    <div data-testid="shop-page" className="pb-4 space-y-6">
      ...
      {!data.enabled && (sent ? (
        <div data-testid="shop-waitlist-done" className="rounded-2xl bg-[#6B8E62]/10 border border-[#6B8E62]/30 p-5 text-center">
          ...
          <p className="text-sm text-[#4A3B34] dark:text-[#C9BBB0]">{tri("Grazie! Ti avviseremo al lancio.","Danke! Wir benachrichtigen dich zum Start.","Thanks! We'll notify you at launch.")}</p>
        </div>
      ) : (
        <div className="rounded-2xl bg-[#D99B26]/10 border border-[#D99B26]/30 p-4">
          ...
          <div className="flex gap-2">
            ...
            <button data-testid="shop-waitlist-btn" onClick={() => join(null)}
              className="bg-[#B34A26] text-white font-semibold px-4 rounded-xl active:scale-97">{tri("Iscrivimi","Eintragen","Join")}</button>
          </div>
        </div>
      ))}
