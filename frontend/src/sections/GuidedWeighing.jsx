...
export default function GuidedWeighing() {
  ...
  const COLORS = { under: "#E0A458", near: "#E0A458", ok: "#B45309", over: "#C0574D", done: "#8C4A27" };
    ...
    return (
      <div className="pb-40" data-testid="gw-run">
        <div className="flex items-center justify-between mb-3">
          ...
          <button data-testid="gw-voice-toggle" onClick={() => setVoiceOn((v) => !v)} className="p-2 rounded-lg border border-[#E6D8C3] dark:border-[#38424B] text-[#8C4A27]">{voiceOn ? <Volume2 className="w-4 h-4" /> : <VolumeX className="w-4 h-4" />}</button>
        </div>
        ...
        {!bleOn && (
          <div className="mt-4 space-y-2">
            ...
            <input data-testid="gw-sim" type="range" min={0} max={Math.round(tgt * 1.3) || 100} value={weight} onChange={(e) => setWeight(Number(e.target.value))} className="w-full accent-[#8C4A27]" />
        ...
        <div className="grid grid-cols-2 gap-2 mt-4">
          <button data-testid="gw-next" onClick={next} disabled={state === "over" || state === "under"} className="flex items-center justify-center gap-2 bg-[#8C4A27] hover:bg-[#336a94] disabled:opacity-40 text-white font-bold py-4 rounded-2xl active:scale-97">{tri("Avanti", "Weiter", "Next")} <ArrowRight className="w-5 h-5" /></button>
          <button data-testid="gw-voicecmd" onClick={startVoiceCmd} className="flex items-center justify-center gap-2 bg-white dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] text-[#2B303B] dark:text-[#e4eff8] font-bold py-4 rounded-2xl active:scale-97"><Volume2 className="w-5 h-5 text-[#8C4A27]" /> {tri("Voce", "Sprache", "Voice")}</button>
        </div>
    ...
    return (
      <div className="pb-40" data-testid="gw-summary">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-11 h-11 rounded-2xl bg-[#B45309] flex items-center justify-center"><Scale className="w-6 h-6 text-white" /></div>
        ...
        <div className="rounded-2xl bg-[#B45309]/10 border border-[#B45309]/30 p-4 mb-4" data-testid="gw-weighed-list">
          <p className="text-xs font-bold uppercase text-[#8C4A27] mb-2">{tri("Pesato realmente", "Tatsächlich gewogen", "Actually weighed")}</p>
          {summary.rows.map((r, i) => (
            <div key={i} className="flex items-center justify-between text-sm py-1 border-b border-[#E6D8C3]/50 dark:border-[#38424B]/50 last:border-0">
        ...
        <div className="rounded-2xl bg-[#8C4A27]/10 border border-[#8C4A27]/30 p-4 mb-4" data-testid="gw-savesession">
          {sessSaved ? (
            ...
            <div className="space-y-2">
              <p className="text-xs font-bold uppercase text-[#8C4A27]">{tri("Aggiungi le temperature", "Temperaturen ergänzen", "Add the temperatures")}</p>
              ...
              <button data-testid="gw-sess-save" onClick={saveAsSession} disabled={sessSaving} className="w-full flex items-center justify-center gap-2 bg-[#8C4A27] hover:bg-[#336a94] disabled:opacity-50 text-white font-bold py-3 rounded-2xl active:scale-98"><Save className="w-5 h-5" /> {sessSaving ? tri("Salvataggio…", "Speichern…", "Saving…") : tri("Salva nel Diario", "Ins Tagebuch", "Save to log")}</button>
            </div>
          )}
        </div>

        {/* Crea lotto tracciabilità dai pesi reali */}
        <button data-testid="gw-create-batch" onClick={createBatch} disabled={batchSaved} className="w-full flex items-center justify-center gap-2 bg-[#B45309] hover:bg-[#336a94] disabled:opacity-60 text-white font-semibold py-3 rounded-2xl mb-4 active:scale-98">
        ...
        <button data-testid="gw-summary-reset" onClick={reset} className="w-full flex items-center justify-center gap-2 bg-[#8C4A27] hover:bg-[#336a94] text-white font-bold py-4 rounded-2xl active:scale-98"><RotateCcw className="w-5 h-5" /> {tri("Nuova pesata", "Neu wiegen", "New weighing")}</button>
      </div>
    );
  }
  ...
  const roleBtn = (ing, role, Icon, label) => (
    <button onClick={() => setIngredients((l) => l.map((x) => x.id === ing.id ? { ...x, role } : x))}
      className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg text-[11px] font-semibold ${ing.role === role ? "bg-[#8C4A27] text-white" : "bg-[#FAF5EC] dark:bg-[#1F252B] text-[#7E8A93] border border-[#E6D8C3] dark:border-[#38424B]"}`}>
  ...
  return (
    <div className="pb-40">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-[#8C4A27] flex items-center justify-center"><Scale className="w-6 h-6 text-white" /></div>
        <div>
      ...
      <button data-testid="gw-ble" onClick={connectBle} className={`w-full flex items-center justify-center gap-2 font-semibold py-3 rounded-2xl mb-4 active:scale-98 ${bleOn ? "bg-[#B45309] text-white" : "bg-[#3F7CAC] text-white hover:bg-[#2E5E82]"}`}>
      ...
      {/* Riscalamento */}
      <div className="bg-[#B45309]/10 border border-[#B45309]/30 rounded-2xl p-4 mb-4 space-y-2">
        <p className="text-xs font-bold uppercase text-[#8C4A27]">{tri("Riscalamento", "Skalierung", "Rescaling")}</p>
        <div className="grid grid-cols-2 gap-2">
        ...
        <p className="text-[11px] text-[#7E8A93]">{tri("Totale", "Gesamt", "Total")}: <b className="font-mono-data">{(totalG / 1000).toFixed(2)} kg</b>{batches > 1 && <span className="text-[#C0574D] font-semibold"> · <Layers className="w-3 h-3 inline" /> {tri(`divisa in ${batches} impastate`, `in ${batches} Teige geteilt`, `split into ${batches} batches`)}</span>}</p>
      </div>

      {/* Ingredienti con ruolo + prezzo */}
      <div className="space-y-3 mb-4" data-testid="gw-ingredients">
        {ingredients.map((ing) => (
          <div key={ing.id} className="rounded-2xl bg-white dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] p-3 space-y-2">
            <div className="grid grid-cols-[1fr_84px_34px] gap-2 items-center">
              ...
              <button onClick={() => setIngredients((l) => l.filter((x) => x.id !== ing.id))} className="text-[#7E8A93] hover:text-[#C0574D] flex justify-center"><Trash2 className="w-4 h-4" /></button>
            </div>
            <div className="flex gap-1.5">
              ...
              <label className="flex items-center gap-1 bg-[#FAF5EC] dark:bg-[#1F252B] border border-[#E6D8C3] dark:border-[#38424B] rounded-lg px-2 text-[11px] text-[#7E8A93]">
        ...
        <button data-testid="gw-add" onClick={() => setIngredients((l) => [...l, { id: uid(), name: "", base: 0, role: "other", price: 0 }])} className="text-sm font-semibold text-[#8C4A27] flex items-center gap-1"><Plus className="w-4 h-4" /> {tri("Aggiungi ingrediente", "Zutat hinzufügen", "Add ingredient")}</button>
      </div>

      <button data-testid="gw-start" onClick={startRun} disabled={ingredients.filter((i) => i.name && i.base > 0).length === 0}
        className="w-full flex items-center justify-center gap-2 bg-[#8C4A27] hover:bg-[#336a94] disabled:opacity-40 text-white font-bold py-4 rounded-2xl active:scale-98"><Scale className="w-5 h-5" /> {tri("Inizia pesata guidata", "Wiegen starten", "Start guided weighing")}</button>
    </div>
  );
}
