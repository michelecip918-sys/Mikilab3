  ...
  return (
    <div className="pb-8" data-testid="cantiere">
      ...
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#161616] shadow-xl mb-5" style={{ background: "linear-gradient(135deg,#ff6b00,#ff6b00 60%,#ff6b00)" }}>
        ...
        <p className="text-[#161616]/85 text-sm mt-2 leading-snug">{L("Genera il PDF stampabile a caratteri grandi da appendere in laboratorio: dosi, tempi e procedimento a colpo d'occhio.", "Druckbares Großdruck-PDF fürs Labor.", "A big-print PDF to hang in your kitchen.", "PDF imprimible de letra grande para el obrador.")}</p>
      </div>
      ...
      {recipes === null ? (
        ...
        <div className="space-y-2.5" data-testid="cantiere-list">
          {recipes.map((r, i) => (
            <div key={r.id || i} data-testid={`cantiere-recipe-${i}`} className="flex items-center gap-3 rounded-2xl bg-[#121212] dark:bg-[#1e1e1e] border border-[#2b2b2b] dark:border-[#2e2e2e] p-3.5 shadow-sm">
              ...
              <button data-testid={`cantiere-pdf-${i}`} disabled={busy} onClick={() => generate(r)}
                className="flex items-center gap-1.5 bg-[#ff6b00] hover:bg-[#ff6b00] disabled:opacity-50 text-[#161616] text-[13px] font-semibold px-3.5 py-2 rounded-xl active:scale-95 transition-all shrink-0">
