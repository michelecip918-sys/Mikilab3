...
export default function CantiereRicetta({ onBack }) {
  ...
  return (
    <div className="pb-8" data-testid="cantiere">
      {onBack && <button data-testid="cantiere-back" onClick={onBack} className="flex items-center gap-1 text-[#8C4A27] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back")}</button>}
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#FFFDF9] shadow-xl mb-5" style={{ background: "linear-gradient(135deg,#8C4A27,#6E371C 60%,#4A3222)" }}>
        ...
        <p className="text-[#FFFDF9]/85 text-sm mt-2 leading-snug">{L("Genera il PDF stampabile a caratteri grandi da appendere in laboratorio: dosi, tempi e procedimento a colpo d'occhio.", "Druckbares Großdruck-PDF fürs Labor.", "A big-print PDF to hang in your kitchen.", "PDF imprimible de letra grande para el obrador.")}</p>
      </div>

      {recipes === null ? (
        <div className="flex justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-[#8C4A27]" /></div>
      ) : recipes.length === 0 ? (
        <p className="text-center text-sm text-[#8C7362] py-8">{L("Nessuna ricetta ancora. Aggiungine una da 'Le Mie Ricette'.", "Noch keine Rezepte.", "No recipes yet. Add one in 'My Recipes'.", "Aún no hay recetas.")}</p>
      ) : (
        <div className="space-y-2.5" data-testid="cantiere-list">
          {recipes.map((r, i) => (
            <div key={r.id || i} data-testid={`cantiere-recipe-${i}`} className="flex items-center gap-3 rounded-2xl bg-[#FAF5EC] dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] p-3.5 shadow-sm">
              {r.image_url ? <img src={r.image_url} alt="" className="w-12 h-12 rounded-xl object-cover shrink-0" /> : <div className="w-12 h-12 rounded-xl bg-[#8C4A27]/12 flex items-center justify-center shrink-0"><FileText className="w-5 h-5 text-[#8C4A27]" /></div>}
              <p className="flex-1 min-w-0 font-display text-[15px] font-bold text-[#2C1E16] dark:text-[#e4eff8] leading-tight truncate">{field(r, "name") || "Ricetta"}</p>
              <button data-testid={`cantiere-pdf-${i}`} disabled={busy} onClick={() => generate(r)}
                className="flex items-center gap-1.5 bg-[#D97706] hover:bg-[#B45309] disabled:opacity-50 text-[#FFFDF9] text-[13px] font-semibold px-3.5 py-2 rounded-xl active:scale-95 transition-all shrink-0">
