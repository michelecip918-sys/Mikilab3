...
export default function GestioneForno() {
  ...
  return (
    <div className="pb-4">
      ...
      {showForm && (
        <div className="mt-5 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-4 space-y-3">
          <input
      ...
      <div className="space-y-3 mt-5">
        {profiles.map((p) => (
          <div key={p.id} data-testid={`oven-profile-${p.id}`} className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-5">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-display text-xl font-semibold text-[#2B303B] dark:text-[#e4eff8]">{p.name}</h3>
                <span data-testid={`oven-type-${p.id}`} className="inline-block mt-1 text-[10px] font-bold uppercase tracking-wide text-[#3E9C93] dark:text-[#8FB0C2] bg-[#3E9C93]/15 px-2 py-0.5 rounded-full border border-[#3E9C93]/30">
                  {p.oven_type === "ventilato" ? t("oven_type_fan") : t("oven_type_static")}
                </span>
              </div>
              <div className="flex gap-1.5">
                <button onClick={() => edit(p)} className="w-8 h-8 rounded-lg bg-[#e4eff8] dark:bg-[#1B2A38] flex items-center justify-center text-[#3E9C93]">✎</button>
                <button data-testid={`delete-oven-${p.id}`} onClick={() => remove(p.id)} className="w-8 h-8 rounded-lg bg-[#e4eff8] dark:bg-[#1B2A38] flex items-center justify-center text-[#3E9C93]">
...
function NumRow({ label, k, form, set }) {
  return (
    <div className="flex items-center gap-2 bg-[#e4eff8] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 px-3 py-2">
      ...
      <input
        ...
        className="w-16 text-right font-mono-data font-bold text-[#3E9C93] dark:text-[#8FB0C2] bg-transparent outline-none"
      />
    </div>
  );
}
...
function PhaseTimer({ label, temp, minutes, onDone }) {
  ...
  return (
    <div className="flex items-center gap-2 bg-[#e4eff8] dark:bg-[#1B2A38] rounded-2xl shadow-md border border-amber-900/40 px-3 py-2 border border-[#2A3B49] dark:border-[#2A3B49]">
      ...
      <span data-testid="timer-display" className="font-mono-data font-bold text-lg text-[#3E9C93] dark:text-[#8FB0C2] tabular-nums">{mm}:{ss}</span>
      <button data-testid="timer-toggle" onClick={start} className="w-8 h-8 rounded-lg bg-[#3E9C93] text-white flex items-center justify-center">
        {running ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
      </button>
      <button data-testid="timer-reset" onClick={() => { setRunning(false); setLeft(totalSec); }} className="w-8 h-8 rounded-lg bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] flex items-center justify-center text-[#7E8A93]">
