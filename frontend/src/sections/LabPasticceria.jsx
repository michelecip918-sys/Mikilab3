    ...
    <div className="pb-8" data-testid="lab-pasticceria">
        ...
        <div className={card} data-testid="pasticceria-lievitati">
          ...
          <div className="rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] p-3 space-y-2" data-testid="gl-schedule">
            {schedule.steps.map((t, i) => (
              <div key={i} className="flex items-center gap-2 text-[13px]"><span className="w-6 h-6 rounded-full bg-[#3E9C93] text-white text-[11px] font-bold flex items-center justify-center">{i + 1}</span><span className="text-[#3E9C93]">{L(`Rinfresco ${i + 1} · lievito raddoppiato in ~4h a 30°C`, `Refresh ${i + 1} · doubled in ~4h at 30°C`)}</span><span className="ml-auto font-mono-data font-bold text-[#3E9C93]">{t}</span></div>
            ))}
            <div className="flex items-center gap-2 text-[13px] pt-2 border-t border-[#2A3B49]"><span className="w-6 h-6 rounded-full bg-[#3E9C93] text-white text-[11px] font-bold flex items-center justify-center">★</span><span className="text-[#3E9C93] font-semibold">{L("Primo impasto pronto", "First dough ready")}</span><span className="ml-auto font-mono-data font-bold text-[#3E9C93]">{schedule.impasto}</span></div>
          </div>
          <p className="text-[12px] text-[#3E9C93] mt-2 leading-snug">{L("Tra un rinfresco e l'altro il lievito madre deve triplicare a 28-30°C. Il bagnetto in acqua (a 18°C con poco zucchero) prima dell'ultimo rinfresco riduce l'acidità.", "Between refreshes the sourdough should triple at 28-30°C. A water bath (18°C, a little sugar) before the last refresh lowers acidity.")}</p>
          <div className="mt-3 pt-3 border-t border-[#2A3B49]">
            ...
            <div data-testid="gl-bagnetto" className="mt-2 rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] p-3 text-[13px] text-[#3E9C93] font-semibold">
      ...
      {tab === "podpac" && (
        <div className={card} data-testid="pasticceria-podpac">
          <p className="text-[12.5px] text-[#3E9C93] mb-3">{L("Inserisci i grammi di ogni zucchero e il peso totale della miscela per bilanciare dolcezza (POD) e struttura al freddo (PAC) per vetrina a -12°C.", "Enter grams of each sugar and total mix weight to balance sweetness (POD) and freezing (PAC) for a -12°C display.")}</p>
          <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <div data-testid="pp-out" className="grid grid-cols-2 gap-2">
            <div className={`rounded-2xl shadow-md border border-amber-900/40 p-3 text-center border ${podpac.podOk ? "bg-[#DCFCE7] border-[#16A34A]" : "bg-[#ffffff] border-[#3E9C93]"}`}><p className="text-[11px] font-semibold text-[#3E9C93]">POD (/kg)</p><p className="font-display text-2xl font-bold text-[#3E9C93]">{num(podpac.podR)}</p><p className="text-[10px] text-[#3E9C93]">{L("ideale 180-280", "ideal 180-280")}</p></div>
            <div className={`rounded-2xl shadow-md border border-amber-900/40 p-3 text-center border ${podpac.pacOk ? "bg-[#DCFCE7] border-[#16A34A]" : "bg-[#ffffff] border-[#3E9C93]"}`}><p className="text-[11px] font-semibold text-[#3E9C93]">PAC (/kg)</p><p className="font-display text-2xl font-bold text-[#3E9C93]">{num(podpac.pacR)}</p><p className="text-[10px] text-[#3E9C93]">{L("ideale 250-300 (-12°C)", "ideal 250-300 (-12°C)")}</p></div>
          </div>
        </div>
      )}
      {tab === "creme" && (
        <div className={card} data-testid="pasticceria-creme">
          ...
          <div data-testid="cr-out" className="rounded-2xl shadow-md border border-amber-900/40 bg-[#ffffff] p-3 divide-y divide-[#2A3B49]">
            {creme.map(([k, v], i) => (
              <div key={i} className="flex justify-between py-1.5 text-[13px]"><span className="text-[#3E9C93]">{k}</span><span className="font-mono-data font-bold text-[#3E9C93]">{num(v)} g</span></div>
            ))}
          </div>
          <p className="text-[12px] text-[#3E9C93] mt-2 leading-snug">{L("Dosi indicative scalate sulla base. Cuoci la pasticcera a 82-85°C; raffredda rapidamente e conserva a +4°C.", "Indicative doses scaled to the base. Cook pastry cream to 82-85°C; cool fast and keep at +4°C.")}</p>
        </div>
      )}
