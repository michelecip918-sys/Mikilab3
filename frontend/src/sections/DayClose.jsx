...
export default function DayClose() {
  ...
  if (!user) {
    return (
      <div className="pb-40" data-testid="dayclose">
        <Header tri={tri} />
        <button data-testid="dayclose-login" onClick={() => setAuthOpen(true)} className="w-full flex items-center justify-center gap-2 bg-[#F26419] text-white font-semibold py-3 rounded-2xl">
  ...
  return (
    <div className="pb-40" data-testid="dayclose">
      ...
      <div data-testid="dayclose-modeswitch" className="grid grid-cols-2 gap-2 mb-4 bg-[#e4eff8] dark:bg-[#18202E] rounded-2xl p-1">
        <button data-testid="dayclose-mode-wizard" onClick={() => setMode("wizard")}
          className={`flex items-center justify-center gap-1.5 py-2.5 rounded-2xl shadow-md border border-amber-900/40 text-sm font-semibold transition-all ${mode === "wizard" ? "bg-white dark:bg-[#18202E] text-[#F26419] dark:text-[#8FB0C2] shadow-sm" : "text-[#7E8A93]"}`}>
        ...
        <button data-testid="dayclose-mode-storico" onClick={() => { setMode("storico"); loadClosures(); }}
          className={`flex items-center justify-center gap-1.5 py-2.5 rounded-2xl shadow-md border border-amber-900/40 text-sm font-semibold transition-all ${mode === "storico" ? "bg-white dark:bg-[#18202E] text-[#F26419] dark:text-[#8FB0C2] shadow-sm" : "text-[#7E8A93]"}`}>
      ...
      {mode === "storico" ? (
        <div data-testid="dayclose-storico" className="space-y-3">
          <div data-testid="consumption-chart" className="bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-2xl p-4">
            <p className="flex items-center gap-1.5 text-xs font-bold uppercase text-[#F26419] mb-3"><BarChart3 className="w-4 h-4" /> {tri("Consumi settimanali (farina e lieviti)", "Wochenverbrauch (Mehl & Hefen)")}</p>
            {hasConsumption ? (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={consumption} margin={{ top: 6, right: 10, left: 2, bottom: 0 }} barGap={2}>
                  ...
                  <Bar dataKey="flour" name={tri("Farina", "Mehl")} fill="#F26419" radius={[4, 4, 0, 0]} maxBarSize={26} />
                  <Bar dataKey="yeast" name={tri("Lievito", "Hefe")} fill="#F26419" radius={[4, 4, 0, 0]} maxBarSize={26} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
          ...
          {(() => {
            ...
            return list.map((c) => (
              <div key={c.id} data-testid={`storico-item-${c.id}`} className="bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-2xl p-3.5 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-bold text-[#2B303B] dark:text-[#e4eff8]">{c.date} · <span className="font-mono-data text-[#F26419] dark:text-[#8FB0C2]">{c.production_lot || tri("senza lotto", "ohne Charge")}</span></p>
                ...
                <button data-testid={`storico-pdf-${c.id}`} onClick={() => downloadPdf(c.id, c.production_lot)}
                  className="shrink-0 inline-flex items-center gap-1.5 bg-[#F26419] text-white text-xs font-semibold px-3 py-2 rounded-2xl shadow-md border border-amber-900/40 active:scale-95">
      ...
      <div data-testid="dayclose-stepper" className="flex items-center gap-1 mb-5">
        {[1, 2, 3].map((n) => (
          <div key={n} className="flex items-center flex-1">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${step >= n ? "bg-[#F26419] text-white" : "bg-[#e4eff8] dark:bg-[#18202E] text-[#7E8A93]"}`}>{n}</div>
            {n < 3 && <div className={`h-1 flex-1 rounded ${step > n ? "bg-[#F26419]" : "bg-[#e4eff8] dark:bg-[#18202E]"}`} />}
          </div>
        ))}
      </div>
      <p className="text-xs font-bold uppercase tracking-wide text-[#F26419] mb-3">
        ...
        <div data-testid="dayclose-step-1" className="space-y-4">
          <Card icon={<QrCode className="w-4 h-4" />} title={tri("Lotto di produzione", "Produktionscharge")}>
            <div className="flex gap-2">
              ...
              <button data-testid="dayclose-lot-regen" onClick={() => setLot(genLot())} className="shrink-0 px-3 rounded-2xl shadow-md border border-amber-900/40 bg-[#e4eff8] dark:bg-[#18202E] text-[#F26419] text-xs font-semibold">{tri("Nuovo", "Neu")}</button>
            </div>
            ...
            <div className="space-y-2">
              {produced.map((p, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <button onClick={() => setProduced((l) => l.filter((_, k) => k !== i))} className="text-[#F26419] p-1 shrink-0"><X className="w-4 h-4" /></button>
                </div>
              ))}
              <button data-testid="dayclose-prod-add" onClick={() => setProduced((l) => [...l, { recipe_id: "", name: "", qty: "", unit: "pz", gpp: "" }])} className="text-sm font-medium text-[#F26419] flex items-center gap-1"><Plus className="w-4 h-4" /> {tri("Aggiungi prodotto", "Produkt hinzufügen")}</button>
            </div>
          </Card>

          <Card icon={<Boxes className="w-4 h-4" />} title={tri("Magazzino materie prime", "Rohstofflager")}>
            {lowStock.length > 0 && (
              <div data-testid="dayclose-lowstock" className="flex flex-wrap items-center gap-2 bg-[#F26419]/10 border border-[#F26419]/30 rounded-2xl shadow-md border border-amber-900/40 px-3 py-2 mb-2 text-[#F26419] text-xs font-semibold">
                ...
                <button data-testid="dayclose-order-supplier" onClick={supplierOrder}
                  className="ml-auto inline-flex items-center gap-1.5 bg-[#F26419] text-white px-2.5 py-1 rounded-full active:scale-95">
            ...
            <div className="space-y-2">
              {inventory.map((it, i) => (
                <div key={it.id || i} className="rounded-2xl shadow-md border border-amber-900/40 border border-[#26324A] dark:border-[#26324A] p-2 space-y-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setInventory((l) => l.filter((_, k) => k !== i))} className="text-[#F26419] p-1 shrink-0"><Trash2 className="w-4 h-4" /></button>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="flex items-center gap-1 text-[11px] font-semibold text-[#F26419] shrink-0"><AlertTriangle className="w-3.5 h-3.5" /> {tri("Soglia avviso", "Warnschwelle")}</span>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button data-testid="inv-add" onClick={() => setInventory((l) => [...l, { name: "", category: "farina", qty: 0, unit: "kg", threshold: null }])} className="text-sm font-medium text-[#F26419] flex items-center gap-1"><Plus className="w-4 h-4" /> {tri("Aggiungi materia", "Rohstoff")}</button>
                <button data-testid="inv-save" onClick={saveInventory} className="text-sm font-semibold text-white bg-[#F26419] px-3 py-1.5 rounded-full flex items-center gap-1"><Save className="w-3.5 h-3.5" /> {tri("Salva magazzino", "Lager speichern")}</button>
              </div>
              <p className="text-[11px] text-[#7E8A93] flex items-start gap-1"><AlertTriangle className="w-3.5 h-3.5 text-[#F26419] shrink-0 mt-0.5" /> {tri("Imposta una soglia: ricevi un'email quando la materia scende sotto quel livello.", "Lege eine Schwelle fest: du erhältst eine E-Mail, wenn der Rohstoff darunter fällt.")}</p>
              <div className="pt-2 border-t border-[#26324A] dark:border-[#26324A]">
                <label className="flex items-center gap-1.5 text-[11px] font-bold uppercase text-[#F26419] mb-1"><Send className="w-3.5 h-3.5" /> {tri("Email fornitore (per ordine rapido)", "Lieferanten-E-Mail (Schnellbestellung)")}</label>
          ...
          <Card icon={<Package className="w-4 h-4" />} title={tri("Scarico materie prime (dal piano)", "Rohstoff-Abbuchung (aus Plan)")}>
            <button data-testid="dayclose-suggest" onClick={suggestConsume} className="w-full mb-2 text-sm font-semibold text-white bg-[#F26419] py-2 rounded-2xl shadow-md border border-amber-900/40 active:scale-98 flex items-center justify-center gap-1.5"><Sparkles className="w-4 h-4" /> {tri("Calcola scarico dal piano", "Abbuchung aus Plan berechnen")}</button>
            <div className="space-y-2">
              {consume.map((c, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <button onClick={() => setConsume((l) => l.filter((_, k) => k !== i))} className="text-[#F26419] p-1 shrink-0"><X className="w-4 h-4" /></button>
                </div>
              ))}
              <button data-testid="consume-add" onClick={() => setConsume((l) => [...l, { name: "", qty: "" }])} className="text-sm font-medium text-[#F26419] flex items-center gap-1"><Plus className="w-4 h-4" /> {tri("Aggiungi voce", "Eintrag")}</button>
            ...
            <div className="space-y-2">
              {temps.map((tp, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <button onClick={() => setTemps((l) => l.filter((_, k) => k !== i))} className="text-[#F26419] p-1 shrink-0"><X className="w-4 h-4" /></button>
                </div>
              ))}
              <button data-testid="temp-add" onClick={() => setTemps((l) => [...l, { name: "", temp_c: "" }])} className="text-sm font-medium text-[#F26419] flex items-center gap-1"><Plus className="w-4 h-4" /> {tri("Aggiungi punto", "Punkt hinzufügen")}</button>
            </div>
          </Card>
              ...
              {CLEAN_ITEMS.map(([key, it, de]) => {
                ...
                return (
                  <button key={key} data-testid={`clean-${key}`} onClick={() => setCleaning((c) => ({ ...c, [tri(it, de)]: !on }))}
                    className={`flex items-center gap-2 rounded-2xl shadow-md border border-amber-900/40 px-3 py-2.5 text-left border transition-all active:scale-97 ${on ? "bg-[#F26419] text-white border-[#F26419]" : "bg-white dark:bg-[#18202E] text-[#2B303B] dark:text-[#e4eff8] border-[#26324A] dark:border-[#26324A]"}`}>
      ...
      {step === 3 && (
        <div data-testid="dayclose-step-3" className="space-y-4">
          ...
          <Card icon={<Archive className="w-4 h-4" />} title={tri("Riepilogo", "Zusammenfassung")}>
            ...
            <div className="mt-3 flex items-start gap-2 bg-[#F26419]/12 border border-[#F26419]/30 rounded-2xl shadow-md border border-amber-900/40 p-2.5">
              <ShieldCheck className="w-4 h-4 text-[#F26419] dark:text-[#a9d2ec] shrink-0 mt-0.5" />
              <p className="text-[12px] text-[#F26419] dark:text-[#8FB0C2]">{tri("Alla conferma i dati vengono archiviati e sincronizzati automaticamente nel Registro HACCP.", "Bei Bestätigung werden die Daten archiviert und automatisch ins HACCP-Register übernommen.")}</p>
            </div>
          </Card>

          <button data-testid="dayclose-confirm" onClick={doClose} disabled={saving}
            className="w-full flex items-center justify-center gap-2 bg-[#F26419] hover:bg-[#E8A838] disabled:opacity-50 text-white font-bold py-3.5 rounded-2xl active:scale-98 shadow-md">
      ...
      <div className="flex items-center justify-between mt-5">
        <button data-testid="dayclose-prev" onClick={() => setStep((s) => Math.max(1, s - 1))} disabled={step === 1}
          className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-full bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] text-[#F26419] dark:text-[#a9d2ec] font-semibold text-sm disabled:opacity-40 active:scale-95">
          <ChevronLeft className="w-4 h-4" /> {tri("Indietro", "Zurück")}
        </button>
        {step < 3 && (
          <button data-testid="dayclose-next" onClick={() => setStep((s) => Math.min(3, s + 1))}
            className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-full bg-[#F26419] text-white font-semibold text-sm active:scale-95">
            ...
            <div className="mt-6 flex flex-col items-center gap-2.5" onClick={(e) => e.stopPropagation()}>
              {lastId && (
                <button data-testid="dayclose-download-pdf" onClick={() => downloadPdf(lastId, lot)}
                  className="inline-flex items-center gap-2 bg-[#F26419] text-white font-bold px-6 py-2.5 rounded-full active:scale-95">
...
function Header({ tri }) {
  return (
    <div className="flex items-center gap-3 mb-4">
      <div className="w-11 h-11 rounded-2xl bg-[#F26419] flex items-center justify-center"><CalendarCheck className="w-6 h-6 text-white" /></div>
      <div>
...
function Card({ icon, title, children }) {
  return (
    <div className="bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-2xl p-4">
      <p className="flex items-center gap-1.5 text-xs font-bold uppercase text-[#F26419] mb-2.5">{icon} {title}</p>
      {children}
    </div>
  );
}
...
function SignaturePad({ value, onChange, tri }) {
  ...
  return (
    <div>
      <div className="relative rounded-2xl shadow-md border border-amber-900/40 border-2 border-dashed border-[#c7d6e5] dark:border-[#26324A] bg-[#f8fbfe] dark:bg-[#18202E] overflow-hidden">
      ...
      <button type="button" data-testid="signature-clear" onClick={clear}
        className="mt-2 text-xs font-semibold text-[#F26419] active:scale-95">
