...
export const TOOL_CATS = [
  { key: "panificazione", Icon: Wheat, color: "#3E9C93", it: "Laboratorio Panificazione", de: "Backlabor", en: "Baking Lab", es: "Lab de Panificación" },
  { key: "pizzeria", Icon: Pizza, color: "#3E9C93", it: "Laboratorio Pizzeria", de: "Pizzeria-Labor", en: "Pizzeria Lab", es: "Lab de Pizzería" },
  ...
  { key: "manisporche", Icon: Hand, color: "#3E9C93", it: "Strumenti Mani in Pasta", de: "Werkzeuge (Hände im Teig)", en: "Hands-in-Dough Tools", es: "Herramientas Manos en Masa" },
...
export default function PianoProduzioneAI({ onOpenTool }) {
  ...
  const renderToolCard = ({ id, Icon, it, de, en }) => {
    ...
    return (
      <div key={id} data-testid={`capo-quicklink-${id}`} onClick={() => { if (!editTools) openToolTracked(id); }}
        ...
        className={`relative flex flex-col items-center justify-center gap-1.5 bg-white dark:bg-[#1B2A38] border rounded-2xl p-3 pt-4 text-center transition-all min-h-[70px] ${editTools ? "cursor-move border-dashed border-[#3E9C93]/50" : "cursor-pointer border-[#2A3B49] dark:border-[#2A3B49] active:scale-95 hover:border-[#3E9C93]/60"} ${isHidden ? "opacity-40" : ""} ${dragId === id ? "opacity-50 scale-95 ring-2 ring-[#3E9C93]" : ""}`}>
        {editTools ? (
          <>
            <div className="absolute top-1 left-1 flex gap-0.5">
              <button type="button" data-testid={`tool-hide-${id}`} aria-label="hide" onClick={(e) => { e.stopPropagation(); toggleHideTool(id); }}
                className="w-6 h-6 rounded-full bg-[#e4eff8] dark:bg-[#1B2A38] flex items-center justify-center text-[#3E9C93] active:scale-90">
                {isHidden ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
              </button>
              <button type="button" data-testid={`tool-pin-${id}`} aria-label="pin" onClick={(e) => { e.stopPropagation(); togglePinTool(id); }}
                className={`w-6 h-6 rounded-full flex items-center justify-center active:scale-90 ${(toolPrefs.pinned || []).includes(id) ? "bg-[#3E9C93] text-white" : "bg-[#e4eff8] dark:bg-[#1B2A38] text-[#3E9C93]"}`}>
          ...
          <>
            <button type="button" data-testid={`tool-fav-${id}`} aria-label="favorite"
              ...
              className={`absolute top-1 left-1 w-6 h-6 rounded-full flex items-center justify-center active:scale-90 ${(toolPrefs.pinned || []).includes(id) ? "bg-[#3E9C93] text-white" : "bg-[#3E9C93]/12 text-[#3E9C93]"}`}>
              <Star className={`w-3.5 h-3.5 ${(toolPrefs.pinned || []).includes(id) ? "fill-white" : ""}`} />
              {id === "generatore" && !toolUsage[id] && (
                <span data-testid={`tool-new-${id}`} className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-[#3E9C93] border border-white animate-pulse" title={tri3(lang, "Nuovo", "Neu", "New")} />
              )}
            </button>
            {guideFor(id, lang) && (
              <button type="button" data-testid={`tool-info-${id}`} aria-label="info"
                onClick={(e) => { e.stopPropagation(); openGuide(id); }}
                className="absolute top-1 right-1 w-6 h-6 rounded-full bg-[#3E9C93] flex items-center justify-center text-white shadow-sm ring-2 ring-white dark:ring-[#1B2A38] active:scale-90">
  ...
  const renderToolReorderRow = ({ id, Icon, it, de, en }) => {
    ...
    return (
      <Reorder.Item as="div" key={id} value={id} data-testid={`capo-quicklink-${id}`}
        ...
        className={`relative flex items-center gap-2 bg-white dark:bg-[#1B2A38] border border-dashed border-[#3E9C93]/50 rounded-2xl px-2.5 py-2.5 select-none touch-none cursor-grab active:cursor-grabbing ${isHidden ? "opacity-40" : ""}`}>
        <GripVertical className="w-4 h-4 text-[#9aa4ac] shrink-0" data-testid={`tool-grip-${id}`} />
        <Icon className="w-5 h-5 text-[#3E9C93] shrink-0" />
        ...
        <button type="button" data-testid={`tool-hide-${id}`} aria-label="hide" onPointerDown={stop} onClick={(e) => { stop(e); toggleHideTool(id); }}
          className="w-7 h-7 rounded-full bg-[#e4eff8] dark:bg-[#1B2A38] flex items-center justify-center text-[#3E9C93] active:scale-90 shrink-0">
          {isHidden ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
        </button>
        <button type="button" data-testid={`tool-pin-${id}`} aria-label="pin" onPointerDown={stop} onClick={(e) => { stop(e); togglePinTool(id); }}
          className={`w-7 h-7 rounded-full flex items-center justify-center active:scale-90 shrink-0 ${isPinned ? "bg-[#3E9C93] text-white" : "bg-[#e4eff8] dark:bg-[#1B2A38] text-[#3E9C93]"}`}>
  ...
  const generate = async () => {
    if (!canGenerate) {
      ...
      if (el) {
        ...
        el.classList.add("ring-2", "ring-[#3E9C93]", "rounded-2xl");
        setTimeout(() => el.classList.remove("ring-2", "ring-[#3E9C93]", "rounded-2xl"), 2200);
      }
      return;
    }
  ...
  return (
    <div className="pb-40">
      ...
      <div data-testid="capo-tabs" className="sticky top-[58px] z-30 mb-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1 p-1 rounded-2xl bg-[#14212C]/95 backdrop-blur border border-[#3E9C93]/40 shadow-lg">
        {[
          ...
          <button key={tb.id} data-testid={`capo-tab-${tb.id}`} onClick={() => goPlanTab(tb.id, tb.sel)}
            className={`py-2.5 rounded-2xl shadow-md border border-amber-900/40 text-[12.5px] font-bold leading-tight transition-all ${planTab === tb.id ? "bg-[#3E9C93] text-white shadow" : "text-[#AEB8BF] hover:text-white"}`}>
            {tb.label}
          </button>
        ))}
      </div>
                    ...
                    <div className="grid grid-cols-1 gap-1">
                      {items.map((tl) => (
                        <button key={tl.id} data-testid={`lab-menu-tool-${tl.id}`} onClick={() => { setMenuOpen(false); onOpenTool(tl.id); window.scrollTo(0, 0); }}
                          className="flex items-center gap-2.5 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] active:scale-98 hover:border-[#3E9C93]/60 transition-all">
      ...
      {planTab === "produci" && (mixers.length === 0 || cells.length === 0) && modules.celle && (
        <div data-testid="capo-setup-hint" className="mb-4 rounded-2xl bg-[#3E9C93]/12 border border-[#3E9C93]/35 p-3.5">
          <p className="text-sm text-[#3E9C93] dark:text-[#8FB0C2] leading-snug">
            {(() => {
              const miss = [];
          ...
          {onOpenTool && (
            <button data-testid="capo-setup-hint-btn" onClick={() => onOpenTool("capo")}
              className="mt-2 inline-flex items-center gap-1.5 text-xs font-semibold text-white bg-[#3E9C93] hover:bg-[#b3781f] px-3 py-1.5 rounded-lg active:scale-95 transition-all">
      ...
      {planTab === "produci" && onOpenTool && (
        <div data-testid="capo-pizza-pastry" className="mb-4 rounded-2xl border border-[#3E9C93]/40 bg-[#14212C] p-3.5">
          <p className="text-[11px] font-extrabold uppercase tracking-wider text-[#3E9C93] mb-1 flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button data-testid="capo-open-pizzeria" onClick={() => onOpenTool("labpizzeria")}
              className="flex items-center gap-2 bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] text-white rounded-2xl p-3 text-left active:scale-95 transition-all shadow-sm">
            ...
            <div className="min-w-0 flex-1">
              ...
              {onOpenTool && guideToolId(guideId) && (
                <button data-testid="tool-guide-open" onClick={() => { const tid = guideToolId(guideId); setGuideId(null); openToolTracked(tid); }}
                  className="mt-2 mr-2 inline-flex items-center gap-1 text-xs font-bold bg-white text-[#3E9C93] px-3 py-1.5 rounded-lg active:scale-95"><Wrench className="w-3.5 h-3.5" /> {tri3(lang, "Apri strumento", "Öffnen", "Open tool")}</button>
              )}
          ...
          <div data-testid="capo-quicklinks" className="mb-4">
            <div className="flex items-center justify-between">
              ...
              <button data-testid="lab-tour-replay" onClick={() => setTourForce((n) => n + 1)}
                className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#3E9C93] px-2.5 py-1 rounded-full border border-[#2A3B49] dark:border-[#2A3B49] bg-white dark:bg-[#1B2A38] active:scale-95 transition-all">
            ...
            <div className="h-px bg-[#2A3B49] dark:bg-[#2A3B49] my-4" />
          </div>
        )}
        <div data-testid="capo-advanced-title" className="mb-2 mt-1 flex items-center gap-2">
          <Settings2 className="w-4 h-4 text-[#3E9C93]" />
          <h3 className="font-display text-lg font-bold text-[#3E9C93]">{tri3(lang, "Impostazioni Avanzate IA", "Erweiterte KI-Einstellungen", "Advanced AI Settings", "Ajustes avanzados IA")}</h3>
        </div>
        <div data-testid="capo-modules-hint" className="mb-3 flex items-center gap-2 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/15 border border-[#3E9C93]/45 px-3 py-2.5">
          <SlidersHorizontal className="w-4 h-4 text-[#3E9C93] shrink-0" />
          <p className="text-[12px] font-bold text-[#3E9C93] dark:text-[#3E9C93] leading-snug">
            {tri3(lang,
          ...
          {MODULES.map(({ id, Icon, it, de, en }) => {
            ...
            return (
              <div key={id} data-testid={`capo-module-${id}`} aria-pressed={on} onClick={() => toggleMod(id)}
                ...
                <span className={`absolute top-1.5 left-1.5 text-[8.5px] font-extrabold px-1.5 py-0.5 rounded-full ${on ? "bg-white/25 text-white" : "bg-[#e4eff8] dark:bg-[#1B2A38] text-[#9aa4ac]"}`}>{on ? "ON" : "OFF"}</span>
                <button type="button" data-testid={`tool-info-${id}`} aria-label="info"
                  onClick={(e) => { e.stopPropagation(); openGuide(id); }}
                  className={`absolute top-1 right-1 w-6 h-6 rounded-full flex items-center justify-center active:scale-90 shadow-sm ring-2 ${on ? "bg-white text-[#3E9C93] ring-[#3E9C93]" : "bg-[#3E9C93] text-white ring-white dark:ring-[#1B2A38]"}`}>
                  <Info className="w-3.5 h-3.5" />
                </button>
                ...
                {onOpenTool && (MODULE_TOOL[id]) && (
                  <button type="button" data-testid={`capo-module-open-${id}`}
                    ...
                    className={`mt-0.5 inline-flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded-full active:scale-95 transition-all ${on ? "bg-white text-[#3E9C93]" : "bg-[#3E9C93]/12 text-[#3E9C93] border border-[#3E9C93]/30"}`}>
      ...
      <Section order={3} icon={<Wrench className="w-4 h-4" />} title={tri3(lang, "Apri anche altri strumenti", "Weitere Werkzeuge öffnen", "Open other tools")}>
        ...
        {onOpenTool && (
          <>
            <div className="mt-4 mb-2 h-px bg-[#2A3B49] dark:bg-[#2A3B49]" />
            {!editTools && (() => {
              ...
              return (
                <div data-testid="tools-discovery" className={`mb-3 rounded-2xl border p-3 ${done ? "bg-[#3E9C93]/10 border-[#3E9C93]/40" : "bg-[#3E9C93]/10 border-[#3E9C93]/40"}`}>
                  ...
                  <Reorder.Group as="div" axis="x" values={pinnedIds} onReorder={(ids) => savePrefs({ ...toolPrefs, pinned: ids })}
                    ...
                    {pinnedFavs.map(({ id, Icon, it, de, en }) => (
                      <Reorder.Item as="div" key={id} value={id} data-testid={`fav-tool-${id}`}
                        ...
                        className="relative shrink-0 w-[104px] flex flex-col items-center justify-center gap-1.5 bg-[#3E9C93]/12 border border-[#3E9C93]/50 rounded-2xl p-3 pt-6 min-h-[70px] cursor-grab active:cursor-grabbing select-none">
                        <button type="button" data-testid={`fav-remove-${id}`} aria-label="remove favorite"
                          ...
                          className="absolute top-1 right-1 w-6 h-6 rounded-full flex items-center justify-center bg-[#3E9C93] active:scale-90">
            ...
            <div className="flex items-center justify-between mb-0.5">
              <p className="font-display text-lg font-bold text-[#3E9C93] dark:text-[#a9d2ec]">
              ...
              <button data-testid="tools-edit-toggle" onClick={() => setEditTools((v) => !v)}
                className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full border active:scale-95 transition-all ${editTools ? "bg-[#3E9C93] text-white border-[#3E9C93]" : "bg-white dark:bg-[#1B2A38] text-[#3E9C93] border-[#2A3B49] dark:border-[#2A3B49]"}`}>
            ...
            {!editTools && (
              <div className="relative mb-2">
                ...
                <input data-testid="tools-search" value={toolQuery} onChange={(e) => setToolQuery(e.target.value)}
                  ...
                  className="w-full rounded-2xl shadow-md border border-amber-900/40 border border-[#2A3B49] dark:border-[#2A3B49] bg-white dark:bg-[#1B2A38] pl-9 pr-3 py-2 text-sm outline-none focus:border-[#3E9C93]" />
              </div>
            )}

            {!editTools && !toolQuery.trim() && suggestedTools.length > 0 && (
              <div data-testid="tools-suggested" className="mb-4">
                <p className="font-display text-lg font-bold text-[#3E9C93] mb-2">{tri3(lang, "Suggeriti per te", "Für dich empfohlen", "Suggested for you", "Sugeridos para ti")}</p>
      ...
      <Section order={2} highlight badge={tri3(lang, "Inizia qui", "Hier starten", "Start here")} icon={<Sparkles className="w-4 h-4" />} title={tri3(lang, "Compila per generare", "Zum Generieren ausfüllen", "Fill in to generate")}>
        ...
        <div data-testid="capo-plan-switch" className="flex items-center gap-1 p-1 rounded-2xl bg-[#e4eff8] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] mb-2">
          ...
          <button data-testid="capo-switch-today" onClick={() => setUseWeekly(false)}
            className={`flex-1 py-2 rounded-2xl shadow-md border border-amber-900/40 text-sm font-bold transition-all ${!useWeekly ? "bg-[#3E9C93] text-white shadow-sm" : "text-[#7E8A93]"}`}>{tri3(lang, "Ordine di oggi", "Heutige Bestellung", "Today's order", "Pedido de hoy")}</button>
        </div>
        {!useWeekly && (
          <p data-testid="capo-today-note" className="text-[11.5px] text-[#3E9C93] dark:text-[#e0b487] font-semibold mb-2 flex items-start gap-1.5 leading-snug">
            <span>☀️</span>
            <span>{tri3(lang,
        ...
        <div data-testid="capo-source-choice" className="grid grid-cols-2 gap-2 mb-3">
            <button data-testid="capo-source-weekly" onClick={() => setUseWeekly(true)}
              className={`rounded-2xl p-3 text-left border-2 transition-all active:scale-97 ${useWeekly ? "bg-[#3E9C93] text-white border-[#3E9C93]" : "bg-white dark:bg-[#1B2A38] text-[#3E9C93] dark:text-[#a9d2ec] border-[#2A3B49] dark:border-[#2A3B49]"}`}>
        ...
        <div data-testid="capo-products-heading" className={`mt-4 mb-2 flex items-start gap-2 ${useWeekly ? "hidden" : ""}`}>
          <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[#3E9C93] text-white text-[13px] font-extrabold shrink-0">1</span>
          <div>
            <p className="text-sm font-bold text-white leading-tight">{tri3(lang, "Cosa produci oggi?", "Was produzierst du heute?", "What are you making today?")} <span className="text-[#3E9C93]">*</span></p>
            ...
            <button type="button" data-testid="capo-fill-example" onClick={fillExample}
              className="mt-1.5 inline-flex items-center gap-1.5 text-[12px] font-bold text-[#3E9C93] border border-[#3E9C93]/50 rounded-full px-3 py-1 active:scale-95">
        ...
        <div className={`space-y-2 ${useWeekly ? "hidden" : ""}`} data-testid="capo-products">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-1">
            <button data-testid="capo-product-add" onClick={() => setProducts((l) => [...l, { recipe_id: "", name: "", qty: "", unit: "pezzi", gpp: "", day: "", start: false }])} className="text-sm font-medium text-[#3E9C93] flex items-center gap-1"><Plus className="w-4 h-4" /> {t("capo_add_product")}</button>
            ...
            {savedProducts.length > 0 && (
              <button data-testid="capo-restore-prev" onClick={restorePrevPlan} className="text-sm font-semibold text-[#3E9C93] dark:text-[#a9d2ec] bg-[#3E9C93]/12 border border-[#3E9C93]/30 px-3 py-1.5 rounded-full flex items-center gap-1 active:scale-95"><RotateCcw className="w-3.5 h-3.5" /> {tri3(lang, "Riparti dall'ultimo piano", "Vom letzten Plan starten", "Reuse last plan")}</button>
            )}
            <button data-testid="capo-suggest-frequent" onClick={suggestFromFrequent} className="text-sm font-semibold text-white bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] px-3 py-1.5 rounded-full flex items-center gap-1 active:scale-95"><Sparkles className="w-3.5 h-3.5" /> {tri3(lang, "Suggerisci dai più usati", "Aus meistgenutzten vorschlagen", "Suggest from most-used", "Sugerir de los más usados")}</button>
          </div>
          ...
          <p className="text-[11px] text-[#7E8A93] leading-snug mt-1.5 flex items-start gap-1">
            <Flag className="w-3.5 h-3.5 text-[#1B2A38] shrink-0 mt-0.5" />
          ...
          {!isAdmin && !recipes.some((r) => !r._own) && (
            <div data-testid="capo-mikilab-buy-hint" className="mt-2 rounded-2xl shadow-md border border-amber-900/40 border border-[#3E9C93]/40 bg-[#3E9C93]/10 p-3 flex items-start gap-2">
              <BookOpen className="w-4 h-4 text-[#3E9C93] shrink-0 mt-0.5" />
              <p className="text-xs text-[#5b4a2a] dark:text-[#3E9C93] leading-snug">
                {tri3(lang,
        ...
        {onOpenTool && (
          <div data-testid="capo-quick-tools" className="mb-3">
            <div className="flex items-center justify-between mb-1.5">
              <p className="text-[11px] font-bold uppercase tracking-wide text-[#3E9C93]">{tri3(lang, "Strumenti rapidi", "Schnellzugriff", "Quick tools", "Herramientas rápidas")}</p>
                  ...
                  {QUICK_CATALOG.map((q) => {
                    ...
                    return (
                      <button key={q.id} data-testid={`capo-quicktoggle-${q.id}`} onClick={() => toggleQuick(q.id)}
                        className={`flex items-center gap-2 py-2 px-2.5 rounded-2xl shadow-md border border-amber-900/40 border text-left active:scale-95 transition-all ${on ? "bg-[#3E9C93] text-white border-[#3E9C93]" : "bg-white dark:bg-[#1B2A38] text-[#2B303B] dark:text-[#e4eff8] border-[#2A3B49] dark:border-[#2A3B49]"}`}>
                        <q.Icon className={`w-4 h-4 shrink-0 ${on ? "text-white" : "text-[#3E9C93]"}`} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {quickTools.map((id) => QUICK_CATALOG.find((c) => c.id === id)).filter(Boolean).map((q) => (
                  <button key={q.id} data-testid={`capo-quicktool-${q.id}`} onClick={() => onOpenTool(q.id)}
                    className="flex flex-col items-center gap-1 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] active:scale-95 hover:border-[#3E9C93] transition-all">
                    <q.Icon className="w-4 h-4 text-[#3E9C93]" />
        ...
        <div data-testid="capo-extra-today" className="mb-3 rounded-2xl shadow-md border border-amber-900/40 border-2 border-[#3E9C93]/70 bg-gradient-to-br from-[#3E9C93]/15 to-[#3E9C93]/10 overflow-hidden shadow-sm ring-1 ring-[#3E9C93]/30">
          <button type="button" data-testid="capo-extra-toggle" onClick={() => { setExtraOpen((s) => !s); if (!extraOpen && extraToday.length === 0) setExtraToday([{ recipe_id: "", name: "", qty: "", unit: "pezzi" }]); }}
            ...
            <span className="w-8 h-8 rounded-lg bg-[#3E9C93] text-white flex items-center justify-center shrink-0 shadow"><Plus className="w-4 h-4" /></span>
            <span className="min-w-0">
              <span className="flex items-center gap-1.5 text-[13.5px] font-extrabold text-[#3E9C93] dark:text-[#3E9C93] leading-tight">
                {tri3(lang, "Ordine extra di oggi", "Extra-Bestellung heute", "Extra order for today")}
                <span className="text-[9px] font-extrabold text-white bg-[#3E9C93] px-1.5 py-0.5 rounded-full uppercase tracking-wide">{tri3(lang, "all'ultimo minuto", "last minute", "last minute")}</span>
              </span>
              <span className="block text-[10.5px] text-[#3E9C93]/80 dark:text-[#3E9C93]/80 leading-snug">{tri3(lang, "Ordini improvvisi? Aggiungili qui: si sommano al piano di oggi, senza modificare il Piano Settimanale.", "Spontane Bestellungen? Hier hinzufügen: wird addiert, ohne den Wochenplan zu ändern.", "Sudden orders? Add them here: added on top of today, without changing the Weekly Plan.")}</span>
            </span>
            {extraToday.filter((x) => x.recipe_id || x.name).length > 0 && (
              <span className="ml-auto text-[10px] font-extrabold text-white bg-[#3E9C93] px-2 py-0.5 rounded-full shrink-0">{extraToday.filter((x) => x.recipe_id || x.name).length}</span>
            )}
          </button>
          {extraOpen && (
                ...
                <div key={i} className="flex items-center gap-2">
                  ...
                  <div className="relative w-[92px] shrink-0">
                    <input data-testid={`capo-extra-qty-${i}`} type="number" value={p.qty} placeholder={tri3(lang, "Qtà", "Menge", "Qty")}
                      ...
                      className="w-full bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-lg p-2 pr-9 text-sm outline-none focus:border-[#3E9C93]" />
                    <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-[#7E8A93]">{p.unit === "kg" ? "kg" : t("capo_unit_pieces")}</span>
                  </div>
                  <button type="button" data-testid={`capo-extra-remove-${i}`} onClick={() => setExtraToday((l) => l.filter((_, k) => k !== i))} className="text-[#3E9C93] p-1 shrink-0"><X className="w-4 h-4" /></button>
                </div>
              ))}
              <button type="button" data-testid="capo-extra-add" onClick={() => setExtraToday((l) => [...l, { recipe_id: "", name: "", qty: "", unit: "pezzi" }])}
                className="text-sm font-medium text-[#3E9C93] flex items-center gap-1"><Plus className="w-4 h-4" /> {t("capo_add_product")}</button>
              <p className="text-[11px] text-[#3E9C93]/80 dark:text-[#3E9C93]/80 leading-snug">
        ...
        {useWeekly && weeklyItems.length === 0 && (
          <div data-testid="capo-weekly-empty" className="mb-3 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/12 border border-[#3E9C93]/40 px-3 py-2.5">
            <p className="text-[12px] text-[#3E9C93] dark:text-[#3E9C93] leading-snug mb-2">
              {tri3(lang,
            ...
            {onOpenTool && (
              <button data-testid="capo-weekly-create" onClick={() => onOpenTool("settimana")}
                className="w-full flex items-center justify-center gap-2 bg-[#3E9C93] text-white font-bold py-2 rounded-2xl shadow-md border border-amber-900/40 active:scale-98 transition-all text-sm">
        ...
        {useWeekly && weeklyItems.length > 0 && (
          <div data-testid="capo-weekly-note" className="mb-3 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/12 border border-[#3E9C93]/35 px-3 py-2.5 space-y-2.5">
            <div className="flex items-start gap-2">
              <CalendarDays className="w-4 h-4 text-[#3E9C93] dark:text-[#8FB0C2] shrink-0 mt-0.5" />
              <p className="text-[12px] text-[#3E9C93] dark:text-[#8FB0C2] leading-snug">
                {tri3(lang,
            ...
            <div data-testid="capo-weekly-start-box" className="rounded-2xl shadow-md border border-amber-900/40 bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] p-3 ring-2 ring-[#D4AF37]/70 shadow-md">
              ...
              <select data-testid="capo-weekly-start" value={weeklyStartId}
                ...
                className="w-full bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-lg p-2.5 text-sm outline-none focus:border-[#D4AF37]">
        ...
        {modules.clima && tempMsg && (
          <div data-testid="capo-temp-msg" className={`mt-2 text-sm rounded-2xl shadow-md border border-amber-900/40 px-3 py-2 border ${tempDelta && Math.abs(tempDelta) >= 1 ? "bg-[#3E9C93]/15 border-[#3E9C93]/40 text-[#3E9C93] dark:text-[#8FB0C2]" : "bg-[#3E9C93]/12 border-[#3E9C93]/30 text-[#3E9C93] dark:text-[#a9d2ec]"}`}>
        ...
        <div className="mt-3">
          ...
          <select data-testid="capo-plan-goal" value={planGoal} onChange={(e) => setPlanGoal(e.target.value)}
            className="mt-1 w-full bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 p-3 text-sm outline-none focus:border-[#3E9C93]">
        ...
        <div className="mt-3">
          ...
          <textarea data-testid="capo-notes" value={notes} onChange={(e) => setNotes(e.target.value)} rows={2}
            className="mt-1 w-full bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 p-3 text-sm outline-none focus:border-[#3E9C93] resize-none" />
        </div>
        ...
        {modules.foodcost && (() => {
          ...
          return (
            <div data-testid="capo-cost-summary" className="mt-4 rounded-2xl bg-[#3E9C93]/8 border border-[#3E9C93]/25 p-4">
              <div className="flex items-center gap-2 mb-2">
                <Euro className="w-4 h-4 text-[#3E9C93] dark:text-[#a9d2ec]" />
                <span className="text-[11px] font-bold uppercase tracking-wide text-[#3E9C93] dark:text-[#a9d2ec]">{tri3(lang, "Costi & Margine", "Kosten & Marge", "Costs & Margin")}</span>
              </div>
              <div className="space-y-1.5">
                {rows.map((x, i) => (
                  <div key={i} data-testid={`capo-cost-row-${i}`} className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-mono-data font-semibold text-[#3E9C93] dark:text-[#a9d2ec]">{x.rev != null ? eur(x.rev) : "—"}</span>
                  </div>
                ))}
              </div>
              <div className="mt-3 pt-3 border-t border-[#3E9C93]/20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 text-center">
                <div><p className="text-[10px] uppercase text-[#7E8A93]">{tri3(lang, "Costo", "Kosten", "Cost")}</p><p className="font-mono-data font-bold text-[#1B2A38]">{eur(totCost)}</p></div>
                <div><p className="text-[10px] uppercase text-[#7E8A93]">{tri3(lang, "Ricavo", "Umsatz", "Revenue")}</p><p className="font-mono-data font-bold text-[#3E9C93] dark:text-[#a9d2ec]">{eur(totRev)}</p></div>
                <div><p className="text-[10px] uppercase text-[#7E8A93]">{tri3(lang, "Margine", "Marge", "Margin")}</p><p className="font-mono-data font-bold text-[#3E9C93]">{eur(margin)}{marginPct != null ? ` · ${marginPct.toFixed(0)}%` : ""}</p></div>
              </div>
              {missing && (
          ...
          <>
            <div data-testid="capo-saved-banner" className="no-print mt-4 rounded-2xl bg-[#3E9C93]/12 border border-[#3E9C93]/35 p-3.5 flex items-center justify-between gap-3">
              ...
              <button data-testid="capo-new-plan" onClick={clearPlan}
                className="shrink-0 inline-flex items-center gap-1.5 text-xs font-semibold text-[#3E9C93] dark:text-[#e4eff8] bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] px-3 py-2 rounded-2xl shadow-md border border-amber-900/40 active:scale-95 transition-all">
                <RotateCcw className="w-3.5 h-3.5" /> {tri3(lang, "Nuovo piano", "Neuer Plan", "New plan")}
              </button>
            </div>
            <div data-testid="capo-phase-timers" className="no-print mt-2 rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-3">
              <p className="text-[11px] font-bold uppercase tracking-wide text-[#3E9C93] mb-2 flex items-center gap-1.5"><TimerIcon className="w-4 h-4" /> {tri3(lang, "Timer di fase (Smart Timer)", "Phasen-Timer (Smart Timer)", "Phase timers (Smart Timer)", "Temporizadores de fase")}</p>
            ...
            <button data-testid="capo-quick-archive" onClick={() => capoArchiveRef.current?.openSave()}
              className="no-print mt-2 w-full inline-flex items-center justify-center gap-2 text-sm font-semibold text-[#3E9C93] dark:text-[#8FB0C2] bg-[#e4eff8] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] px-4 py-2.5 rounded-2xl active:scale-98 transition-all">
            ...
            <div className="no-print mt-3">
              <label className="block text-[11px] font-bold uppercase tracking-wide text-[#3E9C93] mb-1.5">{tri3(lang, "Note del fornaio (finiscono nel PDF)", "Notizen des Bäckers (kommen ins PDF)", "Baker's notes (added to the PDF)", "Notas del panadero (van al PDF)")}</label>
            ...
            {!generating && (
              <button data-testid="capo-voice" onClick={() => setPlanHF(true)}
                className="no-print mt-2 w-full bg-[#3E9C93] hover:bg-[#a66f20] text-white font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all flex items-center justify-center gap-2">
            ...
            <button data-testid="capo-share" onClick={() => shareContent(mkTri(lang)("Piano di Produzione — MikiLab", "Produktionsplan — MikiLab", "Production plan — MikiLab", "Plan de producción — MikiLab"), plan, lang)}
              className="no-print mt-2 w-full bg-[#e4eff8] dark:bg-[#1B2A38] text-[#2B303B] dark:text-[#e4eff8] font-medium px-5 py-3 rounded-2xl border border-[#2A3B49] dark:border-[#2A3B49] active:scale-98 transition-all flex items-center justify-center gap-2">
            ...
            <div className="print-area mt-4 space-y-4">
              ...
              {bakerNote.trim() && (
                <div data-testid="capo-baker-note-print" className="print-table rounded-2xl shadow-md border border-amber-900/40 border border-[#3E9C93]/40 bg-[#3E9C93]/8 px-3 py-2.5">
                  <p className="text-[10px] font-bold uppercase tracking-wide text-[#3E9C93] mb-1">{tri3(lang, "Note del fornaio", "Notizen des Bäckers", "Baker's notes", "Notas del panadero")}</p>
                  <p className="text-sm text-[#2B303B] dark:text-[#e4eff8] whitespace-pre-wrap leading-relaxed">{bakerNote}</p>
                </div>
              )}
              <div data-testid="capo-plan" className="markdown-body bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-5 text-sm leading-relaxed text-[#2B303B] dark:text-[#e4eff8]">
                <p className="text-[10px] font-bold uppercase tracking-wide text-[#3E9C93] mb-2">{t("capo_plan_title")}</p>
                {planTruncated && (
                  <div data-testid="capo-plan-truncated" className="no-print mb-3 flex items-start gap-2 rounded-2xl shadow-md border border-amber-900/40 border border-[#3E9C93]/40 bg-[#3E9C93]/10 px-3 py-2.5">
                      ...
                      {showInf && (
                        <div data-testid="capo-infornate-editor" className="mt-4">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <button data-testid="capo-inf-add-row" onClick={infAddRow} className="no-print inline-flex items-center gap-1 text-[11px] font-semibold text-[#3E9C93] border border-[#3E9C93]/40 px-2 py-1 rounded-lg active:scale-95">
                              <Plus className="w-3.5 h-3.5" /> {tri3(lang, "Riga", "Zeile", "Row", "Fila")}
                            </button>
                          </div>
                          <div className="overflow-x-auto print-table rounded-2xl shadow-md border border-amber-900/40 border border-[#2A3B49] dark:border-[#2A3B49]">
                            <table className="w-full text-[12px] border-collapse">
                              <thead>
                                <tr className="bg-[#e4eff8] dark:bg-[#1B2A38]">
                                  {infEdit.headers.map((h, ci) => (
                                    <th key={ci} className="text-left font-bold text-[#3E9C93] dark:text-[#a9d2ec] px-2 py-1.5 whitespace-nowrap">{h}</th>
                                  ))}
                              ...
                              <tbody>
                                {infEdit.rows.map((r, ri) => (
                                  <tr key={ri} data-testid={`capo-inf-row-${ri}`} className="border-t border-[#2A3B49] dark:border-[#2A3B49]">
                                    {infEdit.headers.map((_, ci) => (
                                      <td key={ci} className="px-1 py-1 align-top">
                                        <input data-testid={`capo-inf-cell-${ri}-${ci}`} value={r[ci] ?? ""} onChange={(e) => infSetCell(ri, ci, e.target.value)}
                                          className="w-full min-w-[70px] bg-transparent px-1.5 py-1 rounded-md outline-none focus:bg-[#3E9C93]/8 border border-transparent focus:border-[#3E9C93]/40" />
                                      </td>
                                    ))}
                                    <td className="no-print px-1 py-1 align-middle">
                                      <button data-testid={`capo-inf-del-${ri}`} onClick={() => infDelRow(ri)} aria-label="delete row" className="text-[#3E9C93] p-1 active:scale-90"><X className="w-3.5 h-3.5" /></button>
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
              ...
              {usedRecipes.length > 0 && (
                <div data-testid="capo-recipes" className="space-y-3">
                  <p className="text-[10px] font-bold uppercase tracking-wide text-[#3E9C93]">{t("capo_recipes_title")}</p>
...
function RecipePrint({ r, lang }) {
  ...
  return (
    <div className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-4">
      ...
      <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-0.5">
        {ing.map(([label, g]) => (
          <div key={label} className="flex items-center justify-between text-xs">
            ...
            <span className="font-mono-data font-bold text-[#3E9C93] dark:text-[#8FB0C2]">{g} g</span>
          </div>
        ))}
      </div>
...
function Section({ icon, title, children, highlight, badge, order }) {
  const ACCENTS = { 1: "#3E9C93", 2: "#3E9C93", 3: "#2e8b6f", 4: "#3E9C93", 5: "#3E9C93" };
  const accent = ACCENTS[order] || "#3E9C93";
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} style={order ? { order } : undefined}
      className={`relative mb-4 rounded-2xl p-4 pl-5 overflow-hidden ${highlight
        ? "bg-white dark:bg-[#1B2A38] border-2 border-[#3E9C93] shadow-lg shadow-[#3E9C93]/20 ring-1 ring-[#3E9C93]/30"
        : "bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49]"}`}>
      ...
      <div className="flex items-center gap-2.5 mb-3">
        ...
        {badge && <span className="ml-auto text-[10px] font-extrabold uppercase tracking-wide text-white bg-[#3E9C93] px-2 py-0.5 rounded-full shadow">{badge}</span>}
      </div>
      {children}
    </motion.div>
  );
