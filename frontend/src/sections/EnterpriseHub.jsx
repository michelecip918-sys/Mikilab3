  ...
  return (
    <div data-testid="enterprise-hub">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#c94f00] to-[#121212] flex items-center justify-center"><Building2 className="w-6 h-6 text-white" /></div>
        <div>
      ...
      {stores.length > 0 && (
        <div className="relative mb-4" data-testid="enterprise-store-picker">
          <Store className="w-4 h-4 text-[#c94f00] absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
          <select value={current || ""} onChange={(e) => setCurrent(e.target.value)}
            className="w-full appearance-none bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] rounded-2xl pl-9 pr-9 py-3 font-semibold text-[#2B303B] dark:text-[#e4eff8] outline-none focus:border-[#c94f00]">
      ...
      {/* Sotto-schede */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 bg-[#e4eff8] dark:bg-[#181818] p-1.5 rounded-2xl mb-5 border border-[#2e2e2e] dark:border-[#2e2e2e]">
        {TABS.map(({ id, label, Icon }) => (
          <button key={id} data-testid={`enterprise-tab-${id}`} onClick={() => setSub(id)}
            className={`flex items-center justify-center gap-1.5 py-2.5 rounded-2xl shadow-md border border-amber-900/40 text-sm font-semibold transition-all ${sub === id ? "bg-[#c94f00] text-white shadow" : "text-[#3F4A54] dark:text-[#AEB8BF]"}`}>
