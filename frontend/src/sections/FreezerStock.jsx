...
export default function FreezerStock() {
  ...
  return (
    <div data-testid="freezer-stock" className="pb-4">
      <div className="rounded-2xl bg-gradient-to-br from-[#8C4A27] to-[#374f31] text-white p-5 mb-4">
      ...
      {low.length > 0 && (
        <div data-testid="freezer-low-warning" className="mb-3 rounded-xl bg-[#C0574D]/10 border border-[#C0574D]/30 p-3 flex items-start gap-2">
          ...
          <p className="text-sm text-[#6E371C] dark:text-[#8FB0C2]">
      ...
      <div className="space-y-2">
        {items.map((it, i) => (
          <div key={i} data-testid={`freezer-row-${i}`} className="flex items-center gap-2 bg-white dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] rounded-xl p-2.5">
            ...
            <div className="flex items-center gap-1">
              <input type="number" value={it.qty} onChange={(e) => set(i, { qty: e.target.value })} title={de ? "Bestand" : lang === "en" ? "Stock" : "Scorta"}
                className="w-16 text-center font-mono-data text-sm bg-[#e4eff8] dark:bg-[#2A323A] rounded-lg py-1.5 outline-none" />
              ...
              <input type="number" value={it.min_qty} onChange={(e) => set(i, { min_qty: e.target.value })} title={de ? "Min." : "Min."}
                className="w-16 text-center font-mono-data text-sm bg-[#B45309]/15 rounded-lg py-1.5 outline-none" />
            </div>
            <button onClick={() => remove(i)} className="w-8 h-8 rounded-lg bg-[#e4eff8] dark:bg-[#2A323A] flex items-center justify-center text-[#C0574D] shrink-0"><Trash2 className="w-4 h-4" /></button>
          </div>
        ))}
      </div>

      <button data-testid="freezer-add" onClick={add} className="mt-3 w-full flex items-center justify-center gap-2 border-2 border-dashed border-[#E6D8C3] dark:border-[#38424B] text-[#7E8A93] rounded-xl py-2.5 active:scale-98">
        <Plus className="w-4 h-4" /> {de ? "Produkt hinzufügen" : lang === "en" ? "Add product" : "Aggiungi prodotto"}
      </button>
      <button data-testid="freezer-save" onClick={save} disabled={saving} className="mt-3 w-full flex items-center justify-center gap-2 bg-[#8C4A27] disabled:opacity-50 text-white font-semibold rounded-2xl py-3 active:scale-98">
