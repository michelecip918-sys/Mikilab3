    ...
    <div className="space-y-4 mb-5">
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          <h2 className="font-display text-lg font-bold text-[#2B303B] dark:text-[#EAF0EC] leading-none">{t("tab_stoccarda")}</h2>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {REGIONS.map((r) => (
          <button key={r} data-testid={`news-filter-${r}`} onClick={() => setFilter(r)}
            className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${filter === r ? "bg-[#6B8E62] text-white border-[#6B8E62]" : "bg-white dark:bg-[#232A31] text-[#3F4A54] dark:text-[#AEB8BF] border-[#D7E1DB] dark:border-[#38424B]"}`}>
            {regionLabel(r)}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-2.5" data-testid="news-list">
          {shown.map((a, i) => (
            <a key={i} href={a.link || "#"} target="_blank" rel="noopener noreferrer"
              ...
              className="block bg-white dark:bg-[#232A31] border border-[#D7E1DB] dark:border-[#38424B] rounded-2xl p-4 hover:border-[#6B8E62]/50 transition-colors">
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-[#2B303B] dark:text-[#EAF0EC] leading-snug">{a.title}</p>
