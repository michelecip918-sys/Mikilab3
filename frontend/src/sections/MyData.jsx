...
export default function MyData({ onOpenTool }) {
  ...
  return (
    <div data-testid="my-data" className="pb-4">
      ...
      <div className="grid grid-cols-4 gap-1.5 bg-[#e4eff8] dark:bg-[#181818] p-1.5 rounded-2xl mb-4 border border-[#2b2b2b] dark:border-[#2e2e2e]">
      ...
      {tab === "piani" && (
        <div className="space-y-2" data-testid="mydata-piani">
          {plans.length === 0 ? <Empty text={tri("Nessun piano salvato. Salvane uno dal Piano Settimanale o dal Piano IA.", "Keine Pläne. Speichere einen im Wochenplan oder KI-Plan.", "No saved plans. Save one from the Weekly or AI plan.")} /> :
            plans.map((p) => (
              <div key={p.id} className="flex items-center gap-2 rounded-xl bg-white dark:bg-[#1e1e1e] border border-[#2b2b2b] dark:border-[#2e2e2e] p-3">
      ...
      {tab === "ricette" && (
        <div className="space-y-2" data-testid="mydata-ricette">
          {recipes.length === 0 ? <Empty text={tri("Nessuna ricetta personale. Aggiungile da Ricette → Le Mie Ricette.", "Keine eigenen Rezepte. Füge sie unter Rezepte → Meine Rezepte hinzu.", "No personal recipes. Add them under Recipes → My Recipes.")} /> :
            recipes.map((r) => (
              <div key={r.id} className="rounded-xl bg-white dark:bg-[#1e1e1e] border border-[#2b2b2b] dark:border-[#2e2e2e] p-3">
...
function ChatCard({ chat, fmt, tri, onDelete }) {
  ...
  return (
    <div data-testid={`mydata-chat-${chat.id}`} className="rounded-xl bg-white dark:bg-[#1e1e1e] border border-[#2b2b2b] dark:border-[#2e2e2e] overflow-hidden">
      <div className="flex items-center gap-2 p-3">
        ...
        <button data-testid={`mydata-chat-delete-${chat.id}`} onClick={onDelete} className="w-8 h-8 rounded-lg bg-[#e4eff8] dark:bg-[#242424] flex items-center justify-center text-[#ff6b00] shrink-0"><Trash2 className="w-4 h-4" /></button>
          ...
          {loading ? (
            ...
            msgs.map((m, i) => (
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`markdown-body max-w-[85%] rounded-2xl px-3 py-2 text-sm ${m.role === "user" ? "bg-[#ff6b00] text-white whitespace-pre-wrap" : "bg-[#e4eff8] dark:bg-[#242424] text-[#2B303B] dark:text-[#e4eff8]"}`}>
...
function Empty({ text }) {
  return <div className="rounded-xl bg-[#e4eff8] dark:bg-[#242424] border border-dashed border-[#2b2b2b] dark:border-[#2e2e2e] p-4 text-sm text-[#7E8A93] leading-snug">{text}</div>;
}
