...
export default function SourdoughTracker() {
  ...
  return (
    <div className="pb-24">
      ...
      {/* Finestra ideale */}
      <div className="rounded-2xl bg-[#F26419]/10 border border-[#F26419]/30 p-3 mb-4 text-sm text-[#F26419] dark:text-[#8FB0C2]">
      ...
      {/* Stato attuale */}
      {last && (
        <div data-testid="ph-status" className={`rounded-2xl p-4 mb-4 border ${atPeak ? "bg-[#F26419]/15 border-[#F26419]/40" : "bg-white dark:bg-[#18202E] border-[#26324A] dark:border-[#26324A]"}`}>
          <div className="flex items-center justify-between">
            ...
            {atPeak
              ? <div className="flex items-center gap-2 text-[#F26419] dark:text-[#a9d2ec] font-bold"><CheckCircle2 className="w-6 h-6" /> {tri("PRONTO", "BEREIT", "READY")}</div>
      ...
      <button data-testid="ph-add-btn" onClick={() => add(ph, temp)}
        className="w-full bg-[#F26419] hover:bg-[#E8A838] text-white font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all flex items-center justify-center gap-2 mb-2">
      ...
      <button data-testid="ph-bt-btn" onClick={connectBt} disabled={btBusy}
        className="w-full bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] text-[#2B303B] dark:text-[#e4eff8] font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all flex items-center justify-center gap-2 disabled:opacity-60">
        <Bluetooth className="w-5 h-5 text-[#F26419]" /> {tri("Collega pH-metro (Bluetooth)", "pH-Meter verbinden (Bluetooth)", "Connect pH meter (Bluetooth)")}
      </button>
      ...
      {log.length > 0 && (
        <div className="mt-5" data-testid="ph-log">
          <div className="flex items-center justify-between mb-2">
            <h2 className="font-display text-sm font-bold uppercase tracking-wide text-[#F26419]">{tri("Storico misure", "Messverlauf", "Reading history")}</h2>
            <button data-testid="ph-clear-btn" onClick={() => setLog([])} className="text-xs text-[#F26419] flex items-center gap-1"><Trash2 className="w-3.5 h-3.5" /> {tri("Svuota", "Leeren", "Clear")}</button>
          </div>
          <div className="space-y-1.5">
            {log.map((e) => {
              const ok = inWindow(Number(e.ph), Number(e.temp));
              return (
                <div key={e.id} className={`flex items-center justify-between rounded-2xl shadow-md border border-amber-900/40 px-3 py-2 border text-sm ${ok ? "bg-[#F26419]/12 border-[#F26419]/30" : "bg-white dark:bg-[#18202E] border-[#26324A] dark:border-[#26324A]"}`}>
