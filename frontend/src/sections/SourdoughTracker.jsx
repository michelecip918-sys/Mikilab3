...
export default function SourdoughTracker() {
  ...
  return (
    <div className="pb-24">
      ...
      {/* Finestra ideale */}
      <div className="rounded-2xl bg-[#c94f00]/10 border border-[#c94f00]/30 p-3 mb-4 text-sm text-[#c94f00] dark:text-[#8FB0C2]">
      ...
      {/* Stato attuale */}
      {last && (
        <div data-testid="ph-status" className={`rounded-2xl p-4 mb-4 border ${atPeak ? "bg-[#c94f00]/15 border-[#c94f00]/40" : "bg-white dark:bg-[#1e1e1e] border-[#2e2e2e] dark:border-[#2e2e2e]"}`}>
          <div className="flex items-center justify-between">
            ...
            {atPeak
              ? <div className="flex items-center gap-2 text-[#c94f00] dark:text-[#a9d2ec] font-bold"><CheckCircle2 className="w-6 h-6" /> {tri("PRONTO", "BEREIT", "READY")}</div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-2">
        <div>
          ...
          <input data-testid="ph-input" type="number" step="0.1" value={ph} onChange={(e) => setPh(e.target.value)} placeholder="4.2"
            className="mt-1 w-full font-mono-data bg-white dark:bg-[#181818] border border-[#2e2e2e] dark:border-[#2e2e2e] rounded-2xl shadow-md border border-amber-900/40 p-3 outline-none focus:border-[#c94f00]" />
        </div>
        <div>
      ...
      <button data-testid="ph-add-btn" onClick={() => add(ph, temp)}
        className="w-full bg-[#c94f00] hover:bg-[#d4a373] text-white font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all flex items-center justify-center gap-2 mb-2">
      ...
      <button data-testid="ph-bt-btn" onClick={connectBt} disabled={btBusy}
        className="w-full bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] text-[#2B303B] dark:text-[#e4eff8] font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all flex items-center justify-center gap-2 disabled:opacity-60">
        <Bluetooth className="w-5 h-5 text-[#c94f00]" /> {tri("Collega pH-metro (Bluetooth)", "pH-Meter verbinden (Bluetooth)", "Connect pH meter (Bluetooth)")}
      </button>
      ...
      {log.length > 0 && (
        <div className="mt-5" data-testid="ph-log">
          <div className="flex items-center justify-between mb-2">
            <h2 className="font-display text-sm font-bold uppercase tracking-wide text-[#c94f00]">{tri("Storico misure", "Messverlauf", "Reading history")}</h2>
            <button data-testid="ph-clear-btn" onClick={() => setLog([])} className="text-xs text-[#c94f00] flex items-center gap-1"><Trash2 className="w-3.5 h-3.5" /> {tri("Svuota", "Leeren", "Clear")}</button>
          </div>
          <div className="space-y-1.5">
            {log.map((e) => {
              const ok = inWindow(Number(e.ph), Number(e.temp));
              return (
                <div key={e.id} className={`flex items-center justify-between rounded-2xl shadow-md border border-amber-900/40 px-3 py-2 border text-sm ${ok ? "bg-[#c94f00]/12 border-[#c94f00]/30" : "bg-white dark:bg-[#1e1e1e] border-[#2e2e2e] dark:border-[#2e2e2e]"}`}>
