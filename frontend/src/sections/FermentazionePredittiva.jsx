  ...
  return (
    <div className="pb-40" data-testid="fermentazione-tool">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-[#F26419] flex items-center justify-center"><Activity className="w-6 h-6 text-white" /></div>
        <div>
      ...
      <button data-testid="ferment-geo" onClick={useGeo} disabled={loading}
        className="w-full flex items-center justify-center gap-2 bg-[#F26419] hover:bg-[#E8A838] disabled:opacity-50 text-white font-semibold py-3 rounded-2xl active:scale-98 transition-all mb-2">
      ...
      <div className="grid grid-cols-[1fr_auto] gap-2 mb-2">
        ...
        <button data-testid="ferment-city-search" onClick={searchCity} disabled={loading} className="px-4 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] font-semibold text-[#2B303B] dark:text-[#e4eff8] disabled:opacity-50">{tri("Cerca", "Suchen", "Search", "Buscar")}</button>
      </div>
      {err && <p data-testid="ferment-error" className="text-sm text-[#E4572E] mb-2">{err}</p>}
      {w && (
        <div data-testid="ferment-weather" className="flex items-center gap-2 text-sm text-[#F26419] dark:text-[#a9d2ec] bg-[#F26419]/12 border border-[#F26419]/30 rounded-2xl shadow-md border border-amber-900/40 px-3 py-2 mb-3">
      ...
      <div className="flex gap-2 mb-4">
        {[["double", tri("Raddoppio", "Verdopplung", "Double", "Duplica")], ["plus50", tri("+50% volume", "+50% Volumen", "+50% volume", "+50% volumen")]].map(([id, lbl]) => (
          <button key={id} data-testid={`ferment-level-${id}`} onClick={() => setLevel(id)}
            className={`flex-1 py-2 rounded-2xl shadow-md border border-amber-900/40 text-sm font-semibold border transition-all active:scale-98 ${level === id ? "bg-[#F26419] text-white border-[#F26419]" : "bg-white dark:bg-[#18202E] text-[#7E8A93] border-[#26324A] dark:border-[#26324A]"}`}>{lbl}</button>
        ))}
      </div>

      {/* Risultato + curva */}
      <div className="rounded-3xl bg-gradient-to-br from-[#F26419] to-[#E8A838] text-white p-5 shadow-lg mb-4">
      ...
      <div data-testid="ferment-note" className="rounded-2xl bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] p-4 mb-4 flex items-start gap-2">
      ...
      {run ? (
        <button data-testid="ferment-cancel" onClick={cancelRun}
          className="w-full flex items-center justify-center gap-2 bg-white dark:bg-[#18202E] border border-[#F26419]/40 text-[#F26419] font-bold py-4 rounded-2xl active:scale-98 transition-all">
        ...
        <button data-testid="ferment-remind" onClick={startReminder}
          className="w-full flex items-center justify-center gap-2 bg-[#F26419] hover:bg-[#a66f20] text-white font-bold py-4 rounded-2xl active:scale-98 transition-all">
