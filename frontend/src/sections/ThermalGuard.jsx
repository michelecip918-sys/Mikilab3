    ...
    <div data-testid="thermal-guard" className="space-y-6">
      ...
      <div className="bg-slate-900/80 border border-orange-500/30 rounded-2xl p-5 space-y-4">
        <div className="flex items-start gap-3">
          <span className="w-11 h-11 rounded-2xl bg-orange-500/15 border border-orange-500/40 flex items-center justify-center shrink-0"><Moon className="w-6 h-6 text-orange-400" /></span>
          <div>
            <h3 className="text-xl font-bold text-orange-400">Zero-Night Production</h3>
            <p className="text-slate-400 text-sm mt-0.5">Ciclo diurno/antinotte: calcolo a ritroso della lievitazione.</p>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div><label className="text-[10px] font-bold text-slate-400 uppercase">Chiusura serale</label><input data-testid="zn-close" type="time" value={close} onChange={(e) => setClose(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-center text-orange-300 font-mono font-bold mt-1" /></div>
          <div><label className="text-[10px] font-bold text-slate-400 uppercase">Fine produzione</label><input data-testid="zn-finish" type="time" value={finish} onChange={(e) => setFinish(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-center text-orange-300 font-mono font-bold mt-1" /></div>
          <div><label className="text-[10px] font-bold text-slate-400 uppercase">Ore lievitazione</label><input data-testid="zn-proof" type="number" min="0" value={proof} onChange={(e) => setProof(Math.max(0, Number(e.target.value) || 0))} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-center text-orange-300 font-mono font-bold mt-1" /></div>
        </div>
        <button data-testid="zn-from-recipes" onClick={calcFromRecipes} disabled={calcBusy} className="inline-flex items-center gap-1.5 px-3 py-2 bg-orange-600 hover:bg-orange-500 disabled:opacity-50 text-white text-xs font-bold rounded-lg active:scale-95 transition-all">
          <CalendarClock className="w-3.5 h-3.5" /> {calcBusy ? "Calcolo…" : "Calcola dalle ricette di oggi"}
        </button>
        <div className="p-4 bg-orange-950/20 border border-orange-800/40 rounded-xl text-sm space-y-1" data-testid="zn-report">
          <p className="text-slate-300">🌙 Chiudi il laboratorio alle <b className="text-orange-300">{close}</b>, produzione conclusa alle <b className="text-orange-300">{finish}</b>.</p>
