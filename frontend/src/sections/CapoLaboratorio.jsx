    ...
    <div className="pb-24">
      ...
      <Section icon={<Cog className="w-4 h-4" />} title={t("capo_equip_title")}>
        ...
        <div className="space-y-2" data-testid="capo-mixers">
          {mixers.map((m, i) => (
            <div key={i} className="bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-lg p-2 space-y-2">
              <div className="flex items-center gap-2">
                <input data-testid={`capo-mixer-name-${i}`} value={m.name || ""} placeholder={t("capo_mixer_name")}
                  ...
                  className="flex-1 min-w-0 bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-lg p-2 text-sm outline-none focus:border-[#F26419]" />
                <div className="relative w-24 shrink-0">
                  <input data-testid={`capo-mixer-cap-${i}`} type="number" value={m.capacity_kg ?? ""} placeholder={t("capo_mixer_cap")}
                    ...
                    className="w-full bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-lg p-2 pr-7 text-sm outline-none focus:border-[#F26419]" />
                  <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-[#7E8A93]">kg</span>
                </div>
                <button onClick={() => setMixers((l) => l.filter((_, k) => k !== i))} className="text-[#F26419] p-1 shrink-0"><X className="w-4 h-4" /></button>
              </div>
              <select data-testid={`capo-mixer-type-${i}`} value={m.type || "spirale"}
                onChange={(e) => setMixers((l) => l.map((x, k) => k === i ? { ...x, type: e.target.value } : x))}
                className="w-full bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-lg p-2 text-sm outline-none focus:border-[#F26419]">
          ...
          <button data-testid="capo-mixer-add" onClick={() => setMixers((l) => [...l, { name: "", capacity_kg: "", type: "spirale" }])} className="text-sm font-medium text-[#F26419]">+ {t("capo_mixer_add")}</button>
        </div>
        ...
        <div className="space-y-2" data-testid="capo-cells">
          {cells.map((c, i) => (
            <div key={i} className="bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-2xl shadow-md border border-amber-900/40 p-2.5 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <input data-testid={`capo-cell-name-${i}`} value={c.name || ""} placeholder={t("capo_cell_name")}
                  ...
                  className="flex-1 min-w-0 bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-lg p-2 text-sm outline-none focus:border-[#F26419]" />
                <button onClick={() => setCells((l) => l.filter((_, k) => k !== i))} className="text-[#F26419] p-1"><X className="w-4 h-4" /></button>
              </div>
              <div className="flex items-center gap-2">
                <select data-testid={`capo-cell-type-${i}`} value={c.type || "lievitazione"}
                  onChange={(e) => setCells((l) => l.map((x, k) => k === i ? { ...x, type: e.target.value } : x))}
                  className="flex-1 bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-lg p-2 text-sm outline-none focus:border-[#F26419]">
          ...
          <button data-testid="capo-cell-add" onClick={() => setCells((l) => [...l, { name: "", type: "lievitazione", temp_c: "", contents: "" }])} className="text-sm font-medium text-[#F26419]">+ {t("capo_cell_add")}</button>
        </div>
        ...
        <button data-testid="capo-save-config" onClick={saveConfig} className="mt-3 w-full bg-[#F26419] hover:bg-[#E8A838] text-white font-semibold px-4 py-2.5 rounded-2xl shadow-md border border-amber-900/40 active:scale-98 transition-all">
          {t("capo_save_config")}
        </button>
      </Section>
...
function LabCamera() {
  ...
  return (
    <div className="mt-4 rounded-2xl bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] p-4">
      <div className="flex items-center gap-2 mb-1">
        <Video className="w-4 h-4 text-[#F26419]" />
      ...
      {!open ? (
        <div className="grid grid-cols-2 gap-2">
          <button data-testid="capo-film-start" onClick={start} className="bg-[#F26419] hover:bg-[#E8A838] text-white rounded-2xl shadow-md border border-amber-900/40 px-4 py-3 text-sm font-semibold flex items-center justify-center gap-2">
            <Camera className="w-4 h-4" /> {mkTri(lang)("Filma ora", "Jetzt filmen", "Film now")}
          </button>
          <label data-testid="capo-film-attach" className="cursor-pointer bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-2xl shadow-md border border-amber-900/40 px-4 py-3 text-sm font-semibold flex items-center justify-center gap-2 text-[#2B303B] dark:text-[#e4eff8]">
            <ImagePlus className="w-4 h-4 text-[#F26419]" /> {mkTri(lang)("Allega", "Anhängen", "Attach")}
            <input type="file" accept="image/*" className="hidden" onChange={onAttach} />
          </label>
        </div>
      ) : (
        <div>
          <div className="rounded-2xl overflow-hidden border border-[#26324A] dark:border-[#26324A] bg-black">
            <video ref={videoRef} data-testid="capo-video" playsInline muted className="w-full max-h-72 object-cover" />
          </div>
          <div className="flex gap-2 mt-2">
            <button data-testid="capo-film-snap" onClick={snap} disabled={analyzing} className="flex-1 bg-[#F26419] hover:bg-[#E8A838] disabled:opacity-50 text-white font-semibold px-4 py-2.5 rounded-2xl shadow-md border border-amber-900/40 flex items-center justify-center gap-2">
              <Camera className="w-4 h-4" /> {analyzing ? t("capo_film_analyzing") : t("capo_film_snap")}
            </button>
            <button data-testid="capo-film-stop" onClick={stop} className="bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] px-4 py-2.5 rounded-2xl shadow-md border border-amber-900/40 font-medium text-[#2B303B] dark:text-[#e4eff8]">
              {t("capo_film_stop")}
            </button>
          </div>
        </div>
      ...
      {result && (
        <div data-testid="capo-film-result" className="markdown-body mt-4 bg-[#e4eff8] dark:bg-[#18202E] rounded-2xl p-4 text-sm leading-relaxed text-[#2B303B] dark:text-[#e4eff8]">
...
function Section({ icon, title, children }) {
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-4 rounded-2xl bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] p-4">
...
function CellIcon({ type }) {
  ...
  if (type === "frigo") return <Wind className="w-4 h-4 text-[#F26419] shrink-0" />;
  return <Thermometer className="w-4 h-4 text-[#F26419] shrink-0" />;
}
