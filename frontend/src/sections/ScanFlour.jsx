...
export default function ScanFlour() {
  ...
  const Field = ({ label, value, Icon }) => (
    <div className="flex items-start gap-2.5 py-2 border-b border-[#eef4f9] dark:border-[#2e2e2e] last:border-0">
      {Icon && <Icon className="w-4 h-4 text-[#c94f00] mt-0.5 shrink-0" />}
  ...
  return (
    <div className="pb-24" data-testid="scan-flour">
      <div className="relative rounded-3xl overflow-hidden mb-5 bg-gradient-to-br from-[#c94f00] to-[#6b4a1c] p-6 text-white">
      ...
      <div className="rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-6 text-center">
        ...
        {loading ? (
          <div data-testid="flour-loading" className="inline-flex items-center gap-2 bg-[#c94f00] text-white font-semibold px-5 py-3.5 rounded-2xl opacity-70">
      ...
      {result && (
        <div data-testid="flour-result" className="mt-4 rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-5">
          ...
          {result.ideal_use && (
            <div className="mt-3 rounded-2xl shadow-md border border-amber-900/40 bg-[#c94f00]/10 border border-[#c94f00]/25 p-3">
              <p className="text-[12px] font-bold text-[#c94f00] uppercase tracking-wide mb-0.5">{tri("Uso ideale", "Ideale Verwendung", "Ideal use", "Uso ideal")}</p>
      ...
      {pantry.length > 0 && (
        <div data-testid="flour-pantry" className="mt-4 rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-5">
          <div className="flex items-center gap-2 mb-3 text-[#c94f00]">
          ...
          <ul className="space-y-2">
            {pantry.map((f) => (
              <li key={f.id} data-testid={`flour-pantry-${f.id}`} className="flex items-center gap-3 rounded-2xl shadow-md border border-amber-900/40 bg-[#121212] dark:bg-[#181818] border border-[#2e2e2e] dark:border-[#2e2e2e] px-3.5 py-2.5">
                <Wheat className="w-5 h-5 text-[#c94f00] shrink-0" />
                ...
                <button data-testid={`flour-pantry-del-${f.id}`} onClick={() => removeFlour(f.id)} className="text-[#c94f00] p-1.5 shrink-0 active:scale-90"><Trash2 className="w-4 h-4" /></button>
              </li>
            ))}
          </ul>
        </div>
