  ...
  return (
    <div data-testid="timetable-lievitazione" className="pb-8">
      <div className="flex items-center gap-2 mb-1">
        <Clock className="w-6 h-6 text-[#F26419]" />
      ...
      <div className="rounded-2xl bg-[#18202E] border border-[#26324A] p-4 mb-4">
        <label className="text-[12px] font-bold uppercase tracking-wide text-[#F26419]">{tri("Orario di inizio", "Startzeit", "Start time")}</label>
        <input data-testid="timetable-start" type="time" value={start} onChange={(e) => setStart(e.target.value || nowHM())}
          className="ml-3 font-mono-data text-lg font-extrabold text-white bg-[#0B0E14] border border-[#26324A] rounded-2xl shadow-md border border-amber-900/40 px-3 py-1.5 outline-none focus:border-[#F26419]" />
      </div>

      <div className="space-y-2">
        {rows.map((r, i) => (
          <div key={i} data-testid={`timetable-row-${i}`} className="flex items-center gap-2 rounded-2xl bg-[#18202E] border border-[#26324A] p-3">
            <span className="w-7 h-7 rounded-full bg-[#F26419] text-white text-[13px] font-extrabold flex items-center justify-center shrink-0">{i + 1}</span>
            <div className="flex-1 min-w-0">
              <input data-testid={`timetable-name-${i}`} value={r.name} onChange={(e) => setPhase(i, { name: e.target.value })}
                className="w-full bg-transparent text-white font-semibold text-[15px] outline-none border-b border-transparent focus:border-[#F26419]/50" />
            ...
            <div className="flex items-center gap-1 shrink-0">
              <input data-testid={`timetable-min-${i}`} type="number" min="0" step="5" value={r.min} onChange={(e) => setPhase(i, { min: e.target.value })}
                className="w-16 text-right font-mono-data text-sm font-bold text-[#F26419] bg-[#0B0E14] border border-[#26324A] rounded-lg px-2 py-1.5 outline-none" />
              <span className="text-[11px] text-[#7E8A93]">min</span>
              <button data-testid={`timetable-del-${i}`} onClick={() => del(i)} className="ml-1 w-8 h-8 rounded-lg bg-[#0B0E14] border border-[#26324A] flex items-center justify-center text-[#7E8A93] active:scale-90"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center gap-2 mt-3">
        <button data-testid="timetable-add" onClick={addPhase} className="flex items-center gap-1.5 text-sm font-semibold text-[#F26419] border border-[#F26419]/40 rounded-full px-3.5 py-2 active:scale-95"><Plus className="w-4 h-4" /> {tri("Aggiungi fase", "Phase hinzufügen", "Add phase")}</button>
        <button data-testid="timetable-reset" onClick={reset} className="flex items-center gap-1.5 text-sm font-semibold text-[#AEB8BF] border border-[#26324A] rounded-full px-3.5 py-2 active:scale-95"><RotateCcw className="w-4 h-4" /> {tri("Reimposta", "Zurücksetzen", "Reset")}</button>
      </div>

      <div data-testid="timetable-summary" className="mt-4 rounded-2xl bg-gradient-to-br from-[#F26419] to-[#F26419] text-white p-4 flex items-center justify-between">
        <div>
