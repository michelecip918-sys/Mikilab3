  ...
  return (
    <div data-testid="timetable-lievitazione" className="pb-8">
      ...
      <div className="space-y-2">
        {rows.map((r, i) => (
          <div key={i} data-testid={`timetable-row-${i}`} className="flex items-center gap-2 rounded-2xl bg-[#1B2A38] border border-[#2A3B49] p-3">
            <span className="w-7 h-7 rounded-full bg-[#3E9C93] text-white text-[13px] font-extrabold flex items-center justify-center shrink-0">{i + 1}</span>
            <div className="flex-1 min-w-0">
              <input data-testid={`timetable-name-${i}`} value={r.name} onChange={(e) => setPhase(i, { name: e.target.value })}
                className="w-full bg-transparent text-white font-semibold text-[15px] outline-none border-b border-transparent focus:border-[#3E9C93]/50" />
      ...
      <div className="flex items-center gap-2 mt-3">
        <button data-testid="timetable-add" onClick={addPhase} className="flex items-center gap-1.5 text-sm font-semibold text-[#3E9C93] border border-[#3E9C93]/40 rounded-full px-3.5 py-2 active:scale-95"><Plus className="w-4 h-4" /> {tri("Aggiungi fase", "Phase hinzufügen", "Add phase")}</button>
        <button data-testid="timetable-reset" onClick={reset} className="flex items-center gap-1.5 text-sm font-semibold text-[#AEB8BF] border border-[#2A3B49] rounded-full px-3.5 py-2 active:scale-95"><RotateCcw className="w-4 h-4" /> {tri("Reimposta", "Zurücksetzen", "Reset")}</button>
      </div>

      <div data-testid="timetable-summary" className="mt-4 rounded-2xl bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] text-white p-4 flex items-center justify-between">
        <div>
