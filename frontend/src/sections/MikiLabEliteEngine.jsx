/* ============================================================================
   MIKILAB OS v10.3 - ULTIMATE 3D BAKERY ENTERPRISE EDITION (Miki & Sitor)
...
export default function MikiLabEliteEngine({ open, onClose, locked = false, lockedDept = '', isCapo = false, readOnly = false }) {
  ...
  const guidedProduction = () => {
    const steps = [
      tr("Passo uno: pesare 10 chilogrammi di farina T500.", "Schritt eins: 10 Kilogramm Mehl T500 abwiegen.", "Step one: weigh 10 kilograms of T500 flour.", "Paso uno: pesar 10 kilogramos de harina T500.", "Étape un : peser 10 kilogrammes de farine T500.", "گام یک: ۱۰ کیلوگرم آرد T500 وزن کن."),
      tr("Passo due: aggiungere 6 litri e mezzo di acqua a 22 gradi.", "Schritt zwei: 6,5 Liter Wasser bei 22 Grad hinzufügen.", "Step two: add six and a half litres of water at 22 degrees.", "Paso dos: añadir seis litros y medio de agua a 22 grados.", "Étape deux : ajouter six litres et demi d'eau à 22 degrés.", "گام دو: شش و نیم لیتر آب با دمای ۲۲ درجه اضافه کن."),
      tr("Passo tre: impastare per 8 minuti in prima velocità.", "Schritt drei: 8 Minuten in erster Geschwindigkeit kneten.", "Step three: knead for 8 minutes on first speed.", "Paso tres: amasar 8 minutos en primera velocidad.", "Étape trois : pétrir 8 minutes en première vitesse.", "گام سه: ۸ دقیقه با سرعت اول ورز بده."),
      tr("Passo quattro: puntata di 30 minuti, poi formatura.", "Schritt vier: 30 Minuten Teigruhe, dann formen.", "Step four: 30-minute bulk rest, then shaping.", "Paso cuatro: reposo de 30 minutos, luego formado.", "Étape quatre : pointage de 30 minutes, puis façonnage.", "گام چهار: ۳۰ دقیقه استراحت خمیر، سپس شکل‌دهی."),
    ];
  ...
  const startBake = () => {
    setIsBaking(true);
    speakVoice(tr("Conto alla rovescia forno avviato da Sitor", "Ofen-Countdown von Sitor gestartet", "Oven countdown started by Sitor", "Cuenta atrás del horno iniciada por Sitor", "Compte à rebours du four lancé par Sitor", "شمارش معکوس فر توسط Sitor آغاز شد"));
      ...
      <div style={{ padding: '16px', maxWidth: 760, margin: '0 auto', paddingBottom: '40px' }}>
        ...
        {!isLocked && eliteSection === 'diagnosi' && (
          <div data-testid="elite-panel-diagnosi" style={{ marginBottom: '16px' }}>
            ...
            <div data-testid="elite-guide-scienza" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '8px', marginBottom: '10px' }}>
              <button data-testid="elite-guide-0" onClick={() => speakVoice(tr("Guida Lievito Madre: mantenere il pH tra 4.1 e 4.3 per evitare un'eccessiva acidità acetica.", "Sauerteig-Leitfaden: den pH-Wert zwischen 4,1 und 4,3 halten, um übermäßige Essigsäure zu vermeiden.", "Sourdough guide: keep the pH between 4.1 and 4.3 to avoid excessive acetic acidity.", "Guía de masa madre: mantén el pH entre 4.1 y 4.3 para evitar una acidez acética excesiva.", "Guide du levain : maintenir le pH entre 4,1 et 4,3 pour éviter une acidité acétique excessive.", "راهنمای خمیرترش: pH را بین ۴٫۱ و ۴٫۳ نگه دار تا اسیدیته استیک زیاد نشود."))} style={{ backgroundColor: 'rgba(230,162,60,0.12)', border: '1px solid #E6A23C55', borderRadius: '10px', padding: '10px', textAlign: 'left', cursor: 'pointer' }}>
              ...
              <button data-testid="elite-guide-1" onClick={() => speakVoice(tr("Guida diagnostica: la mollica compatta indica solitamente una lievitazione incompleta o farina troppo debole.", "Diagnose-Leitfaden: eine kompakte Krume deutet meist auf unvollständige Gare oder zu schwaches Mehl hin.", "Diagnostic guide: a dense crumb usually indicates incomplete proofing or too weak a flour.", "Guía de diagnóstico: una miga compacta suele indicar una fermentación incompleta o harina demasiado débil.", "Guide de diagnostic : une mie compacte indique généralement une fermentation incomplète ou une farine trop faible.", "راهنمای عیب‌یابی: مغز متراکم معمولاً نشانه تخمیر ناقص یا آرد بسیار ضعیف است."))} style={{ backgroundColor: `${currentRoom.color}18`, border: `1px solid ${currentRoom.color}55`, borderRadius: '10px', padding: '10px', textAlign: 'left', cursor: 'pointer' }}>
        ...
        {!isLocked && eliteSection === 'ricette' && (
          <div data-testid="elite-panel-ricette" style={{ marginBottom: '16px' }}>
            {/* Calcolatore Sitor — temperatura acqua & idratazione */}
            <div data-testid="elite-bakemix" style={{ backgroundColor: 'rgba(0,0,0,0.5)', border: `1px solid ${currentRoom.color}`, borderRadius: '12px', padding: '12px', marginBottom: '14px' }}>
              ...
              <button data-testid="bakemix-speak" onClick={() => speakVoice(tr(`Aggiungere ${(calcFlour * (calcHydration / 100)).toFixed(1)} litri d'acqua a ${(calcTFinal * 3) - (Number(calcTAmb) + Number(calcTFlour) + 9)} gradi.`, `${(calcFlour * (calcHydration / 100)).toFixed(1)} Liter Wasser bei ${(calcTFinal * 3) - (Number(calcTAmb) + Number(calcTFlour) + 9)} Grad hinzufügen.`, `Add ${(calcFlour * (calcHydration / 100)).toFixed(1)} litres of water at ${(calcTFinal * 3) - (Number(calcTAmb) + Number(calcTFlour) + 9)} degrees.`, `Añadir ${(calcFlour * (calcHydration / 100)).toFixed(1)} litros de agua a ${(calcTFinal * 3) - (Number(calcTAmb) + Number(calcTFlour) + 9)} grados.`, `Ajouter ${(calcFlour * (calcHydration / 100)).toFixed(1)} litres d'eau à ${(calcTFinal * 3) - (Number(calcTAmb) + Number(calcTFlour) + 9)} degrés.`, `${(calcFlour * (calcHydration / 100)).toFixed(1)} لیتر آب با دمای ${(calcTFinal * 3) - (Number(calcTAmb) + Number(calcTFlour) + 9)} درجه اضافه کن.`))} style={{ width: '100%', backgroundColor: currentRoom.color, color: '#000', border: 'none', borderRadius: '8px', padding: '8px', fontSize: '0.72rem', fontWeight: 800, cursor: 'pointer' }}>
        ...
        <div style={{
          ...
          {activeTab === 'guida' ? (
            <div data-testid="elite-guida-content" style={{ fontSize: '0.85rem', lineHeight: '1.6', color: '#DDD' }}>
              <p>✨ <strong>MikiLab | 3D Lab Simulation v10.3</strong>>{tr(" ideato e sviluppato da Sitor & Miki.", " erdacht und entwickelt von Sitor & Miki.", " conceived and developed by Sitor & Miki.", " ideado y desarrollado por Sitor & Miki.", " conçu et développé par Sitor & Miki.", " طراحی و توسعه توسط محمد و میکی.")}</p>
              ...
              {dbRecipes.length > 0 && (
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <select data-testid="elite-db-recipe-select" value={selectedRecipeId}
                    onChange={(e) => { setSelectedRecipeId(e.target.value); const r = dbRecipes.find(x => x.id === e.target.value); if (r) speakVoice(tr(`Ricetta ${r.name}`, `Rezept ${r.name}`, `Recipe ${r.name}`, `Receta ${r.name}`, `Recette ${r.name}`, `دستور ${r.name}`)); }}
        ...
        <div style={{
          ...
          <span>© MikiLab | 3D Lab Simulation v10.3 - Sitor & Miki (Tutti i diritti riservati)</span>
        ...
        <div data-testid="elite-legal-modal" style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.85rem', lineHeight: '1.5', color: '#DDD', marginBottom: '20px' }}>
              ...
              {modalOpen === 'copyright' && (
                <p><strong>© 2026 MikiLab | 3D Lab Simulation - Sitor & Miki.</strong> Tutti i diritti di proprietà intellettuale relativi al codice sorgente, all'interfaccia 3D, ai flussi operativi, alle immagini e ai concetti di laboratorio sono riservati. È vietata la copia, la riproduzione o la distribuzione non autorizzata, anche parziale, dell'opera.</p>
              )}
              {modalOpen === 'impressum' && (
                <p><strong>MikiLab Industrial Systems</strong><br />
                  Sviluppato da Sitor & Miki<br />
