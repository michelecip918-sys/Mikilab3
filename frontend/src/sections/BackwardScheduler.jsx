  ...
  return (
    <div className="pb-4">
        ...
        {PHASES.map((p) => (
          <div key={p.id} className="flex items-center gap-2 bg-white dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] rounded-xl p-2.5">
            ...
            <input data-testid={`bs-dur-${p.id}`} type="number" min="0" value={dur[p.id]}
              ...
              className="w-20 text-right font-mono-data font-bold text-[#6E371C] dark:text-[#8FB0C2] bg-[#e4eff8] dark:bg-[#2A323A] border border-[#E6D8C3] dark:border-[#38424B] rounded-lg px-2 py-1.5 outline-none" />
          </div>
        ))}
      </div>
      ...
      <div data-testid="bs-schedule" className="rounded-2xl overflow-hidden border border-[#E6D8C3] dark:border-[#38424B] mb-4">
        ...
        <ol>
          {schedule.map((s, i) => (
            <li key={s.id} data-testid={`bs-row-${s.id}`} className="flex items-center gap-3 px-4 py-3 border-t border-[#E6D8C3] dark:border-[#38424B] bg-white dark:bg-[#232A31]">
              <span className="font-mono-data text-lg font-extrabold text-[#8C4A27] w-16 shrink-0">{fmt(s.start, lang)}</span>
