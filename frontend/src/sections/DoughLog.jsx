  ...
  return (
    <div className="pb-40" data-testid="doughlog">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-[#ff6b00] flex items-center justify-center"><Thermometer className="w-6 h-6 text-white" /></div>
        <div>
          ...
          <div className="bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] rounded-2xl p-4 mb-4" data-testid="doughlog-chart">
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={data} margin={{ top: 6, right: 8, left: -18, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="impasto" name={tri("Impasto", "Teig", "Dough")} stroke="#ff6b00" strokeWidth={2.5} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="target" name="Target" stroke="#ff6b00" strokeWidth={2} strokeDasharray="5 4" dot={false} connectNulls />
              </LineChart>
            </ResponsiveContainer>
          </div>
        );
              ...
              <div key={s.id} data-testid={`doughlog-item-${s.id}`} className="flex items-center justify-between bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] rounded-xl px-3 py-2">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-[#2B303B] dark:text-[#e4eff8] flex items-center gap-1.5">
                    {s.recipe_name}
                    {s.source === "pesata" && <span data-testid={`doughlog-badge-${s.id}`} className="inline-flex items-center gap-0.5 bg-[#ff6b00]/15 text-[#ff6b00] text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0"><Scale className="w-2.5 h-2.5" /> {tri("da Pesata", "aus Wiegen", "from Weighing")}</span>}
                  </p>
                ...
                <button data-testid={`doughlog-remove-${s.id}`} onClick={() => remove(s.id)} className="text-[#7E8A93] hover:text-[#ff6b00] shrink-0"><Trash2 className="w-4 h-4" /></button>
              </div>
            ))}
          </div>
        </div>
