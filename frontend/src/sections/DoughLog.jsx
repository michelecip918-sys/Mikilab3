...
export default function DoughLog() {
  ...
  const VERDICT = {
    ...
    too_cold: { color: "#3E9C93", Icon: Snowflake, label: tri("Troppo freddo", "Zu kalt", "Too cold") },
  ...
  return (
    <div className="pb-40" data-testid="doughlog">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-2xl bg-[#3E9C93] flex items-center justify-center"><Thermometer className="w-6 h-6 text-white" /></div>
        <div>
      ...
      {!user && (
        <button data-testid="doughlog-login" onClick={() => setAuthOpen(true)} className="w-full flex items-center justify-center gap-2 bg-[#3E9C93] text-white font-semibold py-3 rounded-2xl mb-4">
      ...
      <div className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-4 mb-4 space-y-3">
        <p className="text-xs font-bold uppercase text-[#3E9C93]">{tri("Nuova sessione impasto", "Neue Teig-Sitzung", "New dough session")}</p>
        ...
        return (
          <div className="rounded-2xl p-4 mb-4 border" style={{ background: v.color + "1A", borderColor: v.color + "55" }} data-testid="doughlog-dayafter">
            ...
            {a.suggested_water_c !== null && (
              <div className="flex items-center gap-2 mt-2 bg-[#3E9C93]/10 border border-[#3E9C93]/30 rounded-2xl shadow-md border border-amber-900/40 px-3 py-2">
                <Droplet className="w-4 h-4 text-[#3E9C93]" />
                <p className="text-sm text-[#2B303B] dark:text-[#e4eff8]">{tri("Oggi usa acqua a", "Heute Wasser mit", "Today use water at")} <b data-testid="doughlog-suggest-water" className="font-mono-data text-[#3E9C93]">{a.suggested_water_c}°C</b></p>
              </div>
            )}
            <button data-testid="doughlog-ai" onClick={getAdvice} disabled={loadingAdvice} className="mt-3 w-full flex items-center justify-center gap-2 bg-[#3E9C93] text-white font-semibold py-2.5 rounded-2xl shadow-md border border-amber-900/40 disabled:opacity-50 active:scale-98"><Sparkles className="w-4 h-4" /> {loadingAdvice ? tri("Chiedo al Maestro…", "Frage den Meister…", "Asking the Master…") : tri("Consiglio IA del Maestro", "KI-Rat des Meisters", "Master's AI advice")}</button>
            {advice && <p data-testid="doughlog-advice" className="text-sm text-[#2B303B] dark:text-[#e4eff8] mt-3 whitespace-pre-line bg-white/60 dark:bg-[#1B2A38]/60 rounded-2xl shadow-md border border-amber-900/40 p-3">{advice}</p>}
          </div>
        );
      })()}
      ...
      {(() => {
        ...
        return (
          <div className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-4 mb-4" data-testid="doughlog-chart">
            <p className="text-xs font-bold uppercase text-[#3E9C93] mb-2 flex items-center gap-1"><TrendingUp className="w-3.5 h-3.5" /> {tri("Andamento temperatura impasto", "Verlauf Teigtemperatur", "Dough temperature trend")}</p>
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={data} margin={{ top: 6, right: 8, left: -18, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#2A3B49" strokeOpacity={0.4} />
                ...
                <Line type="monotone" dataKey="impasto" name={tri("Impasto", "Teig", "Dough")} stroke="#3E9C93" strokeWidth={2.5} dot={{ r: 3 }} />
                <Line type="monotone" dataKey="target" name="Target" stroke="#3E9C93" strokeWidth={2} strokeDasharray="5 4" dot={false} connectNulls />
              </LineChart>
            </ResponsiveContainer>
          </div>
        );
      ...
      {sessions.length > 0 && (
        <div data-testid="doughlog-history">
          ...
          <div className="space-y-2">
            {sessions.map((s) => (
              <div key={s.id} data-testid={`doughlog-item-${s.id}`} className="flex items-center justify-between bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 px-3 py-2">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-[#2B303B] dark:text-[#e4eff8] flex items-center gap-1.5">
                    {s.recipe_name}
                    {s.source === "pesata" && <span data-testid={`doughlog-badge-${s.id}`} className="inline-flex items-center gap-0.5 bg-[#3E9C93]/15 text-[#3E9C93] text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0"><Scale className="w-2.5 h-2.5" /> {tri("da Pesata", "aus Wiegen", "from Weighing")}</span>}
                  </p>
                ...
                <button data-testid={`doughlog-remove-${s.id}`} onClick={() => remove(s.id)} className="text-[#7E8A93] hover:text-[#3E9C93] shrink-0"><Trash2 className="w-4 h-4" /></button>
              </div>
            ))}
          </div>
        </div>
