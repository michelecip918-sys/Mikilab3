...
export default function SosImpastoGuida({ onBack, onOpenTool }) {
  ...
  return (
    <div className="pb-8" data-testid="sos-impasto">
      {onBack && <button data-testid="sos-back" onClick={onBack} className="flex items-center gap-1 text-[#8C4A27] font-medium mb-4"><ChevronRight className="w-5 h-5 rotate-180" /> {L("Indietro", "Zurück", "Back", "Atrás")}</button>}
      <div className="relative overflow-hidden rounded-3xl p-6 text-[#FFFDF9] shadow-xl mb-5" style={{ background: "linear-gradient(135deg,#8C4A27,#6E371C 60%,#4A3222)" }}>
        ...
        <p className="text-[#FFFDF9]/85 text-sm mt-2 leading-snug">{L("Scegli il problema che vedi: ti spiego la causa e come rimediare.", "Wähle dein Problem: Ursache und Lösung.", "Pick the problem you see: cause and fix.", "Elige el problema: causa y solución.")}</p>
      </div>
      ...
      <div className="space-y-2.5" data-testid="sos-list">
        {DEFECTS.map((d, i) => {
          const isOpen = open === i;
          return (
            <div key={i} data-testid={`sos-item-${i}`} className="rounded-2xl bg-[#FAF5EC] dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] overflow-hidden shadow-sm">
              <button data-testid={`sos-toggle-${i}`} onClick={() => setOpen(isOpen ? null : i)} className="w-full flex items-center gap-3 p-4 text-left">
                ...
                <p className="flex-1 font-display text-[15px] font-bold text-[#2C1E16] dark:text-[#e4eff8] leading-tight">{d.it}</p>
              ...
              {isOpen && (
                <div className="px-4 pb-4 space-y-2 text-[13.5px] leading-relaxed">
                  <p className="text-[#6B5546] dark:text-[#AEB8BF]"><span className="font-bold text-[#B45309]">{L("Causa:", "Ursache:", "Cause:", "Causa:")} </span>{d.cause}</p>
                  <p className="text-[#6B5546] dark:text-[#AEB8BF]"><span className="font-bold text-[#2e8b6f]">{L("Rimedio:", "Lösung:", "Fix:", "Solución:")} </span>{d.fix}</p>
                </div>
              )}
            </div>
          );
      ...
      {onOpenTool && (
        <button data-testid="sos-open-ai" onClick={() => onOpenTool("diagnosi")}
          className="mt-5 w-full flex items-center justify-center gap-2 rounded-2xl bg-[#D97706] hover:bg-[#B45309] text-[#FFFDF9] font-semibold px-5 py-3.5 active:scale-98 transition-all">
