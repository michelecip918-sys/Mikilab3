...
function HomePlanner() {
  ...
  return (
    <div data-testid="home-planner" className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-5">
        ...
        {products.map((p, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <input data-testid={`home-product-qty-${i}`} type="number" value={p.qty} placeholder={t("capo_qty")}
              ...
              className="w-16 shrink-0 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 py-2.5 px-2 text-sm font-medium text-center outline-none focus:border-[#3E9C93] focus:ring-2 focus:ring-[#3E9C93]/30 transition-all" />
            <div className="relative w-24 shrink-0">
              <select data-testid={`home-product-day-${i}`} value={p.day || ""}
                onChange={(e) => setProducts((l) => l.map((x, k) => k === i ? { ...x, day: e.target.value } : x))}
                className="w-full appearance-none bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 py-2.5 pl-3 pr-8 text-sm font-medium text-[#2B303B] dark:text-white outline-none focus:border-[#3E9C93] focus:ring-2 focus:ring-[#3E9C93]/30 transition-all cursor-pointer">
                {HOME_DAYS.map((d) => <option key={d} value={d}>{d === "" ? t("capo_day_any") : t(`day_${d}`)}</option>)}
              </select>
              <ChevronDown className="w-4 h-4 text-[#3E9C93] absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none" />
            </div>
            {products.length > 1 && <button onClick={() => setProducts((l) => l.filter((_, k) => k !== i))} className="text-[#3E9C93] p-1 shrink-0"><X className="w-4 h-4" /></button>}
          </div>
        ))}
        <div className="flex flex-wrap items-center gap-3 mt-1">
          <button data-testid="home-product-add" onClick={() => setProducts((l) => [...l, { recipe_id: "", qty: "2", gpp: "500", day: "" }])} className="text-sm font-medium text-[#3E9C93] flex items-center gap-1"><Plus className="w-4 h-4" /> {t("capo_add_product")}</button>
      ...
      {plan && (
        <>
          ...
          <div className="print-area mt-4 space-y-4">
            <div data-testid="home-plan" className="markdown-body bg-[#e4eff8] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-5 text-sm leading-relaxed text-[#2B303B] dark:text-[#e4eff8]">
              <p className="text-[10px] font-bold uppercase tracking-wide text-[#3E9C93] mb-2">{t("home_plan_result")}</p>
...
function BakerQuiz() {
  ...
  const record = best > 0 && (
    <p data-testid="quiz-best" className="text-xs font-semibold text-[#3E9C93] flex items-center justify-center gap-1 mb-2">
  ...
  if (done) {
    ...
    return (
      <div data-testid="quiz-result" className="text-center bg-[#3E9C93]/12 border border-[#3E9C93]/30 rounded-2xl p-6">
        <Trophy className="w-10 h-10 text-[#3E9C93] mx-auto mb-2" />
        ...
        {record}
        <button data-testid="quiz-restart-btn" onClick={start} className="inline-flex items-center gap-2 bg-[#3E9C93] text-white font-semibold px-5 py-2.5 rounded-2xl shadow-md border border-amber-900/40">
  ...
  return (
    <div data-testid="quiz-panel" className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-5">
      <p className="text-[10px] font-bold uppercase tracking-wide text-[#3E9C93] mb-1">{t("quiz_question")} {idx + 1} / {questions.length}</p>
      ...
      <div className="space-y-2">
        {cur.options.map((opt, i) => {
          ...
          let cls = "bg-[#e4eff8] dark:bg-[#1B2A38] border-[#2A3B49] dark:border-[#2A3B49]";
          if (picked != null && isCorrect) cls = "bg-[#3E9C93]/20 border-[#3E9C93]";
          else if (picked != null && chosen && !isCorrect) cls = "bg-[#3E9C93]/15 border-[#3E9C93]";
          return (
            <button key={i} data-testid={`quiz-option-${i}`} onClick={() => choose(i)} disabled={picked != null}
              ...
              {picked != null && isCorrect && <CheckCircle2 className="w-4 h-4 text-[#3E9C93] shrink-0" />}
              {picked != null && chosen && !isCorrect && <XCircle className="w-4 h-4 text-[#3E9C93] shrink-0" />}
            </button>
          );
        })}
      </div>
      {picked != null && (
        <div className="mt-3 flex items-center justify-between gap-2">
          ...
          <button data-testid="quiz-next-btn" onClick={next} className="shrink-0 bg-[#3E9C93] text-white font-semibold px-4 py-2 rounded-2xl shadow-md border border-amber-900/40">
            {t("quiz_next")}
          </button>
        </div>
      )}
...
export default function Beginners({ onNavigate }) {
  ...
  if (askMaster) return (
    <div className="pb-4">
      <button data-testid="impara-askmaster-back" onClick={() => setAskMaster(false)} className="flex items-center gap-1 text-[#3E9C93] font-medium mb-4">
  ...
  return (
    <div data-testid="beginners-page" className="space-y-4 pb-4">
      ...
      <div data-testid="impara-quick-access" className="flex gap-2.5 overflow-x-auto pb-1 -mx-1 px-1 snap-x snap-mandatory">
        {[
          ...
          <button key={id} data-testid={`impara-quick-${id}`} onClick={act}
            className="snap-start shrink-0 inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-[#1B2A38] border border-[#3E9C93]/40 text-white active:scale-95 hover:border-[#3E9C93] transition-all">
            <Icon className="w-4.5 h-4.5 text-[#3E9C93]" />
      ...
      <div className="rounded-2xl p-5 bg-[#3E9C93]/12 border border-[#3E9C93]/30">
        <div className="flex items-center gap-2 mb-2">
          <Sprout className="w-5 h-5 text-[#3E9C93] dark:text-[#a9d2ec]" />
        ...
        <button data-testid="impara-tour-replay" onClick={() => setTourForce((n) => n + 1)}
          className="mt-2 inline-flex items-center gap-1.5 text-xs font-semibold text-[#3E9C93] dark:text-[#a9d2ec] bg-white dark:bg-[#1B2A38] border border-[#3E9C93]/30 px-3 py-1.5 rounded-lg active:scale-95 transition-all">
        ...
        {(() => {
          ...
          return (
            <div data-testid="beginners-level" className="mt-3">
              ...
              {!lvl.isMax && (
                <div className="mt-1.5 h-2 w-full max-w-[240px] rounded-full bg-[#2A3B49] dark:bg-[#2A3B49] overflow-hidden">
      ...
      <div data-testid="beginner-path" className="rounded-2xl p-5 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49]">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-xs font-bold text-[#3E9C93]">{doneCount}/{PATH.length}</span>
        </div>
        <div className="h-2 rounded-full bg-[#e4eff8] dark:bg-[#1B2A38] overflow-hidden mb-3">
          <div className="h-full bg-[#3E9C93] transition-all" style={{ width: `${(doneCount / PATH.length) * 100}%` }} />
        </div>
        <div className="space-y-2">
          {PATH.map((label, i) => {
            const ok = !!pathDone[i];
            return (
              <button key={i} data-testid={`beginner-step-${i}`} onClick={() => toggleStep(i)}
                ...
                {ok
                  ? <CheckCircle2 className="w-6 h-6 text-[#3E9C93] shrink-0" />
                  : <span className="w-6 h-6 rounded-full border-2 border-[#2A3B49] dark:border-[#4a5560] flex items-center justify-center text-[11px] font-bold text-[#7E8A93] shrink-0">{i + 1}</span>}
                <span className={`text-sm ${ok ? "line-through text-[#7E8A93]" : "text-[#3F4A54] dark:text-[#AEB8BF]"}`}>{label}</span>
              </button>
            );
          })}
        </div>
        {doneCount === PATH.length && <p className="text-sm font-semibold text-[#3E9C93] mt-3">🎉 {tri3(lang, "Percorso completato! Sei pronto per il tuo primo pane.", "Weg abgeschlossen! Bereit für dein erstes Brot.", "Path completed! Ready for your first bread.", "¡Recorrido completado! Estás listo para tu primer pan.")}</p>}
      </div>

      {/* Ricetta del giorno gratis — cambia ogni giorno */}
      <div data-testid="recipe-of-day" className="rounded-2xl p-5 text-white bg-gradient-to-br from-[#1B2A38] to-[#16202b] shadow-md">
      ...
      <button data-testid="impara-toggle-more" onClick={() => setShowMore((v) => !v)}
        className="w-full flex items-center justify-between gap-2 rounded-2xl px-4 py-3.5 bg-[#1B2A38] border border-[#2A3B49] hover:border-[#3E9C93]/50 text-white active:scale-98 transition-all text-left">
        <span className="flex items-center gap-2.5">
          <span className="w-9 h-9 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/15 border border-[#3E9C93]/30 flex items-center justify-center shrink-0"><Sprout className="w-5 h-5 text-[#3E9C93]" /></span>
        ...
        {showMore ? <ChevronUp className="w-5 h-5 text-[#3E9C93] shrink-0" /> : <ChevronDown className="w-5 h-5 text-[#3E9C93] shrink-0" />}
      </button>

      {showMore && (
      <div data-testid="impara-more" className="space-y-4">
      {beginners.map((s, i) => (
        <div key={i} data-testid={`beg-section-${i}`} className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl p-5">
