...
export default function PublicBatch({ id }) {
  ...
  const Row = ({ Icon, label, value }) => value ? (
    <div className="flex items-start gap-3 py-3 border-b border-[#e4eff8] dark:border-[#2A3B49] last:border-0">
      <Icon className="w-5 h-5 text-[#3E9C93] shrink-0 mt-0.5" />
  ...
  return (
    <div data-testid="public-batch" className="fixed inset-0 z-[75] bg-[#0E1620] dark:bg-[#0E1620] overflow-auto">
      <div className="max-w-md mx-auto px-4 py-8">
        ...
        {loading ? (
          <div className="flex justify-center py-20"><Loader2 className="w-7 h-7 animate-spin text-[#3E9C93]" /></div>
        ) : error || !batch ? (
          ...
          <>
            <div className="rounded-3xl bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] text-white p-6 shadow-xl mb-5">
            ...
            <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-4 shadow-sm">
            ...
            {batch.note && (
              <div className="rounded-2xl bg-[#3E9C93]/10 border border-[#3E9C93]/30 p-4 mt-4">
                <p className="text-[11px] font-bold uppercase tracking-wide text-[#3E9C93] mb-1">{tri("Note", "Notizen", "Notes")}</p>
            ...
            <a href="/" className="block text-center text-sm font-semibold text-[#3E9C93] underline underline-offset-2 mt-3">mikilab.de</a>
          </>
        )}
      </div>
    </div>
