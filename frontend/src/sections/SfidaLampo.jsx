    ...
    <div data-testid="sfida-lampo" className="mb-5 rounded-3xl overflow-hidden border border-[#ff6b00]/40 bg-[#181818] shadow-lg">
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-center justify-between text-[12px] text-[#AEB8BF]">
          <span className="inline-flex items-center gap-1.5"><Users className="w-4 h-4 text-[#ff6b00]" /> {data?.participants || 0} {L("fornai in gara", "Bäcker dabei", "bakers competing", "panaderos compitiendo", "boulangers en lice", "نانوا در رقابت")}</span>
        </div>
        ...
        {data?.already_submitted ? (
          ...
          <button data-testid="sfida-lampo-join" disabled={busy} onClick={() => (user ? fileRef.current?.click() : (setAuthOpen && setAuthOpen(true)))}
            className="w-full flex items-center justify-center gap-2 rounded-2xl bg-[#ff6b00] text-[#121212] font-bold py-3.5 active:scale-98 transition-all disabled:opacity-60">
        ...
        <div data-testid="sfida-lampo-leaderboard">
          <p className="text-[11px] font-extrabold uppercase tracking-wider text-[#ff6b00] flex items-center gap-1.5 mb-2 mt-1"><Trophy className="w-4 h-4" /> {L("Classifica dei fornai", "Bäcker-Rangliste", "Bakers leaderboard", "Clasificación de panaderos", "Classement des boulangers", "جدول نانواها")}</p>
          {entries.length === 0 ? (
