...
export default function AntiWaste() {
  ...
  return (
    <div className="pb-40">
      ...
      <p className="text-xs font-bold uppercase tracking-wide text-[#3E9C93] mb-2">{tri("Ricetta di recupero", "Verwertungsrezept", "Recovery recipe")}</p>
      <div className="grid grid-cols-2 gap-2 mb-4">
        {RECIPES.map((x) => (
          <button key={x.id} data-testid={`aw-rec-${x.id}`} onClick={() => setRec(x.id)}
            className={`px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 text-sm font-semibold border text-left ${rec === x.id ? "bg-[#3E9C93] text-white border-[#3E9C93]" : "bg-white dark:bg-[#1B2A38] text-[#3F4A54] dark:text-[#AEB8BF] border-[#2A3B49] dark:border-[#2A3B49]"}`}>
            {x.label}
          </button>
        ))}
      </div>

      <div data-testid="aw-result" className="rounded-3xl bg-gradient-to-br from-[#3E9C93] to-[#374f31] text-white p-6 shadow-lg">
