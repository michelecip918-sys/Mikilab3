...
const Field = ({ label, value, onChange, unit }) => (
  <label className="flex flex-col gap-1">
    ...
    <span className="flex items-center gap-1 bg-background border border-border/30 rounded-lg px-2 py-1.5 focus-within:border-border">
      <input type="number" value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-transparent text-sm text-foreground outline-none" />
...
export default function EliteTools() {
  ...
  return (
    <div className="space-y-7" data-testid="elite-tools">
      {/* Food cost */}
      <div>
        <p className="flex items-center gap-2 font-mono-data text-[10px] tracking-[0.25em] uppercase text-muted-foreground mb-3"><Calculator className="w-3.5 h-3.5" /> {tri("Food cost al grammo", "Food Cost pro Gramm", "Food cost per gram", "Food cost por gramo", "Coût matière au gramme", "بهای مواد بر گرم")}</p>
        {recipes.length > 0 && (
          <div className="mb-3">
            <span className="text-[10px] uppercase tracking-wider text-muted-foreground">{tri("Da ricetta salvata (1-clic)", "Aus gespeichertem Rezept", "From saved recipe (1-click)", "Desde receta guardada", "Depuis recette", "از دستور ذخیره‌شده")}</span>
            <select data-testid="fc-recipe-select" onChange={(e) => pickRecipe(e.target.value)} defaultValue=""
              className="mt-1 w-full bg-background border border-border/30 rounded-lg px-3 py-2 text-sm text-foreground focus:border-border outline-none">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
          ...
          <button data-testid="fc-run" onClick={runFc} className="self-end h-[38px] rounded-lg bg-accent/20 border border-border/50 text-accent-foreground font-bold text-sm active:scale-95 transition-all">{tri("Calcola", "Rechnen", "Compute", "Calcular", "Calculer", "محاسبه")}</button>
        </div>
        {fcRes && (
          <div data-testid="fc-result" className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
            {[
              ...
              <div key={i} className="bg-background/70 border border-border/25 rounded-lg py-2">
                <p className="text-[9px] uppercase tracking-wider text-muted-foreground">{k}</p>
                <p className="text-lg font-black text-muted-foreground">{v}</p>
              </div>
            ))}
            {fcRes.margin != null && <p className="col-span-2 sm:col-span-4 text-xs text-foreground">{tri("Margine per infornata", "Marge/Charge", "Margin/batch", "Margen/hornada", "Marge/fournée", "حاشیه")}: <span className="font-bold text-muted-foreground">€ {fcRes.margin}</span></p>}
          </div>
        )}
      </div>
      ...
      {/* Environment */}
      <div>
        <p className="flex items-center gap-2 font-mono-data text-[10px] tracking-[0.25em] uppercase text-muted-foreground mb-3"><Thermometer className="w-3.5 h-3.5" /> {tri("Controllo ambientale predittivo", "Prädiktive Umweltsteuerung", "Predictive environment control", "Control ambiental predictivo", "Contrôle environnemental prédictif", "کنترل محیطی پیش‌بین")}</p>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-3">
          ...
          <button data-testid="env-run" onClick={runEnv} className="self-end h-[38px] rounded-lg bg-accent/20 border border-border/50 text-accent-foreground font-bold text-sm active:scale-95 transition-all">{tri("Suggerisci", "Vorschlag", "Suggest", "Sugerir", "Suggérer", "پیشنهاد")}</button>
          <button data-testid="env-auto" onClick={async () => { try { const w = await envApi.weatherNow(); if (w && w.ok) { const ne = { ...env, temp_c: w.temp_c, humidity_pct: w.humidity_pct }; setEnv(ne); const p = Object.fromEntries(Object.entries(ne).map(([k, v]) => [k, Number(v) || 0])); setEnvRes(await envApi.compute(p)); } } catch (e) { /* */ } }} className="self-end h-[38px] inline-flex items-center justify-center gap-1.5 rounded-lg bg-muted/15 border border-border/50 text-muted-foreground font-bold text-sm active:scale-95 transition-all"><CloudSun className="w-4 h-4" /> {tri("Auto meteo", "Auto Wetter", "Auto weather", "Auto clima", "Auto météo", "خودکار هوا")}</button>
        </div>
        {envRes && (
          <div data-testid="env-result" className="bg-background/70 border border-border/25 rounded-lg p-3">
            <div className="flex gap-4 mb-2">
              <div><p className="text-[9px] uppercase tracking-wider text-muted-foreground">{tri("Lievitazione", "Gare", "Proof", "Fermentación", "Pousse", "تخمیر")}</p><p className="text-xl font-black text-muted-foreground">{envRes.adjusted_proof_hours} h</p></div>
              <div><p className="text-[9px] uppercase tracking-wider text-muted-foreground">{tri("Idratazione", "Hydratation", "Hydration", "Hidratación", "Hydratation", "هیدراتاسیون")}</p><p className="text-xl font-black text-muted-foreground">{envRes.hydration_percent}%</p></div>
            </div>
            <p className="text-xs text-foreground">{envRes.note}</p>
          </div>
        )}
      </div>
    </div>
