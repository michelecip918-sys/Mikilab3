...
export default function DeptAssign() {
  ...
  return (
    <div data-testid="dept-assign" className="space-y-4">
      ...
      {cur && (
        <div className="rounded-xl bg-background border border-border p-3 text-[11px] text-muted-foreground">
          <span className="text-foreground font-bold">{cur.name}</span> · {cur.machines.length} {tri("macchine", "Maschinen", "machines", "máquinas", "machines", "دستگاه")} · {cur.silos.length} silos · {cur.cells.length} {tri("celle", "Zellen", "cells", "celdas", "cellules", "سلول")}
        </div>
      )}
        ...
        <div className="space-y-2.5" data-testid="dept-team-picker">
          ...
          <div className="space-y-1.5">
            {operators.map((o) => {
              ...
              return (
                <div key={o.name} data-testid={`dept-op-row-${o.name}`} className="rounded-xl bg-background border px-2.5 py-2 transition-all" style={{ borderColor: on ? `${accent}77` : "hsl(var(--card))" }}>
                  <div className="flex items-center gap-2">
                    <button data-testid={`dept-op-toggle-${o.name}`} onClick={() => toggle(o.name)}
                      ...
                      style={{ borderColor: on ? accent : "hsl(var(--secondary))", background: on ? `${accent}22` : "transparent" }}>
                      {on && <Check className="w-3.5 h-3.5" style={{ color: accent }} />}
                    </button>
                    <span className="text-sm font-bold text-foreground flex-1 min-w-0 truncate">{o.name}</span>
                    {on && (
                      <>
                        <input data-testid={`dept-op-task-${o.name}`} value={sel[o.name]} onChange={(e) => setTask(o.name, e.target.value)}
                          ...
                          className="flex-1 min-w-0 rounded-lg bg-background border border-border focus:border-border/60 outline-none text-xs text-foreground px-2.5 py-1.5" />
                        <button data-testid={`dept-op-appr-${o.name}`} onClick={() => toggleAppr(o.name)} title={tri("Modalità apprendista", "Lehrlingsmodus", "Apprentice mode", "Modo aprendiz", "Mode apprenti", "حالت کارآموز")}
                          className={`shrink-0 inline-flex items-center gap-1 px-2 py-1.5 rounded-lg border text-[10px] font-black transition-all active:scale-95 ${appr[o.name] ? "bg-amber-500/20 border-amber-500/60 text-amber-400" : "bg-background border-border text-muted-foreground"}`}>
          ...
          <div className="flex items-center gap-2">
            <input data-testid="dept-manual-input" value={manual} onChange={(e) => setManual(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addManual(); }}
              ...
              className="flex-1 rounded-xl bg-background border border-border focus:border-border/60 outline-none text-sm text-foreground px-3 py-2" />
            <button data-testid="dept-manual-add" onClick={addManual} className="shrink-0 inline-flex items-center gap-1 px-3 py-2 rounded-xl border border-border text-muted-foreground text-sm active:scale-95"><Plus className="w-4 h-4" /></button>
          </div>
          ...
          <div className="flex items-center gap-2">
            <input data-testid="dept-label-input" value={label} onChange={(e) => setLabel(e.target.value)}
              ...
              className="flex-1 rounded-xl bg-background border border-border focus:border-border/60 outline-none text-sm text-foreground px-3 py-2.5" />
            <input data-testid="dept-target-input" value={target} onChange={(e) => setTarget(e.target.value.replace(/[^0-9]/g, ""))} inputMode="numeric"
              ...
              className="w-20 rounded-xl bg-background border border-border focus:border-border/60 outline-none text-sm text-foreground px-3 py-2.5" />
          </div>
          ...
          {apprConfirm && (
            <div data-testid="dept-appr-confirm" className="rounded-xl border border-amber-500/50 bg-amber-500/8 p-3.5">
              ...
              <p className="text-[12.5px] text-foreground leading-snug mb-3">{tri(
              ...
              <div className="flex items-center gap-2">
                <button data-testid="dept-appr-confirm-yes" onClick={confirmAppr} disabled={busy} className="flex-1 inline-flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-amber-500 text-foreground font-black text-sm active:scale-95 disabled:opacity-50">
                ...
                <button data-testid="dept-appr-confirm-no" onClick={() => doAssign(apprConfirm.items.map((it) => ({ ...it, apprentice: false })))} disabled={busy} className="flex-1 py-2.5 rounded-xl bg-background border border-border text-muted-foreground font-bold text-sm active:scale-95">
          ...
          <button data-testid="dept-assign-btn" onClick={assign} disabled={busy || selCount === 0}
            className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-muted/15 border border-border/50 text-muted-foreground font-bold text-sm active:scale-95 disabled:opacity-40">
      ...
      {board.length > 0 && (
        <div data-testid="dept-board" className="space-y-1.5">
          <p className="text-[10px] uppercase tracking-widest text-muted-foreground">{tri("Lavagna di controllo · live", "Kontrolltafel · live", "Control board · live", "Tablero · live", "Tableau · live", "تابلو · زنده")}</p>
          {board.map((o) => { const pct = o.target > 0 ? Math.min(100, Math.round((o.done / o.target) * 100)) : 0; return (
            <div key={o.dept} data-testid={`board-${o.dept}`} className="rounded-xl bg-background border border-border px-3 py-2">
              <div className="flex items-center justify-between text-xs mb-1"><span className="font-bold text-foreground">{o.dept_name}{o.label ? ` · ${o.label}` : ""}</span><span className="font-black text-muted-foreground">{o.done}/{o.target || "∞"} {o.unit}</span></div>
              <div className="h-1.5 rounded-full bg-background overflow-hidden"><div className="h-full bg-gradient-to-r from-muted to-accent" style={{ width: `${pct}%` }} /></div>
            </div>
          ); })}
        </div>
      )}
      {assignments.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-[10px] uppercase tracking-widest text-muted-foreground">{tri("Assegnazioni di oggi", "Heutige Zuweisungen", "Today's assignments", "Asignaciones de hoy", "Aujourd'hui", "امروز")}</p>
          {assignments.map((a) => (
            <div key={a.id} data-testid={`dept-assignment-${a.id}`} className="flex items-center gap-2 rounded-xl bg-background border border-border px-3 py-2">
              <UserCog className="w-4 h-4 text-muted-foreground shrink-0" />
              <p className="text-xs text-foreground flex-1 min-w-0 truncate"><b>{a.operator}</b> → {a.dept_name}{a.task ? ` · ${a.task}` : ""}</p>
              ...
              <button data-testid={`dept-unassign-${a.id}`} onClick={() => del(a)} className="text-muted-foreground hover:text-rose-400"><Trash2 className="w-4 h-4" /></button>
            </div>
          ))}
        </div>
      )}
