    ...
    <div data-testid="sitor-atelier" className="space-y-3">
      ...
      <div className="relative">
        <textarea data-testid="atelier-input" value={req} onChange={(e) => setReq(e.target.value)} rows={2}
          ...
          className="w-full rounded-xl bg-[#030712] border border-[#7E9A82]/30 focus:border-[#7E9A82]/70 outline-none text-sm text-white p-3 resize-none" />
      </div>
      <div className="flex flex-wrap gap-1.5">
        {SUGGEST.map((sug, i) => (
          <button key={i} data-testid={`atelier-suggest-${i}`} onClick={() => create(sug)} disabled={busy}
            className="text-[11px] font-bold px-2.5 py-1.5 rounded-full bg-[#7E9A82]/10 border border-[#7E9A82]/30 text-[#7E9A82] active:scale-95 disabled:opacity-50">
            + {sug}
          </button>
        ))}
      </div>
      <button data-testid="atelier-create-btn" onClick={() => create()} disabled={busy || !req.trim()}
        ...
        style={{ background: "linear-gradient(90deg,#7E9A82,#8a97a6)", boxShadow: "0 0 18px rgba(166,177,188,0.3)" }}>
          ...
          {widgets.map((w) => {
            ...
            return (
              <motion.div key={w.id} initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
                data-testid={`atelier-widget-${w.id}`} className="rounded-2xl border border-[#7E9A82]/25 bg-[#0C1019]/70 p-3.5">
                <div className="flex items-center gap-2 mb-2">
                  <span className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 bg-[#7E9A82]/12 border border-[#7E9A82]/35"><I className="w-4 h-4 text-[#7E9A82]" /></span>
                ...
                {w.type === "note" && (
                  <textarea data-testid={`atelier-note-${w.id}`} value={w.config.text || ""} rows={3}
                    ...
                    className="w-full rounded-lg bg-[#030712] border border-[#1e293b] focus:border-[#7E9A82]/50 outline-none text-[13px] text-[#CBD5E1] p-2 resize-none" />
                )}
                ...
                {w.type === "counter" && (
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span data-testid={`atelier-counter-val-${w.id}`} className="min-w-[2.5rem] text-center text-xl font-black text-[#7E9A82]">{w.config.value || 0}</span>
                      <button data-testid={`atelier-counter-plus-${w.id}`} onClick={() => patch(w.id, { ...w.config, value: (w.config.value || 0) + (w.config.step || 1) })} className="w-8 h-8 rounded-lg bg-[#7E9A82]/15 border border-[#7E9A82]/40 text-[#7E9A82] flex items-center justify-center active:scale-90"><Plus className="w-4 h-4" /></button>
                    </div>
                  </div>
                )}
                    ...
                    {(w.config.items || []).map((it, k) => (
                      <li key={k}>
                        <button onClick={() => { const items = w.config.items.map((x, j) => (j === k ? { ...x, done: !x.done } : x)); patch(w.id, { ...w.config, items }); }}
                          ...
                          <span className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${it.done ? "bg-[#7E9A82]/20 border-[#7E9A82]/60 text-[#7E9A82]" : "border-[#64748B]/50 text-transparent"}`}><Check className="w-3 h-3" /></span>
                ...
                {w.type === "metric" && (
                  <div className="text-center py-1">
                    <p className="text-2xl font-black text-[#7E9A82]">{w.config.value || "—"} <span className="text-sm text-[#94A3B8]">{w.config.unit}</span></p>
                      ...
                      <div className="flex items-end gap-1.5 h-24 mb-1.5">
                        {series.map((p, k) => (
                          <div key={k} className="flex-1 flex flex-col items-center justify-end h-full">
                            <span className="text-[9px] text-[#7E9A82] font-bold mb-0.5">{Number(p.v) || 0}</span>
                            <div className="w-full rounded-t transition-all" style={{ height: `${Math.max(4, ((Number(p.v) || 0) / max) * 100)}%`, background: "linear-gradient(180deg,#7E9A82,#8a97a6)" }} />
                            <span className="text-[9px] text-[#64748B] mt-1">{p.d}</span>
                          </div>
                        ))}
                      </div>
                      <p className="text-[10px] text-[#94A3B8] mb-1.5">{w.config.label}{w.config.unit ? ` · ${w.config.unit}` : ""}{w.config.auto ? " · " : ""}{w.config.auto && <span data-testid={`atelier-chart-auto-${w.id}`} className="text-[#7E9A82] font-bold">{tri("dati reali", "Echtdaten", "real data", "datos reales", "données réelles", "داده واقعی")}</span>}</p>
                      {!w.config.auto && (
                      <div className="flex gap-1 overflow-x-auto pb-1">
                        {series.map((p, k) => (
                          <input key={k} data-testid={`atelier-chart-input-${w.id}-${k}`} inputMode="decimal" value={p.v}
                            ...
                            className="w-10 shrink-0 bg-[#030712] border border-[#1e293b] focus:border-[#7E9A82]/50 outline-none text-[11px] text-white text-center rounded px-1 py-1" />
                        ))}
                      </div>
                      )}
                    </div>
                ...
                <div className="mt-2 pt-2 border-t border-[#1e293b] flex items-center gap-1.5">
                  ...
                  <select data-testid={`atelier-share-${w.id}`} value={w.share_dept || ""} onChange={(e) => share(w.id, e.target.value)}
                    className="bg-[#030712] border border-[#1e293b] focus:border-[#7E9A82]/50 outline-none text-[11px] text-white rounded px-1.5 py-1">
                    {DEPTS.map((d) => <option key={d} value={d}>{d === "" ? tri("no", "nein", "no", "no", "non", "خیر") : d}</option>)}
                  </select>
                  {w.share_dept && <span className="text-[9px] text-[#7E9A82] font-bold">✓</span>}
                </div>
              </motion.div>
            );
          })}
