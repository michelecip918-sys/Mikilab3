    ...
    <div data-testid="sitor-atelier" className="space-y-3">
      ...
      <div className="relative">
        <textarea data-testid="atelier-input" value={req} onChange={(e) => setReq(e.target.value)} rows={2}
          ...
          className="w-full rounded-xl bg-background border border-accent/30 focus:border-accent/70 outline-none text-sm text-foreground p-3 resize-none" />
      </div>
      <div className="flex flex-wrap gap-1.5">
        {SUGGEST.map((sug, i) => (
          <button key={i} data-testid={`atelier-suggest-${i}`} onClick={() => create(sug)} disabled={busy}
            className="text-[11px] font-bold px-2.5 py-1.5 rounded-full bg-accent/10 border border-accent/30 text-accent active:scale-95 disabled:opacity-50">
            + {sug}
          </button>
        ))}
      </div>
      <button data-testid="atelier-create-btn" onClick={() => create()} disabled={busy || !req.trim()}
        ...
        style={{ background: "linear-gradient(90deg,hsl(var(--accent)),hsl(var(--muted-foreground)))", boxShadow: "0 0 18px rgba(166,177,188,0.3)" }}>
      ...
      <div data-testid="atelier-widgets" className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
          ...
          {widgets.map((w) => {
            ...
            return (
              <motion.div key={w.id} initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
                data-testid={`atelier-widget-${w.id}`} className="rounded-2xl border border-accent/25 bg-background/70 p-3.5">
                <div className="flex items-center gap-2 mb-2">
                  <span className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 bg-accent/12 border border-accent/35"><I className="w-4 h-4 text-accent" /></span>
                  <p className="text-sm font-bold text-foreground flex-1 truncate">{w.title}</p>
                  <button data-testid={`atelier-del-${w.id}`} onClick={() => remove(w.id)} className="text-muted-foreground hover:text-mattone active:scale-90"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>

                {w.type === "note" && (
                  <textarea data-testid={`atelier-note-${w.id}`} value={w.config.text || ""} rows={3}
                    onChange={(e) => patch(w.id, { ...w.config, text: e.target.value })}
                    className="w-full rounded-lg bg-background border border-border focus:border-accent/50 outline-none text-[13px] text-foreground p-2 resize-none" />
                )}

                {w.type === "counter" && (
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[12px] text-muted-foreground">{w.config.label}</span>
                    <div className="flex items-center gap-2">
                      <button onClick={() => patch(w.id, { ...w.config, value: Math.max(0, (w.config.value || 0) - (w.config.step || 1)) })} className="w-8 h-8 rounded-lg bg-background border border-border text-foreground flex items-center justify-center active:scale-90"><Minus className="w-4 h-4" /></button>
                      <span data-testid={`atelier-counter-val-${w.id}`} className="min-w-[2.5rem] text-center text-xl font-black text-accent">{w.config.value || 0}</span>
                      <button data-testid={`atelier-counter-plus-${w.id}`} onClick={() => patch(w.id, { ...w.config, value: (w.config.value || 0) + (w.config.step || 1) })} className="w-8 h-8 rounded-lg bg-accent/15 border border-accent/40 text-accent flex items-center justify-center active:scale-90"><Plus className="w-4 h-4" /></button>
                    </div>
                  </div>
                )}
                    ...
                    {(w.config.items || []).map((it, k) => (
                      <li key={k}>
                        <button onClick={() => { const items = w.config.items.map((x, j) => (j === k ? { ...x, done: !x.done } : x)); patch(w.id, { ...w.config, items }); }}
                          ...
                          <span className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${it.done ? "bg-accent/20 border-accent/60 text-accent" : "border-border/50 text-transparent"}`}><Check className="w-3 h-3" /></span>
                          <span className={it.done ? "line-through text-muted-foreground" : "text-foreground"}>{it.t}</span>
                        </button>
                      </li>
                    ))}
                  </ul>
                ...
                {w.type === "metric" && (
                  <div className="text-center py-1">
                    <p className="text-2xl font-black text-accent">{w.config.value || "—"} <span className="text-sm text-muted-foreground">{w.config.unit}</span></p>
                    <p className="text-[11px] text-muted-foreground">{w.config.label}</p>
                  </div>
                )}
                ...
                {w.type === "reminder" && (
                  <div className="flex items-start gap-2">
                    ...
                    <div><p className="text-[13px] text-foreground leading-snug">{w.config.text}</p>{w.config.date && <p className="text-[11px] text-muted-foreground font-bold mt-0.5">{w.config.date}</p>}</div>
                  </div>
                )}
                      ...
                      <div className="flex items-end gap-1.5 h-24 mb-1.5">
                        {series.map((p, k) => (
                          <div key={k} className="flex-1 flex flex-col items-center justify-end h-full">
                            <span className="text-[9px] text-accent font-bold mb-0.5">{Number(p.v) || 0}</span>
                            <div className="w-full rounded-t transition-all" style={{ height: `${Math.max(4, ((Number(p.v) || 0) / max) * 100)}%`, background: "linear-gradient(180deg,hsl(var(--accent)),hsl(var(--muted-foreground)))" }} />
                            <span className="text-[9px] text-muted-foreground mt-1">{p.d}</span>
                          </div>
                        ))}
                      </div>
                      <p className="text-[10px] text-muted-foreground mb-1.5">{w.config.label}{w.config.unit ? ` · ${w.config.unit}` : ""}{w.config.auto ? " · " : ""}{w.config.auto && <span data-testid={`atelier-chart-auto-${w.id}`} className="text-accent font-bold">{tri("dati reali", "Echtdaten", "real data", "datos reales", "données réelles", "داده واقعی")}</span>}</p>
                      {!w.config.auto && (
                      <div className="flex gap-1 overflow-x-auto pb-1">
                        {series.map((p, k) => (
                          <input key={k} data-testid={`atelier-chart-input-${w.id}-${k}`} inputMode="decimal" value={p.v}
                            ...
                            className="w-10 shrink-0 bg-background border border-border focus:border-accent/50 outline-none text-[11px] text-foreground text-center rounded px-1 py-1" />
                        ))}
                      </div>
                      )}
                    </div>
                ...
                <div className="mt-2 pt-2 border-t border-border flex items-center gap-1.5">
                  <span className="text-[10px] text-muted-foreground">{tri("Condividi in reparto:", "In Bereich teilen:", "Share to dept:", "Compartir en área:", "Partager à l'atelier :", "اشتراک با بخش:")}</span>
                  <select data-testid={`atelier-share-${w.id}`} value={w.share_dept || ""} onChange={(e) => share(w.id, e.target.value)}
                    className="bg-background border border-border focus:border-accent/50 outline-none text-[11px] text-foreground rounded px-1.5 py-1">
                    {DEPTS.map((d) => <option key={d} value={d}>{d === "" ? tri("no", "nein", "no", "no", "non", "خیر") : d}</option>)}
                  </select>
                  {w.share_dept && <span className="text-[9px] text-accent font-bold">✓</span>}
                </div>
              </motion.div>
            );
          })}
        ...
        {widgets.length === 0 && (
          <p data-testid="atelier-empty" className="text-[12px] text-muted-foreground col-span-full text-center py-4">{tri(
