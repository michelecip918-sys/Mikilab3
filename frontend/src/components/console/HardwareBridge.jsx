...
export default function HardwareBridge() {
  ...
  const light = pct == null ? "#334155" : pct <= 0.02 ? "#7DD3FC" : pct <= 0.06 ? "#FFB800" : "#f87171";
  ...
  return (
    <div className="space-y-7" data-testid="hardware-bridge">
      {/* BILANCIA */}
      <div>
        <p className="flex items-center gap-2 font-mono-data text-[10px] tracking-[0.25em] uppercase text-[#5E8CA8] mb-3"><Scale className="w-3.5 h-3.5" /> {tri("Bilancia di precisione", "Präzisionswaage", "Precision scale", "Balanza de precisión", "Balance de précision", "ترازوی دقیق")}</p>
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <label className="flex items-center gap-1 bg-[#0C1019] border border-[#5E8CA8]/30 rounded-lg px-2 py-1.5">
          ...
          <button data-testid="hw-scale-serial" onClick={async () => { try { await ensureScale().connectSerial(); } catch (e) { alert(e.message); } }} disabled={!hwSupport.serial} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#0C1019] border border-[#5E8CA8]/40 text-[#9fc3dc] text-xs font-bold disabled:opacity-40 active:scale-95"><Usb className="w-3.5 h-3.5" /> Serial</button>
          <button data-testid="hw-scale-ble" onClick={async () => { try { await ensureScale().connectBluetooth(); } catch (e) { alert(e.message); } }} disabled={!hwSupport.bluetooth} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#0C1019] border border-[#5E8CA8]/40 text-[#9fc3dc] text-xs font-bold disabled:opacity-40 active:scale-95"><Bluetooth className="w-3.5 h-3.5" /> BLE</button>
          <button data-testid="hw-scale-sim" onClick={() => ensureScale().simulate(target)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#5E8CA8]/20 border border-[#5E8CA8]/50 text-[#9fc3dc] text-xs font-bold active:scale-95"><Cpu className="w-3.5 h-3.5" /> {tri("Simula", "Simulieren", "Simulate", "Simular", "Simuler", "شبیه‌سازی")}</button>
        </div>
      ...
      {/* PLC FORNO */}
      <div>
        <p className="flex items-center gap-2 font-mono-data text-[10px] tracking-[0.25em] uppercase text-[#5E8CA8] mb-3"><Flame className="w-3.5 h-3.5" /> {tri("PLC forno · ciclo termico", "Ofen-SPS · Thermozyklus", "Oven PLC · thermal cycle", "PLC horno · ciclo térmico", "API four · cycle thermique", "پی‌ال‌سی فر")}</p>
        <div className="flex flex-wrap items-center gap-2">
          <label className="flex items-center gap-1 bg-[#0C1019] border border-[#5E8CA8]/30 rounded-lg px-2 py-1.5"><input data-testid="hw-temp-input" type="number" value={temp} onChange={(e) => setTemp(e.target.value)} className="w-16 bg-transparent text-sm text-white outline-none text-center" /><span className="text-[10px] text-[#64748b]">°C</span></label>
          ...
          <button data-testid="hw-steam" onClick={() => setSteam((v) => !v)} className={`px-3 py-1.5 rounded-lg text-xs font-bold border active:scale-95 ${steam ? "bg-[#7DD3FC]/15 border-[#7DD3FC]/50 text-[#7DD3FC]" : "bg-[#0C1019] border-[#1e293b] text-[#64748b]"}`}>{tri("Vapore", "Dampf", "Steam", "Vapor", "Vapeur", "بخار")}</button>
          <button data-testid="hw-oven-serial" onClick={async () => { try { await ensureOven().connectSerial(); } catch (e) { alert(e.message); } }} disabled={!hwSupport.serial} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#0C1019] border border-[#5E8CA8]/40 text-[#9fc3dc] text-xs font-bold disabled:opacity-40 active:scale-95"><Usb className="w-3.5 h-3.5" /> Serial</button>
        ...
        {plcMsg && <p data-testid="hw-plc-msg" className="mt-2 text-sm font-bold text-[#7DD3FC] font-mono-data">{plcMsg}</p>}
