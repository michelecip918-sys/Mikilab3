...
export default function TeamFaces() {
  ...
  return (
    <div data-testid="team-faces" className="space-y-3">
      ...
      {!cam ? (
        <button data-testid="team-faces-add" onClick={openCam} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-muted/10 border border-border/40 text-muted-foreground font-bold text-sm active:scale-95"><UserPlus className="w-4 h-4" /> {tri("Registra un volto", "Gesicht registrieren", "Enroll a face", "Registrar rostro", "Enregistrer un visage", "ثبت چهره")}</button>
      ) : (
        <div className="space-y-2">
          <div className="relative w-full rounded-xl overflow-hidden border border-border/40 bg-background" style={{ aspectRatio: "4/3" }}>
            ...
            <span data-testid="team-faces-status" className={`absolute top-2 left-2 text-[10px] font-black px-2 py-0.5 rounded-full ${faceOk ? "bg-accent text-accent-foreground" : "bg-background/80 text-muted-foreground"}`}>{faceOk ? tri("Volto rilevato", "Erkannt", "Face detected", "Detectado", "Détecté", "تشخیص داده شد") : tri("Inquadra il viso", "Gesicht zeigen", "Center face", "Encuadra", "Cadre", "کادر")}</span>
            <button data-testid="team-faces-cancel" onClick={stop} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-background/80 border border-border text-muted-foreground flex items-center justify-center"><X className="w-4 h-4" /></button>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <input data-testid="team-faces-name" value={name} onChange={(e) => setName(e.target.value)} placeholder={tri("Nome", "Name", "Name", "Nombre", "Nom", "نام")} className="rounded-lg bg-background border border-border text-foreground text-sm px-3 py-2 focus:border-border outline-none" />
            <input data-testid="team-faces-dept" value={dept} onChange={(e) => setDept(e.target.value)} placeholder={tri("Reparto (facolt.)", "Bereich (opt.)", "Dept (optional)", "Área (opc.)", "Atelier (opt.)", "بخش (اختیاری)")} className="rounded-lg bg-background border border-border text-foreground text-sm px-3 py-2 focus:border-border outline-none" />
          </div>
          <button data-testid="team-faces-save" onClick={enroll} disabled={busy || !faceOk || !name.trim()} className="w-full inline-flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gradient-to-r from-muted to-muted text-foreground font-black text-sm active:scale-95 disabled:opacity-40">{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />} {tri("Salva volto", "Speichern", "Save face", "Guardar", "Enregistrer", "ذخیره")}</button>
        </div>
      )}

      <div className="flex flex-wrap gap-2">
        {faces.length === 0 ? <p className="text-[12px] text-muted-foreground">{tri("Nessun volto registrato.", "Keine Gesichter.", "No faces enrolled.", "Sin rostros.", "Aucun visage.", "چهره‌ای نیست.")}</p> : faces.map((f) => (
          <div key={f.name} data-testid={`team-face-${f.name}`} className="relative flex flex-col items-center gap-1 w-20 rounded-xl bg-background border border-border p-2">
            {f.thumb ? <img src={f.thumb} alt={f.name} className="w-12 h-12 rounded-full object-cover border border-border/40" /> : <span className="w-12 h-12 rounded-full bg-muted/10 flex items-center justify-center text-muted-foreground"><ScanFace className="w-6 h-6" /></span>}
            <span className="text-[11px] font-bold text-foreground truncate max-w-full">{f.name}</span>
            {f.dept && <span className="text-[9px] text-muted-foreground truncate max-w-full">{f.dept}</span>}
            <button data-testid={`team-face-del-${f.name}`} onClick={() => remove(f.name)} className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-background border border-border text-muted-foreground hover:text-rose-400 flex items-center justify-center"><Trash2 className="w-3 h-3" /></button>
          </div>
        ))}
      </div>
    </div>
