  ...
  return (
    <div className="mb-3">
      <button data-testid="magazzino-inventory-btn" onClick={() => setOpen(true)}
        className="w-full inline-flex items-center justify-center gap-2 py-2.5 rounded-2xl font-black text-sm border border-accent/50 text-accent bg-accent/10 active:scale-95 transition-transform">
      ...
      <p className="text-[11px] text-muted-foreground mt-1.5">{tri("Scatta una foto agli scaffali: Sitor conta e aggiorna le giacenze.", "Foto der Regale: Sitor zählt und aktualisiert.", "Photograph the shelves: Sitor counts and updates stock.", "Fotografía los estantes: Sitor cuenta y actualiza.", "Photographie les étagères : Sitor compte et met à jour.", "از قفسه‌ها عکس بگیر: سیتور می‌شمارد و به‌روزرسانی می‌کند.")}</p>
  ...
  return (
    <div className="mb-3">
      <button data-testid="celle-phoenix-btn" onClick={() => setOpen(true)}
        className="w-full inline-flex items-center justify-center gap-2 py-2.5 rounded-2xl font-black text-sm border border-border/50 text-foreground bg-muted/10 active:scale-95 transition-transform">
