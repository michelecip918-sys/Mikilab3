  ...
  return (
    <div data-testid="compliance-timelog" className="space-y-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <p className="flex items-center gap-2 text-[12px] font-black uppercase tracking-widest text-[#8a97a6]"><ScrollText className="w-4 h-4" /> {tri("Registro Orari ArbZG", "ArbZG-Zeiterfassung", "ArbZG Time Log", "Registro ArbZG", "Journal ArbZG", "ثبت ساعت ArbZG")}</p>
        <div className="flex items-center gap-2">
          <input type="date" data-testid="timelog-day" value={day} onChange={(e) => setDay(e.target.value)} className="bg-[#0b0f19] border border-[#334155] rounded-lg px-2.5 py-1.5 text-[12px] text-white" />
          {data && (
            <span data-testid="timelog-integrity" className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-bold border ${data.integrity_ok ? "text-[#6e9e85] border-[#6e9e85]/40 bg-[#6e9e85]/10" : "text-[#e0574a] border-[#e0574a]/50 bg-[#e0574a]/10"}`}>
      ...
      {loading && <div className="py-6 text-center"><Loader2 className="w-5 h-5 animate-spin inline text-[#8a97a6]" /></div>}
      {!loading && workers.length === 0 && <p className="text-[12px] text-[#64748B] py-4">{tri("Nessuna timbratura per questo giorno.", "Keine Einträge für diesen Tag.", "No entries for this day.", "Sin registros para este día.", "Aucune entrée pour ce jour.", "برای این روز ثبتی نیست.")}</p>}

      {workers.map((w) => {
        const s = data.summaries[w] || {};
        const evs = (data.entries || []).filter((e) => e.worker === w);
        return (
          <div key={w} data-testid={`timelog-worker-${w}`} className={`rounded-xl border p-3 ${s.compliant ? "border-[#334155]/60 bg-[#0b0f19]" : "border-[#e0574a]/50 bg-[#e0574a]/8"}`}>
            <div className="flex items-center justify-between gap-2 mb-2">
              <span className="text-[13px] font-bold text-white">{w}</span>
              <span className="flex items-center gap-2">
                <span className="text-[11px] text-[#94A3B8]">{tri("Lavoro", "Arbeit", "Work", "Trabajo", "Travail", "کار")} {Math.floor((s.work_min || 0) / 60)}h{(s.work_min || 0) % 60}′ · {tri("pausa", "Pause", "break", "pausa", "pause", "استراحت")} {s.break_min || 0}′</span>
                <button data-testid={`timelog-erase-${w}`} onClick={() => erase(w)} disabled={erasing === w} title={tri("Cancella dati (GDPR)", "Daten löschen", "Erase data (GDPR)", "Borrar (GDPR)", "Supprimer (GDPR)", "حذف")} className="w-7 h-7 inline-flex items-center justify-center rounded-lg border border-[#334155] text-[#94A3B8] hover:text-[#e0574a] hover:border-[#e0574a]/50 active:scale-95 disabled:opacity-50">{erasing === w ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}</button>
              </span>
            </div>
            {!s.compliant && (data.summaries[w].flags || []).map((f, i) => (
              <p key={i} data-testid={`timelog-flag-${w}-${i}`} className="flex items-center gap-1.5 text-[11.5px] text-[#e0574a] font-semibold mb-1"><AlertTriangle className="w-3.5 h-3.5 shrink-0" /> {f}</p>
            ))}
            <div className="flex flex-wrap gap-1.5 mt-1">
              {evs.map((e, i) => (
                <span key={i} className="text-[10.5px] px-2 py-1 rounded-lg bg-[#050810] border border-[#1e293b] text-[#9aa6b2]">
