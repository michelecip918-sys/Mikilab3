    ...
    <div data-testid="shift-templates" className="space-y-3">
      <div className="flex items-center gap-2">
        <input data-testid="shift-template-name" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") save(); }}
          ...
          className="flex-1 rounded-xl bg-[#030712] border border-[#1e293b] focus:border-[#7DD3FC]/60 outline-none text-sm text-white px-3 py-2.5" />
        <button data-testid="shift-template-save" onClick={save} disabled={busy}
          className="shrink-0 inline-flex items-center gap-1.5 px-3 py-2.5 rounded-xl bg-[#7DD3FC]/15 border border-[#7DD3FC]/50 text-[#7DD3FC] font-bold text-sm active:scale-95 disabled:opacity-40">
        ...
        <div className="space-y-1.5">
          {templates.map((t) => (
            <div key={t.id} data-testid={`shift-template-${t.id}`} className="flex items-center gap-2 rounded-xl bg-[#0C1019] border border-[#1e293b] px-3 py-2">
              <CalendarClock className="w-4 h-4 text-[#7DD3FC] shrink-0" />
