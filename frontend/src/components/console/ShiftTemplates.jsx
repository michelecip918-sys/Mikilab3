    ...
    <div data-testid="shift-templates" className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <button data-testid="shift-template-save" onClick={save} disabled={busy}
          className="shrink-0 inline-flex items-center gap-1.5 px-3 py-2.5 rounded-xl bg-muted/15 border border-border/50 text-muted-foreground font-bold text-sm active:scale-95 disabled:opacity-40">
          {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} {tri("Salva", "Speichern", "Save", "Guardar", "Enreg.", "ذخیره")}
        </button>
      </div>
      <p className="text-[10px] text-muted-foreground">{tri("Salva la squadra assegnata oggi come turno-tipo, poi riapplicala con un tocco quando serve.", "Speichere das heutige Team als Vorlage.", "Save today's team as a template and reapply with one tap.", "Guarda el equipo de hoy como plantilla.", "Enregistre l'équipe du jour comme modèle.", "تیم امروز را به‌عنوان الگو ذخیره کن.")}</p>

      {templates.length === 0 ? (
        <p data-testid="shift-templates-empty" className="text-[11px] text-muted-foreground rounded-xl bg-background border border-border px-3 py-2.5">{tri("Nessun turno-tipo salvato.", "Keine Vorlagen.", "No templates saved.", "Sin plantillas.", "Aucun modèle.", "الگویی نیست.")}</p>
      ) : (
        <div className="space-y-1.5">
          {templates.map((t) => (
            <div key={t.id} data-testid={`shift-template-${t.id}`} className="flex items-center gap-2 rounded-xl bg-background border border-border px-3 py-2">
              <CalendarClock className="w-4 h-4 text-muted-foreground shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-foreground truncate">{t.name}</p>
                <p className="text-[10px] text-muted-foreground">{(t.items || []).length} {tri("assegnazioni", "Zuweisungen", "assignments", "asignaciones", "affectations", "واگذاری")}</p>
              </div>
              <button data-testid={`shift-template-apply-${t.id}`} onClick={() => apply(t)} disabled={busy} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-accent/15 border border-accent/50 text-accent text-xs font-bold active:scale-95 disabled:opacity-40"><Play className="w-3.5 h-3.5" /> {tri("Applica", "Anwenden", "Apply", "Aplicar", "Appliquer", "اعمال")}</button>
              <button data-testid={`shift-template-del-${t.id}`} onClick={() => del(t)} className="text-muted-foreground hover:text-rose-400"><Trash2 className="w-4 h-4" /></button>
            </div>
          ))}
        </div>
      )}
