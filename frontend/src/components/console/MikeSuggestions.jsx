...
// Sitor · Suggerimenti predittivi: il "cervello" unico dell'impianto propone azioni concrete.
export default function MikeSuggestions() {
  ...
  const toggleAuto = async (e) => {
    ...
    try { await mikeApi.autopilotSet(next); toast.success(next ? tri("Auto-pilota Sitor attivo", "Autopilot aktiv", "Sitor autopilot on", "Piloto automático activo", "Pilote auto activé", "خلبان خودکار فعال") : tri("Auto-pilota disattivato", "Autopilot aus", "Autopilot off", "Piloto desactivado", "Pilote désactivé", "خلبان خاموش")); load(); } catch { setAutopilot(!next); }
  };
  ...
  return (
    <div data-testid="mike-suggestions" className="rounded-2xl border border-[#FF6B00]/40 bg-gradient-to-br from-[#FF6B00]/8 to-transparent p-3">
      <p className="text-[11px] font-black uppercase tracking-widest text-[#FF6B00] flex items-center gap-1.5 mb-2"><Brain className="w-3.5 h-3.5" /> {tri("Sitor · Suggerimenti", "Sitor · Vorschläge", "Sitor · Suggestions", "Sitor · Sugerencias", "Sitor · Suggestions", "بوکومیکس · پیشنهادها")}
