...
const ICONS = ["🏭", "🥖", "🧁", "🍕", "🥨", "🏷️", "🍨", "☕", "🧊", "🥗", "🍫", "🧂"];
const COLORS = ["hsl(var(--muted-foreground))", "hsl(var(--muted-foreground))", "hsl(var(--muted-foreground))", "hsl(var(--muted-foreground))", "hsl(var(--muted-foreground))", "hsl(var(--muted-foreground))", "hsl(var(--primary))", "hsl(var(--primary))", "hsl(var(--muted-foreground))"];
...
export default function DeptManager() {
  ...
  const [newColor, setNewColor] = useState("hsl(var(--muted-foreground))");
  ...
  return (
    <div data-testid="dept-manager" className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {editKey === d.key ? (
                <>
                  ...
                  <button data-testid={`dept-save-${d.key}`} onClick={() => saveRename(d.key)} className="text-accent"><Check className="w-4 h-4" /></button>
                  <button data-testid={`dept-cancel-${d.key}`} onClick={() => setEditKey("")} className="text-muted-foreground"><X className="w-4 h-4" /></button>
                </>
              ) : (
                <>
                  <span className="text-sm font-bold text-foreground flex-1 truncate">{d.name}{d.custom ? <span className="ml-1.5 text-[9px] text-foreground uppercase">· {tri("mio", "mein", "mine", "mío", "mien", "من")}</span> : null}</span>
                  <button data-testid={`dept-rename-${d.key}`} onClick={() => { setEditKey(d.key); setEditName(d.name); }} className="text-muted-foreground hover:text-foreground"><Pencil className="w-3.5 h-3.5" /></button>
                  <button data-testid={`dept-del-${d.key}`} onClick={() => del(d.key)} className="text-mattone hover:text-muted-foreground" title={d.custom ? tri("Elimina", "Löschen", "Delete", "Eliminar", "Supprimer", "حذف") : tri("Nascondi", "Ausblenden", "Hide", "Ocultar", "Masquer", "پنهان")}>
            ...
            <div className="flex flex-wrap gap-1 mt-2 pl-7">
              {COLORS.map((c) => (
                <button key={c} data-testid={`dept-color-${d.key}-${c.replace('#', '')}`} onClick={() => setColor(d.key, c)}
                  className={`w-4 h-4 rounded-full border ${d.accent === c ? "ring-2 ring-white" : "border-foreground/20"}`} style={{ background: c }} />
              ))}
            </div>
          </div>
        ))}
      ...
      {hidden.length > 0 && (
        <div data-testid="dept-hidden" className="rounded-lg bg-background/60 border border-border/40 p-2.5">
          <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground mb-1.5">{tri("Reparti nascosti", "Ausgeblendet", "Hidden departments", "Ocultos", "Masqués", "پنهان‌ها")}</p>
          <div className="flex flex-wrap gap-1.5">
            {hidden.map((h) => (
              <button key={h.key} data-testid={`dept-restore-${h.key}`} onClick={() => restore(h.key)} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] bg-secondary/40 text-foreground active:scale-95"><Eye className="w-3 h-3" /> {h.name}</button>
            ))}
          </div>
        </div>
      )}

      <div className="rounded-lg bg-background border border-border/30 p-2.5 space-y-2">
        <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">{tri("Nuovo reparto", "Neuer Bereich", "New department", "Nuevo departamento", "Nouveau rayon", "بخش جدید")}</p>
        <div className="flex items-center gap-1.5">
          <button data-testid="dept-new-icon" onClick={() => setNewIcon(ICONS[(ICONS.indexOf(newIcon) + 1) % ICONS.length])} className="text-lg leading-none w-8 h-8 rounded-md bg-background border border-border/30">{newIcon}</button>
          <input data-testid="dept-new-name" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder={tri("Nome (es. Gelateria)", "Name", "Name (e.g. Gelato)", "Nombre", "Nom", "نام")}
            ...
            className="flex-1 min-w-0 bg-background border border-border/30 rounded-md px-2.5 py-1.5 text-sm text-foreground outline-none" />
        </div>
        <div className="flex flex-wrap gap-1">
          {COLORS.map((c) => (
            <button key={c} data-testid={`dept-new-color-${c.replace('#', '')}`} onClick={() => setNewColor(c)} className={`w-5 h-5 rounded-full border ${newColor === c ? "ring-2 ring-white" : "border-foreground/20"}`} style={{ background: c }} />
          ))}
        </div>
        <button data-testid="dept-add-btn" onClick={create} disabled={busy || !newName.trim()}
          className="w-full inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-primary/20 border border-primary/50 text-primary-foreground font-bold text-sm disabled:opacity-40 active:scale-95">
