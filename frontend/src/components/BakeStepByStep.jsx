...
export default function BakeStepByStep({ recipe, lang, onExit, onGoCommunity }) {
  ...
  if (finished) {
    return (
      <div data-testid="bake-finished" className="pb-8">
        <div className="rounded-3xl p-7 text-center text-[#121212] shadow-xl" style={{ background: "linear-gradient(135deg,#ff8a33,#ff6b00)" }}>
        ...
        <button data-testid="bake-finished-community" onClick={() => onGoCommunity && onGoCommunity()}
          className="mt-4 w-full flex items-center justify-center gap-2 bg-[#ff6b00] text-[#121212] font-semibold py-3.5 rounded-2xl active:scale-98 transition-all">
        ...
        {onExit && <button data-testid="bake-finished-exit" onClick={onExit} className="mt-2 w-full text-[#ff6b00] font-medium py-2">{L("Torna a Impara", "Zurück zu Lernen", "Back to Learn", "Volver a Aprender", "Retour à Apprendre", "بازگشت به یادگیری")}</button>}
      </div>
    );
  }
  ...
  return (
    <div data-testid="bake-step-by-step" className="pb-8">
      {onExit && (
        <button data-testid="bake-exit" onClick={onExit} className="flex items-center gap-1 text-[#ff6b00] font-medium mb-3">
      ...
      <div className="rounded-3xl p-5 text-[#121212] shadow-xl" style={{ background: "linear-gradient(135deg,#ff8a33,#ff6b00 70%)" }}>
      ...
      <div className="mt-4 flex items-start gap-3 rounded-2xl bg-[#ff6b00]/10 border border-[#ff6b00]/30 p-3.5">
        <div className="relative shrink-0">
          <img src="/michele-real-lab.jpg" alt="MikiLab" className="w-11 h-11 rounded-full object-cover border-2 border-[#ff6b00]/40"
        ...
        {uniqueBadges.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-3" data-testid="bake-badges">
            {uniqueBadges.map((b, k) => (
              <span key={k} className="inline-flex items-center gap-1.5 rounded-full bg-[#ff6b00]/15 border border-[#ff6b00]/30 px-3 py-1 text-[12px] font-bold text-[#ff6b00]">
      ...
      <button data-testid="bake-shopping-toggle" onClick={() => setShopOpen((v) => !v)}
        className="mt-4 w-full flex items-center justify-center gap-2 rounded-2xl bg-[#1e1e1e] border border-[#ff6b00]/40 text-white font-semibold py-3 active:scale-98 transition-all">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        ...
        {isLast ? (
          <button data-testid="bake-finish" disabled={busy} onClick={finish}
            className="flex items-center justify-center gap-2 rounded-2xl bg-[#ff6b00] text-[#121212] font-bold py-3.5 disabled:opacity-50 active:scale-95 transition-all">
          ...
          <button data-testid="bake-next" onClick={() => setI((n) => Math.min(total - 1, n + 1))}
            className="flex items-center justify-center gap-1.5 rounded-2xl bg-[#ff6b00] text-[#121212] font-bold py-3.5 active:scale-95 transition-all">
