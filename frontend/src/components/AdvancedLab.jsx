...
function Card({ icon: Ic, title, sub, accent, testid, children }) {
  return (
    <div data-testid={testid} className="relative rounded-xl bg-[#070A10]/80 border p-4 overflow-hidden" style={{ borderColor: `${accent}33` }}>
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-2 mb-2">
          ...
          <div><p className="text-[12px] font-black text-white uppercase tracking-wide leading-none">{title}</p>{sub && <p className="text-[9.5px] text-[#8aa0b4] mt-0.5">{sub}</p>}</div>
        </div>
        {children}
      </div>
    </div>
  ...
  const scan = () => {
    ...
    setTimeout(() => {
      setSensors([
        ...
        { id: "H1", type: tri("Umidità", "Feuchte", "Humidity", "Humedad", "Humidité", "رطوبت"), val: "68%", ic: Droplets, c: "#00F0FF" },
        { id: "P1", type: "pH", val: "5.4", ic: Gauge, c: "#7FD8C0" },
        { id: "F1", type: tri("Flusso", "Fluss", "Flow", "Flujo", "Flux", "جریان"), val: "1.2 L/m", ic: Wifi, c: "#7DD3FC" },
      ]);
      setScanning(false);
    }, 1600);
  };
  ...
  return (
    <div data-testid="advanced-lab" className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="w-11 h-11 shrink-0 rounded-xl bg-[#F6D27A]/10 border border-[#F6D27A]/40 flex items-center justify-center"><Zap className="w-6 h-6 text-[#F6D27A]" /></div>
        <div>
          ...
          <p className="text-[11px] text-[#8aa0b4]">{tri("Plancia avanzata di Miki-Nexus & Mike Mix — dati live.", "Erweiterte Konsole — Live-Daten.", "Advanced console — live data.", "Consola avanzada — datos en vivo.", "Console avancée — données en direct.", "کنسول پیشرفته — داده زنده")}</p>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {/* Compensazione climatica */}
        <Card testid="lab-climate" icon={CloudSun} accent="#00F0FF" title={tri("Compensazione Climatica", "Klimaausgleich", "Climate Compensation", "Compensación Climática", "Compensation Climatique", "جبران آب‌وهوا")} sub={tri("Prodotto identico in ogni clima", "Gleiches Produkt in jedem Klima", "Same product in any climate", "Mismo producto en cualquier clima", "Même produit sous tout climat", "محصول یکسان")}>
          <div className="grid grid-cols-3 gap-2 text-center">
            <div><p className="text-[9px] text-[#8aa0b4] uppercase">{tri("Ambiente", "Umgebung", "Ambient", "Ambiente", "Ambiant", "محیط")}</p><p className="font-cyber text-lg font-black text-white">{ambient.toFixed(1)}°</p></div>
            <div><p className="text-[9px] text-[#00F0FF] uppercase">{tri("Acqua", "Wasser", "Water", "Agua", "Eau", "آب")}</p><p className="font-cyber text-lg font-black text-[#00F0FF]">{waterT}°</p></div>
        ...
        {/* IoT Plug & Play */}
        <Card testid="lab-iot" icon={Radio} accent="#7DD3FC" title={tri("IoT Plug & Play", "IoT Plug & Play", "IoT Plug & Play", "IoT Plug & Play", "IoT Plug & Play", "IoT")} sub={tri("Riconoscimento sensori in 1s", "Sensor-Erkennung in 1s", "Sensor recognition in 1s", "Reconocimiento en 1s", "Reconnaissance en 1s", "شناسایی در ۱ ثانیه")}>
          <button data-testid="lab-iot-scan" onClick={scan} disabled={scanning} className="w-full mb-2 py-1.5 rounded-lg text-[11px] font-bold text-[#070A10] disabled:opacity-60" style={{ background: "linear-gradient(90deg,#7DD3FC,#00F0FF)" }}>{scanning ? tri("Scansione…", "Scan…", "Scanning…", "Escaneando…", "Analyse…", "اسکن…") : tri("Scansiona sensori", "Sensoren scannen", "Scan sensors", "Escanear", "Scanner", "اسکن سنسورها")}</button>
        ...
        {/* Edge computing + Voce */}
        <Card testid="lab-edge" icon={Cpu} accent="#F6D27A" title={tri("Edge Enzimatico & Voce", "Edge & Sprache", "Enzymatic Edge & Voice", "Edge & Voz", "Edge & Voix", "Edge و صدا")} sub={tri("Calcolo locale sub-millisecondo", "Lokale Sub-ms-Berechnung", "Sub-ms local compute", "Cómputo local sub-ms", "Calcul local sub-ms", "پردازش محلی")}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] text-[#8aa0b4] uppercase">{tri("Latenza", "Latenz", "Latency", "Latencia", "Latence", "تأخیر")}</span>
            <span className="font-cyber text-lg font-black text-[#F6D27A] tabular-nums">{lat.toFixed(2)} ms</span>
          </div>
