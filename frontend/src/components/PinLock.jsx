...
export default function PinLock({ onUnlock }) {
  ...
  return (
    <div data-testid="pin-lock" className="fixed inset-0 z-[1000] flex flex-col items-center justify-center px-6"
      ...
      <p className="text-[13px] mb-6" style={{ color: "hsl(var(--muted-foreground))" }}>{tri("Inserisci il PIN di sicurezza", "Sicherheits-PIN eingeben", "Enter the security PIN", "Introduce el PIN de seguridad", "Saisis le code PIN", "پین امنیتی را وارد کن")}</p>

      <div className="flex items-center gap-3 mb-2" data-testid="pin-dots">
        {[0, 1, 2, 3].map((i) => (
          <span key={i} className="w-4 h-4 rounded-full transition-all"
            style={{ background: pin.length > i ? "hsl(var(--muted-foreground))" : "transparent", border: `2px solid ${err ? "hsl(var(--muted-foreground))" : "hsl(var(--secondary))"}` }} />
        ))}
      </div>
      <p className="h-5 text-[12px] font-bold mb-4" style={{ color: "hsl(var(--muted-foreground))" }} data-testid="pin-error">
      ...
      <div className="grid grid-cols-3 gap-3 w-full max-w-[260px]">
        {keys.map((k, i) => k === "" ? <span key={i} /> : k === "del" ? (
          <button key={i} data-testid="pin-del" onClick={del} className="h-16 rounded-2xl flex items-center justify-center active:scale-95 transition-all"
            style={{ background: "hsl(var(--card))", border: "2px solid hsl(var(--secondary))", color: "hsl(var(--muted-foreground))" }}><Delete className="w-6 h-6" /></button>
        ) : (
          <button key={i} data-testid={`pin-key-${k}`} onClick={() => press(k)}
            ...
            style={{ background: "hsl(var(--card))", border: "2px solid hsl(var(--secondary))", color: "hsl(var(--foreground))", fontSize: "24px" }}>{k}</button>
        ))}
      </div>
    </div>
  );
