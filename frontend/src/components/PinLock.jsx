...
export default function PinLock({ onUnlock }) {
  ...
  return (
    <div data-testid="pin-lock" className="fixed inset-0 z-[1000] flex flex-col items-center justify-center px-6"
      style={{ background: "radial-gradient(120% 60% at 50% -10%, #14212C 0%, #0D1520 55%)", color: "#F7F9FC" }}>
      <img src={`${process.env.PUBLIC_URL}/logo.png`} alt="MikiLab" className="w-16 h-16 rounded-2xl object-cover ring-2 ring-[#64748B]/60 mb-4" />
      <div className="flex items-center gap-2 mb-1">
        <Lock className="w-5 h-5" style={{ color: "#64748B" }} />
        <h1 className="font-display font-extrabold" style={{ fontSize: "22px", color: "#64748B" }}>MikiLab</h1>
      </div>
      ...
      <div className="flex items-center gap-3 mb-2" data-testid="pin-dots">
        {[0, 1, 2, 3].map((i) => (
          <span key={i} className="w-4 h-4 rounded-full transition-all"
            style={{ background: pin.length > i ? "#64748B" : "transparent", border: `2px solid ${err ? "#E63946" : "#2A3B49"}` }} />
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 w-full max-w-[260px]">
        {keys.map((k, i) => k === "" ? <span key={i} /> : k === "del" ? (
          <button key={i} data-testid="pin-del" onClick={del} className="h-16 rounded-2xl flex items-center justify-center active:scale-95 transition-all"
            style={{ background: "#1B2A38", border: "2px solid #2A3B49", color: "#64748B" }}><Delete className="w-6 h-6" /></button>
        ) : (
