    ...
    <>
      ...
      {speaking && (
        <div data-testid="proactive-avatar-overlay" className="fixed inset-x-0 top-20 z-[70] flex justify-center pointer-events-none px-3">
          <div className="flex items-center gap-3 rounded-2xl bg-[#14212C]/95 backdrop-blur border border-[#3E9C93]/50 px-3.5 py-2.5 shadow-2xl max-w-sm">
            <SpeakingAvatar who={speaking.persona === "momi" ? "momi" : "lab"} active mode="speaking" size={52} testid="proactive-avatar" />
            <div className="min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-wide text-[#3E9C93]">{speaking.persona === "momi" ? "Momi" : "Mickey Lab"} · <span className="text-white/70">{speaking.name}</span></p>
      ...
      <div data-testid="proactive-panel" className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] overflow-hidden mb-4">
        <button data-testid="proactive-toggle" onClick={() => setOpen((o) => !o)} className="w-full flex items-center justify-between gap-2 px-3.5 py-3">
          <span className="flex items-center gap-2 text-sm font-bold text-[#2B303B] dark:text-[#e4eff8]">
            ...
            <Radar className="w-4 h-4 text-[#3E9C93]" />
        ...
        {open && (
          <div className="px-3.5 pb-3.5 border-t border-[#2A3B49]">
            ...
            <div className="flex items-center gap-2 mb-3">
              <button data-testid="proactive-mute" onClick={() => setMuted((m) => !m)}
                className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg active:scale-95 ${muted ? "bg-[#2A3B49] text-[#AEB8BF]" : "bg-[#3E9C93] text-white"}`}>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {PROACTIVE_MODULES.map((m) => (
                <div key={m.id} data-testid={`proactive-mod-${m.id}`} className="flex items-center gap-1.5 text-[11px] text-[#7E8A93] bg-[#f5f5f5] dark:bg-[#151515] rounded-lg px-2 py-1.5">
                  <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${m.persona === "momi" ? "bg-[#3E9C93]" : "bg-[#3B82F6]"}`} />
