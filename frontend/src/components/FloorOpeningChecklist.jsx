...
export default function FloorOpeningChecklist({ lang, role }) {
  ...
  return (
    <div data-testid="floor-opening-checklist" className="rounded-2xl border border-border/40 bg-background p-4 space-y-3">
      <div>
        <p className="flex items-center gap-2 text-[12px] font-black uppercase tracking-widest text-muted-foreground"><ListChecks className="w-4 h-4" /> {it ? "Checklist di apertura" : "Opening checklist"}</p>
        <p className="text-[12px] text-muted-foreground mt-1">{it ? "Il piano non è ancora arrivato. Intanto parti da qui — tocca 🔊 per farti leggere ogni punto." : "The plan isn't in yet. Start here meanwhile — tap 🔊 to hear each point."}</p>
      </div>
        ...
        {list.map((t, i) => (
          <li key={i} data-testid={`floor-opening-item-${i}`} className="flex items-center gap-2">
            <button data-testid={`floor-opening-check-${i}`} onClick={() => toggle(i)}
              className={`w-7 h-7 rounded-lg shrink-0 flex items-center justify-center border active:scale-95 transition-all ${done[i] ? "bg-accent border-accent text-accent-foreground" : "border-border text-muted-foreground"}`}>
              <Check className="w-4 h-4" />
            </button>
            <span className={`flex-1 text-[13px] leading-snug ${done[i] ? "text-muted-foreground line-through" : "text-foreground"}`}>{t}</span>
            <button data-testid={`floor-opening-read-${i}`} onClick={() => read(t)} className="w-7 h-7 rounded-lg shrink-0 flex items-center justify-center bg-muted/15 text-muted-foreground active:scale-95 transition-all">
      ...
      <div className="pt-2 border-t border-border">
        <button data-testid="floor-ask-sitor-btn" onClick={askSitor} disabled={busy}
          className="w-full inline-flex items-center justify-center gap-2 py-2.5 rounded-xl font-black text-sm text-foreground active:scale-95 transition-all disabled:opacity-60"
          style={{ background: "linear-gradient(90deg,hsl(var(--muted-foreground)),hsl(var(--muted-foreground)))" }}>
          {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} {it ? "Chiedi a Sitor cosa fare" : "Ask Sitor what to do"}
        </button>
        {ask && <p data-testid="floor-ask-sitor-reply" className="mt-2 text-[12.5px] text-foreground leading-relaxed whitespace-pre-line">{ask}</p>}
      </div>
    </div>
  );
}
