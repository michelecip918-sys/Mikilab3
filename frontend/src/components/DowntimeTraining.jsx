  ...
  return (
    <div data-testid="downtime-training" className="space-y-4">
      <div className="flex items-start gap-3">
        <div className="w-11 h-11 shrink-0 rounded-xl bg-[#9aa6b2]/10 border border-[#9aa6b2]/40 flex items-center justify-center shadow-[0_0_18px_rgba(125,211,252,0.3)]">
          <GraduationCap className="w-6 h-6 text-[#9aa6b2]" />
        </div>
        <div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <select data-testid="training-recipe-select" value={sel} onChange={(e) => setSel(e.target.value)}
          className="flex-1 rounded-lg bg-[#0b0f19] border border-[#1e293b] text-white text-sm px-3 py-2.5 focus:border-[#9aa6b2] outline-none">
        ...
        <button data-testid="training-start-btn" onClick={start} disabled={busy}
          ...
          style={{ background: "linear-gradient(90deg,#9aa6b2,#8a97a6)", boxShadow: "0 0 18px rgba(125,211,252,0.4)" }}>
      ...
      {lesson && (
        <div data-testid="training-lesson" className="rounded-xl bg-[#0b0f19] border border-[#9aa6b2]/25 p-4 space-y-4">
          <div className="flex items-start justify-between gap-2">
            <div>
              <h4 className="font-bold text-white text-[15px]">{lesson.title}</h4>
              {lesson.duration_min && <span className="text-[10px] font-mono text-[#9aa6b2] uppercase tracking-widest">≈ {lesson.duration_min} min · {lesson.recipe_name}</span>}
            </div>
            <button data-testid="training-listen" onClick={() => speak(`${lesson.title}. ${lesson.intro}. ${(lesson.steps || []).map((s) => `${s.title}. ${s.detail}`).join(". ")}`)}
              className="shrink-0 inline-flex items-center gap-1 text-[11px] font-bold text-[#9aa6b2] active:scale-90 transition-all"><Volume2 className="w-4 h-4" /> {tri("Ascolta", "Hören", "Listen", "Escuchar", "Écouter", "بشنو")}</button>
          </div>
            ...
            <ol data-testid="training-steps" className="space-y-2">
              {lesson.steps.map((s, i) => (
                <li key={i} className="flex gap-2.5">
                  <span className="shrink-0 w-6 h-6 rounded-full bg-[#9aa6b2]/15 border border-[#9aa6b2]/40 text-[#9aa6b2] text-xs font-black flex items-center justify-center">{i + 1}</span>
          ...
          {(lesson.mistakes || []).length > 0 && (
            <div data-testid="training-mistakes" className="rounded-lg bg-[#aaa795]/8 border border-[#aaa795]/30 p-3 space-y-2">
              <p className="flex items-center gap-1.5 text-[11px] font-black text-[#aaa795] uppercase tracking-wide"><AlertTriangle className="w-3.5 h-3.5" /> {tri("Errori da evitare", "Zu vermeidende Fehler", "Mistakes to avoid", "Errores a evitar", "Erreurs à éviter", "خطاهایی که باید پرهیز کرد")}</p>
              {lesson.mistakes.map((m, i) => (
                <p key={i} className="text-[12px] text-[#d7c9a8] leading-snug"><span className="text-[#aaa795] font-bold">✗ {m.wrong}</span> → <span className="text-[#93a2ae]">{m.fix}</span></p>
              ))}
            </div>
          )}
          ...
          {(lesson.quiz || []).length > 0 && (
            <div data-testid="training-quiz" className="space-y-3">
              <p className="flex items-center gap-1.5 text-[11px] font-black text-[#8a97a6] uppercase tracking-wide"><ChevronRight className="w-3.5 h-3.5" /> {tri("Quiz veloce", "Kurzes Quiz", "Quick quiz", "Quiz rápido", "Quiz rapide", "آزمون سریع")}</p>
                ...
                <div key={qi} data-testid={`training-q-${qi}`} className="rounded-lg bg-[#060A10] border border-[#1e293b] p-3">
                  ...
                  <div className="grid gap-1.5">
                    {(q.options || []).map((opt, oi) => {
                      ...
                      if (answered && isCorrect) cls = "border-[#6e9e85]/60 bg-[#6e9e85]/10 text-[#86efac]";
                      else if (answered && isChosen && !isCorrect) cls = "border-[#b06e78]/60 bg-[#b06e78]/10 text-[#fda4af]";
                      return (
                        <button key={oi} data-testid={`training-q-${qi}-opt-${oi}`} disabled={answered}
                          ...
                          <span className="inline-flex items-center gap-1.5">
                            {answered && isCorrect && <CheckCircle2 className="w-3.5 h-3.5 text-[#6e9e85]" />}
                            {answered && isChosen && !isCorrect && <XCircle className="w-3.5 h-3.5 text-[#b06e78]" />}
                            {opt}
                          </span>
                        </button>
                      );
              ...
              {allAnswered && (
                <div data-testid="training-score" className="flex items-center justify-between rounded-lg bg-[#9aa6b2]/10 border border-[#9aa6b2]/40 px-4 py-3">
                  <span className="text-sm font-black text-white">{tri("Punteggio", "Ergebnis", "Score", "Puntuación", "Score", "امتیاز")}: <span className="text-[#8a97a6]">{score}/{lesson.quiz.length}</span></span>
                  <button data-testid="training-restart" onClick={start} className="inline-flex items-center gap-1.5 text-xs font-bold text-[#9aa6b2] active:scale-95"><RefreshCw className="w-3.5 h-3.5" /> {tri("Nuova lezione", "Neue Lektion", "New lesson", "Nueva lección", "Nouvelle leçon", "درس جدید")}</button>
                </div>
              )}
            </div>
          )}
