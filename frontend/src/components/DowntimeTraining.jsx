  ...
  return (
    <div data-testid="downtime-training" className="space-y-4">
      <div className="flex items-start gap-3">
        <div className="w-11 h-11 shrink-0 rounded-xl bg-[#7DD3FC]/10 border border-[#7DD3FC]/40 flex items-center justify-center shadow-[0_0_18px_rgba(125,211,252,0.3)]">
          <GraduationCap className="w-6 h-6 text-[#7DD3FC]" />
        </div>
        <div>
          ...
          <p className="text-[11px] text-[#8aa0b4] mt-0.5 max-w-md">{tri(
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <select data-testid="training-recipe-select" value={sel} onChange={(e) => setSel(e.target.value)}
          className="flex-1 rounded-lg bg-[#0b0f19] border border-[#1e293b] text-white text-sm px-3 py-2.5 focus:border-[#7DD3FC] outline-none">
        ...
        <button data-testid="training-start-btn" onClick={start} disabled={busy}
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg font-cyber font-black text-sm text-[#070A10] active:scale-95 transition-all disabled:opacity-60"
          style={{ background: "linear-gradient(90deg,#7DD3FC,#00F0FF)", boxShadow: "0 0 18px rgba(125,211,252,0.4)" }}>
      ...
      {lesson && (
        <div data-testid="training-lesson" className="rounded-xl bg-[#0b0f19] border border-[#7DD3FC]/25 p-4 space-y-4">
          <div className="flex items-start justify-between gap-2">
            <div>
              <h4 className="font-bold text-white text-[15px]">{lesson.title}</h4>
              {lesson.duration_min && <span className="text-[10px] font-mono text-[#7DD3FC] uppercase tracking-widest">≈ {lesson.duration_min} min · {lesson.recipe_name}</span>}
            </div>
            <button data-testid="training-listen" onClick={() => speak(`${lesson.title}. ${lesson.intro}. ${(lesson.steps || []).map((s) => `${s.title}. ${s.detail}`).join(". ")}`)}
              className="shrink-0 inline-flex items-center gap-1 text-[11px] font-bold text-[#7DD3FC] active:scale-90 transition-all"><Volume2 className="w-4 h-4" /> {tri("Ascolta", "Hören", "Listen", "Escuchar", "Écouter", "بشنو")}</button>
          </div>
            ...
            <ol data-testid="training-steps" className="space-y-2">
              {lesson.steps.map((s, i) => (
                <li key={i} className="flex gap-2.5">
                  <span className="shrink-0 w-6 h-6 rounded-full bg-[#7DD3FC]/15 border border-[#7DD3FC]/40 text-[#7DD3FC] text-xs font-black flex items-center justify-center">{i + 1}</span>
                  <div><p className="text-[13px] font-bold text-white">{s.title}</p><p className="text-[12px] text-[#9fb3c4] leading-snug">{s.detail}</p></div>
                </li>
              ))}
            </ol>
          )}
          ...
          {(lesson.quiz || []).length > 0 && (
            <div data-testid="training-quiz" className="space-y-3">
              <p className="flex items-center gap-1.5 text-[11px] font-black text-[#00F0FF] uppercase tracking-wide"><ChevronRight className="w-3.5 h-3.5" /> {tri("Quiz veloce", "Kurzes Quiz", "Quick quiz", "Quiz rápido", "Quiz rapide", "آزمون سریع")}</p>
              {lesson.quiz.map((q, qi) => (
                <div key={qi} data-testid={`training-q-${qi}`} className="rounded-lg bg-[#070A10] border border-[#1e293b] p-3">
              ...
              {allAnswered && (
                <div data-testid="training-score" className="flex items-center justify-between rounded-lg bg-[#7DD3FC]/10 border border-[#7DD3FC]/40 px-4 py-3">
                  <span className="text-sm font-black text-white">{tri("Punteggio", "Ergebnis", "Score", "Puntuación", "Score", "امتیاز")}: <span className="text-[#00F0FF]">{score}/{lesson.quiz.length}</span></span>
                  <button data-testid="training-restart" onClick={start} className="inline-flex items-center gap-1.5 text-xs font-bold text-[#7DD3FC] active:scale-95"><RefreshCw className="w-3.5 h-3.5" /> {tri("Nuova lezione", "Neue Lektion", "New lesson", "Nueva lección", "Nouvelle leçon", "درس جدید")}</button>
                </div>
              )}
            </div>
          )}
