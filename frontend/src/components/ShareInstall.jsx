...
export default function ShareInstall() {
  ...
  const NETS = [
    ...
    { id: "email", label: "Email", Icon: Mail, color: "#8C4A27", href: `mailto:?subject=${enc("Mikilab")}&body=${enc(text + "\n\n" + url)}` },
  ];
  ...
  return (
    <div data-testid="share-install" className="rounded-3xl bg-white dark:bg-[#232A31] border border-[#E6D8C3] dark:border-[#38424B] p-5">
      ...
      <div className="grid grid-cols-3 gap-2.5" data-testid="share-networks">
        {NETS.map(({ id, label, Icon, color, href }) => (
          <a key={id} data-testid={`share-${id}`} href={href} target="_blank" rel="noopener noreferrer"
            className="flex flex-col items-center gap-1.5 rounded-2xl border border-[#E6D8C3] dark:border-[#38424B] bg-[#e4eff8] dark:bg-[#2A323A] py-3 active:scale-95 transition-transform">
      ...
      <button data-testid="share-native" onClick={nativeShare}
        className="mt-3 w-full bg-[#B45309] hover:bg-[#336a94] text-white font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all flex items-center justify-center gap-2">
        <Share2 className="w-5 h-5" /> {t("share_more")}
      </button>

      {!installed && (
        <button data-testid="install-app" onClick={install}
          className="mt-2 w-full bg-[#8C4A27] hover:bg-[#336a94] text-white font-semibold px-5 py-3.5 rounded-2xl shadow-md active:scale-98 transition-all flex items-center justify-center gap-2">
          <Download className="w-5 h-5" /> {t("install_app")}
        </button>
      )}
      {installed && (
        <p data-testid="install-done" className="mt-3 text-center text-sm text-[#B45309] font-medium flex items-center justify-center gap-1.5">
          <Smartphone className="w-4 h-4" /> {t("install_done")}
        </p>
      )}

      {iosHint && (
        <div data-testid="install-ios-hint" className="mt-3 rounded-2xl bg-[#B45309]/12 border border-[#B45309]/30 p-3 text-sm text-[#3F4A54] dark:text-[#AEB8BF]">
          {t("install_ios_hint")}
        </div>
      )}
    </div>
