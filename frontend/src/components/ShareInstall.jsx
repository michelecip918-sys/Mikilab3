...
export default function ShareInstall() {
  ...
  const NETS = [
    ...
    { id: "email", label: "Email", Icon: Mail, color: "#F26419", href: `mailto:?subject=${enc("MikiLab")}&body=${enc(text + "\n\n" + url)}` },
  ];
  ...
  return (
    <div data-testid="share-install" className="rounded-3xl bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] p-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5" data-testid="share-networks">
        {NETS.map(({ id, label, Icon, color, href }) => (
          <a key={id} data-testid={`share-${id}`} href={href} target="_blank" rel="noopener noreferrer"
            className="flex flex-col items-center gap-1.5 rounded-2xl border border-[#26324A] dark:border-[#26324A] bg-[#e4eff8] dark:bg-[#18202E] py-3 active:scale-95 transition-transform">
      ...
      <button data-testid="share-native" onClick={nativeShare}
        className="mt-3 w-full bg-[#F26419] hover:bg-[#E8A838] text-white font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all flex items-center justify-center gap-2">
        <Share2 className="w-5 h-5" /> {t("share_more")}
      </button>

      {!installed && (
        <button data-testid="install-app" onClick={install}
          className="mt-2 w-full bg-[#F26419] hover:bg-[#E8A838] text-white font-semibold px-5 py-3.5 rounded-2xl shadow-md active:scale-98 transition-all flex items-center justify-center gap-2">
          <Download className="w-5 h-5" /> {t("install_app")}
        </button>
      )}
      {installed && (
        <p data-testid="install-done" className="mt-3 text-center text-sm text-[#F26419] font-medium flex items-center justify-center gap-1.5">
          <Smartphone className="w-4 h-4" /> {t("install_done")}
        </p>
      )}

      {iosHint && (
        <div data-testid="install-ios-hint" className="mt-3 rounded-2xl bg-[#F26419]/12 border border-[#F26419]/30 p-3 text-sm text-[#3F4A54] dark:text-[#AEB8BF]">
          {t("install_ios_hint")}
        </div>
      )}
    </div>
