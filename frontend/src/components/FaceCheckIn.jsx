...
export default function FaceCheckIn({ tri, onRecognized }) {
  ...
  return (
    <div data-testid="face-checkin" className="rounded-2xl border border-border/40 bg-background p-4 mb-3">
      <p className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-muted-foreground mb-2"><ScanFace className="w-4 h-4" /> {tri("Entra col volto", "Mit Gesicht anmelden", "Face check-in", "Entrar con el rostro", "Connexion par visage", "ورود با چهره")}</p>

      {mode === "" && (
        <>
          {enrolled.length > 0 && (
            <>
              <p className="text-[11px] text-muted-foreground mb-2">{tri("Tocca il tuo volto per entrare:", "Tippe dein Gesicht:", "Tap your face to enter:", "Toca tu rostro:", "Touche ton visage :", "چهره‌ات را بزن:")}</p>
              <div className="flex flex-wrap gap-2 mb-3">
                {enrolled.map((e) => (
                  <div key={e.name} className="relative">
                    <button data-testid={`face-login-${e.name}`} onClick={() => openCam("login", e.name)} className="flex flex-col items-center gap-1 w-20 rounded-xl bg-background border border-border hover:border-border/60 p-2 active:scale-95">
                      {e.thumb ? <img src={e.thumb} alt={e.name} className="w-12 h-12 rounded-full object-cover border border-border/40" /> : <span className="w-12 h-12 rounded-full bg-muted/10 flex items-center justify-center text-muted-foreground"><ScanFace className="w-6 h-6" /></span>}
                      <span className="text-[11px] font-bold text-foreground truncate max-w-full">{e.name}</span>
                    </button>
                    <button data-testid={`face-forget-${e.name}`} onClick={() => forget(e.name)} className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-background border border-border text-muted-foreground flex items-center justify-center"><X className="w-3 h-3" /></button>
                  </div>
                ))}
              </div>
            </>
          )}
          <button data-testid="face-enroll-open" onClick={() => openCam("enroll")} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-muted/10 border border-border/40 text-muted-foreground font-bold text-sm active:scale-95">
      ...
      {mode !== "" && (
        <div className="space-y-2">
          <div className="relative w-full rounded-xl overflow-hidden border border-border/40 bg-background" style={{ aspectRatio: "4/3" }}>
            ...
            <span data-testid="face-status" className={`absolute top-2 left-2 text-[10px] font-black px-2 py-0.5 rounded-full ${faceOk ? "bg-accent text-accent-foreground" : "bg-background/80 text-muted-foreground"}`}>{faceOk ? tri("Volto rilevato", "Gesicht erkannt", "Face detected", "Rostro detectado", "Visage détecté", "چهره تشخیص داده شد") : tri("Inquadra il viso", "Gesicht zeigen", "Center your face", "Encuadra el rostro", "Cadre ton visage", "صورت را در کادر بگذار")}</span>
            <button data-testid="face-cancel" onClick={stop} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-background/80 border border-border text-muted-foreground flex items-center justify-center"><X className="w-4 h-4" /></button>
          </div>
          {mode === "enroll" && (
            <input data-testid="face-enroll-name" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder={tri("Il tuo nome", "Dein Name", "Your name", "Tu nombre", "Ton nom", "نام تو")} className="w-full rounded-lg bg-background border border-border text-foreground text-sm px-3 py-2 focus:border-border outline-none" />
          )}
          <button
            data-testid="face-confirm"
            onClick={mode === "enroll" ? confirmEnroll : confirmLogin}
            disabled={busy || !faceOk || (mode === "enroll" && !newName.trim())}
            className="w-full inline-flex items-center justify-center gap-2 py-3 rounded-xl bg-gradient-to-r from-muted to-muted text-foreground font-black text-sm active:scale-95 disabled:opacity-40">
