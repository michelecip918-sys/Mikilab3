...
import { TRADIZIONI, GIORNI_KEY, stampsInfo, letteraDaAprire } from "@/components/Almanacco";
...
export default function OggiInBottega({ onNav }) {
  ...
  const lettera = letteraDaAprire(); // V104
  const stamp = (e) => {
  ...
  return (
    <div data-testid="oggi-in-bottega" className="mb-4 rounded-3xl border border-primary/30 bg-primary/8 p-4 flex items-center gap-3 cursor-pointer active:scale-[0.99]" onClick={() => onNav("oggi")}>
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[13.5px] text-foreground leading-snug mt-1">{lettera ? tri("C'è una lettera per te: l'hai scritta tu, e oggi si apre.", "Ein Brief für dich: du hast ihn geschrieben, und heute öffnet er sich.", "There's a letter for you: you wrote it, and today it opens.") : trad ? L(trad) : tri("Un proverbio, un gesto da due minuti e il pane di oggi ti aspettano nell'almanacco.", "Ein Sprichwort, ein Handgriff für zwei Minuten und das Brot des Tages warten im Almanach.", "A proverb, a two-minute gesture and today's bread await you in the almanac.")}</p>
