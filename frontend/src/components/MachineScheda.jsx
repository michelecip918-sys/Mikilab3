...
export default function MachineScheda() {
  ...
  return (
    <div data-testid="recipe-machine-scheda" className="mt-4 rounded-2xl border border-primary/40 bg-primary/8 p-4">
      <p className="text-[11px] font-bold uppercase tracking-wide text-primary dark:text-foreground flex items-center gap-1.5 mb-2">
        <Cog className="w-4 h-4" /> {tri("Scheda Macchina", "Maschinen-Blatt", "Machine Sheet")}
      </p>
      <div className="space-y-1 text-sm text-foreground dark:text-foreground">
        ...
        {s.count > 0 ? (
          ...
          <p className="text-[12px] text-muted-foreground">{tri("Nessuna macchina attiva: lavorazione manuale. Attiva il Parco Macchine nel Laboratorio per adattare produzione e resa.", "Keine Maschine aktiv: Handarbeit. Aktiviere den Maschinenpark im Labor.", "No machine active: manual work. Turn on the Machine Park in the Lab.")}</p>
        )}
      </div>
    </div>
  );
