...
export const REGIONS = [
  { k: "basilicata", flag: "🇮🇹", name: D("Basilicata", "Basilikata", "Basilicata"), kw: ["matera", "lucan", "oro di terra", "gran riserva"],
    story: D("La terra di Michele: Miglionico, sulle colline di Matera. Pane di grano duro, forme grandi che durano una settimana, forno a legna e crosta spessa: il pane di Matera è il modello di tutto il sito.", "Micheles Heimat: Miglionico, auf den Hügeln von Matera. Hartweizenbrot, große Laibe, die eine Woche halten, Holzofen und dicke Kruste: das Brot von Matera ist das Vorbild der ganzen Seite.", "Michele's homeland: Miglionico, on the hills of Matera. Durum wheat bread, big loaves that last a week, wood oven and a thick crust: Matera bread is the model for the whole site."), dedica: true },
...
export default function PaneDelPaese({ onBack, onOpenRecipe }) {
  ...
  const pick = (k) => { setSel(k); award("radici"); try { localStorage.setItem(KEY, k); } catch { /* */ } };
      ...
      {region && (
        <section data-testid="paese-region" className="space-y-3">
          <div className="rounded-2xl border border-primary/40 bg-primary/8 p-4">
            ...
            {region.dedica ? <button data-testid="paese-dedica" onClick={() => { try { window.dispatchEvent(new CustomEvent("mikilab-nav", { detail: { route: "dedica" } })); } catch { /* */ } }} className="mt-2 px-3 py-1.5 rounded-full text-xs font-bold border border-primary/40 bg-primary/10 text-foreground active:scale-95">{tri("Da Miglionico a Stoccarda: la dedica di Michele", "Von Miglionico nach Stuttgart: Micheles Widmung", "From Miglionico to Stuttgart: Michele's dedication")}</button> : <p className="text-[10px] text-muted-foreground mt-2">{tri("Testo: bozza di Sitor (IA), da rivedere da Michele.", "Text: Entwurf von Sitor (KI), von Michele zu prüfen.", "Text: draft by Sitor (AI), to be reviewed by Michele.")}</p>}
            {region.k === "basilicata" && <button data-testid="paese-miglionico" onClick={() => { try { window.dispatchEvent(new CustomEvent("mikilab-nav", { detail: { route: "dedica" } } /* V94: il bottone apriva una pagina inesistente */)); } catch { /* */ } }} className="mt-2 px-3 py-1.5 rounded-full bg-mattone/10 border border-mattone/40 text-xs font-bold text-foreground active:scale-95">{tri("Miglionico: la dedica di Michele", "Miglionico: Micheles Widmung", "Miglionico: Michele's dedication")}</button>}
          </div>
