          ...
          <motion.div key={idx} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.12 }}
            ...
            <div className="relative shrink-0">
              <img src={AV[m.who]} alt={NAME[m.who]}
                className="w-11 h-11 rounded-full object-cover shadow-sm ring-2 ring-[#c94f00]/60"
                onError={(e) => { e.currentTarget.style.display = "none"; }} />
              {!isMichele && hint && (
                <span data-testid="assistant-dot" className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-[#c94f00] ring-2 ring-[#f7efe6] dark:ring-[#121212] animate-pulse" />
              )}
            </div>
            <div data-testid={`bubble-${m.who}`}
              onClick={act || undefined}
              role={act ? "button" : undefined}
              className={`relative max-w-[80%] rounded-2xl px-3.5 py-2.5 border transition-all bg-[#c94f00]/5 border-[#c94f00]/20 ${isMichele ? "rounded-bl-sm" : "rounded-br-sm"} ${act ? "cursor-pointer hover:border-[#c94f00]/60 hover:bg-[#c94f00]/10 active:scale-98" : ""}`}>
              ...
              {!isMichele && hint && (
                hintClickable ? (
                  <button data-testid={`bubble-hint-${variant}`} onClick={(e) => { e.stopPropagation(); openChallenges(); }}
                    className="text-[12px] font-bold text-[#c94f00] leading-snug mt-1 underline decoration-[#c94f00]/40 underline-offset-2 active:scale-98">{hint}</button>
                ) : (
                  <p data-testid={`bubble-hint-${variant}`} className="text-[12px] font-semibold text-[#c94f00] leading-snug mt-1">{hint}</p>
                )
              )}
              {act && (
                <span className="mt-1.5 inline-flex items-center gap-1 text-[11px] font-bold text-[#c94f00]">
