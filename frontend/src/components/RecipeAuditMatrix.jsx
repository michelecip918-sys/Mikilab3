...
// DUAL-MODE STRATEGICO: BakoMix (Master Baker) critica la ricetta e presenta al
  ...
  return (
    <div data-testid="recipe-audit" className="fixed inset-0 z-[80] bg-[#030712]/97 backdrop-blur-xl overflow-y-auto">
      <div className="max-w-lg mx-auto p-4 pb-16">
        ...
        {/* Robot BakoMix: stato oro/ambra in modalità strategica */}
        <div className="flex items-center gap-3 my-3 rounded-2xl border p-3" style={{ borderColor: goldState ? "#f59e0b66" : "#1e293b", background: goldState ? "#f59e0b12" : "#0b0f19" }}>
          <div className="relative">
            ...
            <img src={`${PUB}/avatar_bigmix.jpg`} alt="BakoMix" className="relative w-12 h-12 rounded-full object-cover border-2" style={{ borderColor: goldState ? "#f59e0b" : "#5EEAD4" }} />
          </div>
          <p className="text-[12px] text-[#cfe0ec] flex-1">{audit ? audit.bako_note : tri("Scegli una ricetta: la analizzo come Master Baker.", "Wähle ein Rezept: ich prüfe es als Master Baker.", "Pick a recipe: I'll audit it as Master Baker.", "Elige una receta: la audito como Master Baker.", "Choisis une recette : je l'audite en Master Baker.", "دستوری انتخاب کن تا مثل استاد نان بررسی کنم.")}</p>
        </div>
