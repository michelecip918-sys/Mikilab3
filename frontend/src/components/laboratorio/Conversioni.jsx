...
export default function Conversioni({ recipes, lang, t }) {
  ...
  if (!r || !C) return <p className="text-[12px] text-muted-foreground">{tri("Nessuna ricetta con le dosi.", "Kein Rezept mit Mengen.", "No recipe with quantities.")}</p>;
  ...
  const Card = ({ k, title, hours, rows, note }) => (
    <div data-testid={`cv-${k}`} className={`rounded-xl border p-3 ${current === k ? "border-primary/60 bg-primary/8" : "border-border bg-card"}`}>
      ...
      <div className="space-y-0.5 text-[12px]">{rows.map(([a, b], i) => <div key={i} className="flex justify-between gap-2"><span className="text-muted-foreground">{a}</span><span className="font-mono-data font-semibold text-foreground">{b}</span></div>)}</div>
      {note && <p className="text-[11.5px] text-salvia mt-1.5 leading-snug">{note}</p>}
    </div>
  );
  ...
  return (
    <div data-testid="conversioni" className="space-y-3">
      <p className="text-[12.5px] text-foreground/85 leading-snug">{tri("La stessa ricetta con quattro metodi, a parità di farina e acqua totali. Scegli quello che entra nel tuo orario, non il contrario.", "Dasselbe Rezept mit vier Methoden, bei gleicher Gesamtmenge Mehl und Wasser. Wähl die, die in deinen Zeitplan passt, nicht umgekehrt.", "The same recipe with four methods, keeping total flour and water. Choose the one that fits your schedule, not the other way round.")}</p>
      <div className="rounded-xl border border-border bg-card p-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
        <select data-testid="cv-recipe" value={r.id} onChange={(e) => set({ id: e.target.value })} className="col-span-2 text-[12px] font-semibold bg-background text-foreground border border-border rounded-lg px-2 py-2 outline-none">
          {recipes.map((x) => <option key={x.id} value={x.id}>{rLoc(x, "name", lang)}</option>)}
        </select>
        <label className="block text-[12px] text-muted-foreground">{tri("kg farina", "kg Mehl", "kg flour")}<input data-testid="cv-kg" type="number" min="0.5" step="0.5" value={st.kg} onChange={(e) => set({ kg: e.target.value })} className="mt-0.5 w-full font-mono-data font-bold text-foreground bg-background border border-border rounded-md px-2 py-1.5 outline-none" /></label>
        <label className="block text-[12px] text-muted-foreground">{tri("pezzi", "Stück", "pieces")}<input data-testid="cv-pieces" type="number" min="1" value={st.pieces} onChange={(e) => set({ pieces: e.target.value })} className="mt-0.5 w-full font-mono-data font-bold text-foreground bg-background border border-border rounded-md px-2 py-1.5 outline-none" /></label>
      </div>
      <p className="text-[12px] text-foreground">{tri("Totali", "Gesamt", "Totals")}: <b className="font-mono-data">{g(C.F)}</b> {t("ing_flour").toLowerCase()} · <b className="font-mono-data">{g(C.W)}</b> {t("ing_water").toLowerCase()} · {tri("idratazione", "Hydration", "hydration")} <b className="font-mono-data">{fmt1(C.hyd)} %</b> · TA <b className="font-mono-data">{Math.round(100 + C.hyd)}</b></p>
      ...
      <div className="rounded-xl border border-border bg-card p-3 grid grid-cols-2 sm:grid-cols-4 gap-2 text-[12px]">
