...
export default function Cartellini({ recipes, lang, t }) {
  ...
  return (
    <div data-testid="cartellini" className="space-y-3">
      <p className="text-[12.5px] text-foreground/85 leading-snug">{tri("Cartellini per il banco: nome, peso, prezzo e cosa contiene, letto dagli ingredienti della ricetta. Si stampano in una pagina e si ritagliano.", "Thekenschilder: Name, Gewicht, Preis und Inhaltsstoffe, aus den Rezeptzutaten gelesen. Auf einer Seite drucken und ausschneiden.", "Counter tags: name, weight, price and what it contains, read from the recipe ingredients. Print on one page and cut out.")}</p>
      <div className="no-print flex gap-1.5">
        <select data-testid="ct-pick" value={pick} onChange={(e) => setPick(e.target.value)} className="flex-1 text-[12px] font-semibold bg-background text-foreground border border-border rounded-lg px-2 py-2 outline-none">
        ...
        <button data-testid="ct-add" onClick={() => { if (pick) { save([...items, { id: pick, g: 500, price: "" }]); setPick(""); } }} disabled={!pick} className="inline-flex items-center gap-1 text-[12px] font-bold px-3 rounded-lg bg-primary text-white disabled:opacity-40 active:scale-95"><Plus className="w-4 h-4" /></button>
      </div>
          ...
          return (
            <div key={i} data-testid={`ct-tag-${i}`} className="rounded-xl border border-foreground/30 bg-background p-3 text-center min-h-[120px] flex flex-col justify-between">
              <div>
                ...
                <p className="text-[10.5px] text-muted-foreground mt-1 leading-snug">{tri("Contiene", "Enthält", "Contains")}: {cont.join(", ") || tri("— (controlla gli ingredienti)", "— (Zutaten prüfen)", "— (check ingredients)")}</p>
              </div>
              <div className="flex items-end justify-between mt-2">
                <span className="font-mono-data text-[12px] text-foreground">{num(it.g) > 0 ? `${Math.round(num(it.g))} g` : ""}</span>
      ...
      {rows.length > 0 && <button data-testid="ct-printbtn" onClick={() => window.print()} className="no-print inline-flex items-center gap-1.5 text-[12px] font-bold px-3 py-2 rounded-xl bg-primary text-white active:scale-95"><Printer className="w-4 h-4" />{tri("Stampa i cartellini", "Schilder drucken", "Print the tags")}</button>}
      <p className="text-[10.5px] text-muted-foreground">{tri("Il \"contiene\" è dedotto dalla ricetta: verifica sempre le etichette degli ingredienti che usi. Per la vendita vale l'etichettatura obbligatoria (Reg. UE 1169/2011); questo è un cartellino, non l'etichetta.", "\"Enthält\" wird aus dem Rezept abgeleitet: prüfe immer die Etiketten deiner Zutaten. Für den Verkauf gilt die Pflichtkennzeichnung (VO (EU) 1169/2011); das ist ein Schild, kein Etikett.", "\"Contains\" is derived from the recipe: always check the labels of the ingredients you use. For sale, mandatory labelling (Reg. EU 1169/2011) applies; this is a tag, not the label.")}</p>
    </div>
  );
}
