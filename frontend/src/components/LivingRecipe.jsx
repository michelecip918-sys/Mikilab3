  ...
  return (
    <div data-testid="living-recipe" className="space-y-3">
      <div className="flex items-center gap-2">
        <Dna className="w-5 h-5 text-[#a6b1bc]" />
      ...
      <div className="flex flex-wrap gap-1.5">
        {PRESETS.map((p) => (<button key={p} data-testid={`living-preset-${p.slice(0,5)}`} onClick={() => setObj(p)} className={`px-2.5 py-1 rounded-full text-[10.5px] font-bold border active:scale-95 transition-all ${obj === p ? "bg-[#a6b1bc] text-[#060A10] border-[#a6b1bc]" : "bg-[#060A10] text-[#94A3B8] border-[#1e293b]"}`}>{p}</button>))}
      </div>
      <textarea data-testid="living-objective" value={obj} onChange={(e) => setObj(e.target.value)} rows={2} placeholder={tri("Es. pizza altamente digeribile e croccante…", "Z. B. hoch verdauliche knusprige Pizza…", "E.g. highly digestible crispy pizza…", "Ej. pizza muy digerible y crujiente…", "Ex. pizza très digeste et croustillante…", "مثلاً پیتزای ترد و گوارا…")}
        className="w-full rounded-lg bg-[#060A10] border border-[#1e293b] text-white text-sm px-3 py-2 focus:border-[#a6b1bc] outline-none resize-none" />
      <button data-testid="living-generate" onClick={gen} disabled={busy || !obj.trim()}
        className="inline-flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm text-[#060A10] active:scale-95 transition-all disabled:opacity-50" style={{ background: "linear-gradient(90deg,#a6b1bc,#8a97a6)" }}>
      ...
      {r && r.maturation_curve && (
        <div data-testid="living-result" className="rounded-xl border border-[#a6b1bc]/30 bg-gradient-to-b from-[#0b0f19] to-[#060A10] p-4 space-y-3">
          <h5 className="font-cyber text-base font-black text-white">{r.name}</h5>
          {r.why && <p className="flex items-start gap-1.5 text-[12px] text-[#f3e6c4] leading-snug"><Sparkles className="w-3.5 h-3.5 text-[#a6b1bc] shrink-0 mt-0.5" />{r.why}</p>}
          {Object.keys(M).length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {Object.entries(M).map(([k, v]) => (
                <div key={k} className="rounded-lg bg-[#060A10] border border-[#1e293b] p-2"><p className="text-[9px] font-mono uppercase tracking-widest text-[#9aa6b2]">{k.replace(/_/g, " ")}</p><p className="text-[12.5px] font-bold text-white">{String(v)}</p></div>
              ))}
            </div>
          )}
          {r.maturation_curve.length > 0 && (
            <div data-testid="living-curve">
              <p className="flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-widest text-[#8a97a6] mb-1.5"><Clock className="w-3.5 h-3.5" /> {tri("Curva di maturazione", "Reifekurve", "Maturation curve", "Curva de maduración", "Courbe de maturation", "منحنی رسیدن")}</p>
              <div className="space-y-1.5">
                {r.maturation_curve.map((p, i) => (
                  <div key={i} className="flex items-center gap-2 rounded-lg bg-[#060A10] border border-[#1e293b] p-2">
                    <span className="w-6 h-6 shrink-0 rounded-full bg-[#8a97a6]/15 border border-[#8a97a6]/40 text-[#8a97a6] text-[10px] font-black flex items-center justify-center">{i + 1}</span>
                    ...
                    <span className="shrink-0 inline-flex items-center gap-0.5 text-[11px] font-bold text-[#a4afbb]"><Thermometer className="w-3 h-3" />{p.temp_c}°</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {r.sensory && (
            <div data-testid="living-sensory" className="rounded-lg bg-[#9aa6b2]/8 border border-[#9aa6b2]/25 p-3 space-y-1">
              <p className="text-[10px] font-mono uppercase tracking-widest text-[#9aa6b2]">{tri("Predizione sensoriale", "Sensorik-Vorhersage", "Sensory prediction", "Predicción sensorial", "Prédiction sensorielle", "پیش‌بینی حسی")}</p>
