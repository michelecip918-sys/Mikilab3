...
export const DeckAlarmBar = ({ tri, refreshKey }) => {
  ...
  return (
    <div data-testid="deck-alarm-bar" className="mb-6 rounded-xl border border-border/25 bg-background/70 backdrop-blur px-3 py-2.5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <button data-testid="alarm-history-toggle" onClick={() => setOpen((v) => !v)} className="flex items-center gap-2 text-foreground hover:text-foreground transition-colors">
          ...
          <span data-testid="alarm-count" className={`text-xs font-mono-data px-2 py-0.5 rounded-full ${items.length ? "bg-mattone/20 text-mattone" : "bg-accent/15 text-accent"}`}>{items.length}</span>
          {open ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
        </button>
        <div className="flex items-center gap-2">
          <button data-testid="alarm-export-btn" onClick={onExport} className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-full border border-border/40 text-foreground hover:border-border hover:text-muted-foreground transition-colors active:scale-95">
            <Download size={14} /> {tri("Esporta", "Export", "Export", "Exportar", "Exporter", "خروجی")}
          </button>
          <button data-testid="alarm-push-btn" onClick={onEnable} className={`flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-full border transition-colors active:scale-95 ${pushOn ? "border-accent/50 text-accent" : "border-border/50 text-muted-foreground hover:bg-muted/10"}`}>
      ...
      {open && (
        <div data-testid="alarm-timeline" className="mt-3 pt-3 border-t border-border/20 space-y-1.5 max-h-52 overflow-y-auto">
          {items.length === 0 ? (
            <p className="text-xs text-muted-foreground py-2">{tri("Nessun allarme critico oggi. Turno regolare.", "Heute keine kritischen Alarme.", "No critical alarms today. Smooth shift.", "Sin alarmas críticas hoy.", "Aucune alarme critique aujourd'hui.", "امروز هشدار بحرانی نبود.")}</p>
          ) : items.map((it, i) => (
            <div key={i} data-testid={`alarm-row-${i}`} className="flex items-center gap-2.5 text-xs">
              <AlertTriangle size={14} className="text-mattone shrink-0" />
              <span className="font-mono-data text-muted-foreground font-bold shrink-0">{it.hm}</span>
              <span className="text-foreground truncate">
                {(it.departments || []).map((d) => DEPT_IT[d] || d).join(", ") || "—"}
                {it.stations && it.stations.length ? <span className="text-muted-foreground"> · {it.stations.join(", ")}</span> : null}
              </span>
              <span className="ml-auto font-mono-data text-muted-foreground shrink-0">{it.heartbeat} BPM</span>
            </div>
          ))}
        </div>
      )}
