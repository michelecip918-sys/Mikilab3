    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-[#0E1620] dark:bg-[#0E1620]">
      <div className="w-full max-w-sm relative" data-testid="auth-screen">
        ...
        <div className="text-center mb-6">
          <img src={`${process.env.PUBLIC_URL}/logo.png`} alt="MikiLab" className="w-20 h-20 rounded-2xl object-cover ring-2 ring-[#3E9C93]/70 shadow-lg mx-auto mb-3" />
        ...
        {info && (
          <div data-testid="auth-info" className="mb-4 rounded-2xl bg-[#3E9C93]/10 border border-[#3E9C93]/30 p-4 text-center">
            <Mail className="w-7 h-7 text-[#3E9C93] mx-auto mb-1.5" />
            ...
            {needVerify && (
              <button type="button" data-testid="auth-resend-verify" onClick={resendVerify}
                className="mt-2 text-xs font-semibold text-[#3E9C93] underline">
        ...
        {mode !== "forgot" && (
          <>
            ...
            <div className="flex items-center gap-3 mb-4">
              <span className="flex-1 h-px bg-[#2A3B49] dark:bg-[#2A3B49]" />
        ...
        {mode === "forgot" && sent ? (
          <div data-testid="forgot-sent" className="rounded-2xl bg-[#3E9C93]/10 border border-[#3E9C93]/30 p-5 text-center">
            <Mail className="w-8 h-8 text-[#3E9C93] mx-auto mb-2" />
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          {mode === "register" && (
            <>
              <p className={`text-xs -mt-1 ${password ? (pwStrong ? "text-[#3E7C59]" : "text-[#3E9C93]") : "text-[#7E8A93]"}`}>
          ...
          {mode === "login" && (
            <button type="button" data-testid="auth-forgot-link" onClick={() => { setMode("forgot"); setSent(false); }}
              className="block w-full text-right text-xs text-[#7E8A93] hover:text-[#3E9C93] -mt-1">
              {T.forgot}
            </button>
          )}
...
function Field({ icon, children }) {
  return (
    <div className="flex items-center gap-2 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl px-4 py-3">
      <span className="text-[#3E9C93]">{icon}</span>
      {children}
    </div>
  );
}
