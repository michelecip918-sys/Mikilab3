...
const COLORS = ["#3f7cac", "#5aa0cf", "#2e8b6f", "#C88A2B", "#a05eb5", "#C0574D", "#6E8CA0"];
...
export default function RecipeTimeline({ recipe, lang: langProp }) {
  ...
  const inp = "bg-[#f0f6fb] dark:bg-[#1F252B] border border-[#d5e4f0] dark:border-[#38424B] rounded-lg px-2 py-1.5 outline-none text-sm font-mono-data text-[#2B303B] dark:text-[#e4eff8] focus:border-[#3f7cac]";

  if (phases.length < 2) {
    return (
      <div data-testid={`recipe-timeline-${recipe.id}`} className="rounded-xl bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] p-4">
        <p className="text-[11px] font-bold uppercase tracking-wide text-[#3f7cac] flex items-center gap-1 mb-2"><Clock className="w-3.5 h-3.5" /> {tri("Linea del tempo", "Zeitplan", "Timeline", "Línea de tiempo")}</p>
  ...
  return (
    <div data-testid={`recipe-timeline-${recipe.id}`} className="rounded-xl bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] p-4">
      <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
        <p className="text-[11px] font-bold uppercase tracking-wide text-[#3f7cac] flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {tri("Linea del tempo", "Zeitplan", "Timeline", "Línea de tiempo")}</p>
      ...
      <p className="text-xs text-[#7E8A93] mb-3">{tri("Inizia a impastare alle", "Beginne zu kneten um", "Start mixing at", "Empieza a amasar a las")} <b data-testid={`timeline-start-${recipe.id}`} className="text-[#234b6e] dark:text-[#a9d2ec] font-mono-data">{rows.startClock}</b> · {tri("durata totale", "Gesamtdauer", "total", "duración total")} {durStr(totalMins)}</p>
      <div className="relative pl-4">
        <div className="absolute left-[6px] top-1 bottom-1 w-0.5 bg-[#d5e4f0] dark:bg-[#38424B]" />
        {rows.out.map((p) => (
          <div key={p.i} data-testid={`timeline-phase-${recipe.id}-${p.i}`} className="relative mb-3 last:mb-0">
            ...
            <div className="flex items-baseline justify-between gap-2">
              ...
              <span className="font-mono-data text-xs text-[#234b6e] dark:text-[#8FB0C2] shrink-0">{p.start} → {p.end}</span>
            </div>
