...
export default function SiteMenu({ onNavigate, onOpenSfide, tab }) {
  ...
  const SECTIONS = [
    { id: "home", Icon: HomeIcon, label: tri("Home", "Home", "Home", "Inicio"), color: "#F26419" },
    { id: "ricette", Icon: BookOpen, label: tri("Le Ricette di MikiLab", "Die MikiLab-Rezepte", "The MikiLab Recipes", "Las Recetas de MikiLab"), color: "#F26419" },
    { id: "maestro", Icon: Wrench, label: tri("Il Tuo Laboratorio", "Dein Labor", "Your Lab", "Tu Laboratorio"), color: "#F26419" },
    { id: "imparacon", Icon: GraduationCap, label: tri("Scienza & Guide", "Wissen & Guides", "Science & Guides", "Ciencia y Guías"), color: "#A16207" },
    { id: "community", Icon: Users, label: tri("Community", "Community", "Community", "Comunidad"), color: "#F26419" },
  ];
  const SOCIAL = [
    { v: "feed", Icon: Rss, label: tri("Bacheca (Feed)", "Pinnwand (Feed)", "Feed", "Muro (Feed)"), color: "#18202E" },
    { v: "friends", Icon: UserPlus, label: tri("Amici & Colleghi", "Freunde & Kollegen", "Friends & Colleagues", "Amigos y Colegas"), color: "#F26419" },
    { v: "messages", Icon: MessageCircle, label: tri("Messaggi", "Nachrichten", "Messages", "Mensajes"), color: "#F26419" },
    { v: "map", Icon: MapPin, label: tri("Mappa dei Fornai", "Bäcker-Karte", "Bakers Map", "Mapa de Panaderos"), color: "#2e8b6f" },
    { v: "profile", Icon: User, label: tri("Il mio profilo", "Mein Profil", "My profile", "Mi perfil"), color: "#F26419" },
  ];
  const FEED_ORDER = [
    ...
    return (
      <div className="flex items-center gap-1">
        <button data-testid={testid || `site-menu-tool-${tl.id}`} onClick={() => goTool(tl.id)}
          className="flex-1 flex items-center gap-2.5 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] active:scale-98 hover:border-[#F26419]/60 transition-all min-w-0">
        ...
        <button data-testid={`site-menu-fav-${tl.id}`} onClick={() => toggleFav(tl.id)} aria-label="Preferito"
          className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0 active:scale-90 transition-all border border-[#26324A] dark:border-[#26324A] bg-white dark:bg-[#18202E]">
  ...
  return createPortal(
    <div data-testid="site-menu" className="fixed inset-0 z-[300]" onClick={() => setOpen(false)}>
      ...
      <div onClick={(e) => e.stopPropagation()}
        className="absolute top-0 left-0 h-full w-[88%] max-w-sm bg-[#0B0E14] dark:bg-[#1A1F24] shadow-2xl overflow-y-auto animate-in slide-in-from-left duration-300">
        <div className="sticky top-0 z-10 bg-[#F26419] text-white">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 mb-4">
                {FEED_ORDER.map((f) => (
                  <button key={f.o} data-testid={`site-menu-feed-${f.o}`} onClick={() => goFeed(f.o)}
                    className="flex flex-col items-center gap-1 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] active:scale-95 hover:border-[#F26419]/60 transition-all">
                    <f.Icon className="w-4 h-4 text-[#F26419]" /><span className="text-[12px] font-semibold text-[#2B303B] dark:text-[#e4eff8]">{f.label}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 gap-1.5">
                {SOCIAL.map((s) => (
                  <button key={s.v} data-testid={`site-menu-social-${s.v}`} onClick={() => goSocial(s.v)}
                    className="flex items-center gap-3 text-left px-3 py-3 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] active:scale-98 hover:border-[#F26419]/60 transition-all">
          ...
          <div className="grid grid-cols-1 gap-1.5">
            {SECTIONS.map((s) => (
              <button key={s.id} data-testid={`site-menu-section-${s.id}`} onClick={() => goTab(s.id)}
                className={`flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 border active:scale-98 transition-all ${tab === s.id ? "bg-[#F26419]/10 border-[#F26419]/40" : "bg-white dark:bg-[#18202E] border-[#26324A] dark:border-[#26324A] hover:border-[#F26419]/60"}`}>
                ...
                {tab === s.id && <span className="ml-auto text-[10px] font-bold text-[#F26419] bg-[#F26419]/15 px-2 py-0.5 rounded-full">{tri("qui", "hier", "here", "aquí")}</span>}
              </button>
            ))}
            <button data-testid="site-menu-section-sfide" onClick={goSfide}
              className="flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-gradient-to-br from-[#F26419] to-[#F26419] text-white active:scale-98 transition-all">
            ...
            <button data-testid="site-menu-guida" onClick={() => { setOpen(false); window.dispatchEvent(new Event("mikilab-open-guida")); }}
              className="flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#18202E] border border-[#F26419]/40 active:scale-98 hover:border-[#F26419]/70 transition-all">
              <span className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 bg-[#F26419]/15"><BookOpenCheck className="w-4 h-4 text-[#F26419]" /></span>
            ...
            <div data-testid="site-menu-voice" className="mt-1 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#18202E] border border-[#F26419]/40 p-2.5">
              <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-[#7E8A93] mb-1.5 px-0.5"><Mic className="w-3.5 h-3.5 text-[#F26419]" /> {tri("Voce assistente", "Assistenten-Stimme", "Assistant voice", "Voz del asistente")}</p>
              <div className="grid grid-cols-2 gap-1.5 p-1 rounded-lg bg-[#e4eff8] dark:bg-[#0B0E14]">
                {[
                  ...
                  <button key={v.id} data-testid={`site-menu-voice-${v.id}`} onClick={() => choosePersona(v.id)}
                    className={`py-2 px-1 rounded-md text-[13px] font-bold leading-tight transition-all ${persona === v.id ? "bg-[#F26419] text-white shadow" : "text-[#7E8A93] hover:text-[#F26419]"}`}>
            ...
            <div data-testid="site-menu-security" className="mt-1 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#18202E] border border-[#F26419]/40 p-2.5">
              <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-[#7E8A93] mb-1.5 px-0.5"><Lock className="w-3.5 h-3.5 text-[#F26419]" /> {tri("Sicurezza · PIN", "Sicherheit · PIN", "Security · PIN", "Seguridad · PIN")}</p>
              <div className="flex items-center justify-between gap-2">
                ...
                <button data-testid="site-menu-pin-toggle" onClick={togglePin} role="switch" aria-checked={pinOn}
                  className={`w-11 h-6 rounded-full transition-all relative ${pinOn ? "bg-[#F26419]" : "bg-[#c7d2da] dark:bg-[#333]"}`}>
              ...
              {pinOn && (
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  {!pinEditing ? (
                    <>
                      <button data-testid="site-menu-pin-change" onClick={() => { setPinEditing(true); setPinMsg(""); }} className="text-[12px] font-bold px-2.5 py-1.5 rounded-lg bg-[#F26419]/12 text-[#F26419]">{tri("Cambia PIN", "PIN ändern", "Change PIN", "Cambiar PIN")}</button>
                      <button data-testid="site-menu-lock-now" onClick={lockNowBtn} className="text-[12px] font-bold px-2.5 py-1.5 rounded-lg bg-[#18202E] text-[#E8A838] border border-[#26324A]">{tri("Blocca ora", "Jetzt sperren", "Lock now", "Bloquear ahora")}</button>
                    </>
                  ) : (
                    <>
                      <input data-testid="site-menu-pin-input" value={newPin} onChange={(e) => setNewPin(e.target.value.replace(/\D/g, "").slice(0, 4))}
                        ...
                        className="w-20 text-center tracking-[6px] font-extrabold text-[16px] py-1.5 rounded-lg bg-[#f0f4f8] dark:bg-[#0B0E14] border border-[#F26419]/40 text-[#2b3640] dark:text-white" />
                      <button data-testid="site-menu-pin-save" onClick={savePin} className="text-[12px] font-bold px-2.5 py-1.5 rounded-lg bg-[#F26419] text-white">{tri("Salva", "Speichern", "Save", "Guardar")}</button>
            ...
            <div data-testid="site-menu-mode" className="mt-1 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#18202E] border border-[#F26419]/40 p-2.5">
              ...
              <div className="grid grid-cols-2 gap-1.5 p-1 rounded-lg bg-[#e4eff8] dark:bg-[#0B0E14]">
                {[
                  ...
                  <button key={m.id} data-testid={`site-menu-mode-${m.id}`} onClick={() => chooseProfile(m.id)}
                    className={`py-2 rounded-md text-[13px] font-bold transition-all ${profile === m.id ? "bg-[#F26419] text-white shadow" : "text-[#7E8A93] hover:text-[#F26419]"}`}>
                    {m.label}
                  </button>
                ))}
              </div>
            ...
            {user?.role === "admin" && (
              <button data-testid="site-menu-admin" onClick={() => { setOpen(false); window.dispatchEvent(new Event("mikilab-open-admin")); }}
                className="flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#18202E] border border-[#F26419]/40 active:scale-98 hover:border-[#F26419]/70 transition-all">
                <span className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 bg-[#F26419]/15"><Shield className="w-4 h-4 text-[#F26419]" /></span>
                <span className="font-display text-sm font-semibold text-[#F26419]">{tri("Pannello Admin", "Admin-Panel", "Admin Panel", "Panel Admin")}</span>
              </button>
            )}
          </div>
        </div>
