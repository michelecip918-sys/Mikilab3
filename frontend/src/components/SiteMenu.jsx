...
export default function SiteMenu({ onNavigate, onOpenSfide, tab }) {
  ...
  const SECTIONS = [
    ...
    { id: "ricette", Icon: BookOpen, label: tri("Le Ricette di MikiLab", "Die MikiLab-Rezepte", "The MikiLab Recipes", "Las Recetas de MikiLab"), color: "#3E9C93" },
    { id: "maestro", Icon: Wrench, label: tri("Il Tuo Laboratorio", "Dein Labor", "Your Lab", "Tu Laboratorio"), color: "#3E9C93" },
  ...
  const SOCIAL = [
    { v: "feed", Icon: Rss, label: tri("Bacheca (Feed)", "Pinnwand (Feed)", "Feed", "Muro (Feed)"), color: "#1B2A38" },
    { v: "friends", Icon: UserPlus, label: tri("Amici & Colleghi", "Freunde & Kollegen", "Friends & Colleagues", "Amigos y Colegas"), color: "#3E9C93" },
    { v: "messages", Icon: MessageCircle, label: tri("Messaggi", "Nachrichten", "Messages", "Mensajes"), color: "#3E9C93" },
    { v: "map", Icon: MapPin, label: tri("Mappa dei Fornai", "Bäcker-Karte", "Bakers Map", "Mapa de Panaderos"), color: "#2e8b6f" },
    { v: "profile", Icon: User, label: tri("Il mio profilo", "Mein Profil", "My profile", "Mi perfil"), color: "#3E9C93" },
  ];
  const FEED_ORDER = [
    ...
    return (
      <div className="flex items-center gap-1">
        <button data-testid={testid || `site-menu-tool-${tl.id}`} onClick={() => goTool(tl.id)}
          className="flex-1 flex items-center gap-2.5 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] active:scale-98 hover:border-[#3E9C93]/60 transition-all min-w-0">
        ...
        <button data-testid={`site-menu-fav-${tl.id}`} onClick={() => toggleFav(tl.id)} aria-label="Preferito"
          className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0 active:scale-90 transition-all border border-[#2A3B49] dark:border-[#2A3B49] bg-white dark:bg-[#1B2A38]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 mb-4">
                {FEED_ORDER.map((f) => (
                  <button key={f.o} data-testid={`site-menu-feed-${f.o}`} onClick={() => goFeed(f.o)}
                    className="flex flex-col items-center gap-1 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] active:scale-95 hover:border-[#3E9C93]/60 transition-all">
                    <f.Icon className="w-4 h-4 text-[#3E9C93]" /><span className="text-[12px] font-semibold text-[#2B303B] dark:text-[#e4eff8]">{f.label}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 gap-1.5">
                {SOCIAL.map((s) => (
                  <button key={s.v} data-testid={`site-menu-social-${s.v}`} onClick={() => goSocial(s.v)}
                    className="flex items-center gap-3 text-left px-3 py-3 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] active:scale-98 hover:border-[#3E9C93]/60 transition-all">
          ...
          <div className="grid grid-cols-1 gap-1.5">
            {SECTIONS.map((s) => (
              <button key={s.id} data-testid={`site-menu-section-${s.id}`} onClick={() => goTab(s.id)}
                className={`flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 border active:scale-98 transition-all ${tab === s.id ? "bg-[#3E9C93]/10 border-[#3E9C93]/40" : "bg-white dark:bg-[#1B2A38] border-[#2A3B49] dark:border-[#2A3B49] hover:border-[#3E9C93]/60"}`}>
                ...
                {tab === s.id && <span className="ml-auto text-[10px] font-bold text-[#3E9C93] bg-[#3E9C93]/15 px-2 py-0.5 rounded-full">{tri("qui", "hier", "here", "aquí")}</span>}
              </button>
            ))}
            <button data-testid="site-menu-section-sfide" onClick={goSfide}
              className="flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] text-white active:scale-98 transition-all">
            ...
            <button data-testid="site-menu-guida" onClick={() => { setOpen(false); window.dispatchEvent(new Event("mikilab-open-guida")); }}
              className="flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#3E9C93]/40 active:scale-98 hover:border-[#3E9C93]/70 transition-all">
              ...
              <div className="grid grid-cols-2 gap-1.5 p-1 rounded-lg bg-[#e4eff8] dark:bg-[#0E1620]">
                {[
                  ...
                  <button key={v.id} data-testid={`site-menu-voice-${v.id}`} onClick={() => choosePersona(v.id)}
                    className={`py-2 px-1 rounded-md text-[13px] font-bold leading-tight transition-all ${persona === v.id ? "bg-[#3E9C93] text-white shadow" : "text-[#7E8A93] hover:text-[#3E9C93]"}`}>
            ...
            <div data-testid="site-menu-security" className="mt-1 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#3E9C93]/40 p-2.5">
              <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-[#7E8A93] mb-1.5 px-0.5"><Lock className="w-3.5 h-3.5 text-[#3E9C93]" /> {tri("Sicurezza · PIN", "Sicherheit · PIN", "Security · PIN", "Seguridad · PIN")}</p>
              <div className="flex items-center justify-between gap-2">
                ...
                <button data-testid="site-menu-pin-toggle" onClick={togglePin} role="switch" aria-checked={pinOn}
                  className={`w-11 h-6 rounded-full transition-all relative ${pinOn ? "bg-[#3E9C93]" : "bg-[#c7d2da] dark:bg-[#333]"}`}>
              ...
              <div className="grid grid-cols-2 gap-1.5 p-1 rounded-lg bg-[#e4eff8] dark:bg-[#0E1620]">
                {[
                  ...
                  <button key={m.id} data-testid={`site-menu-mode-${m.id}`} onClick={() => chooseProfile(m.id)}
                    className={`py-2 rounded-md text-[13px] font-bold transition-all ${profile === m.id ? "bg-[#3E9C93] text-white shadow" : "text-[#7E8A93] hover:text-[#3E9C93]"}`}>
                    {m.label}
                  </button>
                ))}
              </div>
            ...
            {user?.role === "admin" && (
              <button data-testid="site-menu-admin" onClick={() => { setOpen(false); window.dispatchEvent(new Event("mikilab-open-admin")); }}
                className="flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#3E9C93]/40 active:scale-98 hover:border-[#3E9C93]/70 transition-all">
                <span className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 bg-[#3E9C93]/15"><Shield className="w-4 h-4 text-[#3E9C93]" /></span>
                <span className="font-display text-sm font-semibold text-[#3E9C93]">{tri("Pannello Admin", "Admin-Panel", "Admin Panel", "Panel Admin")}</span>
              </button>
            )}
          </div>
        </div>
