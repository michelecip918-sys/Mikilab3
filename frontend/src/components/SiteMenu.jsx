...
export default function SiteMenu({ onNavigate, onOpenSfide, tab }) {
  ...
  const SECTIONS = [
    { id: "home", Icon: HomeIcon, label: tri("Home", "Home", "Home", "Inicio"), color: "#c94f00" },
    { id: "ricette", Icon: BookOpen, label: tri("Le Ricette di MikiLab", "Die MikiLab-Rezepte", "The MikiLab Recipes", "Las Recetas de MikiLab"), color: "#c94f00" },
    { id: "maestro", Icon: Wrench, label: tri("Il Tuo Laboratorio", "Dein Labor", "Your Lab", "Tu Laboratorio"), color: "#c94f00" },
    { id: "imparacon", Icon: GraduationCap, label: tri("Scienza & Guide", "Wissen & Guides", "Science & Guides", "Ciencia y Guías"), color: "#A16207" },
    { id: "community", Icon: Users, label: tri("Community", "Community", "Community", "Comunidad"), color: "#c94f00" },
  ];
  const SOCIAL = [
    { v: "feed", Icon: Rss, label: tri("Bacheca (Feed)", "Pinnwand (Feed)", "Feed", "Muro (Feed)"), color: "#1e1e1e" },
    { v: "friends", Icon: UserPlus, label: tri("Amici & Colleghi", "Freunde & Kollegen", "Friends & Colleagues", "Amigos y Colegas"), color: "#c94f00" },
    { v: "messages", Icon: MessageCircle, label: tri("Messaggi", "Nachrichten", "Messages", "Mensajes"), color: "#c94f00" },
    { v: "map", Icon: MapPin, label: tri("Mappa dei Fornai", "Bäcker-Karte", "Bakers Map", "Mapa de Panaderos"), color: "#2e8b6f" },
    { v: "profile", Icon: User, label: tri("Il mio profilo", "Mein Profil", "My profile", "Mi perfil"), color: "#c94f00" },
  ];
  const FEED_ORDER = [
    ...
    return (
      <div className="flex items-center gap-1">
        <button data-testid={testid || `site-menu-tool-${tl.id}`} onClick={() => goTool(tl.id)}
          className="flex-1 flex items-center gap-2.5 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] active:scale-98 hover:border-[#c94f00]/60 transition-all min-w-0">
  ...
  return createPortal(
    <div data-testid="site-menu" className="fixed inset-0 z-[300]" onClick={() => setOpen(false)}>
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="sticky top-0 z-10 bg-[#c94f00] text-white">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 mb-4">
                {FEED_ORDER.map((f) => (
                  <button key={f.o} data-testid={`site-menu-feed-${f.o}`} onClick={() => goFeed(f.o)}
                    className="flex flex-col items-center gap-1 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] active:scale-95 hover:border-[#c94f00]/60 transition-all">
                    <f.Icon className="w-4 h-4 text-[#c94f00]" /><span className="text-[12px] font-semibold text-[#2B303B] dark:text-[#e4eff8]">{f.label}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 gap-1.5">
                {SOCIAL.map((s) => (
                  <button key={s.v} data-testid={`site-menu-social-${s.v}`} onClick={() => goSocial(s.v)}
                    className="flex items-center gap-3 text-left px-3 py-3 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] active:scale-98 hover:border-[#c94f00]/60 transition-all">
          ...
          <div className="grid grid-cols-1 gap-1.5">
            {SECTIONS.map((s) => (
              <button key={s.id} data-testid={`site-menu-section-${s.id}`} onClick={() => goTab(s.id)}
                className={`flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 border active:scale-98 transition-all ${tab === s.id ? "bg-[#c94f00]/10 border-[#c94f00]/40" : "bg-white dark:bg-[#1e1e1e] border-[#2e2e2e] dark:border-[#2e2e2e] hover:border-[#c94f00]/60"}`}>
                ...
                {tab === s.id && <span className="ml-auto text-[10px] font-bold text-[#c94f00] bg-[#c94f00]/15 px-2 py-0.5 rounded-full">{tri("qui", "hier", "here", "aquí")}</span>}
              </button>
            ))}
            <button data-testid="site-menu-section-sfide" onClick={goSfide}
              className="flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-gradient-to-br from-[#c94f00] to-[#c94f00] text-white active:scale-98 transition-all">
            ...
            <button data-testid="site-menu-guida" onClick={() => { setOpen(false); window.dispatchEvent(new Event("mikilab-open-guida")); }}
              className="flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1e1e1e] border border-[#c94f00]/40 active:scale-98 hover:border-[#c94f00]/70 transition-all">
              <span className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 bg-[#c94f00]/15"><BookOpenCheck className="w-4 h-4 text-[#c94f00]" /></span>
            ...
            <div data-testid="site-menu-voice" className="mt-1 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1e1e1e] border border-[#c94f00]/40 p-2.5">
              <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-[#7E8A93] mb-1.5 px-0.5"><Mic className="w-3.5 h-3.5 text-[#c94f00]" /> {tri("Voce assistente", "Assistenten-Stimme", "Assistant voice", "Voz del asistente")}</p>
              <div className="grid grid-cols-2 gap-1.5 p-1 rounded-lg bg-[#e4eff8] dark:bg-[#121212]">
                {[
                  ...
                  <button key={v.id} data-testid={`site-menu-voice-${v.id}`} onClick={() => choosePersona(v.id)}
                    className={`py-2 px-1 rounded-md text-[13px] font-bold leading-tight transition-all ${persona === v.id ? "bg-[#c94f00] text-white shadow" : "text-[#7E8A93] hover:text-[#c94f00]"}`}>
            ...
            <div data-testid="site-menu-security" className="mt-1 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1e1e1e] border border-[#c94f00]/40 p-2.5">
              <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-[#7E8A93] mb-1.5 px-0.5"><Lock className="w-3.5 h-3.5 text-[#c94f00]" /> {tri("Sicurezza · PIN", "Sicherheit · PIN", "Security · PIN", "Seguridad · PIN")}</p>
              <div className="flex items-center justify-between gap-2">
                ...
                <button data-testid="site-menu-pin-toggle" onClick={togglePin} role="switch" aria-checked={pinOn}
                  className={`w-11 h-6 rounded-full transition-all relative ${pinOn ? "bg-[#c94f00]" : "bg-[#c7d2da] dark:bg-[#333]"}`}>
              ...
              {pinOn && (
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  {!pinEditing ? (
                    <>
                      <button data-testid="site-menu-pin-change" onClick={() => { setPinEditing(true); setPinMsg(""); }} className="text-[12px] font-bold px-2.5 py-1.5 rounded-lg bg-[#c94f00]/12 text-[#c94f00]">{tri("Cambia PIN", "PIN ändern", "Change PIN", "Cambiar PIN")}</button>
                    ...
                    <>
                      <input data-testid="site-menu-pin-input" value={newPin} onChange={(e) => setNewPin(e.target.value.replace(/\D/g, "").slice(0, 4))}
                        ...
                        className="w-20 text-center tracking-[6px] font-extrabold text-[16px] py-1.5 rounded-lg bg-[#f0f4f8] dark:bg-[#121212] border border-[#c94f00]/40 text-[#2b3640] dark:text-white" />
                      <button data-testid="site-menu-pin-save" onClick={savePin} className="text-[12px] font-bold px-2.5 py-1.5 rounded-lg bg-[#c94f00] text-white">{tri("Salva", "Speichern", "Save", "Guardar")}</button>
            ...
            <div data-testid="site-menu-mode" className="mt-1 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1e1e1e] border border-[#c94f00]/40 p-2.5">
              ...
              <div className="grid grid-cols-2 gap-1.5 p-1 rounded-lg bg-[#e4eff8] dark:bg-[#121212]">
                {[
                  ...
                  <button key={m.id} data-testid={`site-menu-mode-${m.id}`} onClick={() => chooseProfile(m.id)}
                    className={`py-2 rounded-md text-[13px] font-bold transition-all ${profile === m.id ? "bg-[#c94f00] text-white shadow" : "text-[#7E8A93] hover:text-[#c94f00]"}`}>
                    {m.label}
                  </button>
                ))}
              </div>
            ...
            {user?.role === "admin" && (
              <button data-testid="site-menu-admin" onClick={() => { setOpen(false); window.dispatchEvent(new Event("mikilab-open-admin")); }}
                className="flex items-center gap-3 text-left px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1e1e1e] border border-[#c94f00]/40 active:scale-98 hover:border-[#c94f00]/70 transition-all">
                <span className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 bg-[#c94f00]/15"><Shield className="w-4 h-4 text-[#c94f00]" /></span>
                <span className="font-display text-sm font-semibold text-[#c94f00]">{tri("Pannello Admin", "Admin-Panel", "Admin Panel", "Panel Admin")}</span>
              </button>
            )}
          </div>
        </div>
