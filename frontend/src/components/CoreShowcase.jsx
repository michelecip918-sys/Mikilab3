...
const CAPS = [
  ...
  { icon: Languages, tag: "i18n", color: "#3E9C93",
    ...
    b: (l) => mkTri(l)("UI, ricette e audio Miki-Nexus in 6 lingue, per una squadra internazionale.", "UI, Rezepte und Miki-Nexus-Audio in 6 Sprachen für ein internationales Team.", "UI, recipes and Miki-Nexus audio in 6 languages for an international team.", "UI, recetas y audio Miki-Nexus en 6 idiomas para un equipo internacional.", "UI, recettes et audio Miki-Nexus en 6 langues pour une équipe internationale.", "رابط، دستورها و صدای باکومیکس در ۶ زبان.") },
  { icon: Cpu, tag: "ai", color: "#64748B",
    t: (l) => mkTri(l)("Core Miki-Nexus", "Miki-Nexus-KI-Kern", "Miki-Nexus Core", "Núcleo Miki-Nexus IA", "Cœur Miki-Nexus IA", "هسته هوش باکومیکس"),
  ...
  { icon: ShieldCheck, tag: "pin", color: "#D95200",
...
export default function CoreShowcase() {
  ...
  return (
    <div data-testid="core-showcase" className="space-y-3">
      <div className="flex items-center gap-2 p-4 rounded-2xl bg-gradient-to-r from-[#D95200]/10 to-transparent border border-[#D95200]/30">
        <Sparkles className="w-5 h-5 text-[#D95200]" />
        <div>
          <h3 className="text-sm font-extrabold text-[#D95200]">{tri("Cosa fa MikiLab · Capacità chiave", "Was MikiLab kann · Kernfunktionen", "What MikiLab does · Core capabilities", "Qué hace MikiLab · Capacidades clave", "Ce que fait MikiLab · Capacités clés", "توانمندی‌های کلیدی میکی‌لب")}</h3>
        ...
        {caps.map((c, k) => {
          ...
          return (
            <motion.div
              ...
              className="rounded-2xl bg-[#0b0f19] border border-[#1e293b] p-4 hover:border-[#D95200]/50 transition-all group"
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-sm font-bold text-white group-hover:text-[#D95200] transition-colors">{c.t(lang)}</h4>
              </div>
