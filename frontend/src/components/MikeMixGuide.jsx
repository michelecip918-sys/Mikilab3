...
import MikeMixHardware from "@/components/MikeMixHardware";
...
// MikeMix: l'AI che "ha in testa" tutto MikiLab e spiega il sito passo-passo.
export default function MikeMixGuide() {
  const { lang } = useLang();
  ...
  const CH = [
    { icon: BookOpen, t: tri("Cos'è MikiLab", "Was ist MikiLab", "What is MikiLab", "Qué es MikiLab", "Qu'est-ce que MikiLab", "میکی‌لب چیست"),
      b: tri("MikiLab è il laboratorio digitale del panettiere. Io sono MikeMix: ho in testa tutto il sistema e ti guido passo-passo.", "MikiLab ist das digitale Labor des Bäckers. Ich bin MikeMix und führe dich Schritt für Schritt.", "MikiLab is the baker's digital lab. I'm MikeMix and I guide you step by step.", "MikiLab es el laboratorio digital del panadero. Soy MikeMix y te guío paso a paso.", "MikiLab est le laboratoire numérique du boulanger. Je suis MikeMix et je te guide pas à pas.", "میکی‌لب آزمایشگاه دیجیتال نانوا است. من بیک‌میکس هستم و قدم‌به‌قدم راهنمایی‌ات می‌کنم.") },
    ...
    { icon: Mic, t: tri("La produzione con Miki-Nexus", "Produktion mit Miki-Nexus", "Production with Miki-Nexus", "Producción con Miki-Nexus", "Production avec Miki-Nexus", "تولید با Miki-Nexus"),
      b: tri("In produzione tocchi l'avatar di Miki-Nexus: ti legge il piano del Capo a voce, passo dopo passo. Di' «avanti», «indietro», «ripeti».", "In der Produktion tippst du Miki-Nexus an: er liest den Plan vor. Sag «weiter», «zurück», «nochmal».", "In production you tap Miki-Nexus: he reads the Capo's plan aloud. Say «next», «back», «repeat».", "En producción tocas a Miki-Nexus: te lee el plan en voz alta. Di «siguiente», «atrás», «repite».", "En production tu touches Miki-Nexus : il lit le plan à voix haute. Dis «suivant», «précédent», «répète».", "در تولید روی Miki-Nexus می‌زنی: برنامه را با صدا می‌خواند. بگو «بعدی»، «قبلی»، «تکرار».") },
  ...
  return (
    <div data-testid="bakemix-guide" className="space-y-4">
      <div className="flex items-center gap-3 p-4 rounded-2xl bg-gradient-to-r from-[#06b6d4]/10 to-transparent border border-[#06b6d4]/30">
        <img src={`${PUB}/avatar_nexus.jpg`} alt="MikeMix" className="w-12 h-12 rounded-xl object-cover border border-[#06b6d4]/60" onError={(e) => { e.currentTarget.style.display = "none"; }} />
        <div className="min-w-0">
          <h3 className="text-sm font-extrabold text-[#06b6d4]">{tri("Miki-Nexus ti spiega tutto", "Miki-Nexus erklärt alles", "Miki-Nexus explains everything", "Miki-Nexus te explica todo", "Miki-Nexus explique tout", "Miki-Nexus همه‌چیز را توضیح می‌دهد")}</h3>
