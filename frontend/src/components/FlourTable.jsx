...
export default function FlourTable({ embedded = false }) {
  ...
  const th = "text-left text-[10px] font-bold uppercase tracking-wide text-[#5E8B7E] px-2 py-1.5";
  const td = "px-2 py-1.5 text-xs text-[#2B303B] dark:text-[#EAF0EC] border-t border-[#EAF0EC] dark:border-[#38424B]";

  const body = (
    <div className="space-y-4">
      <div>
        <p className="text-xs font-bold uppercase text-[#B34A26] mb-1.5">{tri("Farine — sigle nei 3 mercati", "Mehle — Codes in 3 Märkten", "Flours — codes in 3 markets")}</p>
        <div className="overflow-x-auto rounded-xl border border-[#D7E1DB] dark:border-[#38424B]">
          <table className="w-full border-collapse">
            <thead className="bg-[#F6F8F5] dark:bg-[#1F252B]">
            ...
            <tbody>
              {FLOURS.map((r, i) => (
                <tr key={i} className={i % 2 ? "bg-[#F6F8F5]/50 dark:bg-[#1F252B]/50" : ""}>
      ...
      <div>
        <p className="text-xs font-bold uppercase text-[#B34A26] mb-1.5">{tri("Abbreviazioni d'impasto", "Teig-Abkürzungen", "Dough abbreviations")}</p>
        <div className="overflow-x-auto rounded-xl border border-[#D7E1DB] dark:border-[#38424B]">
          <table className="w-full border-collapse">
            <thead className="bg-[#F6F8F5] dark:bg-[#1F252B]">
      ...
      <div className="rounded-xl bg-[#6E8CA0]/12 border border-[#6E8CA0]/30 p-3 text-xs text-[#33564E] dark:text-[#8FB0C2] leading-relaxed">
        ⚠️ {tri(
    ...
    return (
      <div data-testid="flour-table" className="pb-24">
        <div className="flex items-center gap-2 mb-3 text-[#5E8B7E]">
          ...
          <h1 className="font-display text-xl font-bold text-[#2B303B] dark:text-[#EAF0EC]">{tri("Tabella Farine & Sigle", "Mehl- & Kürzel-Tabelle", "Flour & Codes Table")}</h1>
        </div>
        {body}
      </div>
    );
  ...
  return (
    <div data-testid="flour-table" className="mb-4 rounded-2xl bg-white dark:bg-[#232A31] border border-[#D7E1DB] dark:border-[#38424B] overflow-hidden">
      <button data-testid="flour-table-toggle" onClick={() => setOpen((o) => !o)} className="w-full flex items-center gap-2 px-4 py-3 text-left">
        <Table2 className="w-5 h-5 text-[#5E8B7E]" />
        <span className="font-display text-sm font-bold text-[#2B303B] dark:text-[#EAF0EC] flex-1">{tri("Tabella Farine & Sigle (DE / IT / EN)", "Mehl- & Kürzel-Tabelle", "Flour & Codes Table")}</span>
