  ...
  return (
    <div className="rounded-xl bg-[#030712] border border-[#1e293b] overflow-hidden">
      <button data-testid={testid} onClick={() => setOpen((o) => !o)} className="w-full flex items-center justify-between gap-2 px-4 py-3 text-left">
        ...
        <ChevronDown className={`w-4 h-4 text-[#D95200] transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
  ...
  return (
    <div data-testid="guida-sos" className="space-y-5">
      <div className="p-5 rounded-2xl bg-[#0b0f19] border border-[#1e293b]">
        <h3 className="text-sm font-extrabold text-[#D95200] flex items-center gap-2"><ShieldCheck className="w-4 h-4" /> Il laboratorio MikiLab</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="p-4 rounded-xl bg-[#0b0f19] border border-[#1e293b]"><Sparkles className="w-5 h-5 text-[#D95200] mb-2" /><h4 className="text-xs font-bold text-white">AI a supporto</h4><p className="text-[11px] text-[#94A3B8] mt-1">Rigenera piani, guida in cuffia e risponde alle domande.</p></div>
        <div className="p-4 rounded-xl bg-[#0b0f19] border border-[#1e293b]"><Mic className="w-5 h-5 text-[#D95200] mb-2" /><h4 className="text-xs font-bold text-white">Voce ovunque</h4><p className="text-[11px] text-[#94A3B8] mt-1">Comandi vocali in ogni sezione, hands-free.</p></div>
        <div className="p-4 rounded-xl bg-[#0b0f19] border border-[#1e293b]"><ShieldCheck className="w-5 h-5 text-[#D95200] mb-2" /><h4 className="text-xs font-bold text-white">Dati protetti</h4><p className="text-[11px] text-[#94A3B8] mt-1">PIN e ruoli separati: Capo e Produzione.</p></div>
      </div>

      <div>
        <h3 className="text-sm font-bold text-white flex items-center gap-2 mb-2"><HelpCircle className="w-4 h-4 text-[#D95200]" /> Guida completa · SOS · FAQ</h3>
        <div className="space-y-2">
          {FAQ.map((f, i) => <Acc key={i} testid={`faq-${i}`} q={f.q} a={f.a} />)}
        </div>
      </div>

      <div className="p-4 rounded-xl bg-[#D95200]/5 border border-[#D95200]/30">
        <p className="text-xs text-[#cbd5e1]">Hai altre domande sul funzionamento del sito? Tocca l'<strong className="text-[#D95200]">assistente AI</strong> in basso a destra: risponde in tempo reale su impostazioni, pulsanti e funzioni.</p>
      </div>
    </div>
  );
}
