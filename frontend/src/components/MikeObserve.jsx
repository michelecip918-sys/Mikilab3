...
// FASE 10 · Ecosistema Autonomo — Sitor osserva la scelta dell'operatore, impara e allerta il Capo.
  ...
  return (
    <div data-testid="mike-observe" className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <div><h4 className="font-cyber text-sm font-black text-white uppercase tracking-wide">{tri("Sitor osserva", "Sitor beobachtet", "Sitor observes", "Sitor observa", "Sitor observe", "Sitor مشاهده می‌کند")}</h4>
        <p className="text-[10.5px] text-[#94A3B8]">{tri("Scrivi come hai lavorato: Sitor impara e, se serve, avvisa il Capo.", "Schreib, wie du gearbeitet hast: Sitor lernt und warnt ggf. den Chef.", "Write how you worked: Sitor learns and alerts the Capo if needed.", "Escribe cómo trabajaste: Sitor aprende y avisa al Capo.", "Écris comment tu as travaillé : Sitor apprend et alerte le Capo.", "بنویس چطور کار کردی: Sitor یاد می‌گیرد.")}</p></div>
      </div>
      ...
      <button data-testid="observe-submit" onClick={submit} disabled={busy || !action.trim()}
        ...
        {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />} {tri("Invia a Sitor", "An Sitor senden", "Send to Sitor", "Enviar a Sitor", "Envoyer à Sitor", "ارسال به Sitor")}
      </button>
      {res && (
