        ...
        {recipes.map((r, i) => {
          ...
          return (
            <motion.button
              ...
              className="group relative overflow-hidden rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] shadow-sm active:scale-[0.98] hover:border-[#c94f00]/60 transition-all text-left"
            >
              ...
              <div className="p-3">
                ...
                {rLoc(r, "real_name", lang) && rLoc(r, "real_name", lang).toLowerCase() !== (rLoc(r, "name", lang) || "").toLowerCase() ? (
                  <p className="text-[11px] font-medium text-[#c94f00] truncate mt-0.5">{rLoc(r, "real_name", lang)}</p>
                ) : null}
                {r.flour_type ? (
