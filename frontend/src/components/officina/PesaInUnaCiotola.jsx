...
export default function PesaInUnaCiotola({ r, dough, lang, t }) {
  ...
  if (steps.length === 0) return <p className="text-[13px] text-muted-foreground">{tri("Questa ricetta non ha dosi in grammi.", "Dieses Rezept hat keine Grammangaben.", "This recipe has no gram amounts.")}</p>;
    ...
    <div data-testid="pesa-ciotola" className="space-y-3">
        ...
        {dough.biga && (
          <div className="flex gap-1">
            {[[1, dough.biga.kind === "poolish" ? tri("Fase 1 · Poolish", "Phase 1 · Poolish", "Phase 1 · Poolish") : tri("Fase 1 · Biga", "Phase 1 · Biga", "Phase 1 · Biga")], [2, tri("Fase 2 · Impasto", "Phase 2 · Teig", "Phase 2 · Dough")]].map(([p, l]) => (
              <button key={p} data-testid={`pc-phase-${p}`} onClick={() => setPhase(p)} className={`text-[12px] font-bold px-2.5 py-1 rounded-full border active:scale-95 ${phase === p ? "bg-primary text-white border-primary" : "bg-card text-muted-foreground border-border"}`}>{l}</button>
            ))}
          </div>
        )}
        <label className="inline-flex items-center gap-1.5 text-[12px] text-muted-foreground ml-auto"><input data-testid="pc-tare" type="checkbox" checked={tare} onChange={(e) => setTare(e.target.checked)} /> {tri("Faccio la tara ogni volta", "Ich tariere jedes Mal", "I tare every time")}</label>
        <button data-testid="pc-voice" onClick={() => setVoice((v) => !v)} className={`inline-flex items-center gap-1 text-[12px] font-bold px-2.5 py-1 rounded-full border active:scale-95 ${voice ? "bg-salvia text-white border-salvia" : "bg-card text-muted-foreground border-border"}`}><Volume2 className="w-3.5 h-3.5" /> {tri("Voce", "Stimme", "Voice")}</button>
      </div>
      ...
      {!finished ? (
        <div data-testid="pc-step" className="rounded-2xl border border-salvia/50 bg-salvia/10 p-4">
          ...
          <p className="text-[13px] text-muted-foreground mt-1">{tri("Aggiungi", "Gib dazu", "Add")}</p>
          ...
          {topUps.length > 0 && (
            <p data-testid="pc-topup" className="text-[12.5px] text-foreground mt-2 rounded-lg bg-background border border-border p-2">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            {BIG.has(cur.kind) && (
              editing === cur.id ? (
                ...
                <button data-testid="pc-more" onClick={() => setEditing(cur.id)} className="text-[12.5px] font-semibold text-muted-foreground underline decoration-dotted px-1 active:scale-95">{tri("Ne ho messo di più…", "Ich habe mehr genommen…", "I put more…")}</button>
              )
            )}
            {idx > 0 && <button data-testid="pc-back" onClick={() => setIdx(idx - 1)} className="text-[12.5px] font-semibold text-muted-foreground px-1 active:scale-95">{tri("Indietro", "Zurück", "Back")}</button>}
          </div>
        ...
        <div data-testid="pc-finished" className="rounded-2xl border border-salvia/50 bg-salvia/10 p-4">
          ...
          {factor > 1.005 && <p className="text-[12.5px] text-muted-foreground mt-1">{tri(`Hai fatto un impasto del ${Math.round((factor - 1) * 100)} % più grande, con le stesse proporzioni della ricetta.`, `Du hast einen um ${Math.round((factor - 1) * 100)} % größeren Teig gemacht, mit denselben Verhältnissen wie im Rezept.`, `You made a dough ${Math.round((factor - 1) * 100)} % bigger, with the same proportions as the recipe.`)}</p>}
          <button data-testid="pc-restart" onClick={() => { setIdx(0); setActual({}); }} className="inline-flex items-center gap-1 mt-3 text-[12.5px] font-semibold text-muted-foreground active:scale-95"><RotateCcw className="w-3.5 h-3.5" /> {tri("Ricomincia", "Nochmal", "Start again")}</button>
        </div>
      )}

      <ol className="space-y-0.5">
        {steps.map((s, i) => (
          <li key={s.id} className={`flex items-center justify-between text-[12.5px] px-1 ${i === idx ? "text-foreground font-semibold" : i < idx ? "text-muted-foreground line-through" : "text-muted-foreground"}`}>
        ...
        <li className="flex items-center justify-between text-[12.5px] px-1 pt-1 border-t border-border/60 text-muted-foreground"><span>{tri("Totale", "Gesamt", "Total")}</span><span className="font-mono-data font-bold text-foreground">{total} g</span></li>
      </ol>
