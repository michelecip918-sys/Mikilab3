...
export default function DisegnaIlTaglio({ r, lang }) {
  ...
  return (
    <div data-testid="disegna-il-taglio" className="space-y-3">
      <p className="text-[12.5px] text-foreground/85 leading-snug">{tri("Il taglio è la valvola del pane: decide dove si apre. Scegli la forma e lo schema, poi guarda come viene dopo il forno.", "Der Schnitt ist das Ventil des Brotes: er entscheidet, wo es aufgeht. Wähle Form und Muster, dann sieh, wie es nach dem Ofen aussieht.", "The score is the loaf's valve: it decides where it opens. Pick the shape and the pattern, then see how it looks after the oven.")}</p>
      <div className="flex flex-wrap gap-1.5">
        {SHAPES.map((s) => <button key={s.k} data-testid={`dt-shape-${s.k}`} onClick={() => { setShape(s.k); setPat(0); }} className={`text-[11.5px] font-semibold px-2.5 py-1.5 rounded-full border active:scale-95 ${shape === s.k ? "bg-primary text-white border-primary" : "bg-card text-muted-foreground border-border"}`}>{L(s)}</button>)}
      </div>
      <div className="flex flex-wrap gap-1.5">
        {list.map((p, i) => <button key={p.k} data-testid={`dt-pat-${p.k}`} onClick={() => setPat(i)} className={`text-[11.5px] font-semibold px-2.5 py-1.5 rounded-full border active:scale-95 ${pat === i ? "bg-salvia text-white border-salvia" : "bg-card text-muted-foreground border-border"}`}>{L(p)}</button>)}
      </div>
      <div className="rounded-xl border border-border bg-card p-3">
        ...
        <label className="mt-2 flex items-center gap-2 text-[12px] text-foreground cursor-pointer">
      ...
      {P.angle > 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-border bg-card p-3">
          ...
          <p className="text-[12px] text-foreground/85 leading-snug flex items-start gap-1.5"><Scissors className="w-3.5 h-3.5 text-primary shrink-0 mt-0.5" />{tip}</p>
        </div>
      )}
      {P.angle === 0 && <p className="text-[12px] text-foreground/85 leading-snug">{tip}</p>}
      <p className="text-[12px] text-salvia leading-snug">{tri("Sitor: taglia l'impasto freddo di frigo, se puoi: la lametta scorre e non si impasta. Spolvera un velo di farina prima: il disegno resta chiaro anche dopo la cottura.", "Sitor: schneide den Teig kühlschrankkalt, wenn du kannst: die Klinge gleitet und verklebt nicht. Vorher hauchdünn bemehlen: das Muster bleibt auch nach dem Backen hell.", "Sitor: score the dough fridge-cold if you can: the blade glides and doesn't stick. Dust a veil of flour first: the pattern stays pale even after baking.")}</p>
    </div>
  );
}
