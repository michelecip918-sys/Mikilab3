    ...
    <div data-testid="prima-che-succeda" className="space-y-2.5">
      ...
      {picks.map((s) => (
        <div key={s.k} data-testid={`pcs-${s.k}`} className="rounded-xl border border-border bg-card p-3">
          ...
          <p className="text-[12px] text-foreground/85 leading-snug mt-1">{L(s.why)}</p>
          <p className="text-[12px] text-salvia leading-snug mt-1">{tri("Per evitarlo", "So vermeidest du es", "To avoid it")}: {L(s.next)}</p>
        </div>
      ))}
