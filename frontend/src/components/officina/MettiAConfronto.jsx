...
function RecipeSelect({ list, value, onChange, testid, lang, t }) {
  return (
    <select data-testid={testid} value={value} onChange={(e) => onChange(e.target.value)} className="w-full text-[12px] font-semibold bg-background text-foreground border border-border rounded-lg px-2 py-1.5 outline-none">
      {CATS.map((c) => {
...
export default function MettiAConfronto({ recipes, lang, t }) {
  ...
  if (!ra || !rb) return <p className="text-[12.5px] text-muted-foreground">{tri("Servono almeno due ricette con le dosi per fare un confronto.", "Für einen Vergleich braucht es mindestens zwei Rezepte mit Mengen.", "You need at least two recipes with quantities to compare.")}</p>;
  ...
  return (
    <div data-testid="metti-a-confronto" className="space-y-3">
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="grid grid-cols-[1.1fr_1fr_1fr] text-[10.5px] font-bold uppercase tracking-wide text-muted-foreground bg-muted/40 px-2.5 py-1.5">
          <span />
          <span className="truncate text-primary normal-case tracking-normal text-[11.5px]">{nA}</span>
          <span className="truncate text-primary normal-case tracking-normal text-[11.5px]">{nB}</span>
        </div>
        {rows.map((x, i) => (
          <div key={i} data-testid={`mac-row-${i}`} className={`grid grid-cols-[1.1fr_1fr_1fr] gap-x-2 px-2.5 py-1.5 text-[12px] border-t border-border ${x.d ? "bg-primary/8" : ""}`}>
            <span className="text-muted-foreground">{x.k}</span>
            <span className={`font-mono-data ${x.d ? "font-bold text-foreground" : "text-foreground/80"}`}>{x.a}</span>
            <span className={`font-mono-data ${x.d ? "font-bold text-foreground" : "text-foreground/80"}`}>{x.b}</span>
          </div>
        ))}
      </div>
      <p className="text-[10.5px] text-muted-foreground">{tri("Le righe evidenziate sono le differenze vere. Valori calcolati dalle dosi alla scala originale.", "Hervorgehobene Zeilen sind die echten Unterschiede. Werte aus den Mengen im Originalmaßstab berechnet.", "Highlighted rows are the real differences. Values calculated from the quantities at the original scale.")}</p>
      <div className="space-y-1.5">
        {say.map((s, i) => <p key={i} data-testid={`mac-say-${i}`} className="text-[12px] text-salvia leading-snug">{i === 0 ? "Sitor: " : ""}{s}</p>)}
      </div>
    </div>
  );
}
