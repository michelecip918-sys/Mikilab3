  ...
  return (
    <div data-testid="pane-agenda" className="space-y-3">
      ...
      <div className="rounded-xl border border-border bg-card p-3 flex flex-wrap items-center gap-3">
        <label className="flex items-center gap-1.5 text-[12px] text-muted-foreground"><Calendar className="w-3.5 h-3.5" /> {tri("Pronto per", "Fertig am", "Ready by")}
        ...
        <label className="flex items-center gap-1.5 text-[12px] text-muted-foreground">{tri("Cucina a", "Küche bei", "Kitchen at")}
          ...
          <ol className="space-y-1.5">
            {ev.map((e, i) => (
              <li key={i} className="flex items-start gap-3 text-[13px]">
                <span className="font-mono-data text-[12px] text-muted-foreground shrink-0 w-[7.5rem] leading-tight">{fmtDate(new Date(e.t), lang)}<br /><b className="text-foreground">{fmtTime(new Date(e.t), lang)}</b></span>
                <span><span className="font-semibold text-foreground">{e.title}</span>{e.note ? <span className="block text-[11.5px] text-muted-foreground">{e.note}</span> : null}</span>
              </li>
            ))}
          </ol>
          <p className="text-[11.5px] text-muted-foreground mt-2 pt-2 border-t border-salvia/30">{tri("Dal primo passo al pane pronto", "Vom ersten Schritt bis zum fertigen Brot", "From the first step to the finished bread")}: {Math.round(hoursTotal)} h{pastStart ? ` — ${tri("il primo passo sarebbe già passato: scegli un orario più avanti", "der erste Schritt wäre schon vorbei: wähle eine spätere Zeit", "the first step would already be past: choose a later time")}.` : "."}</p>
          <div className="flex flex-wrap gap-2 mt-3">
            <button data-testid="pa-download" onClick={download} className="inline-flex items-center gap-1.5 bg-primary text-white font-semibold text-[12.5px] px-3.5 py-2 rounded-xl active:scale-95"><Download className="w-4 h-4" /> {tri("Metti in agenda (.ics)", "In den Kalender (.ics)", "Add to calendar (.ics)")}</button>
            <button data-testid="pa-copy" onClick={copy} className="inline-flex items-center gap-1.5 bg-card border border-border text-foreground font-semibold text-[12.5px] px-3.5 py-2 rounded-xl active:scale-95"><Copy className="w-4 h-4" /> {tri("Copia come testo", "Als Text kopieren", "Copy as text")}</button>
          </div>
        </div>
      ) : <p className="text-[12.5px] text-muted-foreground">{tri("Scegli una data e un'ora.", "Wähle Datum und Uhrzeit.", "Pick a date and time.")}</p>}
