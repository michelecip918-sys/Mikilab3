...
export default function TazzeOnce({ r, dough, lang, t }) {
  ...
  return (
    <div data-testid="tazze-once" className="space-y-2.5">
      <p className="text-[12.5px] text-foreground/85 leading-snug">{tri("Le dosi in tazze, cucchiai e once, per chi cucina con le misure americane. Densità diversa per ogni ingrediente: una tazza di farina non pesa come una di zucchero.", "Die Mengen in Cups, Löffeln und Unzen, für alle, die mit US-Maßen backen. Jede Zutat mit eigener Dichte: eine Tasse Mehl wiegt nicht wie eine Tasse Zucker.", "Quantities in cups, spoons and ounces, for those who cook with US measures. Each ingredient has its own density: a cup of flour doesn't weigh like a cup of sugar.")}</p>
      <div className="rounded-xl border border-border bg-card overflow-hidden text-[12px]">
        <div className="grid grid-cols-[1.3fr_0.7fr_0.8fr_0.6fr] px-2.5 py-1.5 bg-muted/40 text-[10.5px] font-bold uppercase tracking-wide text-muted-foreground"><span /><span className="text-right">g</span><span className="text-right">cups</span><span className="text-right">oz</span></div>
        {rows.filter((x) => x.g > 0).map((x, i) => <div key={i} data-testid={`to-row-${i}`} className="grid grid-cols-[1.3fr_0.7fr_0.8fr_0.6fr] px-2.5 py-1.5 border-t border-border"><span className="text-foreground/85 truncate">{x.l}</span><span className="font-mono-data text-right text-muted-foreground">{Math.round(x.g)}</span><span className="font-mono-data text-right font-semibold text-foreground">{toUS(x.g, x.fam).txt}</span><span className="font-mono-data text-right text-muted-foreground">{fmt1(x.g / 28.35)}</span></div>)}
      </div>
      <p className="text-[12px] text-salvia leading-snug">{tri("Sitor: le tazze sono un'approssimazione (farina pressata o setacciata cambia del 20 %). Se hai una bilancia, usa i grammi: il pane ti ringrazia.", "Sitor: Cups sind eine Näherung (gedrücktes oder gesiebtes Mehl macht 20 % aus). Hast du eine Waage, nimm Gramm: das Brot dankt es dir.", "Sitor: cups are an approximation (packed or sifted flour changes by 20 %). If you have a scale, use grams: the bread will thank you.")}</p>
    </div>
  );
}
