...
const PlanArchive = forwardRef(function PlanArchive({ kind, getPayload, canSave, onRepeat, repeatLabel, describe }, ref) {
  ...
  return (
    <div ref={rootRef} data-testid={`plan-archive-${kind}`} className="mt-4 rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-4">
      <div className="flex items-center gap-2 mb-1">
        <Archive className="w-5 h-5 text-[#3E9C93]" />
      ...
      {!naming ? (
        <button
          ...
          onClick={openNaming}
          disabled={!canSave}
          className="w-full flex items-center justify-center gap-2 bg-[#e4eff8] dark:bg-[#1B2A38] text-[#3E9C93] dark:text-[#8FB0C2] font-semibold px-4 py-2.5 rounded-2xl shadow-md border border-amber-900/40 border border-[#2A3B49] dark:border-[#2A3B49] disabled:opacity-40 active:scale-98 transition-all"
        >
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 min-w-0 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl shadow-md border border-amber-900/40 px-3 py-2.5 text-sm outline-none focus:border-[#3E9C93] text-[#2B303B] dark:text-[#e4eff8]"
          />
          <button
          ...
          <button
            ...
            className="shrink-0 w-10 h-10 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] flex items-center justify-center text-[#7E8A93]"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="mt-3 space-y-2">
          {plans.map((p) => (
            <div
              ...
              className="flex items-center gap-2 rounded-2xl shadow-md border border-amber-900/40 bg-[#e4eff8] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-2.5"
            >
                ...
                {editingId === p.id ? (
                  <div className="flex items-center gap-1.5">
                    <input
                      ...
                      className="flex-1 min-w-0 bg-white dark:bg-[#1B2A38] border border-[#3E9C93] rounded-lg px-2 py-1.5 text-sm outline-none text-[#2B303B] dark:text-[#e4eff8]"
                    />
                    <button
                      data-testid={`plan-archive-rename-confirm-${p.id}`}
                      onClick={() => doRename(p.id)}
                      className="shrink-0 w-8 h-8 rounded-lg bg-[#3E9C93] text-white flex items-center justify-center"
                    >
              ...
              {editingId !== p.id && (
                <>
                  <button
                    ...
                    className="shrink-0 w-9 h-9 rounded-lg bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] flex items-center justify-center text-[#7E8A93]"
                    aria-label="rename"
                  >
                  ...
                  <button
                    ...
                    className="shrink-0 w-9 h-9 rounded-lg bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] flex items-center justify-center text-[#3E9C93]"
                    aria-label="delete"
                  >
