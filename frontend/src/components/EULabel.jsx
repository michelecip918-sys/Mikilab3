...
export default function EULabel({ recipe, lang }) {
  ...
  return (
    <div data-testid="eu-label" className="mt-4 rounded-2xl border border-[#26324A] dark:border-[#26324A] bg-white dark:bg-[#18202E] p-4">
      <div className="flex items-center justify-between gap-2 mb-2">
        <p className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide text-[#F26419]">
        ...
        <button data-testid="eu-label-print-btn" onClick={() => printEULabel(recipe, lang)}
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#F26419] dark:text-[#8FB0C2] bg-[#e4eff8] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] px-3 py-1.5 rounded-lg active:scale-95">
      ...
      <table className="w-full text-xs">
        <tbody>
          {rows(label, lang).map(([k, v, bold], i) => (
            <tr key={i} className="border-b border-dashed border-[#e4eff8] dark:border-[#18202E] last:border-0">
              <td className={`py-1 ${bold ? "font-semibold text-[#2B303B] dark:text-[#e4eff8]" : "pl-3 text-[#7E8A93]"}`}>{k}</td>
              <td className="py-1 text-right font-mono-data text-[#F26419] dark:text-[#8FB0C2]">{v}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>${esc(name)} — ${L(lang, "Etichetta", "Etikett", "Label")}</title>
    <style>
      *{box-sizing:border-box}
      ...
      .brand{font-size:9px;font-weight:800;color:#F26419;letter-spacing:.05em}
