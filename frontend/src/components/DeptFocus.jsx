  ...
  if (!opName || mine.length === 0) {
    return (
      <div data-testid="dept-focus-picker" className="w-full mb-3 rounded-2xl border border-[#1e293b] bg-[#0b0f19] p-4 text-left">
        <p className="text-[10px] uppercase tracking-widest text-[#64748B] mb-1 flex items-center gap-1"><UserCheck className="w-3.5 h-3.5 text-[#FF6B00]" /> {tri("Chi sei?", "Wer bist du?", "Who are you?", "¿Quién eres?", "Qui es-tu ?", "تو کی هستی؟")}</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          {distinct.map((n) => (
            <button key={n} data-testid={`dept-focus-op-${n}`} onClick={() => setOp(n)}
              className="py-2.5 px-3 rounded-xl bg-[#030712] border border-[#1e293b] text-sm font-bold text-white active:scale-95 hover:border-[#FF6B00]/60 transition-all">{n}</button>
          ))}
        </div>
        <div className="mt-3 pt-3 border-t border-[#1e293b]">
          <p className="text-[10px] uppercase tracking-widest text-[#64748B] mb-1.5 flex items-center gap-1"><KeyRound className="w-3.5 h-3.5 text-[#FF6B00]" /> {tri("…oppure entra col tuo PIN", "…oder mit deinem PIN", "…or enter with your PIN", "…o entra con tu PIN", "…ou entre avec ton PIN", "…یا با پین وارد شو")}</p>
          <div className="flex items-center gap-2">
            <input data-testid="dept-focus-pin-input" value={pin} onChange={(e) => setPin(e.target.value.replace(/[^0-9]/g, "").slice(0, 6))} onKeyDown={(e) => { if (e.key === "Enter") verifyPin(); }}
              ...
              className="flex-1 rounded-xl bg-[#030712] border border-[#1e293b] focus:border-[#FF6B00]/60 outline-none text-center tracking-[0.4em] text-white px-3 py-2.5" />
            <button data-testid="dept-focus-pin-go" onClick={verifyPin} disabled={pinBusy || pin.length < 4}
              className="shrink-0 inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-[#FF6B00]/15 border border-[#FF6B00]/50 text-[#FF6B00] font-bold text-sm active:scale-95 disabled:opacity-40">
    ...
    <div data-testid="dept-focus" className="w-full mb-3 rounded-2xl border p-4 text-left" style={{ borderColor: `${dept.accent}66`, background: `${dept.accent}0d` }}>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button data-testid="dept-focus-reset-op" onClick={resetOp} className="text-[10px] font-bold text-[#64748B] hover:text-[#FF6B00]">{tri("non sei tu?", "nicht du?", "not you?", "¿no eres tú?", "pas toi ?", "تو نیستی؟")}</button>
        </div>
      </div>
      {mine.length > 1 && (
        <div className="flex flex-wrap gap-1.5 mb-2" data-testid="dept-focus-switch">
          {mine.map((m, i) => { const dd = depts.find((d) => d.key === m.dept); return (
            <button key={m.id} data-testid={`dept-focus-switch-${m.dept}`} onClick={() => setIdx(i)}
              ...
              style={i === idx ? { background: `${dd?.accent || "#FF6B00"}22`, border: `1px solid ${dd?.accent || "#FF6B00"}66`, color: dd?.accent } : { border: "1px solid #1e293b" }}>
              {dd?.icon} {dd?.name}
            </button>
          ); })}
        </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-2">
        {dept.machines.map((m) => (
          <button key={m.id} data-testid={`dept-machine-${m.id}`} onClick={() => { try { playTTS(`${m.name}. ${tri("Modulo vocale pronto.", "Sprachmodul bereit.", "Voice module ready.", "Módulo de voz listo.", "Module vocal prêt.", "ماژول صوتی آماده.")}`, { lang, voice: "mikemix" }); } catch { /* */ } }}
            ...
            <Volume2 className="w-3 h-3 text-[#FF6B00]" /> {m.name}
          </button>
        ))}
      </div>
      {obj && (
