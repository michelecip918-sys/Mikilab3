  ...
  return (
    <div data-testid="lab-mode-big" className="pb-28">
      <div className="flex items-center justify-between mb-4">
        <div className="min-w-0">
          <h1 className="font-display text-2xl font-extrabold text-white leading-tight flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-[#3E9C93] animate-pulse" />
        ...
        <button data-testid="lab-big-exit" onClick={onExit}
          className="shrink-0 flex items-center gap-1.5 rounded-2xl px-4 py-3 bg-[#1B2A38] border-2 border-[#2A3B49] text-white font-bold text-sm active:scale-95 transition-all">
      ...
      <button data-testid="lab-big-plan" onClick={onOpenPlan}
        className="w-full flex items-center gap-4 min-h-[92px] rounded-3xl px-5 mb-3 bg-gradient-to-r from-[#3E9C93] to-[#3E9C93] text-white shadow-[0_6px_0_rgba(0,0,0,.35)] active:translate-y-1 active:shadow-[0_2px_0_rgba(0,0,0,.35)] transition-all text-left">
        ...
        {TILES.map(({ id, Icon, label, primary }) => (
          <button key={id} data-testid={`lab-big-tile-${id}`} onClick={() => onOpenTool(id)}
            className={`flex flex-col items-center justify-center gap-2.5 min-h-[118px] rounded-3xl px-3 py-4 text-center active:scale-97 transition-all shadow-[0_5px_0_rgba(0,0,0,.3)] active:translate-y-0.5 ${
              primary
                ? "bg-gradient-to-br from-[#3E9C93] to-[#3E9C93] text-white border-2 border-[#3E9C93]"
                : "bg-[#1B2A38] text-white border-2 border-[#3E9C93]/70"
            }`}>
            <Icon className={`w-9 h-9 shrink-0 ${primary ? "text-white" : "text-[#3E9C93]"}`} />
