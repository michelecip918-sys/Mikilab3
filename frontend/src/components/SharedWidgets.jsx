...
export default function SharedWidgets({ dept = "" }) {
  ...
  return (
    <div data-testid="shared-widgets" className="rounded-2xl border border-[#EAB308]/25 bg-[#0C1019]/50 p-4 space-y-2.5">
      ...
      {widgets.map((w) => (
        <div key={w.id} data-testid={`shared-widget-${w.id}`} className="rounded-xl bg-[#060A10] border border-[#1e293b] p-3">
          ...
          {w.type === "metric" && <p className="text-xl font-black text-[#EAB308]">{w.config.value || "—"} <span className="text-xs text-[#94A3B8]">{w.config.unit}</span></p>}
          {w.type === "reminder" && <p className="text-[12px] text-[#CBD5E1] flex items-center gap-1.5"><Bell className="w-3.5 h-3.5 text-[#FF9D42]" />{w.config.text}{w.config.date ? ` · ${w.config.date}` : ""}</p>}
          {w.type === "counter" && <p className="text-xl font-black text-[#EAB308]">{w.config.value || 0} <span className="text-xs text-[#94A3B8]">{w.config.label}</span></p>}
          {w.type === "checklist" && (
              ...
              <div className="flex items-end gap-1.5 h-20">
                {s.map((p, k) => (
                  <div key={k} className="flex-1 flex flex-col items-center justify-end h-full">
                    <span className="text-[9px] text-[#EAB308] font-bold">{Number(p.v) || 0}</span>
                    <div className="w-full rounded-t" style={{ height: `${Math.max(4, ((Number(p.v) || 0) / max) * 100)}%`, background: "linear-gradient(180deg,#EAB308,#FF6B00)" }} />
