...
export default function OnboardingActivity({ onChoose, onClose }) {
  ...
  const OPTS = [
    { id: "panificio", ic: "🥖", accent: "hsl(var(--muted-foreground))",
    ...
    { id: "pizzeria", ic: "🍕", accent: "hsl(var(--muted-foreground))",
    ...
    { id: "pasticceria", ic: "🧁", accent: "hsl(var(--muted-foreground))",
  ...
  return (
    <div data-testid="onboarding-activity" className="fixed inset-0 z-[900] bg-background/96 backdrop-blur-md flex flex-col items-center justify-center px-5 py-8 overflow-auto">
      ...
      <p className="mt-2 text-[12.5px] text-muted-foreground text-center max-w-sm">{tri(
      ...
      <div className="mt-6 grid grid-cols-1 gap-3 w-full max-w-md">
        {OPTS.map((o) => (
          <button key={o.id} data-testid={`onboarding-opt-${o.id}`} onClick={() => setSel(o.id)}
            ...
            style={sel === o.id ? { borderColor: o.accent, background: `${o.accent}18`, boxShadow: `0 0 22px ${o.accent}44` } : { borderColor: "hsl(var(--card))", background: "hsl(var(--card))" }}>
            <span className="text-3xl shrink-0">{o.ic}</span>
            <span className="min-w-0">
              <span className="block font-black text-foreground text-base" style={{ color: sel === o.id ? o.accent : "#fff" }}>{o.label}</span>
              <span className="block text-[11.5px] text-muted-foreground leading-snug">{o.desc}</span>
            </span>
          </button>
        ))}
      </div>
      <motion.button data-testid="onboarding-confirm" onClick={confirm} disabled={!sel}
        whileTap={{ scale: 0.96 }}
        className="mt-6 w-full max-w-md py-3.5 rounded-full font-black text-base text-foreground disabled:opacity-40"
