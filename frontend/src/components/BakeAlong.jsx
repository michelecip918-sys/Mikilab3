              ...
              {entries.map((e) => (
                <li key={e.id} data-testid={`bake-along-entry-${e.id}`}
                  ...
                  <button data-testid={`bake-along-vote-${e.id}`} onClick={() => vote(e.id)}
                    className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-full text-sm font-bold shrink-0 active:scale-90 transition-all ${e.liked_by_me ? "bg-[#ff6b00] text-white" : "bg-white dark:bg-[#1e1e1e] text-[#ff6b00] border border-[#ff6b00]/40"}`}>
