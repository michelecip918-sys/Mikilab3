...
export default function BakeAlong() {
  ...
  return (
    <div data-testid="bake-along" className="rounded-2xl overflow-hidden bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49]">
      {/* Header sfida */}
      <div className="p-5 text-white" style={{ background: "linear-gradient(135deg,#7a1f1f,#3E9C93 90%)" }}>
      ...
      <div className="p-4 space-y-4">
        ...
        {data.already_submitted ? (
          ...
          <div>
            ...
            <button data-testid="bake-along-participate" disabled={busy}
              ...
              className="w-full inline-flex items-center justify-center gap-2 bg-[#3E9C93] hover:bg-[#3E9C93] text-white font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all disabled:opacity-60">
              ...
              {entries.map((e) => (
                <li key={e.id} data-testid={`bake-along-entry-${e.id}`}
                  ...
                  <button data-testid={`bake-along-vote-${e.id}`} onClick={() => vote(e.id)}
                    className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-full text-sm font-bold shrink-0 active:scale-90 transition-all ${e.liked_by_me ? "bg-[#3E9C93] text-white" : "bg-white dark:bg-[#1B2A38] text-[#3E9C93] border border-[#3E9C93]/40"}`}>
        ...
        {champion && (
          <div data-testid="bake-along-champion" className="flex items-center gap-2 rounded-2xl shadow-md border border-amber-900/40 bg-gradient-to-r from-[#3E9C93]/12 to-[#3E9C93]/12 border border-[#3E9C93]/30 px-3 py-2">
