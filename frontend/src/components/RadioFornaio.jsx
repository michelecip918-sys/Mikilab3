        ...
        {list.map((st) => {
          ...
          return (
            ...
            >
              ...
              >
                <Star className={`w-4 h-4 ${isFav ? "fill-[#F26419] text-[#F26419]" : active ? "text-white/70" : "text-[#c9b17e]"}`} />
              </button>
            </div>
          );
        })}
    ...
    <>
      {open && (
        <div className="fixed inset-x-0 bottom-40 z-40 px-4 flex justify-center pointer-events-none">
          <div data-testid="radio-panel" className="pointer-events-auto w-full max-w-xl max-h-[70vh] overflow-y-auto bg-[#0B0E14] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-2xl shadow-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              <Radio className="w-5 h-5 text-[#F26419]" />
              ...
              <button data-testid="radio-voice" onClick={listenStation}
                className={`p-1.5 rounded-lg mr-1 ${listening ? "bg-[#F26419] text-white animate-pulse" : "text-[#F26419]"}`}
            ...
            <button
              ...
              className={`w-full mb-3 flex items-center gap-2.5 px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 border text-left transition-all active:scale-98 ${
                ambientOn ? "bg-[#F26419] text-white border-[#F26419]" : "bg-[#F26419]/10 text-[#F26419] dark:text-[#8FB0C2] border-[#F26419]/40"
              }`}
            >
              <Flame className={`w-5 h-5 shrink-0 ${ambientOn ? "text-white" : "text-[#F26419]"}`} />
            ...
            {ambientOn && (
              <div className="-mt-1 mb-3">
                  ...
                  {AMB.map((a) => (
                    <button key={a.id} data-testid={`ambient-mode-${a.id}`} onClick={() => setAmbientMode(a.id)}
                      className={`flex flex-col items-center gap-0.5 py-1.5 rounded-2xl shadow-md border border-amber-900/40 border text-[10px] font-semibold transition-all active:scale-95 ${
                        ambientMode === a.id ? "bg-[#F26419] text-white border-[#F26419]" : "bg-white dark:bg-[#18202E] text-[#7E8A93] border-[#26324A] dark:border-[#26324A]"
                      }`}>
                ...
                <div className="flex items-center gap-2 px-1">
                  <Flame className="w-4 h-4 text-[#F26419] shrink-0" />
                  <input
                    ...
                    className="flex-1 accent-[#F26419]"
                  />
                </div>
              </div>
            )}
            ...
            <button
              ...
              onClick={toggleSfx}
              className={`w-full mb-3 flex items-center gap-2.5 px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 border text-left transition-all active:scale-98 ${
                sfxEnabled ? "bg-[#F26419] text-white border-[#F26419]" : "bg-[#F26419]/10 text-[#F26419] dark:text-[#d3ab6b] border-[#F26419]/40"
              }`}
            >
              <Volume2 className={`w-5 h-5 shrink-0 ${sfxEnabled ? "text-white" : "text-[#F26419]"}`} />
              <span className="flex-1 min-w-0">
            ...
            {sfxEnabled && (
              <div className="-mt-1 mb-3 flex items-center gap-2 px-1">
                <Volume2 className="w-4 h-4 text-[#F26419] shrink-0" />
                <input data-testid="sfx-volume" type="range" min="0" max="1" step="0.05" value={sfxVolume}
                  onChange={(e) => setSfxVol(Number(e.target.value))} className="flex-1 accent-[#F26419]" />
              </div>
            )}
            ...
            <div className="relative mb-3">
              ...
              <input data-testid="radio-search" value={q} onChange={(e) => setQ(e.target.value)}
                ...
                className="w-full bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-2xl shadow-md border border-amber-900/40 pl-9 pr-9 py-2.5 outline-none text-sm text-[#2B303B] dark:text-[#e4eff8] focus:border-[#F26419]" />
            ...
            <button data-testid="radio-add-toggle" onClick={() => setShowAdd((v) => !v)}
              className={`w-full mb-3 flex items-center gap-2 px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 border text-sm font-semibold text-left transition-all active:scale-98 ${showAdd ? "bg-[#F26419] text-white border-[#F26419]" : "bg-white dark:bg-[#18202E] text-[#F26419] border-[#26324A] dark:border-[#26324A]"}`}>
              <Plus className="w-4 h-4 shrink-0" />
              {tri("Aggiungi la tua radio (URL)", "Eigenen Sender hinzufügen (URL)", "Add your radio (URL)", "Añade tu radio (URL)")}
            </button>
            {showAdd && (
              <div data-testid="radio-add-form" className="mb-3 rounded-2xl shadow-md border border-amber-900/40 border border-[#F26419]/40 bg-[#F26419]/8 p-3 space-y-2">
                <input data-testid="radio-add-name" value={cName} onChange={(e) => setCName(e.target.value)}
                  ...
                  className="w-full bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-lg px-3 py-2 outline-none text-sm text-[#2B303B] dark:text-[#e4eff8] focus:border-[#F26419]" />
                ...
                <button data-testid="radio-add-save" onClick={addCustom}
                  className="w-full bg-[#F26419] text-white font-semibold px-3 py-2 rounded-lg active:scale-98 text-sm">
            ...
            {lastStation && current !== lastStation.id && (
              <button data-testid="radio-resume" onClick={() => playStation(lastStation)}
                className="w-full flex items-center gap-2 mb-3 px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-[#F26419]/12 border border-[#F26419]/40 text-[#8a5e17] dark:text-[#e0b566] text-sm font-semibold active:scale-98">
                    ...
                    return (
                      <div key={st.id} data-testid={`radio-station-${st.id}`} onClick={() => playStation(st)}
                        ...
                        <button type="button" data-testid={`radio-custom-remove-${st.id}`} aria-label="remove" onClick={(e) => { e.stopPropagation(); removeCustom(st.id); }}
                          className="absolute right-1.5 top-1/2 -translate-y-1/2 p-1 active:scale-90">
                          <Trash2 className={`w-4 h-4 ${active ? "text-white/70" : "text-[#F26419]"}`} />
                        </button>
                      </div>
                    );
                  })}
            ...
            {nowPlaying && (
              <div data-testid="radio-now-playing" className="mt-1 flex items-center gap-2 bg-[#F26419]/12 border border-[#F26419]/30 rounded-2xl shadow-md border border-amber-900/40 px-3 py-2">
                <span className={`w-2 h-2 rounded-full shrink-0 ${status === "playing" ? "bg-[#F26419] animate-pulse" : status === "error" ? "bg-[#F26419]" : "bg-[#F26419]"}`} />
                ...
                <button data-testid="radio-stop-btn" onClick={stop} className="text-[#F26419] p-1"><Square className="w-4 h-4 fill-current" /></button>
              </div>
            )}
      ...
      <div className={`fixed z-40 left-3 bottom-24 flex flex-col items-center gap-1 transition-all duration-300 ${scrolling && !open ? "translate-y-24 opacity-0 pointer-events-none" : "translate-y-0 opacity-100"}`} style={{ marginBottom: "env(safe-area-inset-bottom)" }}>
        <button
          ...
          className={`relative w-12 h-12 rounded-full shadow-lg flex items-center justify-center transition-all active:scale-95 ${
            status === "playing" ? "bg-[#F26419]" : "bg-[#F26419] hover:bg-[#F26419]"
          }`}
        >
          {status !== "playing" && !open && <span aria-hidden className="absolute inset-0 rounded-full bg-[#F26419] opacity-50 animate-ping" />}
          {status === "playing" ? (
        ...
        <span className="text-[9px] font-bold text-[#F26419] bg-[#0B0E14]/90 dark:bg-[#0B0E14]/90 px-1.5 py-0.5 rounded-full shadow-sm">{t("radio_label")}</span>
      </div>
    </>
  );
}
