...
export default function SupplierOrder({ totals }) {
  ...
  return (
    <div data-testid="supplier-order" className="space-y-3">
        ...
        {!data ? (
          ...
          <div className="space-y-1.5" data-testid="supplier-shopping">
            {Object.entries(totals.flourByType).sort((a, b) => b[1] - a[1]).map(([k, v]) => (
              <Row key={k} icon={<Wheat className="w-3.5 h-3.5 text-[#3E9C93]" />} label={k} value={fmtQty(v)} />
            ))}
            {Object.entries(totals.others).map(([f, v]) => (
              <Row key={f} icon={<Droplets className="w-3.5 h-3.5 text-[#3E9C93]" />} label={otherLabel(f, lang)} value={fmtQty(v)} />
            ))}
      ...
      {/* Ordine al fornitore */}
      <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-4 no-print">
        <div className="flex items-center gap-2 mb-2 text-[#3E9C93]">
        ...
        <a href={supplier.web} target="_blank" rel="noopener noreferrer"
          className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-[#3E9C93]">
      ...
      {/* Directory fornitori */}
      <div className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] overflow-hidden no-print">
        <button data-testid="supplier-dir-toggle" onClick={() => setShowDir((v) => !v)}
          className="w-full flex items-center justify-between p-4 text-left">
          <div className="flex items-center gap-2 text-[#3E9C93]">
            <Store className="w-4 h-4" />
        ...
        {showDir && (
          <div className="px-4 pb-4 space-y-2" data-testid="supplier-directory">
            ...
            {SUPPLIERS.map((s) => (
              <div key={s.id} className="rounded-2xl shadow-md border border-amber-900/40 bg-[#e4eff8] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] font-bold uppercase text-[#3E9C93] dark:text-[#8FB0C2] bg-[#3E9C93]/15 px-1.5 py-0.5 rounded-full shrink-0">{cats[s.category] || s.category}</span>
                </div>
                <p className="text-xs text-[#3F4A54] dark:text-[#AEB8BF] mt-1.5 leading-relaxed">{lang === "de" ? s.de : lang === "en" ? s.en : s.it}</p>
                <div className="flex items-center gap-3 mt-2">
                  <a href={s.web} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs font-medium text-[#3E9C93]">
                    <ExternalLink className="w-3 h-3" /> {t("shop_visit_site")}
                  </a>
                  <button onClick={() => pickSupplier(s.id)} className="inline-flex items-center gap-1 text-xs font-medium text-[#3E9C93]">
...
function Row({ icon, label, value }) {
  return (
    <div className="flex items-center justify-between text-sm gap-2">
      ...
      <span className="font-mono-data font-bold text-[#3E9C93] dark:text-[#8FB0C2] shrink-0">{value}</span>
    </div>
  );
}
