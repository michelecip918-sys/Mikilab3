        ...
        {!data ? (
          ...
          <div className="space-y-1.5" data-testid="supplier-shopping">
            {Object.entries(totals.flourByType).sort((a, b) => b[1] - a[1]).map(([k, v]) => (
              <Row key={k} icon={<Wheat className="w-3.5 h-3.5 text-[#ff6b00]" />} label={k} value={fmtQty(v)} />
            ))}
            {Object.entries(totals.others).map(([f, v]) => (
              <Row key={f} icon={<Droplets className="w-3.5 h-3.5 text-[#ff6b00]" />} label={otherLabel(f, lang)} value={fmtQty(v)} />
            ))}
      ...
      <div className="rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-4 no-print">
        ...
        <div className="grid grid-cols-2 gap-2 mt-3">
          <button data-testid="supplier-mail-btn" onClick={sendEmail} disabled={!data}
            className="bg-[#ff6b00] hover:bg-[#ff8a33] disabled:opacity-50 text-white font-semibold px-4 py-2.5 rounded-2xl shadow-md border border-amber-900/40 active:scale-98 transition-all flex items-center justify-center gap-2">
            ...
            {SUPPLIERS.map((s) => (
              <div key={s.id} className="rounded-2xl shadow-md border border-amber-900/40 bg-[#e4eff8] dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] font-bold uppercase text-[#ff6b00] dark:text-[#8FB0C2] bg-[#ff6b00]/15 px-1.5 py-0.5 rounded-full shrink-0">{cats[s.category] || s.category}</span>
                </div>
                <p className="text-xs text-[#3F4A54] dark:text-[#AEB8BF] mt-1.5 leading-relaxed">{lang === "de" ? s.de : lang === "en" ? s.en : s.it}</p>
                <div className="flex items-center gap-3 mt-2">
                  <a href={s.web} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs font-medium text-[#ff6b00]">
                    <ExternalLink className="w-3 h-3" /> {t("shop_visit_site")}
                  </a>
                  <button onClick={() => pickSupplier(s.id)} className="inline-flex items-center gap-1 text-xs font-medium text-[#ff6b00]">
...
function Row({ icon, label, value }) {
  return (
    <div className="flex items-center justify-between text-sm gap-2">
      ...
      <span className="font-mono-data font-bold text-[#ff6b00] dark:text-[#8FB0C2] shrink-0">{value}</span>
    </div>
  );
}
