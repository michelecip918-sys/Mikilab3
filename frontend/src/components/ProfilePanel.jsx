  ...
  return (
    <div data-testid="profile-panel" className="fixed inset-0 z-[60] bg-black/50 flex items-end sm:items-center justify-center" onClick={onClose}>
      <div className="bg-white dark:bg-[#121212] w-full sm:max-w-lg sm:rounded-3xl rounded-t-3xl max-h-[88vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="relative p-6 text-white" style={{ background: "linear-gradient(135deg,#0f2231,#1e1e1e 45%,#1f5a68 72%,#c94f00)" }}>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                {(data.badges || []).includes("diplomato") && (
                  <span data-testid="profile-badge-diplomato" className="mt-1.5 inline-flex items-center gap-1 text-[11px] font-bold bg-[#c94f00] text-white px-2.5 py-1 rounded-full">
                    🎓 {tri("Fornaio Diplomato", "Diplom-Bäcker", "Certified Baker", "Panadero Diplomado")}
                  </span>
                )}
                {(data.badges || []).includes("fornaio_settimana") && (
                  <span data-testid="profile-badge-champion" className="mt-1.5 ml-1.5 inline-flex items-center gap-1 text-[11px] font-bold bg-gradient-to-r from-[#c94f00] to-[#7a531d] text-white px-2.5 py-1 rounded-full">
                ...
                {((data.badges || []).includes("bakealong_champion") || (data.bakealong_wins || 0) > 0) && (
                  <span data-testid="profile-badge-bakealong" className="mt-1.5 ml-1.5 inline-flex items-center gap-1 text-[11px] font-bold bg-gradient-to-r from-[#c94f00] to-[#c94f00] text-white px-2.5 py-1 rounded-full">
          ...
          <div className="p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {PRESET_AVATARS.map((a) => (
                      <button key={a.id} data-testid={`avatar-preset-${a.id}`} onClick={() => setPic(a.url)}
                        className={`rounded-2xl shadow-md border border-amber-900/40 overflow-hidden border-2 transition-all active:scale-95 ${pic === a.url ? "border-[#c94f00] ring-2 ring-[#c94f00]/40" : "border-[#2e2e2e] dark:border-[#2e2e2e]"}`}>
            ...
            {isMe && (
              <div data-testid="profile-email-pref">
                <p className="text-[11px] font-bold uppercase tracking-wide text-[#7E8A93] mb-2 flex items-center gap-1.5"><Mail className="w-3.5 h-3.5 text-[#c94f00]" /> {tri("Notifiche email dai canali seguiti", "E-Mail-Benachrichtigungen der gefolgten Kanäle", "Email notifications from followed channels", "Notificaciones por email de los canales seguidos")}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  {[
                    ...
                    <button key={o.v} data-testid={`email-pref-${o.v}`} onClick={() => changeEmailMode(o.v)}
                      className={`rounded-2xl shadow-md border border-amber-900/40 px-2 py-2.5 text-[12px] font-bold border transition-all active:scale-95 ${emailMode === o.v ? "bg-[#c94f00] text-white border-[#c94f00]" : "bg-[#121212] dark:bg-[#1e1e1e] text-[#7E8A93] border-[#2e2e2e] dark:border-[#2e2e2e]"}`}>
            ...
            {isMe && sosItems.length > 0 && (
              <div data-testid="profile-sos-history">
                <p className="text-[11px] font-bold uppercase tracking-wide text-[#7E8A93] mb-2 flex items-center gap-1.5"><Stethoscope className="w-3.5 h-3.5 text-[#c94f00]" /> {tri("Diagnosi SOS salvate", "Gespeicherte SOS-Diagnosen", "Saved SOS diagnoses", "Diagnósticos SOS guardados")} ({sosItems.length})</p>
                <div className="space-y-2">
                  {sosItems.map((s) => (
                    <div key={s.id} data-testid={`sos-history-${s.id}`} className="rounded-2xl shadow-md border border-amber-900/40 bg-[#fbeeec] dark:bg-[#2a1f1e] border border-[#e6cfc9] dark:border-[#4a2e2b] p-3 flex gap-3">
                      <div className="relative w-14 h-14 rounded-lg bg-[#c94f00]/15 flex items-center justify-center shrink-0 overflow-hidden">
                        <Stethoscope className="w-6 h-6 text-[#c94f00]" />
                      ...
                      <button data-testid={`sos-history-delete-${s.id}`} onClick={() => deleteSos(s.id)} className="text-[#7E8A93] hover:text-[#c94f00] p-1 self-start"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  ))}
                </div>
              </div>
