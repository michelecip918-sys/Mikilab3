    ...
    <div data-testid="mohamed-inbox" className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <img src={`${PUB}/avatar_mohamed.jpg`} alt="Mohamed" className="w-9 h-9 rounded-full object-cover border border-[#5E8CA8]/50 grayscale-[0.2]" onError={(e) => { e.currentTarget.style.display = "none"; }} />
          <div>
            ...
            <p className="text-[10px] text-[#8aa0b4]">{tri("Smistate da Mohamed · assistente subordinato", "Sortiert von Mohamed · untergeordnet", "Routed by Mohamed · subordinate assistant", "Clasificadas por Mohamed", "Triées par Mohamed", "توسط محمد")}</p>
          </div>
        </div>
        <button data-testid="inbox-refresh" onClick={load} className="text-[#7DD3FC] active:scale-90"><RefreshCw className={`w-4 h-4 ${busy ? "animate-spin" : ""}`} /></button>
      </div>
      {reqs.length === 0 ? (
        <p data-testid="inbox-empty" className="text-[12px] text-[#8aa0b4]">{tri("Nessuna richiesta in arrivo.", "Keine Anfragen.", "No incoming requests.", "Sin solicitudes.", "Aucune demande.", "درخواستی نیست.")}</p>
      ) : (
        <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
          {reqs.map((r) => (
            <div key={r.id} data-testid={`inbox-req-${r.id}`} className="rounded-lg border border-[#1e293b] bg-[#070A10] p-3">
      ...
      {pending > 0 && <p className="flex items-center gap-1 text-[10px] text-[#F6D27A]"><Clock className="w-3 h-3" /> {pending} {tri("in attesa del Capo", "warten auf den Chef", "awaiting the Capo", "esperando al Capo", "en attente du Capo", "منتظر کاپو")}</p>}
    </div>
  );
}
