...
export default function FloorInviteLanding({ token, onEnter }) {
  ...
  return (
    <div data-testid="floor-invite-landing" className="relative min-h-screen overflow-hidden bg-background text-foreground flex flex-col items-center justify-center px-6">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div className="flex flex-col items-center text-center mb-5">
          ...
          {info == null ? (
            ...
            <>
              <h1 className="font-display text-2xl font-black tracking-wide">{info.org_name}</h1>
              {info.note && <p className="text-[12px] text-muted-foreground mt-1">{info.note}</p>}
            ...
            <div className="mt-2 inline-flex flex-col items-center gap-1.5 text-mattone">
              ...
              <p className="text-[11px] text-muted-foreground">{tri("Chiedi alla Direzione un nuovo link.", "Bitte die Leitung um einen neuen Link.", "Ask management for a new link.", "Pide a la Dirección un nuevo enlace.", "Demandez un nouveau lien à la Direction.", "از مدیریت لینک جدید بخواهید.")}</p>
            </div>
          )}
        </div>
        ...
        {valid && (
          <form data-testid="floor-invite-form" onSubmit={(e) => { e.preventDefault(); submit(); }} className="space-y-3">
            <input data-testid="floor-invite-name" value={name} onChange={(e) => setName(e.target.value)}
              ...
              className="w-full rounded-xl bg-background border border-border/30 text-foreground text-sm px-3.5 py-3 outline-none focus:border-primary" />
            <input data-testid="floor-invite-pin" value={pin} onChange={(e) => setPin(e.target.value.replace(/\D/g, "").slice(0, 4))}
              ...
              className="w-full rounded-xl bg-background border border-border/30 text-foreground text-center text-2xl tracking-[0.5em] px-3.5 py-3 outline-none focus:border-primary" />
            {err && <p data-testid="floor-invite-error" className="text-[12px] text-mattone font-semibold text-center">{err}</p>}
            <button data-testid="floor-invite-enter" type="submit" disabled={busy}
              className="w-full inline-flex items-center justify-center gap-2 py-3 rounded-xl bg-primary text-primary-foreground font-black text-sm active:scale-95 transition-all disabled:opacity-60">
