...
export default function CapoCombos({ products, setProducts, lang, getSaveItems, onApply }) {
  ...
  return (
    <div className="mt-2.5 rounded-2xl shadow-md border border-amber-900/40 border border-[#2A3B49] dark:border-[#2A3B49] bg-[#1B2A38] p-2.5" data-testid="capo-combos">
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        {combos.map((c) => (
          <span key={c.id} className="inline-flex items-center rounded-full bg-[#3E9C93]/12 border border-[#3E9C93]/30 overflow-hidden">
            <button type="button" data-testid={`capo-combo-apply-${c.id}`} onClick={() => apply(c)}
              className="text-xs font-semibold text-[#3E9C93] dark:text-[#ffd9b8] pl-3 pr-2 py-1.5 active:scale-95 transition-all max-w-[220px] truncate flex items-center gap-1">
            ...
            <button type="button" data-testid={`capo-combo-del-${c.id}`} onClick={() => remove(c.id)}
              className="text-[#3E9C93]/70 hover:text-[#3E9C93] pr-2 pl-0.5 py-1.5"><X className="w-3 h-3" /></button>
          </span>
        ))}
        {hasRecipes && !saveOpen && (
          <button type="button" data-testid="capo-combo-save-open" onClick={() => setSaveOpen(true)}
            className="text-xs font-semibold text-white bg-[#3E9C93] px-3 py-1.5 rounded-full flex items-center gap-1 active:scale-95">
      ...
      {saveOpen && (
        <div className="flex items-center gap-2 mt-2">
          <input data-testid="capo-combo-name" value={name} onChange={(e) => setName(e.target.value)}
            ...
            className="flex-1 min-w-0 bg-[#1B2A38] border border-[#2A3B49] rounded-lg py-1.5 px-2.5 text-sm text-white outline-none focus:border-[#3E9C93]" />
          <button type="button" data-testid="capo-combo-save" onClick={doSave} disabled={!name.trim()}
            className="text-xs font-semibold text-white bg-[#3E9C93] disabled:opacity-40 px-3 py-1.5 rounded-lg active:scale-95 shrink-0">{tri3("Salva", "Speichern", "Save", "Guardar")}</button>
