...
export default function ScaleDialog({ recipe, open, onOpenChange, onSave }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md bg-[#f0f6fb] dark:bg-[#1B2127] border-[#d5e4f0] dark:border-[#38424B]">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl text-[#2B303B] dark:text-[#e4eff8] flex items-center gap-2">
            <Scale className="w-6 h-6 text-[#5aa0cf]" /> {t("scale_title")}
          </DialogTitle>
        ...
        <div className="space-y-4 py-1">
          ...
          <div>
            ...
            <input
              ...
              className="mt-1 w-full font-mono-data font-bold text-[#234b6e] dark:text-[#8FB0C2] bg-white dark:bg-[#1F252B] border border-[#d5e4f0] dark:border-[#38424B] focus:border-[#3f7cac] focus:ring-2 focus:ring-[#3f7cac]/20 rounded-xl p-3 text-lg outline-none"
            />
          </div>

          <div className="space-y-2" data-testid="scale-preview">
            {scaled.map((f) => (
              <div key={f.key} className="flex items-center justify-between bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] rounded-xl px-4 py-2.5">
                <span className="text-sm text-[#3F4A54] dark:text-[#AEB8BF]">{t(f.labelKey)}</span>
                <span className="font-mono-data font-bold text-[#234b6e] dark:text-[#8FB0C2]">
        ...
        <DialogFooter className="gap-2 sm:gap-2">
          ...
          <button
            ...
            onClick={submit}
            disabled={!factor}
            className="flex-1 bg-[#3f7cac] hover:bg-[#336a94] disabled:opacity-50 text-white font-semibold px-4 py-3 rounded-xl shadow-md active:scale-98 transition-all"
          >
            {t("save_as_new")}
          </button>
        </DialogFooter>
