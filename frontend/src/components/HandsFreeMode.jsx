  ...
  if (!steps.length) {
    return createPortal((
      <div className="fixed inset-0 z-[95] bg-[#121212] text-white flex flex-col items-center justify-center p-6 text-center" data-testid="handsfree-overlay">
        ...
        <button data-testid="handsfree-close" onClick={onClose} className="bg-[#c94f00] px-5 py-2.5 rounded-2xl shadow-md border border-amber-900/40 font-semibold">{tri("Chiudi", "Schließen", "Close", "Cerrar")}</button>
      </div>
    ), document.body);
  }

  return createPortal((
    <div className="fixed inset-0 z-[95] bg-gradient-to-b from-[#c94f00] to-[#12212e] text-white flex flex-col" data-testid="handsfree-overlay">
      ...
      <div className="p-5 pb-8 space-y-3" style={{ paddingBottom: "calc(2rem + env(safe-area-inset-bottom))" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          <button data-testid="handsfree-tts" onClick={() => { setTtsOn((v) => { const nv = !v; if (!nv) { try { window.speechSynthesis.cancel(); } catch { /* */ } } return nv; }); }}
            className={`flex items-center justify-center gap-2 py-3 rounded-2xl font-semibold text-sm active:scale-95 ${ttsOn ? "bg-white text-[#c94f00]" : "bg-white/12"}`}>{ttsOn ? <Volume2 className="w-5 h-5" /> : <VolumeX className="w-5 h-5" />}{tri("Voce", "Stimme", "Voice", "Voz")}</button>
          ...
          <button data-testid="handsfree-mic" onClick={toggleMic}
            className={`flex items-center justify-center gap-2 py-3 rounded-2xl font-semibold text-sm active:scale-95 ${micOn ? "bg-[#c94f00] text-white animate-pulse" : "bg-white/12"}`}>{micOn ? <Mic className="w-5 h-5" /> : <MicOff className="w-5 h-5" />}{tri("Comandi", "Befehle", "Commands", "Comandos")}</button>
        </div>
      </div>
    </div>
  ), document.body);
