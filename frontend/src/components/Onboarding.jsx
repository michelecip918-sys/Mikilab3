      ...
      body: (
        <div className="space-y-3">
          <input data-testid="ob-labname" value={p.labName} onChange={(e) => set({ labName: e.target.value })}
            ...
            className="w-full bg-[#FAF5EC] dark:bg-[#1F252B] border border-[#E6D8C3] dark:border-[#38424B] rounded-xl px-3 py-3 outline-none text-[#2B303B] dark:text-[#e4eff8]" />
          <div className="grid grid-cols-2 gap-2">
            {TYPES.map((tp) => (
              <button key={tp.id} data-testid={`ob-type-${tp.id}`} onClick={() => set({ type: tp.id })}
                className={`px-3 py-2.5 rounded-xl text-sm font-semibold border ${p.type === tp.id ? "bg-[#8C4A27] text-white border-[#8C4A27]" : "bg-white dark:bg-[#232A31] text-[#3F4A54] dark:text-[#AEB8BF] border-[#E6D8C3] dark:border-[#38424B]"}`}>{tp.label}</button>
            ))}
          </div>
        </div>
      ),
        ...
        <div className="flex flex-wrap gap-2">
          {EQUIP.map((e) => (
            <button key={e.id} data-testid={`ob-equip-${e.id}`} onClick={() => toggleEquip(e.id)}
              className={`px-3 py-2 rounded-full text-sm font-semibold border ${p.equip.includes(e.id) ? "bg-[#B45309] text-white border-[#B45309]" : "bg-white dark:bg-[#232A31] text-[#3F4A54] dark:text-[#AEB8BF] border-[#E6D8C3] dark:border-[#38424B]"}`}>
        ...
        <div className="grid grid-cols-2 gap-2">
          {FOCUS.map((f) => (
            <button key={f.id} data-testid={`ob-focus-${f.id}`} onClick={() => set({ focus: f.id })}
              className={`px-3 py-3 rounded-xl text-sm font-semibold border ${p.focus === f.id ? "bg-[#8C4A27] text-white border-[#8C4A27]" : "bg-white dark:bg-[#232A31] text-[#3F4A54] dark:text-[#AEB8BF] border-[#E6D8C3] dark:border-[#38424B]"}`}>{f.label}</button>
          ))}
        </div>
      ),
    },
    ...
    <div className="fixed inset-0 z-[70] bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center p-3" data-testid="onboarding">
      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-md bg-white dark:bg-[#1B2127] rounded-3xl p-6 shadow-2xl">
        <div className="flex items-center gap-2 mb-1">
          {STEPS.map((_, i) => <div key={i} className={`h-1.5 flex-1 rounded-full ${i <= step ? "bg-[#8C4A27]" : "bg-[#E6D8C3] dark:bg-[#38424B]"}`} />)}
        </div>
        <p className="text-[11px] font-bold uppercase tracking-wide text-[#8C4A27] mt-3">{tri("Passo", "Schritt", "Step")} {step + 1}/3</p>
        <div className="flex items-center gap-2 mb-4">
          <cur.icon className="w-6 h-6 text-[#8C4A27]" />
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          {step < 2 ? (
            <button data-testid="ob-next" onClick={() => setStep((x) => x + 1)}
              className="flex items-center gap-1 px-5 py-2.5 rounded-2xl bg-[#8C4A27] hover:bg-[#336a94] text-white font-semibold">
              {tri("Avanti", "Weiter", "Next")} <ChevronRight className="w-5 h-5" />
            </button>
          ) : (
            <button data-testid="ob-finish" onClick={finish}
              className="flex items-center gap-1 px-5 py-2.5 rounded-2xl bg-[#B45309] hover:bg-[#336a94] text-white font-semibold">
