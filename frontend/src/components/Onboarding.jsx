          ...
          <div className="grid grid-cols-2 gap-2">
            {TYPES.map((tp) => (
              <button key={tp.id} data-testid={`ob-type-${tp.id}`} onClick={() => set({ type: tp.id })}
                className={`px-3 py-2.5 rounded-2xl shadow-md border border-amber-900/40 text-sm font-semibold border ${p.type === tp.id ? "bg-[#3E9C93] text-white border-[#3E9C93]" : "bg-white dark:bg-[#1B2A38] text-[#3F4A54] dark:text-[#AEB8BF] border-[#2A3B49] dark:border-[#2A3B49]"}`}>{tp.label}</button>
            ))}
          </div>
        </div>
      ),
        ...
        <div className="flex flex-wrap gap-2">
          {EQUIP.map((e) => (
            <button key={e.id} data-testid={`ob-equip-${e.id}`} onClick={() => toggleEquip(e.id)}
              className={`px-3 py-2 rounded-full text-sm font-semibold border ${p.equip.includes(e.id) ? "bg-[#3E9C93] text-white border-[#3E9C93]" : "bg-white dark:bg-[#1B2A38] text-[#3F4A54] dark:text-[#AEB8BF] border-[#2A3B49] dark:border-[#2A3B49]"}`}>
        ...
        <div className="grid grid-cols-2 gap-2">
          {FOCUS.map((f) => (
            <button key={f.id} data-testid={`ob-focus-${f.id}`} onClick={() => set({ focus: f.id })}
              className={`px-3 py-3 rounded-2xl shadow-md border border-amber-900/40 text-sm font-semibold border ${p.focus === f.id ? "bg-[#3E9C93] text-white border-[#3E9C93]" : "bg-white dark:bg-[#1B2A38] text-[#3F4A54] dark:text-[#AEB8BF] border-[#2A3B49] dark:border-[#2A3B49]"}`}>{f.label}</button>
          ))}
        </div>
      ),
    },
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center p-3" data-testid="onboarding">
      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-md bg-white dark:bg-[#0D1520] rounded-3xl p-6 shadow-2xl">
        ...
        <p className="text-[11px] font-bold uppercase tracking-wide text-[#3E9C93] mt-3">{tri("Passo", "Schritt", "Step")} {step + 1}/3</p>
        <div className="flex items-center gap-2 mb-4">
          <cur.icon className="w-6 h-6 text-[#3E9C93]" />
