...
export default function CoursesPanel() {
  ...
  return (
    <div className="space-y-4" data-testid="courses-panel">
      ...
      {courses.map((c, i) => (
        <div key={i} data-testid={`course-${i}`} className="bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] rounded-2xl overflow-hidden">
          ...
          <div className="p-4">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] font-bold uppercase tracking-wide text-[#3E9C93]">{c.category}</span>
              {c.isNew && (
