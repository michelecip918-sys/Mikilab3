...
1. Verantwortlicher
Michele Signorella, Stitzenburgstraße 15, 70182 Stuttgart, Deutschland, E-Mail: michelecip918@gmail.com
...
Foto-Funktion („Com'è venuto?"): derzeit deaktiviert. Falls sie aktiviert wird: Das Foto wird in Ihrem Browser verkleinert und neu kodiert (Metadaten entfernt) und nur nach Ihrer ausdrücklichen Einwilligung (Art. 6 Abs. 1 lit. a DSGVO) zur Analyse an den KI-Anbieter gesendet. Es wird nicht gespeichert.
...
1. Titolare
Michele Signorella, Stitzenburgstraße 15, 70182 Stoccarda, Germania, email: michelecip918@gmail.com
...
Funzione foto ("Com'è venuto?"): al momento disattivata. Se verrà attivata: la foto viene ridotta e ricodificata nel tuo browser (metadati rimossi) e inviata al fornitore di IA per l'analisi solo dopo il tuo consenso esplicito (art. 6, par. 1, lett. a GDPR). Non viene salvata.
...
1. Controller
Michele Signorella, Stitzenburgstraße 15, 70182 Stuttgart, Germany, email: michelecip918@gmail.com
...
Photo function ("Com'è venuto?"): currently disabled. If enabled: the photo is resized and re-encoded in your browser (metadata removed) and sent to the AI provider for analysis only after your explicit consent (Art. 6(1)(a) GDPR). It is not stored.
...
7. Radio and links
The radio loads audio only when you press play. Your browser then connects directly to the streaming servers (RAI, Shoutcast, United Radio), which receive your IP address. We link to our TikTok profile; only when you click the link is your IP address sent to TikTok. We do not embed any content. TikTok's privacy policy applies. Fonts and images are served by our server; we do not use Google Fonts, maps or analytics services.
  ...
  return (
    <div data-testid={`legal-${kind}`} className="max-w-2xl mx-auto pt-6">
      ...
      <div className="rounded-2xl border border-border bg-background/70 p-5 space-y-4">
        {isImp
          ? <p className="text-sm text-foreground leading-relaxed whitespace-pre-line">{pick(IMPRESSUM, lang)}</p>
          : renderBlocks(pick(PRIVACY, lang))}
      </div>
    </div>
  );
}
