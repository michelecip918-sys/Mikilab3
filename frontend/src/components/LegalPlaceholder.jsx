...
Die Werkzeuge der Seite (Werkstatt, Werkzeuge im Rezept, Backstube, Bäckerrechner, Brot-Sommelier, Gäste, Brotkarte, Urkunde, Jahr als Bäcker) rechnen ausschließlich in Ihrem Browser. Fotos für „Sitors Auge" werden nur auf Ihrem Gerät ausgewertet und weder gespeichert noch gesendet. Kalenderdateien (.ics), Postkarten, Karten, Schilder und Plakate werden auf Ihrem Gerät erzeugt und nur über das Teilen-Menü Ihres Geräts weitergegeben. Ein optionaler Spitzname (kein echter Name nötig) bleibt im localStorage Ihres Geräts. Die Stimme von Sitor ist die Sprachsynthese Ihres Geräts; eine Server-Stimme ist standardmäßig abgeschaltet und würde nur den vorzulesenden Text an den Anbieter übermitteln, falls der Betreiber sie einschaltet. Das Klassenheft von „MikiLab in der Schule“ und das Spiel in „Das Brot der Kleinen“ speichern nur auf Ihrem Gerät (Spitzname der Klasse, Stempel, Notizen, Ergebnisse); Kindernamen werden nicht abgefragt, und nichts wird gesendet. Die Rezepturen des Bäckerrechners bleiben im localStorage Ihres Geräts (mikilab_formule); teilen Sie eine Rezeptur, stehen die Zahlen im Link hinter dem Zeichen #, das der Browser nicht an den Server sendet.`,
...
Gli attrezzi del sito (Officina, attrezzi nella ricetta, Laboratorio, Calcolatrice del fornaio, Sommelier del pane, Ospiti, Carta dei pani, Attestato, Anno da fornaio) calcolano solo nel tuo browser. Le foto per "L'occhio di Sitor" vengono lette solo sul tuo dispositivo, mai salvate né inviate. File calendario (.ics), cartoline, carte, cartellini e manifesti vengono creati sul tuo dispositivo e condivisi solo tramite il menu di condivisione del telefono. Un soprannome facoltativo (non serve il nome vero) resta nel localStorage del dispositivo. La voce di Sitor è la sintesi vocale del tuo dispositivo; una voce del server è spenta di default e, se il gestore la accendesse, trasmetterebbe al fornitore solo il testo da leggere. Il quaderno della classe di «MikiLab a scuola» e il gioco de «Il pane dei piccoli» salvano solo sul tuo dispositivo (soprannome della classe, timbri, appunti, risultati); non chiedono nomi di bambini e non inviano nulla. Le formule della Calcolatrice del fornaio restano nel localStorage del dispositivo (mikilab_formule); se condividi una formula, i numeri viaggiano dentro il link dopo il segno #, che il browser non invia al server.`,
...
The site's tools (Workshop, tools inside recipes, Bakery, Baker's calculator, Bread sommelier, Guests, Bread menu, Certificate, Year as a baker) compute only in your browser. Photos for "Sitor's eye" are read only on your device, never stored or sent. Calendar files (.ics), postcards, menus, tags and posters are created on your device and shared only through your phone's share menu. An optional nickname (no real name needed) stays in your device's localStorage. Sitor's voice is your device's speech synthesis; a server voice is off by default and, if the operator switched it on, would send only the text to be read to the provider. The class notebook of “MikiLab at school” and the game in “Bread for little ones” store only on your device (class nickname, stamps, notes, results); they ask for no children's names and send nothing. Formulas saved in the Baker's calculator stay in your device's localStorage (mikilab_formule); if you share a formula, the numbers travel inside the link after the # sign, which the browser does not send to the server.`,
};
