...
export default function TalkWithMiki({ tab }) {
  ...
  const send = async (text) => {
    ...
    const HINT = {
      mikemix: "[Rispondi in prima persona come Sitor, il braccio destro persiano del laboratorio] ",
            ...
            {["chat", "mikemix"].map((v) => {
              ...
              return (
                <button key={v} data-testid={`talk-tab-${v}`} onClick={() => setView(v)}
                  ...
                  <span className="text-[11px] font-bold" style={{ color: ACCENT[key] }}>{v === "chat" ? "MikiLab" : "Sitor"}</span>
                </button>
              );
            })}
            ...
            <>
              ...
              <div className="p-3 border-t border-slate-800 space-y-2">
                ...
                <div data-testid="copilot-selector" className="flex items-center gap-1">
                  {[["miki", "MikiLab"], ["mikemix", "Sitor"]].map(([id, lbl]) => (
          ...
          {view === "mikemix" && (
            <div data-testid="mikemix-lab-live" className="flex-1 overflow-y-auto p-4">
              <div className="relative rounded-2xl overflow-hidden border border-[#3E9C93]/40 aspect-video bg-slate-950">
                <img src={AV("avatar_nexus.jpg")} alt="Sitor" className="w-full h-full object-cover" style={{ animation: "cyberGlitch 4s steps(6) infinite" }} />
              ...
              <p className="text-[12px] text-slate-400 mt-3">Sitor esegue le operazioni reali del laboratorio: pulizia, gestione carrelli e infornata sincronizzata.</p>
