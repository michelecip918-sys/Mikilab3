    ...
    <div data-testid="batch-phoenix" className="fixed inset-0 z-[80] bg-[#030712]/97 backdrop-blur-xl overflow-y-auto">
      <div className="max-w-md mx-auto p-4 pb-16">
        <div className="flex items-center justify-between sticky top-0 bg-[#030712]/95 py-2 z-10">
          <h2 className="text-lg font-black text-white flex items-center gap-2"><Flame className="w-5 h-5 text-[#a89a8c]" /> Batch Phoenix</h2>
        ...
        <input data-testid="phoenix-dough" value={dough} onChange={(e) => setDough(e.target.value)} placeholder={tri("Tipo impasto (es. impasto pane 70%)", "Teigart", "Dough type (e.g. 70% bread dough)", "Tipo de masa", "Type de pâte", "نوع خمیر")} className="w-full bg-[#0b0f19] border border-[#1e293b] rounded-xl p-3 text-sm text-white outline-none focus:border-[#a89a8c] mb-2" />
        <div className="flex items-center gap-2 mb-2">
          ...
          <select data-testid="phoenix-state" value={state} onChange={(e) => setState(e.target.value)} className="flex-1 bg-[#0b0f19] border border-[#1e293b] rounded-xl p-2.5 text-sm text-white outline-none focus:border-[#a89a8c]">
            {STATES.map((s) => <option key={s.id} value={s.id}>{tri(s.it, s.de, s.en, s.es, s.fr, s.fa)}</option>)}
          </select>
        </div>
        <button data-testid="phoenix-run" onClick={run} disabled={!dough.trim() || loading} className="w-full inline-flex items-center justify-center gap-2 py-3 rounded-2xl bg-gradient-to-r from-[#a89a8c] to-[#a08f82] text-white font-black text-sm disabled:opacity-40 active:scale-98">
        ...
        {res && res.error && <p className="mt-3 text-sm text-[#bb8489] text-center">{tri("Errore. Riprova.", "Fehler.", "Error. Retry.", "Error.", "Erreur.", "خطا.")}</p>}
        {res && !res.error && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 space-y-2" data-testid="phoenix-result">
            ...
            {(res.options || []).map((o, i) => (
              <div key={i} className="rounded-2xl border border-[#a89a8c]/40 bg-[#f973160d] p-3">
