  ...
  return (
    <AnimatePresence>
      <motion.div data-testid="sfide-overlay"
        ...
        className="fixed inset-0 z-[75] bg-[#121212] dark:bg-[#121212] overflow-auto">
        <div className="max-w-xl mx-auto px-4 py-5 pb-24">
          <button data-testid="sfide-close" onClick={onClose}
            className="mb-4 flex items-center gap-1.5 text-sm font-semibold text-[#c94f00]">
            <X className="w-4 h-4" /> {tri("Chiudi", "Schließen", "Close", "Cerrar")}
          </button>

          {/* Hero */}
          <div className="relative overflow-hidden rounded-3xl p-6 text-[#121212] shadow-xl"
            ...
            <p className="text-[#121212]/85 text-sm mt-2 leading-snug">
          ...
          {/* Progresso */}
          <div data-testid="sfide-progress" className="mt-5 rounded-2xl bg-[#121212] dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-4">
            <div className="flex items-center justify-between mb-2">
              <p className="font-display text-lg font-bold text-[#c94f00] dark:text-[#e4eff8]">{tri("I tuoi progressi", "Dein Fortschritt", "Your progress", "Tu progreso")}</p>
              <span className="text-sm font-bold text-[#c94f00]">{count}/{total}</span>
            </div>
            <div className="h-3 rounded-full bg-[#2e2e2e] dark:bg-[#2e2e2e] overflow-hidden">
              <motion.div className="h-full rounded-full bg-gradient-to-r from-[#c94f00] to-[#c94f00]"
                initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.6 }} />
            </div>
            <div className="grid grid-cols-2 gap-2 mt-3">
              <div data-testid="sfide-milestone-panettoni" className={`rounded-2xl shadow-md border border-amber-900/40 p-2.5 border text-center ${count >= needPan ? "bg-[#ffffff] border-[#c94f00]" : "bg-white/60 dark:bg-[#181818] border-[#2e2e2e] dark:border-[#2e2e2e]"}`}>
                <p className="text-[11px] font-semibold text-[#c94f00] dark:text-[#AEB8BF]">{count >= needPan ? "✅ " : `🔒 ${needPan} `}{tri("sfide", "Challenges", "challenges", "retos")}</p>
                <p className="text-[13px] font-bold text-[#c94f00] dark:text-[#e4eff8] leading-tight">{tri("Panettoni + Academy", "Panettoni + Academy", "Panettoni + Academy", "Panettones + Academy")}</p>
              </div>
              <div data-testid="sfide-milestone-all" className={`rounded-2xl shadow-md border border-amber-900/40 p-2.5 border text-center ${count >= needAll ? "bg-[#ffffff] border-[#c94f00]" : "bg-white/60 dark:bg-[#181818] border-[#2e2e2e] dark:border-[#2e2e2e]"}`}>
                <p className="text-[11px] font-semibold text-[#c94f00] dark:text-[#AEB8BF]">{count >= needAll ? "✅ " : `🔒 ${needAll} `}{tri("sfide", "Challenges", "challenges", "retos")}</p>
                <p className="text-[13px] font-bold text-[#c94f00] dark:text-[#e4eff8] leading-tight">{tri("Sblocca tutto", "Alles freischalten", "Unlock everything", "Desbloquea todo")}</p>
              </div>
            </div>
            {user && count >= (state?.need_panettoni || 3) && (
              <button data-testid="sfide-certificate" onClick={downloadCertificate}
                className="mt-3 w-full flex items-center justify-center gap-2 rounded-2xl shadow-md border border-amber-900/40 bg-[#c94f00] hover:bg-[#c94f00] text-[#121212] font-semibold px-4 py-2.5 active:scale-98 transition-all">
          ...
          {user && [...completed].some((c) => c.startsWith("learn_")) && (
            <div data-testid="sfide-learn" className="mt-4 rounded-2xl bg-[#121212] dark:bg-[#1e1e1e] border border-[#2e2e2e] dark:border-[#2e2e2e] p-4">
              <p className="font-display text-base font-bold text-[#c94f00] dark:text-[#e4eff8] flex items-center gap-2 mb-2.5">
                <GraduationCap className="w-5 h-5 text-[#c94f00]" /> {tri("Percorsi Impara completati", "Abgeschlossene Lernpfade", "Completed Learn paths", "Rutas de aprendizaje completadas")}
              </p>
              <div className="flex flex-wrap gap-2" data-testid="sfide-learn-list">
                {[...completed].filter((c) => c.startsWith("learn_")).map((c) => {
                  ...
                  return (
                    <span key={c} data-testid={`sfide-learn-${k}`} className="inline-flex items-center gap-1.5 bg-[#ffffff] border border-[#c94f00] text-[#c94f00] text-[12.5px] font-bold px-3 py-1.5 rounded-full">
          ...
          {!user && (
            <div className="mt-4 rounded-2xl bg-[#1e1e1e] border border-[#2e2e2e] p-4 text-center">
              <Lock className="w-6 h-6 text-[#c94f00] mx-auto mb-2" />
              <p className="text-sm text-[#c94f00] mb-3">{tri("Registrati gratis per partecipare alle sfide e sbloccare i contenuti.", "Registriere dich, um an Challenges teilzunehmen.", "Register to join challenges and unlock content.", "Regístrate para participar en los retos.")}</p>
              <button data-testid="sfide-register" onClick={() => setAuthOpen(true)}
                className="w-full bg-[#c94f00] hover:bg-[#c94f00] text-[#121212] font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all">
              ...
              {catalog.map((c) => {
                ...
                return (
                  <div key={c.id} data-testid={`sfide-item-${c.id}`}
                    className={`flex items-start gap-3 rounded-2xl p-4 border shadow-sm transition-all ${done ? "bg-[#ffffff] border-[#c94f00]" : "bg-[#121212] dark:bg-[#1e1e1e] border-[#2e2e2e] dark:border-[#2e2e2e]"}`}>
                    <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${done ? "bg-[#c94f00] text-white" : "bg-[#c94f00]/12 text-[#c94f00]"}`}>
                      {done ? <Check className="w-6 h-6" /> : <Icon className="w-5 h-5" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="font-display text-[15px] font-bold text-[#c94f00] dark:text-[#e4eff8] leading-tight">{txt[0]}</p>
                        <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-full ${isHonor ? "bg-[#d4a373]/20 text-[#c94f00]" : "bg-[#2e8b6f]/15 text-[#2e8b6f]"}`}>
                          {isHonor ? tri("Onore", "Ehre", "Honor", "Honor") : tri("Verificata", "Geprüft", "Verified", "Verificada")}
                        </span>
                      </div>
                      <p className="text-[12.5px] text-[#c94f00] dark:text-[#AEB8BF] leading-snug mt-0.5">{txt[1]}</p>
                      {!done && (
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <button data-testid={`sfide-complete-${c.id}`} disabled={busy === c.id}
                            ...
                            className="ml-auto flex items-center gap-1.5 bg-[#c94f00] hover:bg-[#c94f00] disabled:opacity-50 text-[#121212] text-[13px] font-semibold px-4 py-2 rounded-2xl shadow-md border border-amber-900/40 active:scale-95 transition-all">
