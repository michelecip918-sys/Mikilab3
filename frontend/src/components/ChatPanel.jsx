...
const AvatarImg = ({ pic, name, size = "w-10 h-10" }) => (
  <div className={`${size} rounded-full overflow-hidden bg-[#18202E] flex items-center justify-center text-white font-bold shrink-0`}>
...
export default function ChatPanel({ open, onClose, initialUser = null }) {
  ...
  return (
    <div className="fixed inset-0 z-[75] flex items-end sm:items-center justify-center" data-testid="chat-panel">
      ...
      <div className="relative w-full sm:max-w-md bg-white dark:bg-[#0B0E14] rounded-t-3xl sm:rounded-3xl shadow-2xl h-[85vh] sm:h-[75vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center gap-3 p-4 text-white shrink-0" style={{ background: "linear-gradient(135deg,#0f2231,#18202E 50%,#1f5a68)" }}>
          {active ? (
            <>
        ...
        {!user ? (
          ...
          /* ---- Thread 1-a-1 ---- */
          <>
            <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-[#0B0E14] dark:bg-[#161b20]" data-testid="chat-thread">
              {loadingThread ? (
                  ...
                  return (
                    <div key={m.id} data-testid={`chat-msg-${m.id}`} className={`flex flex-col ${mine ? "items-end" : "items-start"}`}>
                      <div className={`group relative flex items-end gap-1 max-w-[85%] ${mine ? "flex-row-reverse" : ""}`}>
                        <div className={`rounded-2xl px-3.5 py-2 ${mine ? "bg-[#F26419] text-white rounded-br-sm" : "bg-white dark:bg-[#18202E] text-[#2B303B] dark:text-[#e4eff8] rounded-bl-sm border border-[#26324A] dark:border-[#26324A]"}`}>
                        ...
                        {reactFor === m.id && (
                          <div data-testid={`chat-react-picker-${m.id}`} className={`absolute -top-9 ${mine ? "right-8" : "left-8"} z-10 flex items-center gap-1 bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-full px-2 py-1 shadow-lg`}>
                            {REACTIONS.map((e) => (
                      ...
                      {summary.length > 0 && (
                        <div data-testid={`chat-reactions-${m.id}`} className={`flex gap-1 mt-0.5 ${mine ? "pr-1" : "pl-1"}`}>
                          {summary.map(([e, n]) => (
                            <button key={e} onClick={() => react(m.id, e)} className="inline-flex items-center gap-0.5 text-[11px] bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] rounded-full px-1.5 py-0.5">
            ...
            <div className="p-3 border-t border-[#26324A] dark:border-[#26324A] flex items-center gap-2 shrink-0">
              ...
              <button data-testid="chat-photo" onClick={() => fileRef.current?.click()} disabled={uploading} className="w-10 h-10 rounded-full bg-[#e4eff8] dark:bg-[#18202E] text-[#F26419] flex items-center justify-center active:scale-90 disabled:opacity-50 shrink-0">
              ...
              <button data-testid="chat-send" data-sfx="confirm" onClick={send} disabled={sending || !text.trim()} className="w-11 h-11 rounded-full bg-[#F26419] text-white flex items-center justify-center active:scale-90 disabled:opacity-50 shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2" data-testid="chat-new">
            <button data-testid="chat-new-back" onClick={() => setShowNew(false)} className="flex items-center gap-1.5 text-sm font-semibold text-[#F26419] mb-1"><ArrowLeft className="w-4 h-4" />{tri("Indietro", "Zurück", "Back", "Atrás")}</button>
            <div className="flex items-center gap-2 bg-[#e4eff8] dark:bg-[#18202E] rounded-2xl shadow-md border border-amber-900/40 px-3 py-2">
            ...
            {filteredFriends.length === 0 ? (
              ...
              <button key={f.user_id} data-testid={`chat-friend-${f.user_id}`} onClick={() => { setShowNew(false); setActive({ user_id: f.user_id, name: f.name, picture: f.picture }); }}
                className="w-full flex items-center gap-3 p-2.5 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] active:scale-98 transition-all">
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-1.5" data-testid="chat-convos">
              {loadingList ? (
                ...
                <button key={c.other_id} data-testid={`chat-convo-${c.other_id}`} onClick={() => setActive({ user_id: c.other_id, name: c.name, picture: c.picture })}
                  className="w-full flex items-center gap-3 p-2.5 rounded-2xl shadow-md border border-amber-900/40 hover:bg-[#0B0E14] dark:hover:bg-[#18202E] active:scale-98 transition-all text-left">
                  ...
                  {c.unread > 0 && <span data-testid={`chat-unread-${c.other_id}`} className="min-w-[20px] h-5 px-1.5 rounded-full bg-[#F26419] text-white text-[11px] font-bold flex items-center justify-center shrink-0">{c.unread}</span>}
                </button>
              ))}
            </div>
            <div className="p-3 border-t border-[#26324A] dark:border-[#26324A] shrink-0">
              <button data-testid="chat-new-btn" onClick={openNew} className="w-full flex items-center justify-center gap-2 bg-[#F26419] hover:bg-[#E8A838] text-white font-semibold py-3 rounded-2xl shadow-md border border-amber-900/40 active:scale-98 transition-all">
