      ...
      <div className="relative w-full sm:max-w-md bg-white dark:bg-[#121212] rounded-t-3xl sm:rounded-3xl shadow-2xl h-[85vh] sm:h-[75vh] flex flex-col overflow-hidden">
        ...
        {!user ? (
          ...
          <>
                  ...
                  return (
                    <div key={m.id} data-testid={`chat-msg-${m.id}`} className={`flex flex-col ${mine ? "items-end" : "items-start"}`}>
                      <div className={`group relative flex items-end gap-1 max-w-[85%] ${mine ? "flex-row-reverse" : ""}`}>
                        <div className={`rounded-2xl px-3.5 py-2 ${mine ? "bg-[#ff6b00] text-white rounded-br-sm" : "bg-white dark:bg-[#1e1e1e] text-[#2B303B] dark:text-[#e4eff8] rounded-bl-sm border border-[#2b2b2b] dark:border-[#2e2e2e]"}`}>
                        ...
                        {reactFor === m.id && (
                          <div data-testid={`chat-react-picker-${m.id}`} className={`absolute -top-9 ${mine ? "right-8" : "left-8"} z-10 flex items-center gap-1 bg-white dark:bg-[#242424] border border-[#2b2b2b] dark:border-[#2e2e2e] rounded-full px-2 py-1 shadow-lg`}>
                            {REACTIONS.map((e) => (
                      ...
                      {summary.length > 0 && (
                        <div data-testid={`chat-reactions-${m.id}`} className={`flex gap-1 mt-0.5 ${mine ? "pr-1" : "pl-1"}`}>
                          {summary.map(([e, n]) => (
                            <button key={e} onClick={() => react(m.id, e)} className="inline-flex items-center gap-0.5 text-[11px] bg-white dark:bg-[#242424] border border-[#2b2b2b] dark:border-[#2e2e2e] rounded-full px-1.5 py-0.5">
            ...
            <div className="p-3 border-t border-[#2b2b2b] dark:border-[#2e2e2e] flex items-center gap-2 shrink-0">
              ...
              <button data-testid="chat-photo" onClick={() => fileRef.current?.click()} disabled={uploading} className="w-10 h-10 rounded-full bg-[#e4eff8] dark:bg-[#242424] text-[#ff6b00] flex items-center justify-center active:scale-90 disabled:opacity-50 shrink-0">
              ...
              <input data-testid="chat-input" value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && (e.preventDefault(), send())}
                ...
                className="flex-1 bg-[#121212] dark:bg-[#181818] border border-[#2b2b2b] dark:border-[#2e2e2e] rounded-full px-4 py-2.5 outline-none text-sm text-[#2B303B] dark:text-[#e4eff8] focus:border-[#ff6b00]" />
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2" data-testid="chat-new">
            ...
            {filteredFriends.length === 0 ? (
              ...
              <button key={f.user_id} data-testid={`chat-friend-${f.user_id}`} onClick={() => { setShowNew(false); setActive({ user_id: f.user_id, name: f.name, picture: f.picture }); }}
                className="w-full flex items-center gap-3 p-2.5 rounded-xl bg-white dark:bg-[#1e1e1e] border border-[#2b2b2b] dark:border-[#2e2e2e] active:scale-98 transition-all">
