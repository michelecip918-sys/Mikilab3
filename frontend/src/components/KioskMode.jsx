...
import { Tablet, Maximize2, Download, Share, Plus, X, Power, Lock } from "lucide-react";
...
export default function KioskMode() {
  ...
  // Uscita anti-tocco accidentale: tieni premuto ~1.2s
  const startHold = () => {
    const started = Date.now();
    holdRef.current = setInterval(() => {
      const pct = Math.min(100, ((Date.now() - started) / 1200) * 100);
      setHoldPct(pct);
      if (pct >= 100) { clearInterval(holdRef.current); holdRef.current = null; exitKiosk(); }
    }, 40);
  };
      ...
      {resume ? (
        ...
        >
          ...
          <span className="relative flex items-center gap-2">
            ...
            {tri("KIOSK · tieni premuto per uscire", "KIOSK · halten zum Beenden", "KIOSK · hold to exit", "KIOSK · mantén para salir", "KIOSK · maintiens pour quitter", "کیوسک · برای خروج نگه دار")}
          </span>
        </button>
      )}
    </div>
