...
export default function KioskMode() {
  ...
  // Modalità Tablet: NON si apre più da sola. Resta disponibile dal pulsante Tablet,
  // così non ricompare a ogni accesso.
  useEffect(() => {
    ...
    <div data-testid="kiosk-badge" className="fixed bottom-3 left-3 z-[9998] flex items-center gap-2">
      {resume ? (
        <button data-testid="kiosk-resume-btn" onClick={enterKiosk}
          className="flex items-center gap-2 px-3.5 py-2 rounded-full bg-background border border-border/50 text-muted-foreground text-xs font-bold shadow-[0_0_16px_rgba(138,151,166,0.25)] active:scale-95 transition-all">
        ...
        <button
          ...
          className="relative flex items-center gap-2 px-3.5 py-2 rounded-full bg-background border border-border/40 text-foreground text-xs font-bold overflow-hidden select-none active:scale-95 transition-all"
        ...
        >
          <span className="absolute inset-y-0 left-0 bg-muted/25 transition-none" style={{ width: `${holdPct}%` }} />
    ...
    <>
      {!kiosk && (
        <button data-testid="kiosk-chip" onClick={() => setOpen(true)} title={tri("Modalità Tablet", "Tablet-Modus", "Tablet Mode", "Modo Tablet", "Mode Tablette", "حالت تبلت")}
          className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-muted/10 border border-border/30 text-muted-foreground font-bold text-xs hover:bg-muted/20 active:scale-95 transition-all">
      ...
      {exitPinOpen && createPortal(
        <div data-testid="kiosk-exit-pin" className="fixed inset-0 z-[145] flex items-center justify-center bg-background/80 backdrop-blur-md p-4" onClick={() => { setExitPinOpen(false); setPin(""); setPinErr(false); }}>
          <div className="w-full max-w-xs rounded-3xl bg-background border border-border/25 shadow-[0_0_40px_rgba(138,151,166,0.2)] p-6 text-center" onClick={(e) => e.stopPropagation()}>
            <div className="w-14 h-14 mx-auto rounded-2xl overflow-hidden border border-border/40 bg-background mb-3 flex items-center justify-center">
              <Lock className="w-6 h-6 text-muted-foreground" />
            </div>
            <h3 className="font-display text-base font-black tracking-wide text-foreground uppercase">{tri("Uscita Kiosk", "Kiosk verlassen", "Exit Kiosk", "Salir de Kiosk", "Quitter Kiosk", "خروج از کیوسک")}</h3>
            <p className="mt-1.5 text-xs text-muted-foreground flex items-center justify-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-muted-foreground" /> {tri("Inserisci il PIN della Direzione", "Capo-PIN eingeben", "Enter the Capo PIN", "Introduce el PIN del Capo", "Saisis le PIN du Capo", "PIN کاپو را وارد کن")}</p>

            <div className={`mt-5 flex justify-center gap-3 ${pinErr ? "animate-shake" : ""}`}>
              {[0, 1, 2, 3].map((i) => (
                <div key={i} data-testid={`kiosk-pin-dot-${i}`} className={`w-3.5 h-3.5 rounded-full border-2 transition-all ${pin.length > i ? "bg-muted border-border" : "border-border"}`} />
              ))}
            </div>
            ...
            <div className="mt-6 grid grid-cols-3 gap-2.5">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => (
                <button key={n} data-testid={`kiosk-pin-key-${n}`} onClick={() => pushPin(String(n))}
                  className="h-14 rounded-2xl bg-background border border-border text-xl font-bold text-foreground hover:border-border active:scale-95 transition-all">{n}</button>
              ))}
              <button data-testid="kiosk-pin-cancel" onClick={() => { setExitPinOpen(false); setPin(""); setPinErr(false); }} className="h-14 rounded-2xl bg-background border border-border text-muted-foreground hover:border-border active:scale-95 transition-all flex items-center justify-center"><X className="w-5 h-5" /></button>
              <button data-testid="kiosk-pin-key-0" onClick={() => pushPin("0")} className="h-14 rounded-2xl bg-background border border-border text-xl font-bold text-foreground hover:border-border active:scale-95 transition-all">0</button>
              <button data-testid="kiosk-pin-back" onClick={() => setPin((p) => p.slice(0, -1))} className="h-14 rounded-2xl bg-background border border-border text-muted-foreground hover:border-border active:scale-95 transition-all flex items-center justify-center"><Delete className="w-5 h-5" /></button>
            </div>
          </div>
        </div>,
        document.body
      ...
      {open && createPortal(
        <div data-testid="kiosk-modal" className="fixed inset-0 z-[130] overflow-y-auto bg-background/75 backdrop-blur-sm" onClick={() => setOpen(false)}>
          <div className="min-h-full flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-3xl bg-background border border-border/25 shadow-[0_0_40px_rgba(138,151,166,0.15)] p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-1">
              ...
              <button data-testid="kiosk-modal-close" onClick={() => setOpen(false)} className="text-muted-foreground hover:text-foreground"><X className="w-5 h-5" /></button>
            </div>
            <p className="text-xs text-muted-foreground mb-5">{tri(
            ...
            {/* Step 1 — Installa app */}
            <div className="rounded-2xl bg-background border border-border p-4 mb-3">
              <div className="flex items-center gap-2 mb-2">
                <span className="w-6 h-6 rounded-full bg-muted/15 border border-border/40 text-muted-foreground text-xs font-black flex items-center justify-center">1</span>
                <span className="text-sm font-bold text-foreground">{tri("Installa l'app", "App installieren", "Install the app", "Instala la app", "Installe l'app", "نصب اپ")}</span>
                {installed && <span className="ml-auto text-[10px] font-bold text-muted-foreground">✓ {tri("Installata", "Installiert", "Installed", "Instalada", "Installée", "نصب‌شده")}</span>}
              </div>
              {!installed && (
                <button data-testid="kiosk-install-btn" onClick={installClick}
                  className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-muted/10 border border-border/40 text-muted-foreground font-bold text-sm hover:bg-muted/20 active:scale-95 transition-all">
            ...
            <div className="rounded-2xl bg-background border border-border p-4">
              <div className="flex items-center gap-2 mb-2">
                <span className="w-6 h-6 rounded-full bg-muted/15 border border-border/40 text-muted-foreground text-xs font-black flex items-center justify-center">2</span>
                <span className="text-sm font-bold text-foreground">{tri("Avvia modalità Kiosk", "Kiosk-Modus starten", "Start Kiosk mode", "Inicia modo Kiosk", "Démarre le mode Kiosk", "شروع حالت کیوسک")}</span>
              </div>
              ...
              <button data-testid="kiosk-start-btn" onClick={enterKiosk}
                className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-br from-muted/25 to-muted/10 border border-border/60 text-muted-foreground font-black text-sm shadow-[0_0_18px_rgba(138,151,166,0.25)] hover:from-muted/35 active:scale-95 transition-all">
      ...
      {iosHelp && createPortal(
        <div data-testid="kiosk-ios-help" className="fixed inset-0 z-[140] flex items-end sm:items-center justify-center bg-background/75 p-4" onClick={() => setIosHelp(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-background border border-border p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-extrabold text-foreground">{tri("Aggiungi a Home", "Zum Home hinzufügen", "Add to Home", "Añadir a inicio", "Ajouter à l'accueil", "افزودن به صفحه اصلی")}</h3>
              <button onClick={() => setIosHelp(false)} className="text-muted-foreground hover:text-foreground"><X className="w-5 h-5" /></button>
            </div>
            <ol className="space-y-2 text-sm text-foreground">
              <li className="flex items-center gap-2"><Share className="w-4 h-4 text-muted-foreground" /> {tri("Tocca «Condividi» nel browser", "Tippe auf «Teilen»", "Tap «Share» in the browser", "Toca «Compartir»", "Touche «Partager»", "روی «اشتراک» بزن")}</li>
              <li className="flex items-center gap-2"><Plus className="w-4 h-4 text-muted-foreground" /> {tri("Scegli «Aggiungi a Home»", "Wähle «Zum Home-Bildschirm»", "Choose «Add to Home Screen»", "Elige «Añadir a inicio»", "Choisis «Sur l'écran d'accueil»", "«افزودن به صفحه اصلی» را انتخاب کن")}</li>
            </ol>
          </div>
        </div>,
        document.body
