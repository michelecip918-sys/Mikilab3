    ...
    <div data-testid="kiosk-badge" className="fixed bottom-3 left-3 z-[9998] flex items-center gap-2">
      {resume ? (
        <button data-testid="kiosk-resume-btn" onClick={enterKiosk}
          className="flex items-center gap-2 px-3.5 py-2 rounded-full bg-[#0C1019] border border-[#00F0FF]/50 text-[#00F0FF] text-xs font-bold shadow-[0_0_16px_rgba(0,240,255,0.25)] active:scale-95 transition-all">
        ...
        <button
          ...
          className="relative flex items-center gap-2 px-3.5 py-2 rounded-full bg-[#0C1019] border border-[#00F0FF]/40 text-white text-xs font-bold overflow-hidden select-none active:scale-95 transition-all"
        ...
        >
          <span className="absolute inset-y-0 left-0 bg-[#00F0FF]/25 transition-none" style={{ width: `${holdPct}%` }} />
    ...
    <>
      {!kiosk && (
        <button data-testid="kiosk-chip" onClick={() => setOpen(true)} title={tri("Modalità Tablet", "Tablet-Modus", "Tablet Mode", "Modo Tablet", "Mode Tablette", "حالت تبلت")}
          className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-[#00F0FF]/10 border border-[#00F0FF]/30 text-[#00F0FF] font-bold text-xs hover:bg-[#00F0FF]/20 active:scale-95 transition-all">
      ...
      {exitPinOpen && createPortal(
        <div data-testid="kiosk-exit-pin" className="fixed inset-0 z-[145] flex items-center justify-center bg-black/80 backdrop-blur-md p-4" onClick={() => { setExitPinOpen(false); setPin(""); setPinErr(false); }}>
          <div className="w-full max-w-xs rounded-3xl bg-[#070A10] border border-[#00F0FF]/25 shadow-[0_0_40px_rgba(0,240,255,0.2)] p-6 text-center" onClick={(e) => e.stopPropagation()}>
            <div className="w-14 h-14 mx-auto rounded-2xl overflow-hidden border border-[#00F0FF]/40 bg-[#030712] mb-3 flex items-center justify-center">
            ...
            <p className="mt-1.5 text-xs text-[#94A3B8] flex items-center justify-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-[#00F0FF]" /> {tri("Inserisci il PIN del Capo", "Capo-PIN eingeben", "Enter the Capo PIN", "Introduce el PIN del Capo", "Saisis le PIN du Capo", "PIN کاپو را وارد کن")}</p>

            <div className={`mt-5 flex justify-center gap-3 ${pinErr ? "animate-shake" : ""}`}>
              {[0, 1, 2, 3].map((i) => (
                <div key={i} data-testid={`kiosk-pin-dot-${i}`} className={`w-3.5 h-3.5 rounded-full border-2 transition-all ${pin.length > i ? "bg-[#00F0FF] border-[#00F0FF]" : "border-[#334155]"}`} />
              ))}
            </div>
            ...
            <div className="mt-6 grid grid-cols-3 gap-2.5">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => (
                <button key={n} data-testid={`kiosk-pin-key-${n}`} onClick={() => pushPin(String(n))}
                  className="h-14 rounded-2xl bg-[#0C1019] border border-[#1e293b] text-xl font-bold text-white hover:border-[#00F0FF] active:scale-95 transition-all">{n}</button>
              ))}
              <button data-testid="kiosk-pin-cancel" onClick={() => { setExitPinOpen(false); setPin(""); setPinErr(false); }} className="h-14 rounded-2xl bg-[#0C1019] border border-[#1e293b] text-[#64748B] hover:border-[#00F0FF] active:scale-95 transition-all flex items-center justify-center"><X className="w-5 h-5" /></button>
              <button data-testid="kiosk-pin-key-0" onClick={() => pushPin("0")} className="h-14 rounded-2xl bg-[#0C1019] border border-[#1e293b] text-xl font-bold text-white hover:border-[#00F0FF] active:scale-95 transition-all">0</button>
              <button data-testid="kiosk-pin-back" onClick={() => setPin((p) => p.slice(0, -1))} className="h-14 rounded-2xl bg-[#0C1019] border border-[#1e293b] text-[#64748B] hover:border-[#00F0FF] active:scale-95 transition-all flex items-center justify-center"><Delete className="w-5 h-5" /></button>
            </div>
          </div>
        </div>,
        document.body
      ...
      {open && createPortal(
        <div data-testid="kiosk-modal" className="fixed inset-0 z-[130] overflow-y-auto bg-black/75 backdrop-blur-sm" onClick={() => setOpen(false)}>
          ...
          <div className="w-full max-w-md rounded-3xl bg-[#070A10] border border-[#00F0FF]/25 shadow-[0_0_40px_rgba(0,240,255,0.15)] p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2.5">
                <span className="w-10 h-10 rounded-xl overflow-hidden border border-[#00F0FF]/40 bg-[#030712] shrink-0">
            ...
            <div className="rounded-2xl bg-[#0C1019] border border-[#1e293b] p-4 mb-3">
              <div className="flex items-center gap-2 mb-2">
                <span className="w-6 h-6 rounded-full bg-[#00F0FF]/15 border border-[#00F0FF]/40 text-[#00F0FF] text-xs font-black flex items-center justify-center">1</span>
              ...
              {!installed && (
                <button data-testid="kiosk-install-btn" onClick={installClick}
                  className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#00F0FF]/10 border border-[#00F0FF]/40 text-[#00F0FF] font-bold text-sm hover:bg-[#00F0FF]/20 active:scale-95 transition-all">
            ...
            <div className="rounded-2xl bg-[#0C1019] border border-[#1e293b] p-4">
              <div className="flex items-center gap-2 mb-2">
                <span className="w-6 h-6 rounded-full bg-[#00F0FF]/15 border border-[#00F0FF]/40 text-[#00F0FF] text-xs font-black flex items-center justify-center">2</span>
              ...
              <button data-testid="kiosk-start-btn" onClick={enterKiosk}
                className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-br from-[#00F0FF]/25 to-[#00F0FF]/10 border border-[#00F0FF]/60 text-[#00F0FF] font-black text-sm shadow-[0_0_18px_rgba(0,240,255,0.25)] hover:from-[#00F0FF]/35 active:scale-95 transition-all">
      ...
      {iosHelp && createPortal(
        <div data-testid="kiosk-ios-help" className="fixed inset-0 z-[140] flex items-end sm:items-center justify-center bg-black/75 p-4" onClick={() => setIosHelp(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-[#070A10] border border-[#1e293b] p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <ol className="space-y-2 text-sm text-[#cbd5e1]">
              <li className="flex items-center gap-2"><Share className="w-4 h-4 text-[#00F0FF]" /> {tri("Tocca «Condividi» nel browser", "Tippe auf «Teilen»", "Tap «Share» in the browser", "Toca «Compartir»", "Touche «Partager»", "روی «اشتراک» بزن")}</li>
              <li className="flex items-center gap-2"><Plus className="w-4 h-4 text-[#00F0FF]" /> {tri("Scegli «Aggiungi a Home»", "Wähle «Zum Home-Bildschirm»", "Choose «Add to Home Screen»", "Elige «Añadir a inicio»", "Choisis «Sur l'écran d'accueil»", "«افزودن به صفحه اصلی» را انتخاب کن")}</li>
            </ol>
          </div>
        </div>,
        document.body
