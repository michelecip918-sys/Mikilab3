...
// Fase 9 · Simulatore AR per forni tradizionali — inquadri il forno e ricevi le indicazioni di Sitor.
export default function AROven() {
  ...
  const scanOven = async () => {
    ...
    try {
      // Collegato al motore reale di Sitor: ricalcolo per la ricetta scelta.
      ...
      setGuide({
        ...
        note: data.summary || tri("Adattato da Sitor per il tuo forno.", "Von Sitor angepasst.", "Adapted by Sitor for your oven.", "Adaptado por Sitor.", "Adapté par Sitor.", "تنظیم شد."),
        live: true,
      });
    } catch { fallback(); }
    setScanning(false);
  ...
  return (
    <div data-testid="ar-oven" className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-[10.5px] text-[#94A3B8]">{tri("Inquadra il forno: Sitor calcola tempi e temperature.", "Ofen anvisieren: Sitor rechnet.", "Point at the oven: Sitor computes times and temps.", "Enfoca el horno: Sitor calcula.", "Vise le four : Sitor calcule.", "تنور را بگیر: Sitor محاسبه می‌کند.")}</p></div>
      </div>
      ...
      {!on ? (
        ...
        <div className="relative rounded-xl overflow-hidden border border-[#FFB800]/40 bg-[#060A10] aspect-video">
          ...
          <div className="absolute top-2 left-2 flex items-center gap-1 text-[9px] font-mono uppercase tracking-widest text-[#FFB800] bg-black/50 px-1.5 py-0.5 rounded"><Flame className="w-3 h-3" /> AR · Sitor</div>
          {/* Overlay indicazioni */}
          {guide && (
            <div data-testid="ar-guide" className="absolute bottom-2 left-2 right-2 rounded-lg bg-black/70 backdrop-blur-md border border-[#FFB800]/40 p-2.5 grid grid-cols-3 gap-2">
              {guide.live && <span className="absolute -top-2 right-2 px-1.5 py-0.5 rounded bg-[#22c55e] text-[#060A10] text-[8px] font-black uppercase tracking-widest">Live · Sitor</span>}
