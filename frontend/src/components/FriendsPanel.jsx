...
const Avatar = ({ c }) => (
  <div className="w-10 h-10 rounded-2xl shadow-md border border-amber-900/40 bg-[#3E9C93]/15 flex items-center justify-center overflow-hidden shrink-0">
    {c.picture ? <img src={c.picture} alt={c.name} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = "none"; }} />
      : <span className="font-bold text-[#3E9C93]">{(c.name || "?").slice(0, 1).toUpperCase()}</span>}
  </div>
);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 p-3">
              {TABS.map((t) => (
                <button key={t.id} data-testid={`friends-tab-${t.id}`} onClick={() => setTab(t.id)}
                  className={`py-2 rounded-2xl shadow-md border border-amber-900/40 text-xs font-semibold transition-all ${tab === t.id ? "bg-[#3E9C93] text-white" : "bg-[#e4eff8] dark:bg-[#1B2A38] text-[#7E8A93]"}`}>
                  {t.label}{t.n ? ` (${t.n})` : ""}
                </button>
              ))}
            </div>
                ...
                : rel.incoming.map((c) => (
                  <Row key={c.user_id} c={c} testid={`friend-incoming-${c.user_id}`}>
                    <button data-testid={`friend-accept-${c.user_id}`} onClick={() => act(() => friendsApi.respond(c.user_id, "accept"), tri("Ora siete amici!", "Ihr seid jetzt Freunde!", "You are now friends!"))}
                      className="px-3 py-1.5 rounded-lg bg-[#3E9C93] text-white text-xs font-bold flex items-center gap-1"><Check className="w-3.5 h-3.5" />{tri("Accetta", "Annehmen", "Accept")}</button>
                    <button data-testid={`friend-decline-${c.user_id}`} onClick={() => act(() => friendsApi.respond(c.user_id, "decline"))}
                      className="px-3 py-1.5 rounded-lg bg-[#e4eff8] dark:bg-[#1B2A38] text-[#7E8A93] text-xs font-semibold">{tri("Rifiuta", "Ablehnen", "Decline")}</button>
                  </Row>
                )))}
                  ...
                  <Row key={c.user_id} c={c} testid={`friend-item-${c.user_id}`}>
                    {onMessage && (
                      <button data-testid={`friend-message-${c.user_id}`} onClick={() => onMessage({ user_id: c.user_id, name: c.name, picture: c.picture })}
                        className="px-3 py-1.5 rounded-lg bg-[#3E9C93] text-white text-xs font-bold flex items-center gap-1"><Send className="w-3.5 h-3.5" />{tri("Scrivi", "Schreiben", "Message")}</button>
                    )}
                    <button data-testid={`friend-remove-${c.user_id}`} onClick={() => act(() => friendsApi.remove(c.user_id))}
                      className="px-3 py-1.5 rounded-lg bg-[#e4eff8] dark:bg-[#1B2A38] text-[#3E9C93] text-xs font-semibold flex items-center gap-1"><UserMinus className="w-3.5 h-3.5" />{tri("Rimuovi", "Entfernen", "Remove")}</button>
                  </Row>
                )))}

              {tab === "trova" && !loading && (
                <>
                  <div className="flex items-center gap-2 bg-[#e4eff8] dark:bg-[#1B2A38] rounded-2xl shadow-md border border-amber-900/40 px-3 py-2 mb-1">
                  ...
                  {!q.trim() && sugg.length > 0 && (
                    <div data-testid="friends-suggestions" className="mb-2">
                      <p className="text-[11px] font-bold uppercase tracking-wide text-[#3E9C93] px-1 py-1.5 flex items-center gap-1"><Sparkles className="w-3.5 h-3.5" />{tri("Suggeriti per te", "Für dich vorgeschlagen", "Suggested for you")}</p>
                      {sugg.map((c) => {
                        ...
                        return (
                          <Row key={`sg-${c.user_id}`} c={c} testid={`friend-sugg-${c.user_id}`} sub={reason}>
                            <button data-testid={`friend-sugg-add-${c.user_id}`} onClick={() => act(() => friendsApi.request(c.user_id), tri("Richiesta inviata!", "Anfrage gesendet!", "Request sent!"))}
                              className="px-3 py-1.5 rounded-lg bg-[#3E9C93] text-white text-xs font-bold flex items-center gap-1"><UserPlus className="w-3.5 h-3.5" />{tri("Aggiungi", "Hinzufügen", "Add")}</button>
                          </Row>
                        );
                      })}
                  ...
                  {filtered.length === 0 ? <Empty text={tri("Nessun utente trovato.", "Keine Nutzer.", "No users found.")} />
                    : filtered.map((c) => (
                      <Row key={c.user_id} c={c} testid={`friend-dir-${c.user_id}`} sub={c.email}>
                        {c.status === "friends" && <span className="text-xs font-semibold text-[#3E9C93] flex items-center gap-1"><Check className="w-3.5 h-3.5" />{tri("Amici", "Freunde", "Friends")}</span>}
                        ...
                        {c.status === "incoming" && <button data-testid={`friend-accept2-${c.user_id}`} onClick={() => act(() => friendsApi.respond(c.user_id, "accept"), tri("Ora siete amici!", "Freunde!", "Now friends!"))}
                          className="px-3 py-1.5 rounded-lg bg-[#3E9C93] text-white text-xs font-bold">{tri("Accetta", "Annehmen", "Accept")}</button>}
...
function Row({ c, children, testid, sub }) {
  return (
    <div data-testid={testid} className="flex items-center gap-3 rounded-2xl shadow-md border border-amber-900/40 bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-2.5">
      <Avatar c={c} />
...
function Empty({ text }) {
  return <div className="rounded-2xl shadow-md border border-amber-900/40 bg-[#e4eff8] dark:bg-[#1B2A38] border border-dashed border-[#2A3B49] dark:border-[#2A3B49] p-4 text-sm text-[#7E8A93]">{text}</div>;
}
