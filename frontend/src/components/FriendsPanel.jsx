...
export default function FriendsPanel({ open, onClose, onCount }) {
  ...
  const filtered = dir.filter((c) => c.name.toLowerCase().includes(q.toLowerCase()));
              ...
              {tab === "trova" && !loading && (
                <>
                  <div className="flex items-center gap-2 bg-[#e4eff8] dark:bg-[#232A31] rounded-xl px-3 py-2 mb-1">
                    ...
                    <input data-testid="friends-search" value={q} onChange={(e) => setQ(e.target.value)} placeholder={tri("Cerca un fornaio…", "Bäcker suchen…", "Search a baker…")}
                  ...
                  {filtered.length === 0 ? <Empty text={tri("Nessun utente trovato.", "Keine Nutzer.", "No users found.")} />
                    : filtered.map((c) => (
                      <Row key={c.user_id} c={c} testid={`friend-dir-${c.user_id}`}>
...
function Row({ c, children, testid }) {
  return (
    <div data-testid={testid} className="flex items-center gap-3 rounded-xl bg-white dark:bg-[#232A31] border border-[#d5e4f0] dark:border-[#38424B] p-2.5">
      <Avatar c={c} />
      <p className="flex-1 min-w-0 text-sm font-semibold text-[#2B303B] dark:text-[#EAF0EC] truncate">{c.name}</p>
