...
// MOTORE MIKILAB — il Capo detta l'ordine, calcoliamo il piano A RITROSO e lo inviamo a Mike Mix.
export default function OrdineCapo() {
  ...
  const sendToMikeMix = async () => {
    ...
    try {
      ...
      toast.success(tri("Piano inviato a Mike Mix! Coordinerà il team a voce.", "Plan an Mike Mix gesendet!", "Plan sent to Mike Mix! He'll coordinate the team by voice.", "¡Plan enviado a Mike Mix!", "Plan envoyé à Mike Mix !", "برنامه به Mike Mix ارسال شد!"));
      ...
      {res && (
        <div data-testid="ordine-result" className="rounded-xl bg-[#030712] border border-[#14b8a6]/30 p-4">
          ...
          <button data-testid="ordine-send-mikemix" onClick={sendToMikeMix} disabled={sending}
            ...
            {sending ? tri("Invio…", "Senden…", "Sending…", "Enviando…", "Envoi…", "ارسال…") : tri("Invia a Mike Mix (coordina il team)", "An Mike Mix senden", "Send to Mike Mix (coordinates the team)", "Enviar a Mike Mix", "Envoyer à Mike Mix", "ارسال به Mike Mix")}
          </button>
        </div>
      )}
    </div>
