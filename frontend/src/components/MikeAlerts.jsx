  ...
  return (
    <div data-testid="mike-alerts" className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button data-testid="alerts-refresh" onClick={load} className="text-[#7DD3FC] active:scale-90"><RefreshCw className={`w-4 h-4 ${busy ? "animate-spin" : ""}`} /></button>
      ...
      {alerts.length === 0 ? (
        <p data-testid="alerts-empty" className="text-[12px] text-[#8aa0b4]">{tri("Nessuna anomalia. Il campo è sereno.", "Keine Anomalien. Alles ruhig.", "No anomalies. The floor is calm.", "Sin anomalías.", "Aucune anomalie.", "بدون ناهنجاری.")}</p>
      ) : (
        <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
          {alerts.map((a) => (
            <div key={a.id} data-testid={`alert-${a.id}`} className={`rounded-lg border p-3 ${a.read ? "border-[#1e293b] bg-[#070A10]" : "border-[#f59e0b]/40 bg-[#f59e0b]/8"}`}>
              ...
              {a.advice && <p className="mt-1 text-[11.5px] text-[#9fb3c4] leading-snug">→ {a.advice}</p>}
