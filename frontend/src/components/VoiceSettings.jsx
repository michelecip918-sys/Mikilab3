...
export default function VoiceSettings({ open, onClose }) {
  ...
  const Group = ({ title, sel, setSel, who }) => (
    <div className="mb-4">
      <p className="font-display font-bold text-[#2B303B] dark:text-[#EAF0EC] mb-2">{title}</p>
      <div className="space-y-2">
        {VOICE_OPTIONS.map((v) => {
          const on = sel === v.id;
          return (
            <div key={v.id} className={`flex items-center gap-2 rounded-2xl border p-2.5 ${on ? "border-[#5E8B7E] bg-[#5E8B7E]/8" : "border-[#D7E1DB] dark:border-[#38424B]"}`}>
              <button data-testid={`voice-${who}-${v.name}`} onClick={() => setSel(v.id)} className="flex-1 text-left min-w-0">
                <span className="font-semibold text-sm text-[#2B303B] dark:text-[#EAF0EC]">{v.name}</span>
                <span className="text-xs text-[#7E8A93] block truncate">{v.desc[lang] || v.desc.it}</span>
              </button>
              <button data-testid={`voice-preview-${who}-${v.name}`} onClick={() => preview(v.id, who)} className="w-9 h-9 rounded-full bg-[#EAF0EC] dark:bg-[#2A323A] flex items-center justify-center shrink-0 active:scale-90 transition-transform">
                {previewing === v.id + who ? <Loader2 className="w-4 h-4 animate-spin text-[#5E8B7E]" /> : <Volume2 className="w-4 h-4 text-[#5E8B7E]" />}
              </button>
              {on && <Check className="w-5 h-5 text-[#5E8B7E] shrink-0" />}
            </div>
          );
        })}
      </div>
      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div data-testid="voice-settings" initial={{ y: 24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 24, opacity: 0 }}
            ...
            className="w-full max-w-md rounded-3xl bg-[#F6F8F5] dark:bg-[#1B2127] border border-[#D7E1DB] dark:border-[#38424B] shadow-2xl p-5 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display text-xl font-bold text-[#2B303B] dark:text-[#EAF0EC]">{tri("Scegli le voci 🎙️", "Stimmen wählen 🎙️", "Choose the voices 🎙️")}</h2>
              <button data-testid="voice-settings-close" onClick={onClose} className="w-9 h-9 rounded-full bg-[#EAF0EC] dark:bg-[#2A323A] flex items-center justify-center"><X className="w-4 h-4" /></button>
            </div>
            ...
            <button data-testid="voice-settings-save" onClick={save}
              className="w-full bg-[#5E8B7E] hover:bg-[#4C7368] text-white font-semibold px-5 py-3 rounded-2xl active:scale-98 transition-all">
