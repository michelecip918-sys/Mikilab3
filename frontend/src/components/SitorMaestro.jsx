...
export default function SitorMaestro({ role }) {
  ...
  if (!role) return null;
  const LEVEL_BADGE = { novizio: { c: "hsl(var(--accent))", t: tri("Novizio", "Anfänger", "Novice", "Novato", "Novice", "تازه‌کار") },
    esperto: { c: "hsl(var(--muted-foreground))", t: tri("Esperto", "Erfahren", "Expert", "Experto", "Expert", "ماهر") },
    maestro: { c: "hsl(var(--muted-foreground))", t: tri("Maestro", "Meister", "Master", "Maestro", "Maître", "استاد") } }[level] || { c: "hsl(var(--accent))", t: level };

  return (
    <div data-testid="sitor-maestro" className="relative rounded-2xl border border-border/35 bg-background overflow-hidden">
      ...
      <div className="relative z-10 p-4 space-y-3">
        <div className="flex items-center gap-3">
          <img src={`${PUB}/sitor_official.webp`} alt="Sitor" className="w-11 h-11 rounded-xl object-cover object-top border-2 border-border/60 shrink-0" onError={(e) => { e.currentTarget.style.display = "none"; }} />
          <div className="min-w-0 flex-1">
            <p className="text-sm font-black text-foreground uppercase tracking-wide flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-muted-foreground" /> {tri("Sitor Maestro", "Sitor Meister", "Sitor Master", "Sitor Maestro", "Sitor Maître", "استاد سیتور")}
            ...
            <p className="text-[11px] text-muted-foreground leading-snug">{tri("Ti guido passo-passo e sorveglio ogni fase, con o senza macchinari.", "Ich führe dich Schritt für Schritt, mit oder ohne Maschinen.", "I guide you step-by-step and watch every phase, with or without machines.", "Te guío paso a paso, con o sin máquinas.", "Je te guide pas à pas, avec ou sans machines.", "قدم‌به‌قدم راهنمایی می‌کنم.")}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button data-testid="maestro-machines-toggle" onClick={() => setHasMachines((v) => !v)}
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold border transition-all active:scale-95 ${hasMachines ? "bg-accent/12 border-accent/45 text-accent" : "bg-muted/12 border-border/45 text-muted-foreground"}`}>
        ...
        <button data-testid="maestro-guide-btn" onClick={() => askGuide("")} disabled={busy}
          ...
          style={{ background: "linear-gradient(90deg,hsl(var(--muted-foreground)),hsl(var(--muted-foreground)))", boxShadow: "0 0 18px rgba(166,177,188,0.3)" }}>
        ...
        <div className="flex items-center gap-2">
          <input data-testid="maestro-ask-input" value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") askGuide(q); }}
            ...
            className="flex-1 min-w-0 rounded-lg bg-background border border-border focus:border-border/60 outline-none text-sm text-foreground px-3 py-2" />
          <button data-testid="maestro-ask-btn" onClick={() => askGuide(q)} disabled={busy || !q.trim()}
            className="shrink-0 w-10 h-10 rounded-lg bg-muted/12 border border-border/40 text-muted-foreground flex items-center justify-center active:scale-95 disabled:opacity-40"><Send className="w-4 h-4" /></button>
        </div>

        {/* Guida generata */}
        <AnimatePresence>
          {guide && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} data-testid="maestro-guide" className="rounded-xl bg-background border border-border/25 p-3 space-y-2.5">
              {guide.spoken && (
                <p className="text-sm text-foreground leading-snug flex items-start gap-2">
                  <button onClick={() => speak(guide.spoken)} className="text-muted-foreground shrink-0 mt-0.5 active:scale-90"><Volume2 className="w-4 h-4" /></button>
                  {guide.spoken}
                </p>
              )}
              {(guide.steps || []).length > 0 && (
                <div>
                  <p className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-muted-foreground mb-1.5"><ListChecks className="w-3.5 h-3.5" /> {tri("Passi", "Schritte", "Steps", "Pasos", "Étapes", "مراحل")}</p>
                  <ol className="space-y-1.5">
                    {guide.steps.map((s, i) => (
                      <li key={i} data-testid={`maestro-step-${i}`} className="flex gap-2 text-[13px] text-foreground leading-snug">
                        <span className="shrink-0 w-5 h-5 rounded-full bg-muted/15 border border-border/40 text-muted-foreground text-[10px] font-black flex items-center justify-center">{i + 1}</span>
                        <span>{s}</span>
                      </li>
                    ))}
                  </ol>
              ...
              {(guide.watch || []).length > 0 && (
                <div className="rounded-lg bg-muted/8 border border-border/25 p-2.5">
                  <p className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-muted-foreground mb-1"><Eye className="w-3.5 h-3.5" /> {tri("Sorveglia", "Achte auf", "Watch", "Vigila", "Surveille", "مراقب باش")}</p>
                  <ul className="space-y-0.5">
                    {guide.watch.map((w, i) => <li key={i} className="text-[12px] text-foreground leading-snug">• {w}</li>)}
                  </ul>
                </div>
              )}
              {guide.encourage && <p className="text-[12px] italic text-muted-foreground/90 leading-snug">“{guide.encourage}”</p>}
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="pt-2 border-t border-border">
          {!showChange ? (
            <button data-testid="maestro-change-open" onClick={() => setShowChange(true)}
              className="w-full inline-flex items-center justify-center gap-2 py-2 rounded-lg bg-background border border-border/30 text-muted-foreground font-bold text-xs active:scale-95 transition-all">
            ...
            <div className="space-y-2">
              <textarea data-testid="maestro-change-input" value={proposal} onChange={(e) => setProposal(e.target.value)} rows={2}
                ...
                className="w-full rounded-lg bg-background border border-border focus:border-border/60 outline-none text-sm text-foreground p-2.5 resize-none" />
              <div className="flex items-center gap-2">
                <button data-testid="maestro-change-send" onClick={sendChange} disabled={changeBusy || !proposal.trim()}
                  className="flex-1 inline-flex items-center justify-center gap-2 py-2 rounded-lg font-bold text-xs text-foreground active:scale-95 disabled:opacity-40" style={{ background: "linear-gradient(90deg,hsl(var(--muted-foreground)),hsl(var(--muted-foreground)))" }}>
                ...
                <button data-testid="maestro-change-cancel" onClick={() => { setShowChange(false); setProposal(""); }} className="px-3 py-2 rounded-lg bg-background border border-border text-muted-foreground text-xs font-bold active:scale-95">{tri("Annulla", "Abbrechen", "Cancel", "Cancelar", "Annuler", "لغو")}</button>
              </div>
            </div>
          )}
          {lastChange && (
            <div data-testid="maestro-change-result" className={`mt-2 rounded-lg p-2.5 text-[12px] leading-snug border ${lastChange.status === "auto_applied" ? "bg-accent/8 border-accent/30 text-accent-foreground" : "bg-muted/8 border-border/30 text-foreground"}`}>
