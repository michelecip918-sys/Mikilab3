...
export default function SitorMaestro({ role }) {
  ...
  if (!role) return null;
  const LEVEL_BADGE = { novizio: { c: "#22c55e", t: tri("Novizio", "Anfänger", "Novice", "Novato", "Novice", "تازه‌کار") },
    esperto: { c: "#EAB308", t: tri("Esperto", "Erfahren", "Expert", "Experto", "Expert", "ماهر") },
    maestro: { c: "#FF6B00", t: tri("Maestro", "Meister", "Master", "Maestro", "Maître", "استاد") } }[level] || { c: "#22c55e", t: level };

  return (
    <div data-testid="sitor-maestro" className="relative rounded-2xl border border-[#EAB308]/35 bg-[#070B12] overflow-hidden">
      <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(circle at 50% -10%, rgba(234,179,8,0.10), transparent 60%)" }} />
      <div className="relative z-10 p-4 space-y-3">
        <div className="flex items-center gap-3">
          <img src={`${PUB}/avatar_nexus.jpg`} alt="Sitor" className="w-11 h-11 rounded-xl object-cover object-top border-2 border-[#EAB308]/60 shrink-0" onError={(e) => { e.currentTarget.style.display = "none"; }} />
          <div className="min-w-0 flex-1">
            <p className="text-sm font-black text-white uppercase tracking-wide flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-[#EAB308]" /> {tri("Sitor Maestro", "Sitor Meister", "Sitor Master", "Sitor Maestro", "Sitor Maître", "استاد سیتور")}
        ...
        <div className="flex items-center gap-2">
          <button data-testid="maestro-machines-toggle" onClick={() => setHasMachines((v) => !v)}
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold border transition-all active:scale-95 ${hasMachines ? "bg-[#22c55e]/12 border-[#22c55e]/45 text-[#22c55e]" : "bg-[#FF6B00]/12 border-[#FF6B00]/45 text-[#FF9D42]"}`}>
        ...
        <div className="flex items-center gap-2">
          <input data-testid="maestro-ask-input" value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") askGuide(q); }}
            ...
            className="flex-1 min-w-0 rounded-lg bg-[#030712] border border-[#1e293b] focus:border-[#EAB308]/60 outline-none text-sm text-white px-3 py-2" />
          <button data-testid="maestro-ask-btn" onClick={() => askGuide(q)} disabled={busy || !q.trim()}
            className="shrink-0 w-10 h-10 rounded-lg bg-[#EAB308]/12 border border-[#EAB308]/40 text-[#EAB308] flex items-center justify-center active:scale-95 disabled:opacity-40"><Send className="w-4 h-4" /></button>
        </div>

        {/* Guida generata */}
        <AnimatePresence>
          {guide && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} data-testid="maestro-guide" className="rounded-xl bg-[#030712] border border-[#EAB308]/25 p-3 space-y-2.5">
              {guide.spoken && (
                <p className="text-sm text-[#E8EEF5] leading-snug flex items-start gap-2">
                  <button onClick={() => speak(guide.spoken)} className="text-[#EAB308] shrink-0 mt-0.5 active:scale-90"><Volume2 className="w-4 h-4" /></button>
                  {guide.spoken}
                </p>
              )}
              {(guide.steps || []).length > 0 && (
                <div>
                  <p className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-[#EAB308] mb-1.5"><ListChecks className="w-3.5 h-3.5" /> {tri("Passi", "Schritte", "Steps", "Pasos", "Étapes", "مراحل")}</p>
                  <ol className="space-y-1.5">
                    {guide.steps.map((s, i) => (
                      <li key={i} data-testid={`maestro-step-${i}`} className="flex gap-2 text-[13px] text-[#CBD5E1] leading-snug">
                        <span className="shrink-0 w-5 h-5 rounded-full bg-[#EAB308]/15 border border-[#EAB308]/40 text-[#EAB308] text-[10px] font-black flex items-center justify-center">{i + 1}</span>
                        <span>{s}</span>
                      </li>
                    ))}
                  </ol>
              ...
              {(guide.watch || []).length > 0 && (
                <div className="rounded-lg bg-[#FF6B00]/8 border border-[#FF6B00]/25 p-2.5">
                  <p className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-[#FF9D42] mb-1"><Eye className="w-3.5 h-3.5" /> {tri("Sorveglia", "Achte auf", "Watch", "Vigila", "Surveille", "مراقب باش")}</p>
                  <ul className="space-y-0.5">
                    {guide.watch.map((w, i) => <li key={i} className="text-[12px] text-[#d7c9a8] leading-snug">• {w}</li>)}
                  </ul>
                </div>
              )}
              {guide.encourage && <p className="text-[12px] italic text-[#EAB308]/90 leading-snug">“{guide.encourage}”</p>}
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="pt-2 border-t border-[#1e293b]">
          {!showChange ? (
            <button data-testid="maestro-change-open" onClick={() => setShowChange(true)}
              className="w-full inline-flex items-center justify-center gap-2 py-2 rounded-lg bg-[#0C1019] border border-[#EAB308]/30 text-[#EAB308] font-bold text-xs active:scale-95 transition-all">
            ...
            <div className="space-y-2">
              <textarea data-testid="maestro-change-input" value={proposal} onChange={(e) => setProposal(e.target.value)} rows={2}
                ...
                className="w-full rounded-lg bg-[#030712] border border-[#1e293b] focus:border-[#EAB308]/60 outline-none text-sm text-white p-2.5 resize-none" />
              <div className="flex items-center gap-2">
                <button data-testid="maestro-change-send" onClick={sendChange} disabled={changeBusy || !proposal.trim()}
                  className="flex-1 inline-flex items-center justify-center gap-2 py-2 rounded-lg font-bold text-xs text-[#060A10] active:scale-95 disabled:opacity-40" style={{ background: "linear-gradient(90deg,#EAB308,#FF6B00)" }}>
          ...
          {lastChange && (
            <div data-testid="maestro-change-result" className={`mt-2 rounded-lg p-2.5 text-[12px] leading-snug border ${lastChange.status === "auto_applied" ? "bg-[#22c55e]/8 border-[#22c55e]/30 text-[#7ee2a8]" : "bg-[#EAB308]/8 border-[#EAB308]/30 text-[#e8d48a]"}`}>
