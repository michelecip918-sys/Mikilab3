  ...
  return (
    <div data-testid="anno-page" className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <div data-testid="anno-stats" className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {rows.map((r, i) => <div key={i} className="rounded-xl border border-border bg-card p-3 text-center"><p className="font-display text-2xl font-bold text-foreground">{r.n}</p><p className="text-[10.5px] uppercase tracking-wide text-muted-foreground">{r.l}</p></div>)}
      </div>
      <p className="text-[12.5px] text-salvia leading-snug">Sitor: {sitor}</p>
      {url ? <img data-testid="anno-img" src={url} alt="" className="w-full rounded-2xl border border-border shadow-md" /> : <p className="text-[12px] text-muted-foreground">{tri("Sto disegnando…", "Ich zeichne…", "Drawing…")}</p>}
      <button data-testid="anno-share" onClick={share} className="inline-flex items-center gap-1.5 text-[12px] font-bold px-3 py-2 rounded-xl bg-primary text-white active:scale-95"><Share2 className="w-4 h-4" />{tri("Condividi il manifesto", "Plakat teilen", "Share the poster")}</button>
      <p className="text-[10.5px] text-muted-foreground">{tri("I numeri vivono nel tuo browser: se cambi telefono ripartono da zero. Il soprannome è quello della tua bottega, facoltativo.", "Die Zahlen leben in deinem Browser: mit neuem Handy fangen sie bei null an. Der Spitzname ist der deiner Bottega, optional.", "The numbers live in your browser: a new phone starts from zero. The nickname is the one from your bottega, optional.")}</p>
    </div>
  );
}
