...
export default function HomeNews() {
  ...
  const F = (k, ph) => (
    <input value={form[k]} onChange={(e) => setForm((f) => ({ ...f, [k]: e.target.value }))} placeholder={ph}
      className="w-full bg-white dark:bg-[#181818] border border-[#2b2b2b] dark:border-[#2e2e2e] rounded-lg px-2.5 py-2 text-sm outline-none focus:border-[#ff6b00] mb-1.5" />
  );

  return (
    <div data-testid="home-news" className="rounded-2xl bg-white/70 dark:bg-[#1e1e1e]/70 border border-[#2b2b2b] dark:border-[#2e2e2e] p-4 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-2">
        ...
        {isAdmin && editing === null && (
          <button data-testid="news-add-btn" onClick={openNew} className="inline-flex items-center gap-1 text-xs font-semibold text-white bg-[#ff6b00] px-2.5 py-1.5 rounded-lg active:scale-95">
      ...
      {isAdmin && editing !== null && (
        <div data-testid="news-editor" className="rounded-xl bg-[#e4eff8] dark:bg-[#242424] p-3 mb-3">
          ...
          <textarea value={form.body} onChange={(e) => setForm((f) => ({ ...f, body: e.target.value }))} placeholder={tri("Testo (IT)", "Text (IT)", "Body (IT)")} rows={2}
            className="w-full bg-white dark:bg-[#181818] border border-[#2b2b2b] dark:border-[#2e2e2e] rounded-lg px-2.5 py-2 text-sm outline-none focus:border-[#ff6b00] mb-1.5 resize-none" />
          ...
          <div className="flex gap-2 mt-1">
            <button data-testid="news-save-btn" onClick={save} className="flex-1 inline-flex items-center justify-center gap-1.5 bg-[#ff6b00] text-white text-sm font-semibold py-2 rounded-lg active:scale-98"><Save className="w-4 h-4" /> {tri("Salva", "Speichern", "Save")}</button>
            <button onClick={() => setEditing(null)} className="w-10 rounded-lg bg-white dark:bg-[#181818] border border-[#2b2b2b] dark:border-[#2e2e2e] flex items-center justify-center text-[#7E8A93]"><X className="w-4 h-4" /></button>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2.5">
        {items.map((n) => (
          <div key={n.id} data-testid={`news-item-${n.id}`} className="rounded-xl bg-white dark:bg-[#1e1e1e] border border-[#2b2b2b] dark:border-[#2e2e2e] p-3">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                {n.link && <a href={n.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs font-semibold text-[#ff6b00] mt-1.5">{tri("Leggi", "Lesen", "Read")} <ExternalLink className="w-3 h-3" /></a>}
              </div>
              {isAdmin && (
                <div className="flex flex-col gap-1 shrink-0">
                  <button data-testid={`news-edit-${n.id}`} onClick={() => openEdit(n)} className="w-8 h-8 rounded-lg bg-[#e4eff8] dark:bg-[#242424] flex items-center justify-center text-[#7E8A93]"><Pencil className="w-3.5 h-3.5" /></button>
                  <button data-testid={`news-delete-${n.id}`} onClick={() => remove(n.id)} className="w-8 h-8 rounded-lg bg-[#ff6b00]/10 flex items-center justify-center text-[#ff6b00]"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              )}
            </div>
          </div>
