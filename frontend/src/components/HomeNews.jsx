...
export default function HomeNews() {
  ...
  const F = (k, ph) => (
    <input value={form[k]} onChange={(e) => setForm((f) => ({ ...f, [k]: e.target.value }))} placeholder={ph}
      className="w-full bg-white dark:bg-[#181818] border border-[#2e2e2e] dark:border-[#2e2e2e] rounded-lg px-2.5 py-2 text-sm outline-none focus:border-[#ff6b00] mb-1.5" />
  );
    ...
    <div data-testid="home-news" className="rounded-2xl bg-white/70 dark:bg-[#1e1e1e]/70 border border-[#2e2e2e] dark:border-[#2e2e2e] p-4 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-2">
        <h2 className="flex items-center gap-2 font-display text-lg font-bold text-[#2B303B] dark:text-[#e4eff8]">
          <Newspaper className="w-5 h-5 text-[#ff6b00]" /> {tri("News · Arte Bianca", "News · Backhandwerk", "News · Artisan Baking")}
        </h2>
      ...
      {isAdmin && editing !== null && (
        <div data-testid="news-editor" className="rounded-2xl shadow-md border border-amber-900/40 bg-[#e4eff8] dark:bg-[#1e1e1e] p-3 mb-3">
          ...
          <textarea value={form.body} onChange={(e) => setForm((f) => ({ ...f, body: e.target.value }))} placeholder={tri("Testo (IT)", "Text (IT)", "Body (IT)")} rows={2}
            className="w-full bg-white dark:bg-[#181818] border border-[#2e2e2e] dark:border-[#2e2e2e] rounded-lg px-2.5 py-2 text-sm outline-none focus:border-[#ff6b00] mb-1.5 resize-none" />
