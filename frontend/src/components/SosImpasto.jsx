...
export default function SosImpasto({ open, onClose, onNavigate }) {
  ...
  return (
    <div className="fixed inset-0 z-[75] flex items-end sm:items-center justify-center" data-testid="sos-panel">
      ...
      <div className="relative w-full sm:max-w-md bg-white dark:bg-[#0B0E14] rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[88vh] flex flex-col overflow-hidden">
        <div className="flex items-center gap-3 p-4 text-white shrink-0" style={{ background: "linear-gradient(135deg,#7a1f1f,#F26419 55%,#F26419)" }}>
        ...
        <div className="p-4 overflow-y-auto space-y-3">
          ...
          {(busy || result) && (
            <div data-testid="sos-result" className="rounded-2xl bg-[#0B0E14] dark:bg-[#18202E] border border-[#26324A] dark:border-[#26324A] p-4">
              {busy && !result ? (
                <div className="flex items-center gap-2 text-[#F26419]"><Loader2 className="w-5 h-5 animate-spin" /> {tri("Analisi in corso…", "Analyse läuft…", "Analysing…", "Analizando…")}</div>
              ) : (
          ...
          {rec && (
            <button data-testid="sos-recipe-suggestion" onClick={openRecipe}
              className="w-full flex items-center gap-3 rounded-2xl p-3.5 text-left bg-gradient-to-br from-[#F26419] to-[#7a531d] text-white shadow-md active:scale-98 transition-all">
          ...
          {recLoading && !rec && (
            <p className="text-[11px] text-[#F26419] text-center flex items-center justify-center gap-1.5"><Loader2 className="w-3.5 h-3.5 animate-spin" /> {tri("Cerco la ricetta più adatta…", "Suche das passende Rezept…", "Finding the best recipe…", "Buscando la mejor receta…")}</p>
          )}
        </div>
      </div>
    </div>
