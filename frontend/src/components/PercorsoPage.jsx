            ...
            <div className="space-y-1.5">
              {lv.recipes.map((r) => (
                <div key={r.id} className="flex items-center gap-2">
                  ...
                  <button onClick={() => onOpenRecipe(r.id)} className={`text-left flex-1 text-sm hover:text-muted-foreground ${done.has(r.id) ? "text-muted-foreground line-through" : "text-foreground"}`}>{r.name}</button>
                </div>
              ))}
            </div>
            {complete && (
