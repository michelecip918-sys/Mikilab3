...
import ComeLeggere from "@/components/ComeLeggere";
import { farroEligible, farroWaterFactor, farroFlourLabel, farroHelp } from "@/lib/farro";
...
export default function RecipeList({ collectionName, heroImage, heroTitle, heroSubtitle, emptyText, readOnly = false, heroPosition, extraHeader, deptScoped = false, hideHero = false }) {
  ...
  useEffect(() => {
    ...
    if (viewing) { applyRecipeSeo(viewing, lang); return () => clearRecipeSeo(lang); }
    return undefined;
  }, [viewing, lang]); // eslint-disable-line react-hooks/exhaustive-deps
  useBackClose(dialogOpen, () => setDialogOpen(false));
  useBackClose(!!scaling, () => setScaling(null));
  // V76: "Prima di iniziare" → Basi & Lieviti apre il ricettario già filtrato
  useEffect(() => {
...
function RecipeDetail({ r, t, readOnly, canEdit, scaleVal, onScaleChange, onImprover, onEdit, onDuplicate, onScaleAction, onDelete, onPromote }) {
  ...
  const farroFac = farroWaterFactor(r);
  useEffect(() => { window.__mkCur = { id: r.id, farro: farroOn }; return () => { window.__mkCur = null; }; }, [r.id, farroOn]);
  ...
  const gWater = (v) => { const b = g(v); return b == null ? null : (farroOn ? Math.round(b * farroFac) : b); };
  const pct = (v) => (flourG > 0 && v != null ? ` · ${Math.round((Number(v) / flourG) * 1000) / 10}%` : "");
  const biga = (!r.locked && r.biga && (r.preferment_type === "biga" || r.biga.show)) ? r.biga : null;
  const isPoolishPre = !!(biga && biga.kind === "poolish");
  ...
  (r.extra_ingredients || []).forEach((e) => {
    if (e && e.name && e.percent != null && e.percent !== "") {
      if (biga && /lievito di birra|hefe/i.test(e.name) && !/impasto finale|hauptteig|final dough/i.test(e.name)) return; // il lievito del prefermento è già nella Fase 1
      if (r.sourdough_grams && /lievito madre|sauerteig|licoli/i.test(e.name)) return; // già nella riga del prefermento: non raddoppiarlo
  ...
  return (
    <div data-testid={`recipe-detail-${r.id}`}>
      ...
      <div className="p-5 space-y-4 print-area">
        ...
        {farroOk && farro && !isPanettone && (
          <div data-testid={`farro-banner-${r.id}`} className="rounded-2xl shadow-md border border-primary/30 bg-primary/12 p-3 text-sm text-muted-foreground leading-relaxed">
            🌾 {farroHelp(lang)}
          </div>
        )}

        {farroOk && farro && isPanettone && (
          <div data-testid={`farro-banner-pan-${r.id}`} className="rounded-2xl shadow-md border border-amber-900/40 bg-primary/12 border border-primary/30 p-3 text-sm text-muted-foreground dark:text-muted-foreground leading-relaxed">
            🌾 {tri(
        ...
        {isPanettone && !r.locked ? (
          ...
          {biga && (
            <div data-testid={`recipe-biga-${r.id}`} className="rounded-2xl shadow-md border border-amber-900/40 bg-primary/10 border border-primary/30 p-3">
              <p className="text-[10px] font-bold uppercase tracking-wide text-primary dark:text-muted-foreground mb-2">🥖 {isPoolishPre ? tri("Fase 1 · Poolish (la sera prima)", "Phase 1 · Poolish (am Vorabend)", "Phase 1 · Poolish (evening before)") : tri("Fase 1 · Biga (la sera prima)", "Phase 1 · Vorteig (Biga)", "Phase 1 · Biga (evening before)")}</p>
          ...
          <div data-testid={`recipe-ingredients-${r.id}`} className="rounded-2xl shadow-md border border-amber-900/40 bg-muted dark:bg-card p-3 print-table">
            ...
            {flourG > 0 && (
              <div data-testid={`scale-chips-${r.id}`} className="no-print flex flex-wrap items-center gap-1.5 mb-2">
                <span className="text-[10px] text-muted-foreground">{tri("Quanta farina usi?", "Wie viel Mehl nimmst du?", "How much flour?")}</span>
                {[250, 500, 750, 1000].map((v) => (
                  <button key={v} data-testid={`scale-chip-${v}`} onClick={() => onScaleChange(String(v))}
                    className={`px-2 py-0.5 rounded-full text-[11px] font-bold border transition-all ${Math.round(target) === v ? "bg-primary text-primary-foreground border-primary" : "bg-card text-muted-foreground border-border"}`}>{v} g</button>
                ))}
                <button data-testid="scale-chip-orig" onClick={() => onScaleChange(String(flourG))}
          ...
          <Sostituzioni r={r} target={target} f={f} farro={farroOn} />
          </>
        ) : null}
...
function PanettoneStructure({ r, t, lang, flourG, farro, scaleVal, onScaleChange }) {
  ...
  const items = [];
  items.push({ name: t("ing_flour"), tot: flourTot, first: 0.6 });
