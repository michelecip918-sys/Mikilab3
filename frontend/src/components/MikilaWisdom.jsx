...
const WISDOM = {
  home: [
    ...
    ["L'impasto, quello autentico e profondo, non è una quantità limitata di farina e acqua da spartire, ma una frequenza viva che il pane impara a sintonizzare con il tempo.", "Der Teig, der echte und tiefe, ist keine begrenzte Menge Mehl und Wasser zum Aufteilen, sondern eine lebendige Frequenz, die das Brot mit der Zeit zu stimmen lernt.", "Dough — the authentic, deep kind — is not a limited amount of flour and water to divide, but a living frequency that bread learns to tune with time.", "La masa, la auténtica y profunda, no es una cantidad limitada de harina y agua para repartir, sino una frecuencia viva que el pan aprende a sintonizar con el tiempo.", "La pâte, l'authentique et la profonde, n'est pas une quantité limitée de farine et d'eau à partager, mais une fréquence vivante que le pain apprend à accorder avec le temps.", "خمیر، آن خمیر اصیل و ژرف، مقدار محدودی آرد و آب برای تقسیم نیست، بلکه بسامدی زنده است که نان کوک‌کردن آن را با گذر زمان می‌آموزد."],
  ],
  ricette: [
...
export default function MikilaWisdom({ section = "home" }) {
  ...
  return (
    <>
      <div data-testid="mikila-wisdom" className={`mb-4 rounded-2xl border px-3.5 py-3 shadow-sm ${special ? "border-[#3E9C93]/50 bg-[#3E9C93]/12" : "border-[#3E9C93]/25 bg-[#1B2A38]"}`}>
        <div className="flex items-center gap-3">
          <img src="/michele-real-lab.jpg" alt="MikiLab" loading="lazy"
            className="w-10 h-10 rounded-full object-cover border-2 border-[#3E9C93]/40 shrink-0"
            onError={(e) => { e.currentTarget.style.display = "none"; }} />
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-wider text-[#3E9C93] flex items-center gap-1">
          ...
          {!special && (
            <button data-testid="mikila-wisdom-next" onClick={() => setOffset((o) => o + 1)} aria-label="next tip"
              className="shrink-0 w-8 h-8 rounded-full bg-[#3E9C93]/15 border border-[#3E9C93]/30 flex items-center justify-center text-[#3E9C93] active:scale-90 transition-transform">
        ...
        <div className="flex items-center gap-2 mt-2 pl-[52px] flex-wrap">
            ...
            <button data-testid="mikila-wisdom-share" onClick={shareCur} className="inline-flex items-center gap-1 text-[11px] font-bold text-[#3E9C93] px-2 py-1 rounded-full border border-[#3E9C93]/40 active:scale-90 transition-all">
              <Share2 className="w-3 h-3" /> {L("Condividi", "Teilen", "Share", "Compartir", "Partager", "اشتراک")}
            </button>
            <button data-testid="mikila-wisdom-card" onClick={() => setCardText({ text, author: special ? null : cur.author })} className="inline-flex items-center gap-1 text-[11px] font-bold text-[#3E9C93] px-2 py-1 rounded-full border border-[#3E9C93]/40 active:scale-90 transition-all">
              <ImageIcon className="w-3 h-3" /> {L("Crea card", "Karte erstellen", "Make card", "Crear tarjeta", "Créer carte", "ساخت کارت")}
            </button>
            <button data-testid="mikila-wisdom-propose" onClick={() => setProposeOpen(true)} className="inline-flex items-center gap-1 text-[11px] font-bold text-[#3E9C93] px-2 py-1 rounded-full border border-[#3E9C93]/40 active:scale-90 transition-all">
...
function ProposeModal({ lang, user, onClose }) {
  ...
  return (
    <div data-testid="wisdom-propose-modal" className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-md p-3" onClick={onClose}>
      <div className="w-full max-w-md rounded-3xl bg-[#1B2A38] border border-[#2A3B49] p-5" onClick={(e) => e.stopPropagation()}>
...
function ModerateModal({ lang, onClose }) {
  ...
  return (
    <div data-testid="wisdom-moderate-modal" className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-md p-3" onClick={onClose}>
      <div className="w-full max-w-md max-h-[80vh] overflow-y-auto rounded-3xl bg-[#1B2A38] border border-[#2A3B49] p-5" onClick={(e) => e.stopPropagation()}>
              ...
              {items.map((it) => (
                <div key={it.id} data-testid={`wisdom-pending-${it.id}`} className="rounded-2xl shadow-md border border-amber-900/40 bg-[#0D1520] border border-[#2A3B49] p-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    ...
                    <button data-testid={`wisdom-reject-${it.id}`} onClick={() => act(it.id, false)} className="rounded-lg border border-[#2A3B49] text-[#ff3b5c] font-semibold py-2 text-sm active:scale-95 flex items-center justify-center gap-1"><X className="w-4 h-4" /> {L("Rifiuta", "Ablehnen", "Reject", "Rechazar", "Rejeter", "رد")}</button>
                  </div>
                </div>
              ))}
            </div>
...
function CardModal({ lang, text, author, onClose }) {
  ...
  useEffect(() => {
    ...
    // Testo virgolette
    ctx.fillStyle = "#3E9C93"; ctx.font = "bold 160px Georgia, serif"; ctx.textAlign = "center"; ctx.fillText("“", S / 2, 330);
    // Proverbio
    ...
    img.onload = () => {
      ...
      ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.strokeStyle = "#3E9C93"; ctx.lineWidth = 8; ctx.stroke();
      finish();
    };
    img.onerror = finish;
  ...
  return (
    <div data-testid="wisdom-card-modal" className="fixed inset-0 z-[70] flex items-center justify-center bg-black/85 backdrop-blur-md p-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-3xl bg-[#1B2A38] border border-[#2A3B49] p-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-display text-base font-bold text-white flex items-center gap-2"><ImageIcon className="w-5 h-5 text-[#3E9C93]" /> {L("Card condivisibile", "Teilbare Karte", "Shareable card", "Tarjeta para compartir", "Carte à partager", "کارت اشتراکی")}</h3>
          <button onClick={onClose} className="text-[#AEB8BF]"><X className="w-5 h-5" /></button>
        </div>
