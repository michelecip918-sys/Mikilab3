...
export default function EvolvingQuiz() {
  ...
  const LEVELS = [
    ...
    { id: "avanzato", label: tri("Home Baker Avanzato", "Fortgeschritten", "Advanced", "Avanzado"), color: "#ffc700" },
  ...
  return (
    <div data-testid="evolving-quiz" className="rounded-2xl bg-white dark:bg-[#1e1e1e] border border-[#2b2b2b] dark:border-[#2e2e2e] p-5">
      ...
      {diploma && (
        <div data-testid="diploma-badge" className="flex items-center gap-2 mb-3 rounded-xl bg-gradient-to-r from-[#ffc700] to-[#ff6b00] text-white px-3 py-2 shadow-sm">
      ...
      {level === "master" && !diploma && (
        <p data-testid="master-progress" className="text-[11px] font-semibold text-[#ffc700] mb-2">
      ...
      <div className="flex items-center justify-between mb-3 text-xs">
        ...
        <span data-testid="quiz-best" className="font-semibold text-[#ffc700] flex items-center gap-1"><Trophy className="w-3.5 h-3.5" /> {tri("Record", "Rekord", "Best", "Récord")}: {best}</span>
      </div>
      ...
      {user && showBoard && (
        <div data-testid="quiz-leaderboard" className="mb-3 rounded-xl bg-[#121212] dark:bg-[#181818] border border-[#2b2b2b] dark:border-[#2e2e2e] p-3 space-y-1.5">
          ...
          {champion && (
            <div data-testid="quiz-champion" className="flex items-center gap-2.5 rounded-lg bg-gradient-to-r from-[#ffc700] to-[#ff6b00] text-white px-2.5 py-2 mb-1">
          ...
          {board === null ? (
            ...
            <div key={r.user_id} data-testid={`leaderboard-row-${r.user_id}`} className={`flex items-center gap-2.5 rounded-lg px-2 py-1.5 ${r.me ? "bg-[#ff6b00]/10" : ""} ${idx === 0 && r.points > 0 ? "ring-1 ring-[#ffc700]/40" : ""}`}>
              <span className={`w-5 text-center text-sm font-extrabold ${idx === 0 ? "text-[#ffc700]" : "text-[#7E8A93]"}`}>{idx === 0 && r.points > 0 ? "👑" : idx + 1}</span>
