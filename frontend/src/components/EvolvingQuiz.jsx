...
export default function EvolvingQuiz() {
  ...
  const LEVELS = [
    { id: "apprendista", label: tri("Apprendista", "Anfänger", "Apprentice", "Aprendiz"), color: "#3E9C93" },
    { id: "avanzato", label: tri("Home Baker Avanzato", "Fortgeschritten", "Advanced", "Avanzado"), color: "#3E9C93" },
  ...
  return (
    <div data-testid="evolving-quiz" className="rounded-2xl bg-white dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-5">
      ...
      {diploma && (
        <div data-testid="diploma-badge" className="flex items-center gap-2 mb-3 rounded-2xl shadow-md border border-amber-900/40 bg-gradient-to-r from-[#F0B429] to-[#3E9C93] text-white px-3 py-2 shadow-sm">
      ...
      {level === "master" && !diploma && (
        <p data-testid="master-progress" className="text-[11px] font-semibold text-[#3E9C93] mb-2">
      ...
      {theme && (
        <div data-testid="weekly-theme-card" className="mb-3 rounded-2xl shadow-md border border-amber-900/40 p-3 text-white shadow-sm" style={{ background: themeMode ? "linear-gradient(135deg,#3E9C93,#7a1f1f)" : "linear-gradient(135deg,#3E9C93,#3E9C93)" }}>
          <div className="flex items-center gap-2">
            ...
            {themeMode ? (
              ...
              <button data-testid="weekly-theme-start" onClick={startThemeChallenge} className="text-xs font-bold bg-white text-[#3E9C93] px-3 py-1.5 rounded-full active:scale-95">{tri("Gioca", "Spielen", "Play", "Jugar")}</button>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 mb-3">
        {LEVELS.map((l) => (
          <button key={l.id} data-testid={`quiz-level-${l.id}`}
            ...
            className={`py-2 rounded-2xl shadow-md border border-amber-900/40 text-[11px] font-bold transition-all leading-tight ${level === l.id ? "text-white" : "bg-[#e4eff8] dark:bg-[#1B2A38] text-[#7E8A93]"}`}
      ...
      <div className="flex items-center justify-between mb-3 text-xs">
        <span data-testid="quiz-streak" className="font-semibold text-[#3E9C93] flex items-center gap-1"><Sparkles className="w-3.5 h-3.5" /> {tri("Serie", "Serie", "Streak", "Racha")}: {streak}</span>
        <span data-testid="quiz-best" className="font-semibold text-[#3E9C93] flex items-center gap-1"><Trophy className="w-3.5 h-3.5" /> {tri("Record", "Rekord", "Best", "Récord")}: {best}</span>
      </div>
      ...
      {user && showBoard && (
        <div data-testid="quiz-leaderboard" className="mb-3 rounded-2xl shadow-md border border-amber-900/40 bg-[#0E1620] dark:bg-[#1B2A38] border border-[#2A3B49] dark:border-[#2A3B49] p-3 space-y-1.5">
          ...
          {board === null ? (
            ...
            <div key={r.user_id} data-testid={`leaderboard-row-${r.user_id}`} className={`flex items-center gap-2.5 rounded-lg px-2 py-1.5 ${r.me ? "bg-[#3E9C93]/10" : ""} ${idx === 0 && r.points > 0 ? "ring-1 ring-[#3E9C93]/40" : ""}`}>
              <span className={`w-5 text-center text-sm font-extrabold ${idx === 0 ? "text-[#3E9C93]" : "text-[#7E8A93]"}`}>{idx === 0 && r.points > 0 ? "👑" : idx + 1}</span>
              <div className="w-8 h-8 rounded-full overflow-hidden bg-[#1B2A38] flex items-center justify-center text-white text-xs font-bold shrink-0">{r.picture ? <img src={r.picture} alt={r.name} className="w-full h-full object-cover" /> : (r.name || "F")[0].toUpperCase()}</div>
