...
export default function PuliziaDati({ onBack }) {
  ...
  // Un solo pulsante: scarica il backup e subito dopo cancella (dopo una sola conferma).
  const oneClick = async () => {
    const ok = window.confirm(tri(
      "Scarico il backup e poi cancello TUTTI i dati vecchi (azienda, squadra, chat, community, utenti non admin). Le ricette e il tuo account restano. Vuoi procedere?",
      "Ich lade das Backup herunter und lösche danach ALLE Altdaten (Betrieb, Team, Chat, Community, Nicht-Admin-Nutzer). Rezepte und dein Konto bleiben. Fortfahren?",
      "I'll download the backup and then delete ALL old data (company, team, chat, community, non-admin users). Recipes and your account stay. Continue?"));
    if (!ok) return;
    setBusy(true);
    try {
      const b = await api.get("/admin/data-cleanup/backup", { responseType: "blob" });
      const token = b.headers["x-backup-token"] || "";
      const url = URL.createObjectURL(b.data);
      const a = document.createElement("a");
      a.href = url; a.download = `mikilab_backup_dati_vecchi_${new Date().toISOString().slice(0, 10)}.json.gz`;
      a.click(); URL.revokeObjectURL(url);
      const r = await api.post("/admin/data-cleanup/execute", { confirm: "CANCELLA", backup_token: token });
      setReport(r.data);
      toast.success(tri("Fatto: backup scaricato e dati vecchi cancellati.", "Fertig: Backup geladen, Altdaten gelöscht.", "Done: backup downloaded and old data deleted."));
      doScan();
    } catch (e) {
      toast.error(e?.response?.data?.detail || tri("Operazione non riuscita: non è stato cancellato nulla di sicuro.", "Vorgang fehlgeschlagen.", "Operation failed."));
    } finally { setBusy(false); }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4" data-testid="pulizia-dati-page">
      ...
      <button data-testid="cleanup-oneclick-btn" onClick={oneClick} disabled={busy}
        className="w-full inline-flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-mattone text-white font-black text-base active:scale-95 disabled:opacity-50">
        {busy ? <Loader2 className="w-5 h-5 animate-spin" /> : <Trash2 className="w-5 h-5" />} {tri("Pulisci tutto (backup + cancella)", "Alles bereinigen (Backup + Löschen)", "Clean everything (backup + delete)")}
      </button>
      <p className="text-[11px] text-muted-foreground text-center">{tri("Oppure fai i due passi a mano qui sotto.", "Oder die zwei Schritte unten von Hand.", "Or do the two steps by hand below.")}</p>
